...
static int max97236_shdn_set(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	mutex_lock(&shdn_mutex);

	snd_soc_update_bits(codec, MAX97236_ENABLE_1,
				MAX97236_ENABLE_1_SHDN_MASK,
				(ucontrol->value.integer.value[0]) ? MAX97236_ENABLE_1_SHDN_BIT : 0);

	mutex_unlock(&shdn_mutex);

	return 0;
}

static int max97236_shdn_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
...
static const struct soc_enum max97236_shdn_enum =
SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(max97236_onoff_text),
	max97236_onoff_text);
...
static const struct snd_kcontrol_new max97236_controls[] = {
	...
	SOC_ENUM_EXT("Amp SHDN Sync", max97236_shdn_enum,
		max97236_shdn_get, max97236_shdn_set),
...
static inline int max97236_read_detected_load(struct snd_soc_codec *codec)
{
	...
	detected |= (snd_soc_read(codec, MAX97236_DETECTION_RESULTS_2) << 8);
...
static int max97236_detect_load(struct max97236_priv *max97236,
				int *detected_result, int test, int delay_ms,
				int stable_max)
{
	struct snd_soc_codec *codec = max97236->codec;
...
static void max97236_jack_work(struct work_struct *work)
{
	...
	shdn_bit_change = snd_soc_test_bits(codec, MAX97236_ENABLE_1,
				MAX97236_ENABLE_1_SHDN_MASK,
...
static int max97236_resume(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, MAX97236_ENABLE_2,
			    ...
			    MAX97236_ENABLE_2_FAST_BIT | detection_mode);
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void max97236_early_suspend(struct early_suspend *h)
{
...
static void max97236_late_resume(struct early_suspend *h)
{
	struct max97236_priv *max97236 = container_of(h, struct max97236_priv,
						      early_suspend);
...
int max97236_set_key_div(struct snd_soc_codec *codec, unsigned long clk_freq)
{
	...
	snd_soc_write(codec, MAX97236_KEYSCAN_CLK_DIV_2,
		      (clk_freq / 2000) & 0xFF);
	snd_soc_write(codec, MAX97236_KEYSCAN_CLK_DIV_ADC, (clk_freq / 200000));
