...
static void binder_bwdog_bark(struct binder_transaction *t, struct timespec *cur)
{
	...
	if (binder_bwdog_safe(t)) {
		pr_info("%d watched, but wait nothing\n",
				 t->debug_id);
		return;
	}
		...
		size_t size, int is_async)
{
	...
	pr_info("buffer allocation failed on %d:0 "
			"%s from %d:%d size %zd\n",
			target_proc->pid,
			is_async ? "async" : "call ",
	...
	pr_info("%d:0 pending transactions:\n", target_proc->pid);
	...
	pr_info("%d:0 total pending trans: %d(%d large isze)\n",
	...
	if (size > threshold)
	{
		...
		{
			pr_info("on %d:0 the largest pending trans is:\n",
					target_proc->pid);
		...
		else
		{
			...
			pr_info("sender pid %d (%s) is not javap, pls check SYS_BINDER_BACKTRACE!\n",
	...
	else
	{
		...
		pr_info("target_proc %p pid %d (%s) is not javap, pls check SYS_BINDER_BACKTRACE!\n",
			       ...
			       struct binder_transaction_data *tr, int reply)
{
	...
	if (reply) {
		...
		if (rt_task(current) && (MAX_RT_PRIO != in_reply_to->saved_rt_prio) && 
			...
			{
				pr_info("reply reset %d sched_policy from %d to %d rt_prio from %d to %d\n",
...
# endif
			if (!reply && (t->policy == SCHED_RR || t->policy == SCHED_FIFO)&&
				...
				if (log_disable & BINDER_RT_LOG_ENABLE)
				{	
					pr_info("write set %d sched_policy from %d to %d rt_prio from %d to %d\n",
...
retry:
	...
	if (wait_for_proc_work) {
		...
		if (rt_task(current) && !binder_has_proc_work(proc, thread)) {
			...
			if (log_disable & BINDER_RT_LOG_ENABLE)
			{	
				pr_info("enter threadpool reset %d sched_policy from %u to %d rt_prio from %u to %d\n",
	...
	while (1) {
		...
		if (t->buffer->target_node) {
			...
			if ((SCHED_RR == t->policy || SCHED_FIFO == t->policy) && t->rt_prio > current->rt_priority &&
				...
				if (log_disable & BINDER_RT_LOG_ENABLE)
				{	
					pr_info("read set %d sched_policy from %d to %d rt_prio from %d to %d\n",
...
static int binder_node_release(struct binder_node *node, int refs, int *reg)
{
	...
	hlist_for_each_entry(ref, &node->refs, node_entry) {
		...
		binder_debug(BINDER_DEBUG_DEATH_NOTIFICATION,
				...
				node->proc->pid, ref->death->cookie);
#endif
		if (!(*reg) &&
...
static void binder_deferred_release(struct binder_proc *proc)
{
	...
	for (i = 0; i < BC_STATS_NR; i++) {
		if (proc->bc_stats[i] != NULL) {
			...
			pr_info("binder_release: release %d bc_stats[%d]\n", proc->pid, i);
		}
	}
	}
#endif
...
static int binder_proc_show(struct seq_file *m, void *unused)
{
	...
	if (find == true)
		...
		pr_info("show proc addr 0x%p exit\n", proc);
#endif
	if (do_lock)
		binder_unlock(__func__);
	return 0;
...
{
	pr_info("transaction log is self resumed\n");
	log_disable = 0;
}
...
static int binder_transaction_log_show(struct seq_file *m, void *unused)
{
	...
	if (log_disable & BINDER_LOG_RESUME) {
		pr_info("%d (%s) read transaction log and "
				 "resume\n",
				      ...
				      size_t cnt, loff_t *data)
{
	...
	pr_info("%d (%s) set performance evaluate type %s %s\n",
			 task_pid_nr(current), current->comm,
...
static int __init binder_init(void)
{
	...
	if (IS_ERR(th)) {
		pr_info("fail to create watchdog thread "
				 "(err:%li)\n", PTR_ERR(th));
	} else {
		wake_up_process(th);
	}
