...
int ion_debug_show_backtrace(struct ion_record_basic_info *tracking_info,
			     unsigned int show_backtrace_type)
{
	unsigned int i = 0;
	...
	for (i = 0; i < backtrace_count; i++) {
		...
		ion_get_backtrace_info(tracking_info, tmpString, stringCount, i,
				       show_backtrace_type);
...
char *ion_get_backtrace_info(struct ion_record_basic_info *tracking_info, char *backtrace_string,
			     unsigned int backtrace_string_len, unsigned int backtrace_index,
			     unsigned int show_backtrace_type)
{
	...
	if (tracking_info == NULL) {
		printk(ION_DEBUG_ERROR
		       "[ion_get_backtrace_info]ERROR input tracking_info is NULL\n");
		return NULL;
	}
	...
	if ((tmpBacktrace == NULL) || (tmpBacktrace->numEntries <= 0)) {
		printk(ION_DEBUG_ERROR
		       "[ion_get_backtrace_info]ERROR tmpBacktrace is NULL or tmpBacktrace->numEntries <=0\n");
		return NULL;
	} else {
	...
	if (backtrace_info == USER_BACKTRACE) {
		if (tmpMapping != NULL) {
			...
			if (backtrace_mapping != NULL) {
				snprintf(backtrace_string, backtrace_string_len,
					 "USER[%d] addr: 0x%x mapping address: 0x%x - 0x%x lib: %s\n",
					 backtrace_index, (unsigned int)backtrace[backtrace_index],
					 backtrace_mapping[backtrace_index].address,
					 (backtrace_mapping[backtrace_index].address +
					  backtrace_mapping[backtrace_index].size - 1),
					 backtrace_mapping[backtrace_index].name);
				return backtrace_string;
			} else {
				printk(ION_DEBUG_ERROR
				       "[ion_get_backtrace_info]ERROR backtrace_mapping is NULL\n");
			}
		} else {
			printk(ION_DEBUG_ERROR
			       "[ion_get_backtrace_info]ERROR tmpMapping is NULL\n");
		}
		...
		if (tmpMapping != NULL) {
			...
			if (backtrace_symbol != NULL) {
				snprintf(backtrace_string, backtrace_string_len,
					 "KERNEL[%d] addr: 0x%x symbol: %s\n", backtrace_index,
					 (unsigned int)backtrace[backtrace_index],
					 (char *)*(backtrace_symbol + backtrace_index));
				/* snprintf(backtrace_string,backtrace_string_len,
				   "KERNELSPACE BACKTRACE[%d]2 address: 0x%x\n",
...
void *ion_get_list(unsigned int record_type, void *record, unsigned int list_type)
{
	...
	if (record_type == LIST_BUFFER) {
		...
		case BUFFER_ALLOCATION_LIST:
			{
				printk(ION_DEBUG_INFO "[ion_get_list return]buffer using list %x\n",
				       (unsigned int)buffer_record->buffer_using_list);
				ion_debug_show_buffer_usage_record(buffer_record->
								   buffer_using_list);
		...
		case ADDRESS_ALLOCATION_LIST:
			{
				printk(ION_DEBUG_INFO
				       "[ion_get_list return] address using list %x\n",
				       (unsigned int)buffer_record->address_using_list);
				ion_debug_show_address_usage_record(buffer_record->
								    address_using_list);
		...
		case ADDRESS_FREE_LIST:
			{
				printk(ION_DEBUG_INFO
				       "[ion_get_list return] address freed list %x\n",
				       (unsigned int)buffer_record->address_freed_list);
				ion_debug_show_address_usage_record(buffer_record->
								    address_freed_list);
		...
		case FD_ALLOCATION_LIST:
			{
				printk(ION_DEBUG_INFO
				       "[ion_get_list return]process fd using list %x\n",
				       (unsigned int)process_record->fd_using_list);
		...
		case FD_FREE_LIST:
			{
				printk(ION_DEBUG_INFO
				       "[ion_get_list return]process fd free list %x\n",
				       (unsigned int)process_record->fd_freed_list);
		...
		case ADDRESS_ALLOCATION_LIST:
			{
				printk(ION_DEBUG_INFO
				       "[ion_get_list return]process address using list %x\n",
				       (unsigned int)process_record->address_using_list);
				ion_debug_show_address_usage_record(process_record->
								    address_using_list);
		...
		case ADDRESS_FREE_LIST:
			{
				printk(ION_DEBUG_INFO
				       "[ion_get_list return]process  address freed list %x\n",
				       (unsigned int)process_record->address_freed_list);
				ion_debug_show_address_usage_record(process_record->
								    address_freed_list);
	...
	printk(ION_DEBUG_ERROR
	       "[ion_get_list_from_buffer]can't find corresponding record in list_type %d record_type %x\n",
...
struct ion_process_record *ion_get_inuse_process_usage_record2(void)
{
	...
	if (tmp_process != NULL) {
		printk(ION_DEBUG_INFO
		       "[ion_get_inuse_process_record]return process_record  %x pid %d\n",
		...
		printk(ION_DEBUG_WARN
		       "[ion_get_inuse_process_recoed] tmp_process is null process_created_list is %x\n",
...
struct ion_client_usage_record *ion_get_inuse_client_record(void)
{
	...
	pr_info("[ion_get_inuse_client_record]return client_record  %x\n", (unsigned int)tmp_client);
	if (tmp_client != NULL) {
...
struct ion_client_usage_record *ion_get_freed_client_record(void)
{
	...
	pr_info(ION_DEBUG_INFO "[ion_get_inuse_client_record]return client_record  %x\n",
	       (unsigned int)tmp_client);
	if (tmp_client != NULL) {
		printk(ION_DEBUG_INFO "[ion_get_inuse_client_record]return client %x\n",
		       (unsigned int)tmp_client);
...
unsigned int ion_get_data_from_record(void *record, unsigned int data_type)
{
	...
	struct ion_record_basic_info *tracking_info =
	    (struct ion_record_basic_info *)(tmp_record + 1);
	switch (data_type) {
	case RECORD_ID:
		{
	...
	case RECORD_HANDLE:
		{
			if (tracking_info->record_type == NODE_BUFFER) {
				struct ion_buffer_usage_record *buffer_node =
				    (struct ion_buffer_usage_record *)record;
	...
	case RECORD_FD:
		{
			if (tracking_info->record_type == NODE_FD) {
				struct ion_fd_usage_record *fd_node =
				    (struct ion_fd_usage_record *)record;
				return (unsigned int)fd_node->fd;
			} else if (tracking_info->record_type == NODE_MMAP) {
				struct ion_address_usage_record *address_node =
				    (struct ion_address_usage_record *)record;
	...
	default:
		{
			printk(ION_DEBUG_ERROR
			       "[ion_get_data_from_record]can't find data type (%d)error\n",
			       data_type);
			return 0;
		}
	}
	printk(ION_DEBUG_INFO
	       "[ion_get_data_from_record]get data type %d but wrong record type %d\n", data_type,
	       tracking_info->record_type);
	return 0;
}

struct ion_buffer_record *search_record_in_list(struct ion_buffer *buffer,
						struct ion_buffer_record *list)
{
	...
	if (buffer_created_list != NULL) {
		while (tmp_buffer_record != NULL) {
			if (tmp_buffer_record->buffer_address == buffer) {
				printk(ION_DEBUG_INFO
				       "               found record tmp_buffer_record: 0x%x\n",
		...
		printk(ION_DEBUG_INFO
		       "		[search_record_in_list]can't get corresponding buffer %x in buffer list %x\n",
		       (unsigned int)buffer, (unsigned int)list);
	} else {
		printk(ION_DEBUG_INFO
		       "		[search_record_in_list]buffer_created_list is null\n");
	}
	return NULL;
}
...
struct ion_process_record *search_process_in_list(pid_t pid, struct ion_process_record *list)
{
	...
	if (process_created_list != NULL) {
		while (tmp_process_record != NULL) {
			printk(ION_DEBUG_INFO
			       "               tmp_process_record: 0x%x pid %d count %d\n",
			       (unsigned int)tmp_process_record, tmp_process_record->pid,
			       tmp_process_record->count);
			if (tmp_process_record->pid == pid) {
				printk(ION_DEBUG_INFO
				       "               found record tmp_process_record: 0x%x\n",
		...
		printk(ION_DEBUG_INFO
		       "        [search_process_in_list]can't get corresponding process %x in process list %x\n",
		       (unsigned int)pid, (unsigned int)list);
	} else {
		printk(ION_DEBUG_INFO
		       "        [search_process_in_list]process_created_list is null\n");
	}
	return NULL;
}

void get_kernel_symbol(unsigned long *backtrace, unsigned int numEntries,
		       unsigned int *kernel_symbol)
{
	unsigned int i = 0;
	...
	for (i = 0; i < numEntries; i++) {
		...
		*(kernel_symbol + i) =
		    (unsigned int)get_kernelString_from_hashTable(symbol, strlen(symbol));
...
unsigned int get_kernel_backtrace(unsigned long *backtrace)
{
	...
	if (trace.nr_entries > 0) {
		for (i = 0; i < trace.nr_entries; i++) {
			printk(ION_DEBUG_INFO "bactrace[%d] : %x  ", i,
			       (unsigned int)trace.entries[i]);
...
void *find_node_in_list(pid_t pid, unsigned int client_address, unsigned int data,
			unsigned int **previous_node, unsigned int *list, unsigned int search_type,
			unsigned int node_type)
{
	...
	while (current_node != NULL) {
		if ((search_type == SEARCH_PROCESS_PID) && (node_type == LIST_PROCESS)
		    && (client_address == 0)) {
			process_tmp = (struct ion_process_record *)(current_node);
			printk(ION_DEBUG_INFO
			       "            [find_node_in_list]curent_node is %x process_tmp->pid is %d process_tmp->count is %d\n",
		...
		} else if ((search_type == SEARCH_BUFFER) && (node_type == LIST_BUFFER)
			   && (client_address == 0)) {
			buffer_tmp = (struct ion_buffer_record *)(current_node);
			printk(ION_DEBUG_INFO
			       "            [find_node_in_list]curent_node is %x buffer_tmp->buffer_address is 0x%x data 0x%x\n",
			       (unsigned int)current_node, (unsigned int)buffer_tmp->buffer_address,
			       (unsigned int)data);
			...
			printk(ION_DEBUG_INFO
			       "            [find_node_in_list]current_node is %x record_ID_tmp %x record_ID_tmp->pid %d record_ID_tmp->client_address %x\n",
			       (unsigned int)current_node, (unsigned int)record_ID_tmp,
			       record_ID_tmp->recordID.pid, record_ID_tmp->recordID.client_address);
			if ((search_type == SEARCH_PID_CLIENT)
			    && (record_ID_tmp->recordID.pid == pid)
			    && (record_ID_tmp->recordID.client_address == client_address)) {
				/* FIXME client may use the same buffer twice? */
				*previous_node = prev_node;
				return current_node;
			} else if ((search_type == SEARCH_PID)
				   && (record_ID_tmp->recordID.pid == pid)) {
				if (node_type == NODE_FD) {
					struct ion_fd_usage_record *tmp =
					    (struct ion_fd_usage_record *)current_node;
					if (tmp->fd == data) {
						*previous_node = prev_node;
						return current_node;
					}
				} else if (node_type == NODE_MMAP) {
					struct ion_address_usage_record *tmp =
					    (struct ion_address_usage_record *)current_node;
	...
	printk(ION_DEBUG_INFO
	       "              [find_node_in_list]can't find node in list. search_type %d node_type %d pid %d client_address %x\n",
	       search_type, node_type, pid, (unsigned int)client_address);
	return NULL;
}

void *move_node_to_freelist(pid_t pid, unsigned int client_address, unsigned int data,
			    unsigned int **from, unsigned int **to, unsigned int search_type,
			    unsigned int node_type)
{
	unsigned int *previous_node = NULL;
	unsigned int *found_node;
	printk(ION_DEBUG_INFO
	       "          [move_node_to_freelist]pid %d client_address %x from %x to %x search_type %d node_type %d\n",
	       (int)pid, (unsigned int)client_address, (unsigned int)*from, (unsigned int)*to,
	       search_type, node_type);
	found_node =
	    find_node_in_list(pid, client_address, data, (unsigned int **)&previous_node,
			      (unsigned int *)*from, search_type, node_type);
	printk(ION_DEBUG_INFO
	       "          [move_node_to_freelist] found_node %x previous_node is %x *to %x\n",
	...
	if (found_node != NULL) {
		...
		*to = found_node;
		printk(ION_DEBUG_INFO
		       "          [move_node_to_freelist]from list is %x to list is %x found_node is %x\n",
		       (unsigned int)*from, (unsigned int)*to, (unsigned int)found_node);
		return found_node;
	} else {
		printk(ION_DEBUG_ERROR
		       "          [move_node_to_freelist]can't found node in list %x: node info pid %d client address %x\n",
...
{
	static unsigned int buffer_node_size;
	static unsigned int process_node_size;
	static unsigned int list_buffer_size;
	static unsigned int fd_node_size;
	static unsigned int client_node_size;
	static unsigned int mmap_node_size;
	static unsigned int total_size;
	switch (type) {
	case LIST_BUFFER:
		{
			if (!list_buffer_cache_created) {
				ion_list_buffer_cachep =
				    kmem_cache_create("buffer_record",
						      sizeof(struct ion_buffer_record), 0,
			...
			if (ion_list_buffer_cachep != NULL) {
				...
				new_buffer_record =
				    ...
				    kmem_cache_alloc(ion_list_buffer_cachep, GFP_KERNEL);
				if (new_buffer_record != NULL) {
					/* assign data into buffer record */
					new_buffer_record->buffer_address = record_param->buffer;
					new_buffer_record->buffer =
					    get_record(HASH_NODE_BUFFER, record_param);
					new_buffer_record->heap_type =
					    record_param->buffer->heap->type;
					if (new_buffer_record->heap_type != ION_HEAP_TYPE_CARVEOUT) {
						new_buffer_record->priv_virt =
						    record_param->buffer->priv_virt;
					} else {
						new_buffer_record->priv_phys =
						    record_param->buffer->priv_phys;
					}
					...
					list_buffer_size +=
					    (unsigned int)sizeof(struct ion_buffer_record);
					total_size +=
					    (unsigned int)sizeof(struct ion_buffer_record);
					pr_alert(KERN_ALERT
					       "list_buffer[%d] buffer_node[%d] client_node[%d] fd_node[%d] mmap_node[%d] process_node[%d] total_size[%d]\n",
					       list_buffer_size, buffer_node_size, client_node_size,
					       fd_node_size, mmap_node_size, process_node_size,
					       total_size);

					return (void *)new_buffer_record;
				} else {
					pr_alert(KERN_ALERT
					       "can't get enough memory for LIST_BUFFER structure");
					return (void *)NULL;
				}
	...
	case LIST_PROCESS:
		{
			if (!list_process_cache_created) {
				ion_list_process_cachep =
				    kmem_cache_create("process_record",
						      sizeof(struct ion_process_record), 0,
						      SLAB_HWCACHE_ALIGN, NULL);
			...
			if (ion_list_process_cachep != NULL) {
				...
				process_record =
				    ...
				    kmem_cache_alloc(ion_list_process_cachep, GFP_KERNEL);
				if (process_record != NULL) {
					...
					process_node_size +=
					    (unsigned int)sizeof(struct ion_process_record);
					total_size +=
					    (unsigned int)sizeof(struct ion_process_record);
					pr_alert(KERN_ALERT
					       "list_buffer[%d] buffer_node[%d] client_node[%d] fd_node[%d] mmap_node[%d] process_node[%d] total_size[%d]\n",
					...
					pr_alert(KERN_ALERT
					       "can't get enough memory for LIST_PROCESS structure");
					return (void *)NULL;
				}
			}
			break;
	...
	case NODE_BUFFER:
		{
			if (!buffer_cache_created) {
				ion_buffer_usage_cachep =
				    kmem_cache_create("buffer_usage_record",
						      sizeof(ion_buffer_usage_record_t), 0,
						      SLAB_HWCACHE_ALIGN, NULL);
			...
			if (ion_buffer_usage_cachep != NULL) {
				...
				pr_alert(KERN_ALERT
				       "list_buffer[%d] buffer_node[%d] client_node[%d] fd_node[%d] mmap_node[%d] process_node[%d] total_size[%d]\n",
				       list_buffer_size, buffer_node_size, client_node_size,
				       fd_node_size, mmap_node_size, process_node_size, total_size);

				return (void *)kmem_cache_alloc(ion_buffer_usage_cachep,
								GFP_KERNEL);
			}
			break;
		}
	case NODE_FD:
		{
			if (!fd_cache_created) {
				ion_fd_usage_cachep =
				    kmem_cache_create("fd_record", sizeof(ion_fd_usage_record_t), 0,
						      SLAB_HWCACHE_ALIGN, NULL);
	...
	case NODE_CLIENT:
		{
			if (!client_cache_created) {
				ion_client_usage_cachep =
				    kmem_cache_create("client_record",
						      sizeof(ion_client_usage_record_t), 0,
						      SLAB_HWCACHE_ALIGN, NULL);
			...
			if (ion_client_usage_cachep != NULL) {
				...
				/* pr_info("allocate ion_client_usage_record_t (%d), tmp=%x tmp size = %d\n",
	...
	case NODE_MMAP:
		{
			if (!address_cache_created) {
				ion_address_usage_cachep =
				    kmem_cache_create("address_record",
						      sizeof(ion_address_usage_record_t), 0,
						      SLAB_HWCACHE_ALIGN, NULL);
			...
			if (ion_address_usage_cachep != NULL) {
				...
				return (void *)kmem_cache_alloc(ion_address_usage_cachep,
								GFP_KERNEL);
			}
			break;
		}
...
static ObjectEntry *find_entry(ObjectTable *table, unsigned int slot, void *object,
...
static ObjectEntry *find_mapping_entry(ObjectTable *table, unsigned int slot, void *object,
				       unsigned int numEntries)
{
	ObjectEntry *entry = table->slots[slot];
	while (entry != NULL) {
...
static ObjectEntry *find_buffer_entry(ObjectTable *table, unsigned int slot,
				      struct ion_buffer *object)
{
	ObjectEntry *entry = table->slots[slot];
	while (entry != NULL) {
		struct ion_buffer *tmp_buffer = (struct ion_buffer *)entry->object;
		if ((tmp_buffer->size == object->size) && (tmp_buffer->heap == object->heap)
		    && (entry->object == (void *)object)) {
...
static StringEntry *find_string_entry(StringTable *table, unsigned int slot, char *string_name,
...
char *get_string(char *string_name, unsigned int len, StringTable *table)
{
	...
	static unsigned int string_struct_size;
	...
	if (entry != NULL) {
		...
		table->count++;
		pr_alert(KERN_ALERT "string_struct_size [%d] string_size[%d]\n", string_struct_size,
		       string_size);
...
void *get_record(unsigned int type, ion_sys_record_t *param)
{
	...
	if (tmp != NULL) {
		...
		case HASH_NODE_BUFFER:
			{
				hash =
				    (unsigned int)
				    hash_32(((unsigned int)param->buffer + param->buffer->size +
					     param->buffer->heap->type), 16);
				slot = hash % OBJECT_TABLE_SIZE;
				...
				if (entry != NULL) {
					...
					entry =
					    kmalloc(sizeof(ObjectEntry) + sizeof(struct ion_buffer),
						    GFP_KERNEL);
					entry->reference = 1;
					entry->prev = NULL;
					entry->slot = slot;
					...
					memcpy(entry->object, param->buffer,
					       sizeof(struct ion_buffer));
		...
		case HASH_NODE_USER_BACKTRACE:
			{
				...
				slot = hash % OBJECT_TABLE_SIZE;
				entry =
				    find_entry(&gUserBtTable, slot, (void *)param->backtrace,
					       param->backtrace_num);
				if (entry != NULL) {
					entry->reference++;
				} else {
					entry =
					    kmalloc(sizeof(ObjectEntry) +
						    (param->backtrace_num * sizeof(unsigned int)),
						    GFP_KERNEL);
					entry->reference = 1;
					entry->prev = NULL;
					entry->slot = slot;
					...
					memcpy(entry->object, &(param->backtrace[0]),
					       entry->numEntries * sizeof(unsigned int));
		...
		case HASH_NODE_KERNEL_BACKTRACE:
			{
				...
				slot = hash % OBJECT_TABLE_SIZE;
				entry =
				    find_entry(&gKernelBtTable, slot, (void *)param->backtrace,
					       param->backtrace_num);
				if (entry != NULL) {
					entry->reference++;
				} else {
		...
		case HASH_NODE_USER_MAPPING:
			{
				hash =
				    get_mapping_hash(&(param->mapping_record[0]),
						     param->backtrace_num);
				slot = hash % OBJECT_TABLE_SIZE;
				entry =
				    find_mapping_entry(&gUserMappingTable, slot,
						       (void *)&(param->mapping_record[0]),
						       param->backtrace_num);
				if (entry != NULL) {
					entry->reference++;
				} else {
					entry =
					    kmalloc(sizeof(ObjectEntry) +
						    (param->backtrace_num * sizeof(struct mapping)),
						    GFP_KERNEL);
					entry->reference = 1;
					entry->prev = NULL;
					entry->slot = slot;
					...
					entry->numEntries = param->backtrace_num;
					memcpy(entry->object, &(param->mapping_record[0]),
					       entry->numEntries * sizeof(struct mapping));
		...
		case HASH_NODE_KERNEL_SYMBOL:
			{
				...
				slot = hash % OBJECT_TABLE_SIZE;
				entry =
				    find_entry(&gKernelSymbolTable, slot,
					       (void *)param->kernel_symbol, param->backtrace_num);
				if (entry != NULL) {
					entry->reference++;
				} else {
		...
		default:
			{
				printk(ION_DEBUG_ERROR
				       "[get_record][ERROR] get_record error type %d",
				       type);
				break;
			}
		}
...
void *create_new_record_into_process(ion_sys_record_t *record_param,
				     struct ion_process_record *process_record,
				     unsigned int node_type, unsigned int from_kernel)
{
	unsigned int *new_node = NULL;
	struct ion_client *tmp = NULL;
	new_node = (void *)allocate_record(node_type, record_param);
	printk
	    ("[%d][%d][create_new_record_into_process(%d)] process_record 0x%x node_type %d new node %x\n",
	     record_param->pid, record_param->group_id, from_kernel, (unsigned int)process_record,
	     node_type, (unsigned int)new_node);
	...
	if (new_node != NULL) {
		...
		if (!from_kernel) {
			...
			tracking_info->allocate_backtrace =
			    (unsigned int *)get_record(HASH_NODE_USER_BACKTRACE, record_param);
			tracking_info->allocate_map =
			    (unsigned int *)get_record(HASH_NODE_USER_MAPPING, record_param);
		} else {
			tracking_info->allocate_backtrace_type = KERNEL_BACKTRACE;
			tracking_info->allocate_backtrace =
			    (unsigned int *)get_record(HASH_NODE_KERNEL_BACKTRACE, record_param);
			tracking_info->allocate_map =
			    (unsigned int *)get_record(HASH_NODE_KERNEL_SYMBOL, record_param);
		}
		...
		case NODE_MMAP:
			{
				struct ion_address_usage_record *tmp =
				    (struct ion_address_usage_record *)new_node;
				if (from_kernel)
				...
				insert_node_to_list((void **)&process_record->address_using_list,
		...
		case NODE_FD:
			{
				struct ion_fd_usage_record *tmp =
				    (struct ion_fd_usage_record *)new_node;
				tmp->fd = record_param->fd;
				tmp->buffer = record_param->buffer;
				tmp->handle = record_param->handle;
				/* add new node into allocate list */
				mutex_lock(&process_record->ion_fd_usage_mutex);
				insert_node_to_list((void **)&process_record->fd_using_list,
						    (unsigned int *)new_node);
...
void *create_new_record_into_list(ion_sys_record_t *record_param, struct ion_buffer_record *buffer,
				  unsigned int node_type, unsigned int from_kernel)
{
	unsigned int *new_node = NULL;
	struct ion_client *tmp = NULL;
	...
	if (new_node != NULL) {
		...
		case NODE_BUFFER:
			{
				struct ion_buffer_usage_record *tmp =
				    (struct ion_buffer_usage_record *)new_node;
				tmp->handle = record_param->handle;	/* FIXME it should be stored in hash table */
				/* add new node into allocate list */
				mutex_lock(&buffer->ion_buffer_usage_mutex);
				insert_node_to_list((void **)&buffer->buffer_using_list,
						    (unsigned int *)new_node);
		...
		case NODE_MMAP:
			{
				...
				insert_node_to_list((void **)&buffer->address_using_list,
					(unsigned int *)new_node);	/* FIXME */
		...
		case NODE_CLIENT:
			{
				...
				struct ion_client_usage_record *tmp =
				    (struct ion_client_usage_record *)new_node;
				if (!from_kernel)
					tmp->fd = record_param->fd;

				/* add new node into allocate list */
				mutex_lock(&client_usage_mutex);
				insert_node_to_list((void **)&client_using_list,
						    (unsigned int *)new_node);
...
unsigned int record_release_backtrace(ion_sys_record_t *record_param,
				      struct ion_record_basic_info *tracking_info,
...
{
	pr_info("[%d][%d][FUNCTION(%d)_%d][record_ion_info].\n",
		...
		case ION_FUNCTION_DESTROY_CLIENT:
			{
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_CLOSE(%d)]  client_freed_list %x\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       (unsigned int)client_freed_list);

				/* move client node from allocate list into free list */
				mutex_lock(&client_usage_mutex);
				found_node =
				    move_node_to_freelist(record_param->pid,
							  (unsigned int)record_param->client, 0,
							  (unsigned int **)&client_using_list,
							  (unsigned int **)&client_freed_list,
							  SEARCH_PID_CLIENT, NODE_CLIENT);
				mutex_unlock(&client_usage_mutex);
				if (found_node != NULL) {
					record_release_backtrace(record_param,
								 &(found_node->tracking_info),
								 from_kernel);
				} else {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_CLOSE(%d)]ERROR can't find client: 0x%x in client_using_list %x\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)record_param->client,
					       (unsigned int)client_using_list);
					break;
				}
				if (from_kernel == 0) {
					...
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_CLOSE(%d)]  DONE prepare to move process %d into destroyed list %x\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->pid,
					       (unsigned int)process_destroyed_list);
					mutex_lock(&process_lifecycle_mutex);
					move_node_to_freelist(record_param->pid, 0, 0,
							      (unsigned int **)
							      &process_created_list,
							      (unsigned int **)
							      &process_destroyed_list,
							      SEARCH_PROCESS_PID, LIST_PROCESS);
					mutex_unlock(&process_lifecycle_mutex);
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_CLOSE(%d)]  DONE process_destroyed_list %x\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)process_destroyed_list);
				}
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_CLOSE(%d)]  DONE client_freed_list %x\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       (unsigned int)client_freed_list);
		...
		case ION_FUNCTION_ALLOC_CONT:
			{
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_ALLOC(%d)_%d]buffer: 0x%x\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       record_param->action, (unsigned int)record_param->buffer);
				if (record_param->buffer == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_ALLOC(%d)_%d]ERROR!!! buffer: 0x%x is NULL\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->action,
					       (unsigned int)record_param->buffer);
					break;
				}
				/* create buffer node */
				new_buffer_record =
				    (struct ion_buffer_record *)allocate_record(LIST_BUFFER,
										record_param);
				if (new_buffer_record != NULL) {
					/* assign data into buffer usage record */
					new_node =
					    (struct ion_buffer_usage_record *)
					    create_new_record_into_list(record_param,
									new_buffer_record,
									NODE_BUFFER, from_kernel);
					if (new_node != NULL)
						new_node->function_type = ION_FUNCTION_ALLOC;

				} else {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_ALLOC(%d)_%d]ERROR!!! buffer: 0x%x can't get  enough memory for create LIST_BUFFER structure\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->action,
					       (unsigned int)record_param->buffer);
					break;
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_ALLOC(%d)_%d] DONE create buffer_usage_record : 0x%x buffer_created_list: 0x%x new buffer record: 0x%x buffer_using_list: 0x%x\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       record_param->action, (unsigned int)new_node,
				       (unsigned int)buffer_created_list,
				       (unsigned int)new_buffer_record,
				       (unsigned int)new_buffer_record->buffer_using_list);
				break;
			}
		case ION_FUNCTION_IMPORT:
			{
				...
				struct ion_buffer_usage_record *new_node = NULL;
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_IMPORT(%d)]input buffer: 0x%x fd %d\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       (unsigned int)record_param->buffer,
				       (unsigned int)record_param->fd);
				if (record_param->buffer == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_IMPORT(%d)]ERROR!!! buffer: 0x%x is NULL\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)record_param->buffer);
				}
				mutex_lock(&buffer_lifecycle_mutex);
				buffer_record =
				    search_record_in_list(record_param->buffer,
							  buffer_created_list);
				mutex_unlock(&buffer_lifecycle_mutex);
				if (buffer_record == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_IMPORT(%d)]can't found corresponding buffer 0x%x in buffer created list %x",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)record_param->buffer,
					       (unsigned int)buffer_created_list);
					break;
				...
				new_node =
				    (struct ion_buffer_usage_record *)
				    create_new_record_into_list(record_param, buffer_record,
								NODE_BUFFER, from_kernel);
				if (new_node != NULL)
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_IMPORT(%d)]DONE new_node %x buffer_using_list%x\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       (unsigned int)new_node,
				       (unsigned int)buffer_record->buffer_using_list);
				break;
		...
		case ION_FUNCTION_FREE:
			{
				...
				struct ion_buffer_usage_record *found_node = NULL;
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_FREE(%d)]input buffer: 0x%x\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       (unsigned int)record_param->buffer);
				if (record_param->buffer == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_FREE(%d)]ERROR!!! buffer: 0x%x is NULL\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)record_param->buffer);
				}
				...
				if (buffer_record == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_FREE(%d)]can't found corresponding buffer 0x%x in buffer created list %x",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)record_param->buffer,
					       (unsigned int)buffer_created_list);
					break;
				...
				if (found_node != NULL) {
					...
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_FREE(%d)]can't found corresponding buffer usage record client 0x%x in buffer using list 0x%x",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)record_param->client,
					       (unsigned int)buffer_record->buffer_using_list);
					break;
				...
				if (buffer_record->buffer_using_list == NULL) {
					mutex_lock(&buffer_lifecycle_mutex);
					move_node_to_freelist(record_param->pid, 0,
							      (unsigned int)record_param->buffer,
							      (unsigned int **)&buffer_created_list,
							      (unsigned int **)
							      &buffer_destroyed_list, SEARCH_BUFFER,
							      LIST_BUFFER);
					mutex_unlock(&buffer_lifecycle_mutex);
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_FREE(%d)]move buffer record %xfrom buffer_created_list %x to buffer_destroyed_list %x\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)buffer_record,
					       (unsigned int)buffer_created_list,
					       (unsigned int)buffer_destroyed_list);

				}
				/* count total buffer free list if buffer free list is full.
					remove olderest buffer record */
				destroyed_buffer_count++;	/* FIXME waiting for real case */
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_FREE(%d)] DONE buffer_using_list %x buffer_freed_list %x\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       (unsigned int)buffer_record->buffer_using_list,
				       (unsigned int)buffer_record->buffer_freed_list);
		...
		case ION_FUNCTION_MMAP:
			{
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_MMAP(%d)]input buffer: 0x%x mapping_address %x length %d\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       (unsigned int)record_param->buffer,
				       (unsigned int)record_param->address, record_param->length);
				if ((record_param->buffer == NULL) && (record_param->fd != 0)
				    && !from_kernel) {
					...
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_MMAP(%d)]get buffer from fd %d input buffer: 0x%x\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->fd,
					       (unsigned int)record_param->buffer);
				}
				if (record_param->buffer == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_MMAP(%d)]ERROR!!! buffer: 0x%x is NULL\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)record_param->buffer);
					break;
				}
				if (from_kernel) {
					...
					if (buffer_record == NULL) {
						printk(ION_DEBUG_ERROR
						       "[%d][%d][ION_FUNCTION_MMAP(%d)]ERROR !!! can't found corresponding buffer 0x%x in buffer created list %x",
						       record_param->pid, record_param->group_id,
						       from_kernel,
						       (unsigned int)record_param->buffer,
						       (unsigned int)buffer_created_list);
					...
					new_node =
					    ...
					    create_new_record_into_list(record_param, buffer_record,
									NODE_MMAP, from_kernel);
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_MMAP(%d)] DONE new node:  0x%x in buffer record address mapping 0x%x size %d address using list: 0x%x\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)new_node,
					       new_node->mapping_address, new_node->size,
					       (unsigned int)buffer_record->address_using_list);
				} else {
					/* find corresponding process in created process list */
					mutex_lock(&process_lifecycle_mutex);
					process_record =
					    search_process_in_list(record_param->pid,
								   process_created_list);
					mutex_unlock(&process_lifecycle_mutex);
					if (process_record == NULL) {
						printk(ION_DEBUG_ERROR
						       "[%d][%d][ION_FUNCTION_MMAP(%d)]ERROR !!! can't found corresponding process %d in process created list %x",
						       record_param->pid, record_param->group_id,
						       from_kernel, record_param->pid,
						       (unsigned int)process_created_list);
						break;
					...
					new_node2 =
					    (struct ion_address_usage_record *)
					    create_new_record_into_process(record_param,
									   process_record,
									   NODE_MMAP, from_kernel);
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_MMAP(%d)] DONE new node 0x%x in process record address mapping 0x%x size %d address using list 0x%x\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)new_node2,
					       (unsigned int)new_node2->mapping_address,
					       new_node2->size,
					       (unsigned int)process_record->address_using_list);
				}
				break;
		...
		case ION_FUNCTION_MUNMAP:
			{
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_MUNMAP(%d)]input buffer: 0x%x address 0x%x size %d\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       (unsigned int)record_param->buffer, record_param->address,
				       record_param->length);
				if (from_kernel) {
					mutex_lock(&buffer_lifecycle_mutex);
					buffer_record =
					...
					if (buffer_record == NULL) {
						printk(ION_DEBUG_ERROR
						       "[%d][%d][ION_FUNCTION_MUNMAP(%d)]ERROR !!! can't found corresponding buffer 0x%x in buffer created list %x\n",
						       record_param->pid, record_param->group_id,
						       from_kernel,
						       (unsigned int)record_param->buffer,
						       (unsigned int)buffer_created_list);
					...
					found_node =
					    move_node_to_freelist(record_param->pid,
								  (unsigned int)record_param->
								  client, record_param->address,
								  (unsigned int **)&(buffer_record->
										     address_using_list),
								  (unsigned int **)&(buffer_record->
										     address_freed_list),
								  SEARCH_PID, NODE_MMAP);
					...
					if (found_node != NULL) {
						...
						printk(ION_DEBUG_ERROR
						       "[%d][%d][ION_FUNCTION_MUNMAP(%d)]can't found corresponding buffer usage record client 0x%x in address using list 0x%x\n",
						       record_param->pid, record_param->group_id,
						       from_kernel,
						       (unsigned int)record_param->client,
						       (unsigned int)buffer_record->
						       address_using_list);
						break;
					...
					if (process_record == NULL) {
						printk(ION_DEBUG_ERROR
						       "[%d][%d][ION_FUNCTION_MUNMAP(%d)]ERROR !!! can't found corresponding process pid %d in process created list %x\n",
						       record_param->pid, record_param->group_id,
						       from_kernel, record_param->pid,
						       (unsigned int)process_created_list);
						break;
					...
					if (found_node2 != NULL) {
						...
						printk(ION_DEBUG_ERROR
						       "[%d][%d][ION_FUNCTION_MUNMAP(%d)]can't found corresponding address usage record process 0x%d in address using list 0x%x\n",
						       record_param->pid, record_param->group_id,
						       from_kernel, record_param->pid,
						       (unsigned int)process_record->
						       address_using_list);
					...
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_MUNMAP(%d)] DONE client %x process->address_using_list %x process->address_free_list %x\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)record_param->client,
					       (unsigned int)process_record->address_using_list,
					       (unsigned int)process_record->address_freed_list);
		...
		case ION_FUNCTION_SHARE_CLOSE:
			{
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_SHARE_CLOSE(%d)]input buffer: 0x%x fd %d\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       (unsigned int)record_param->buffer, record_param->fd);
				mutex_lock(&process_lifecycle_mutex);
				process_record =
				    search_process_in_list(record_param->pid, process_created_list);
				mutex_unlock(&process_lifecycle_mutex);
				if (process_record == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_SHARE_CLOSE(%d)]ERROR !!!can't found corresponding process %d in process created list %x\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)record_param->pid,
					       (unsigned int)buffer_created_list);
					break;
				}
				...
				if (found_node != NULL) {
					...
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_SHARE_CLOSE(%d)]can't found corresponding process  record client 0x%x in fd using list 0x%x\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, (unsigned int)record_param->client,
					       (unsigned int)process_record->fd_using_list);
					break;
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_SHARE_CLOSE(%d)] client %x fd_using_list %x fd_freed_list %x\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       (unsigned int)record_param->client,
				       (unsigned int)process_record->fd_using_list,
				       (unsigned int)process_record->fd_freed_list);
	...
	pr_info("[%d][%d][FUNCTION(%d)_%d][record_ion_info] DONE\n", record_param->pid,
	       record_param->group_id, from_kernel, record_param->action);
	return 1;
}
