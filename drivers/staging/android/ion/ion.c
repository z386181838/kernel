...
static int ion_debug_kern_rec(struct ion_client *client,
			      ...
			      unsigned int address, unsigned length, int fd);

bool ion_buffer_fault_user_mappings(struct ion_buffer *buffer)
{
	return (buffer->flags & ION_FLAG_CACHED) && !(buffer->flags & ION_FLAG_CACHED_NEEDS_SYNC);
}
...
static struct ion_handle *ion_handle_create(struct ion_client *client, struct ion_buffer *buffer)
{
	struct ion_handle *handle;
...
static struct ion_handle *ion_handle_lookup(struct ion_client *client, struct ion_buffer *buffer)
{
	     ...
	     ion_phys_addr_t *addr, size_t *len)
{
	struct ion_buffer *buffer;
	int ret;
	MMProfileLogEx(ION_MMP_Events[PROFILE_GET_PHYS], MMProfileFlagStart, (unsigned int)client,
		       (unsigned int)handle);
...
static void ion_buffer_kmap_put(struct ion_buffer *buffer)
{
	buffer->kmap_cnt--;
	if (!buffer->kmap_cnt) {
		MMProfileLogEx(ION_MMP_Events[PROFILE_UNMAP_KERNEL], MMProfileFlagStart,
			       buffer->size, 0);
		buffer->heap->ops->unmap_kernel(buffer->heap, buffer);
		MMProfileLogEx(ION_MMP_Events[PROFILE_UNMAP_KERNEL], MMProfileFlagEnd, buffer->size,
			       0);
		buffer->vaddr = NULL;
	}
}
...
void ion_unmap_kernel(struct ion_client *client, struct ion_handle *handle)
{
	...
	ion_debug_kern_rec(client, handle->buffer, handle, ION_FUNCTION_MUNMAP,
			   ADDRESS_KERNEL_VIRTUAL, (unsigned int)handle->buffer->vaddr,
...
static int ion_get_client_serial(const struct rb_root *root, const unsigned char *name)
{
	int serial = -1;
	struct rb_node *node;
...
struct ion_client *__ion_client_create(struct ion_device *dev, const char *name)
{
	struct ion_client *client;
	struct task_struct *task;
	struct rb_node **p;
	...
	client->display_name = kasprintf(GFP_KERNEL, "%s-%d", name, client->display_serial);
...
void __ion_client_destroy(struct ion_client *client, int from_kern)
{
	...
	while ((n = rb_first(&client->handles))) {
		...
		mutex_lock(&client->lock);
		IONMSG
		    ("warning: release handle @ client destory: handle=%p, buf=%p, ref=%d, size=%ld, kmap=%d\n",
		     handle, handle->buffer, handle->buffer->ref, handle->buffer->size,
...
static void ion_buffer_sync_for_device(struct ion_buffer *buffer,
				       struct device *dev, enum dma_data_direction direction);
				       ...
				       struct device *dev, enum dma_data_direction dir)
{
	...
	for (i = 0; i < pages; i++) {
		...
		if (ion_buffer_page_is_dirty(page))
			ion_pages_sync_for_device(dev, ion_buffer_page(page), PAGE_SIZE, dir);
...
static int ion_mmap(struct dma_buf *dmabuf, struct vm_area_struct *vma)
{
	...
	MMProfileLogEx(ION_MMP_Events[PROFILE_MAP_USER], MMProfileFlagStart, buffer->size,
		       vma->vm_start);
	...
	MMProfileLogEx(ION_MMP_Events[PROFILE_MAP_USER], MMProfileFlagEnd, buffer->size,
		       vma->vm_start);
	...
	case ION_IOC_CUSTOM:
		{
			if (!dev->custom_ioctl)
				return -ENOTTY;
			ret = dev->custom_ioctl(client, data.custom.cmd, data.custom.arg);
			break;
		}
	default:
		return -ENOTTY;
...
static size_t ion_debug_heap_total(struct ion_client *client, unsigned int id)
{
	size_t size = 0;
	struct rb_node *n;
...
void ion_device_add_heap(struct ion_device *dev, struct ion_heap *heap)
{
	...
	debug_file = debugfs_create_file(heap->name, 0664,
					 dev->heaps_debug_root, heap, &debug_heap_fops);
	...
	if (heap->shrinker.shrink) {
		...
		debug_file = debugfs_create_file(debug_name, 0644, dev->heaps_debug_root, heap,
						 &debug_shrink_fops);
		if (!debug_file) {
			char buf[256], *path;
			path = dentry_path(dev->heaps_debug_root, buf, 256);
	...
	for (i = 0; i < data->nr; i++) {
		...
		if (data->heaps[i].base == 0) {
			...
			paddr = memblock_alloc_base(data->heaps[i].size,
						    data->heaps[i].align, MEMBLOCK_ALLOC_ANYWHERE);
			if (!paddr) {
		...
		pr_info("%s: %s reserved base %lx size %zu\n", __func__,
			data->heaps[i].name, data->heaps[i].base, data->heaps[i].size);
	}
}
			      ...
			      unsigned int address, unsigned length, int fd)
{
	...
	get_kernel_symbol((unsigned long *)record_param.backtrace, record_param.backtrace_num,
			  &(record_param.kernel_symbol[0]));
...
static void *ion_get_list_from_buffer(struct ion_buffer *buf, unsigned int type)
{
	...
	if (!buf_rec) {
		pr_warn(KERN_WARNING "No inuse buffers!\n");
		return NULL;
	}
...
static void *ion_get_list_from_process(pid_t pid, unsigned int type)
{
	...
	if (!process_rec) {
		pr_warn(KERN_WARNING "No inuse process!\n");
		return NULL;
	}
...
static void *ion_get_client_record(struct ion_client *client)
{
	...
	if (!client_rec) {
		pr_warn(KERN_WARNING "No inuse client!\n");
		return NULL;
	}
	...
	do {
		/* We only need to find out the record with corresponding buffer */
		if ((client_rec->tracking_info.from_kernel)
		    && (client_rec->tracking_info.recordID.client_address == (unsigned int)client)
		    && (client_rec->tracking_info.recordID.group_pid == client->pid)) {
			return (void *)client_rec;
		} else if ((!client_rec->tracking_info.from_kernel)
			   && (client_rec->tracking_info.recordID.client_address ==
			       (unsigned int)client)
			   && (client_rec->tracking_info.recordID.pid == client->pid)) {
			return (void *)client_rec;
		}
		/* Next record */
 ...
 */
static int ion_debugdb_show_backtrace(struct seq_file *s, struct ion_record_basic_info *ti,
				      unsigned int sbt)
{
	unsigned int i = 0;
...
static int ion_debug_dbcl_show(struct seq_file *s, void *unused)
{
	 ...
	 *  (No-use)2. For kernel modules, we use (raw_key | dbcl_types) as the key,
	 *  in which raw_key is the virtual address the module is resident in.
	...
	case DBCL_BUFFER:
		...
		pr_info(KERN_INFO "DBCL_BUFFER\n");
		{
				...
				for (hn = rb_first(&client->handles); hn; hn = rb_next(hn)) {
					...
					while (!!usg_rec) {
						seq_printf(s,
								"%s [0x%x] %10s [%d] (%s [%d])\n",
								"    client",
								usg_rec->tracking_info.recordID.
						...
						usg_rec =
								...
								ion_get_data_from_record((void *)
										usg_rec,
										RECORD_NEXT);
					}
#if 0
					usg_rec =
							ion_get_list_from_buffer(buffer, BUFFER_FREE_LIST);
					if (usg_rec != NULL)
						seq_printf(s, "%s\n",
								"  <BUFFER_FREE_LIST>");
					while (!!usg_rec) {
			...
			while (buf_rec != NULL) {
				/* Allocation */
				usg_rec =
				    ion_get_list(LIST_BUFFER, buf_rec, BUFFER_ALLOCATION_LIST);

				while ((!!usg_rec)
				       && (usg_rec->tracking_info.recordID.pid == raw_key)) {
					buffer_count++;
					if (buffer_count == 1) {
						seq_printf(s,
							   "%8s[%2d] buffer: 0x%p buffer structure adr: 0x%p size(%d)\n",
							   "buffer", buffer_cnt++, buf_rec->buffer,
							   buf_rec->buffer_address,
							   buf_rec->buffer->size);
					}
					seq_printf(s, "%s\n", "  <BUFFER_ALLOCATION_LIST>");
					...
					/* Show buffer allocation backtrace */
					ion_debugdb_show_backtrace(s, &usg_rec->tracking_info,
								   ALLOCATE_BACKTRACE_INFO);
					/* Next buffer usage record */
					usg_rec =
					    (struct ion_buffer_usage_record *)
					    ion_get_data_from_record((void *)usg_rec, RECORD_NEXT);
				}
				buffer_count = 0;
#if 0
				/* Free */
				...
				for (hn = rb_first(&client->handles); hn; hn = rb_next(hn)) {
					...
					while (!!adr_rec) {
						seq_printf(s,
								...
								"Process",
								adr_rec->tracking_info.recordID.
								pid, "Address",
								adr_rec->mapping_address,
								(adr_rec->mapping_address +
										adr_rec->size), "Size",
								adr_rec->size);
				...
				if (client_rec != NULL) {
					...
					while (!!fd_rec) {
						seq_printf(s, "%10s [%d] %10s [%d]\n",
								"Process",
								fd_rec->tracking_info.recordID.
								pid, "inused fd", fd_rec->fd);
						/* Show address allocation backtrace */
						ion_debugdb_show_backtrace(s,
								&fd_rec->
								tracking_info,
...
static int ion_debug_dbis_show(struct seq_file *s, void *unused)
{
	...
	if (type >= (unsigned long)DBIS_DIR) {
		pr_info(KERN_INFO "ION Debug History Records\n");
		...
		pr_info(KERN_INFO "ION Debug Non-History Records\n");
		switch ((enum dbis_types)type) {
		case DBIS_CLIENTS:
			{
				client_rec = ion_get_inuse_client_record();
	...
	switch ((enum dbis_types)type) {
	case DBIS_CLIENTS:
		pr_info(KERN_INFO "DBIS_CLIENTS\n");
		{
#if 0
			/* All clients */
			for (cn = rb_first(&dev->clients); cn; cn = rb_next(cn)) {
				...
				for (hn = rb_first(&client->handles); hn; hn = rb_next(hn)) {
					...
					seq_printf(s,
						   ...
						   "buffer", buffer_cnt++, buffer->heap->name,
						   buffer, buffer->flags, buffer->size,
						   buffer->kmap_cnt, buffer->vaddr);
					mutex_unlock(&buffer->lock);
				}
				mutex_unlock(&client->lock);
				buffer_cnt = 0;
			...
			while (client_rec != NULL) {
				seq_printf(s, "\n[%2d]%s: fd[%d] 0x%p PID[%d] GROUP_PID[%d]\n",
					   client_cnt++, "client", client_rec->fd,
					   client_rec->tracking_info.recordID.client,
					   client_rec->tracking_info.recordID.pid,
					   client_rec->tracking_info.recordID.group_pid);
				/* Show buffer allocation backtrace */
				seq_printf(s, "    %s\n", "<CLIENT_ALLOCATION_LIST>");
				ion_debugdb_show_backtrace(s, &client_rec->tracking_info,
							   ALLOCATE_BACKTRACE_INFO);
		...
		break;
	case DBIS_BUFFERS:
		pr_info(KERN_INFO "DBIS_BUFFERS\n");
		{
			struct ion_buffer_usage_record *usg_rec;
			...
			while (buf_rec != NULL) {
				seq_printf(s, "%8s[%2d][0x%x] buffer structure: 0x%p size(%d)\n",
					   "buffer", buffer_cnt++, (unsigned int)buf_rec->buffer,
					   buf_rec->buffer_address, buf_rec->buffer->size);
				/* Allocation */
				usg_rec =
				    ion_get_list(LIST_BUFFER, buf_rec, BUFFER_ALLOCATION_LIST);
				if (usg_rec)
	...
	case DBIS_MMAPS:
		pr_info(KERN_INFO "DBIS_MMAPS\n");
		{
			...
			while (process_rec != NULL) {
				/* USER MMAP */
				adr_rec_user =
				    ion_get_list(LIST_PROCESS, process_rec,
						 ADDRESS_ALLOCATION_LIST);
				adr_rec_user_free =
				    ion_get_list(LIST_PROCESS, process_rec, ADDRESS_FREE_LIST);
				...
				seq_printf(s, "[%2d]%8s[0x%x] [%d] group_id [%d]\n", process_cnt++,
					   "process", (unsigned int)process_rec, process_rec->pid,
					   process_rec->group_id);
				...
				while (!!adr_rec_user) {
					seq_printf(s,
						   ...
						   adr_rec_user->tracking_info.recordID.pid,
						   "Address", adr_rec_user->mapping_address,
						   (adr_rec_user->mapping_address +
						    adr_rec_user->size), "Size",
						   adr_rec_user->size);

					/* Show fd allocation backtrace */
					ion_debugdb_show_backtrace(s, &adr_rec_user->tracking_info,
								   ALLOCATE_BACKTRACE_INFO);
					/* Next fd record */
					adr_rec_user =
					    (struct ion_address_usage_record *)
				...
				while (!!adr_rec_user_free) {
					seq_printf(s,
						   "    %s[0x%x] [%d] - %s [0x%x] - [0x%x]%10s [%d]\n",
						   "Process", (unsigned int)process_rec,
						   adr_rec_user_free->tracking_info.recordID.pid,
						   "Address", adr_rec_user_free->mapping_address,
						   (adr_rec_user_free->mapping_address +
						    adr_rec_user_free->size), "Size",
						   adr_rec_user_free->size);
			...
			while (buf_rec != NULL) {
				...
				/* Mapping */
				adr_rec =
				    ion_get_list(LIST_BUFFER, buf_rec, ADDRESS_ALLOCATION_LIST);

				/* Unmapping */
				adr_rec_free =
				    ion_get_list(LIST_BUFFER, buf_rec, ADDRESS_FREE_LIST);
				...
				seq_printf(s, "%8s[%2d] size(%d) %12p\n", "buffer", buffer_cnt++,
					   buf_rec->buffer->size, buf_rec->buffer);
				if (adr_rec != NULL)
					seq_printf(s, "  %s\n", "<ADDRESS_ALLOCATION_LIST>");
				...
				while (!!adr_rec) {
					seq_printf(s, "%8s [%d] - %20s [0x%x] - [0x%x] %10s [%d]\n",
						   ...
						   (adr_rec_user->mapping_address +
						    adr_rec_user->size), "Size", adr_rec->size);
					/* Show address allocation backtrace */
					ion_debugdb_show_backtrace(s, &adr_rec->tracking_info,
								   ALLOCATE_BACKTRACE_INFO);
					/* Next address record */
					adr_rec =
					    (struct ion_address_usage_record *)
					    ion_get_data_from_record((void *)adr_rec, RECORD_NEXT);
				}
				...
				while (!!adr_rec_free) {
					seq_printf(s, "%8s [%d] - %20s [0x%x] - [0x%x] %10s [%d]\n",
						   "Process",
						   adr_rec_free->tracking_info.recordID.pid,
						   "Address", adr_rec_free->mapping_address,
						   (adr_rec_free->mapping_address +
						    adr_rec_free->size), "Size",
						   adr_rec_free->size);
					/* Show address release backtrace */
					ion_debugdb_show_backtrace(s, &adr_rec_free->tracking_info,
								   RELEASE_BACKTRACE_INFO);
					/* Next address record */
					adr_rec_free =
					    (struct ion_address_usage_record *)
		...
		break;
	case DBIS_FDS:
		pr_info(KERN_INFO "DBIS_FDS\n");
		{
			struct ion_fd_usage_record *fd_rec;
			while (process_rec != NULL) {
				/* FD */
				fd_rec =
				    ion_get_list(LIST_PROCESS, process_rec, FD_ALLOCATION_LIST);
				...
				while (!!fd_rec) {
					seq_printf(s, "    %8s[0x%x] [%d] - %8s [%d]\n",
						   "Process", (unsigned int)process_rec,
						   fd_rec->tracking_info.recordID.pid, "inused Fd",
						   fd_rec->fd);
					/* Show fd allocation backtrace */
					ion_debugdb_show_backtrace(s, &fd_rec->tracking_info,
								   ALLOCATE_BACKTRACE_INFO);
					/* Next fd record */
					fd_rec =
					    (struct ion_fd_usage_record *)
					    ion_get_data_from_record((void *)fd_rec, RECORD_NEXT);
				}
#if 0
				if (fd_rec2 != NULL)
					seq_printf(s, "  %s\n", "<FD_FREE_LIST>");
				...
				while (!!fd_rec2) {
					seq_printf(s, "%7s[0x%x] [%d] - %6s [%d]\n",
						   "Process", process_rec,
						   fd_rec2->tracking_info.recordID.pid, "freed Fd",
						   fd_rec2->fd);
					/* Show fd release backtrace */
					ion_debugdb_show_backtrace(s, &fd_rec2->tracking_info,
								   RELEASE_BACKTRACE_INFO);
					/* Next fd record */
					fd_rec2 =
					    (struct ion_fd_usage_record *)
					    ion_get_data_from_record((void *)fd_rec2, RECORD_NEXT);
				}
#endif
				/* Next record */
				process_rec = process_rec->next;
		...
		break;
	case DBIS_PIDS:
		pr_info(KERN_INFO "DBIS_PIDS\n");
		break;
#if 0
	case DBIS_MODS:
		pr_info(KERN_INFO "DBIS_MODS\n");
		break;
#endif
	default:
		break;
	...
	for (index = 0; index <= _TOTAL_DBIS; ++index) {
		if (dbis_child_attr[index].attr == DBIS_FILE) {
			debug_db_root.dbis.child[index]
			    = debugfs_create_file(dbis_child_attr[index].name, 0444,
						  debug_db_root.ion_statistics, (void *)index,
						  &debug_dbis_fops);
		} else {	/* This is only for history now. */
			debug_db_root.dbis.child[index] =
			    debugfs_create_dir(dbis_child_attr[index].name,
					       debug_db_root.ion_statistics);
#if 0
			for (his_index = 0; his_index < _TOTAL_DBIS; ++his_index) {
				debug_db_root.dbis.history_record[his_index]
				    = debugfs_create_file(dbis_child_attr[index + his_index + 1].
							  name, 0444,
							  debug_db_root.dbis.child[index],
							  his_index + index + 1, &debug_dbis_fops);
			}
#endif
			...
			debug_db_root.dbis.history_record[3]
			    = debugfs_create_file(dbis_child_attr[DBIS_FDS].name, 0444,
						  debug_db_root.dbis.child[index],
						  (void *)(DBIS_FDS + DBIS_DIR), &debug_dbis_fops);
			...
			debug_db_root.dbis.history_record[4]
			    = debugfs_create_file(dbis_child_attr[DBIS_PIDS].name, 0444,
						  debug_db_root.dbis.child[index],
						  (void *)(DBIS_PIDS + DBIS_DIR), &debug_dbis_fops);
		}
	}
}
...
static void ion_debug_db_create_clentry(pid_t pid)
{
	...
	for (index = 0; index < _TOTAL_DBCL; ++index) {
		found->type[index] =
		    debugfs_create_file(dbcl_child_name[index], 0444, found->root,
					(void *)((index << 16) | pid), &debug_dbcl_fops);
	}
