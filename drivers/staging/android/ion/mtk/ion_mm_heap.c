...
static int ion_mm_heap_debug_show(struct ion_heap *heap, struct seq_file *s, void *unused)
{
...

	{
		...
		for (n = rb_first(&dev->buffers); n; n = rb_next(n)) {
			...
			ION_PRINT_LOG_OR_SEQ(s, "0x%x %8u %3d %3d %3d %3d %8x %3u %3lu %3d %s "
	...
	for (n = rb_first(&dev->clients); n; n = rb_next(n)) {
...

		{
			...
			for (m = rb_first(&client->handles); m; m = rb_next(m)) {
				...
				ION_PRINT_LOG_OR_SEQ(s,
						     "\thandle=0x%x, buffer=0x%x, heap=%d, backtrace is:\n",
						     (unsigned int)handle,
	...
	case ION_MM_SET_SF_BUF_INFO:
		{
			...
			if (Param.sf_buf_info_param.handle) {
				...
				if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
					...
					IONMSG
					    ("[ion_mm_heap]: Error. Cannot set sf_buf_info buffer that is not from multimedia heap.\n");
					ret = -EFAULT;
				}
				ion_drv_put_kernel_handle(kernel_handle);
			} else {
				IONMSG
				    ("[ion_mm_heap]: Error. set sf_buf_info buffer with invalid handle.\n");
				ret = -EFAULT;
			}
		}
		break;
