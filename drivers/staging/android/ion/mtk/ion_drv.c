			       ...
			       int from_kernel)
{
	...
	if (pParam->sync_type < ION_CACHE_CLEAN_ALL) {
		...
		kernel_handle = ion_drv_get_kernel_handle(client, pParam->handle, from_kernel);
...
if (pParam->sync_type < ION_CACHE_CLEAN_BY_RANGE_USE_VA)
		...
		if ((pParam->sync_type == ION_CACHE_CLEAN_BY_RANGE)
		    || (pParam->sync_type == ION_CACHE_CLEAN_BY_RANGE_USE_VA)) {
			MMProfileLogEx(ION_MMP_Events[PROFILE_DMA_CLEAN_RANGE], MMProfileFlagStart,
				       size, 0);
			/* pr_info("[ion_sys_cache_sync]: ION cache clean by range.
			 *  start=0x%08X size=0x%08X\n", start, size); */
			dmac_map_area((void *)start, size, DMA_TO_DEVICE);
		} else if ((pParam->sync_type == ION_CACHE_INVALID_BY_RANGE)
			   || (pParam->sync_type == ION_CACHE_INVALID_BY_RANGE_USE_VA)) {
			MMProfileLogEx(ION_MMP_Events[PROFILE_DMA_INVALID_RANGE],
				       MMProfileFlagStart, size, 0);
			/* pr_info("[ion_sys_cache_sync]: ION cache invalid by range.
			 * start=0x%08X size=0x%08X\n", start, size); */
			...
			MMProfileLogEx(ION_MMP_Events[PROFILE_DMA_FLUSH_RANGE], MMProfileFlagStart,
				       size, 0);
			/* pr_info("[ion_sys_cache_sync]: ION cache flush by range.
			 * start=0x%08X size=0x%08X\n", start, size); */
			dmac_flush_range((void *)start, (void *)(start + size - 1));
		}
#if 0
		/* L2 cache sync */
		/* pr_info("[ion_sys_cache_sync]: page_start=0x%08X, page_num=%d\n",
		 * page_start, page_num); */
...
if (pParam->sync_type < ION_CACHE_CLEAN_BY_RANGE_USE_VA)
		...
		if (pParam->sync_type == ION_CACHE_CLEAN_ALL) {
			MMProfileLogEx(ION_MMP_Events[PROFILE_DMA_CLEAN_ALL], MMProfileFlagStart, 1,
				       1);
			/* pr_info("[ion_sys_cache_sync]: ION cache clean all.\n"); */
			...
			MMProfileLogEx(ION_MMP_Events[PROFILE_DMA_INVALID_ALL], MMProfileFlagStart,
				       1, 1);
			/* pr_info("[ion_sys_cache_sync]: ION cache invalid all.\n"); */
			smp_inner_dcache_flush_all();
			outer_inv_all();
			/* outer_flush_all(); */
			...
			MMProfileLogEx(ION_MMP_Events[PROFILE_DMA_FLUSH_ALL], MMProfileFlagStart, 1,
				       1);
			/* pr_info("[ion_sys_cache_sync]: ION cache flush all.\n"); */
			smp_inner_dcache_flush_all();
			outer_flush_all();
			  ...
			  int from_kernel)
{
	...
	case ION_SYS_RECORD:
		{
			...
			if (!from_kernel && (Param.record_param.backtrace_num > 0)) {
				for (i = 0; i < Param.record_param.backtrace_num; i++) {
					...
					string_len = strlen_user((void __user *)Param.
									record_param.
									mapping_record[i].name);
					if (string_len <= 0) {
						printk(ION_DEBUG_ERROR
								"[ION_FUNC%d][ion_sys_ioctl]mapping info error can't get right string len\n",
								Param.record_param.action);
						continue;
					}

					int ret;
					tmp_string = kmalloc(string_len, GFP_KERNEL);
					ret = copy_from_user(tmp_string,
							(void __user *)
							Param.
							record_param.
							mapping_record
					...
					if (tmp_string != NULL) {
						Param.record_param.
						mapping_record[i].name =
								get_userString_from_hashTable
								(tmp_string, string_len);
						kfree(tmp_string);
					} else {
						printk(ION_DEBUG_ERROR
								"[ION_FUNC%d][ion_sys_ioctl]tmp_string is NULL\n",
								Param.record_param.action);
					}
				}
			}
			...
			if (Param.record_param.handle != NULL) {
				struct ion_handle *kernel_handle;
				kernel_handle =
					   ion_drv_get_kernel_handle(client,
								     Param.record_param.handle,
								     from_kernel);
				if (IS_ERR(kernel_handle)) {
				...
				Param.record_param.buffer =
					   ion_handle_buffer(kernel_handle);
				if (!from_kernel)
				...
				if (Param.record_param.buffer != NULL) {
					printk(ION_DEBUG_TRACE
							"[ION_FUNC%d][ion_sys_record]BUFFER :[%x] size :[%d] handle: [0x%x]\n",
							Param.record_param.action,
							(unsigned int)Param.record_param.buffer,
							Param.record_param.buffer->size,
							(unsigned int)Param.record_param.handle);
				} else {
					printk(ION_DEBUG_TRACE
							"[ION_FUNC%d]buffer is NULL handle: [0x%x]\n",
							Param.record_param.action,
							(unsigned int)Param.record_param.handle);
				}
#endif
	...
	default:
		pr_info("[ion_dbg][ion_sys_ioctl]: Error. Invalid command.\n");
		ret = -EFAULT;
		break;
	}
	if (from_kernel)
		       ...
		       int from_kernel)
{
	...
	case ION_CMD_MULTIMEDIA:
		...
		break;
	case ION_CMD_MM_CARVEOUT:
		ret = ion_mm_carveout_ioctl(client, cmd, arg, from_kernel);
		break;
	}
	ION_FUNC_LEAVE;
	return ret;
...
DEFINE_SIMPLE_ATTRIBUTE(debug_profile_fops, debug_profile_get, debug_profile_set, "%llu\n");
...
int ion_drv_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(g_ion_device)) {
		pr_info("ion_device_create() error! device=%x\n", (unsigned int)g_ion_device);
...
static struct ion_platform_heap ion_drv_platform_heaps[] = {
	...
	{
	 .type = ION_HEAP_TYPE_MM_CARVEOUT,
	 .id = ION_HEAP_TYPE_MM_CARVEOUT,
	 .name = "ion_mm_carveout_heap",
	 .base = 0,
	 .size = 0, /*reserve in /kernel/arch/arm/mm/init.c ion_reserve();*/
	 .align = 0x1000,	 /*this must not be 0. (or ion_reserve wiil fail) */
	 .priv = NULL,
	 },
};
