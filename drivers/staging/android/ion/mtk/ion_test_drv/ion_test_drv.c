...
{
	pr_info("ion_test_drv: ion_test_release()\n");
	return 0;
}
...
static int ion_test_open(struct inode *inode, struct file *file)
{
	...
	if (gClient) {
		...
		if (IS_ERR_OR_NULL(handle)) {
			pr_info("ion_test_drv: Cannot allocate buffer. handle=0x%08X\n",
			       (unsigned int)handle);
			return 0;
		}
		pBuf = ion_map_kernel(gClient, handle);
		if (IS_ERR_OR_NULL(pBuf)) {
			pr_info("ion_test_drv: Cannot map buffer to kernel. pBuf=0x%08X\n",
			       (unsigned int)pBuf);
			return 0;
		}
		...
		if (ion_phys(gClient, handle, &phy_addr, &len) < 0) {
			pr_info("ion_test_drv: Cannot get physical address.\n");
			return 0;
		}
		pr_info("ion_test_drv: Physical Address: 0x%08X Len: 0x%X\n", (unsigned int)phy_addr,
		       len);
...
static ssize_t ion_test_write(struct file *file, const char __user *data, size_t len,
			      loff_t *ppos)
{
	return 0;
}
...
static long ion_test_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	char *pBuf;
	int i;
	pr_info("ion_test_drv: ion_test_ioctl() fd=%d\n", (int)arg);
	handle = ion_import_dma_buf(gClient, (int)arg);
	if (IS_ERR_OR_NULL(handle)) {
		pr_info("ion_test_drv: Cannot import buffer. handle=0x%08X\n", (unsigned int)handle);
		return 0;
	}
	pBuf = (char *)ion_map_kernel(gClient, handle);
	if (IS_ERR_OR_NULL(pBuf)) {
		pr_info("ion_test_drv: Cannot map kernel. pBuf=0x%08X\n", (unsigned int)pBuf);
		return 0;
	}
...
/*	for (i = 0; i < 1024 * 1024 * 20 + 256; i += 4) {
		if (*(volatile unsigned int *)(pBuf + i) != i) {
			...
			pr_info("ion_test_drv: read data error from kernel!!!!!!!!!!!!\n\n");
		}
	}*/
	ion_unmap_kernel(gClient, handle);
/*
...
const struct file_operations ion_test_fops = {
	.owner = THIS_MODULE,
...
static int __init ion_test_init(void)
{
	...
	int ret;
	pr_info("ion_test_drv: ion_test_init()\n");
	...
	class_dev =
	    (struct class_device *)device_create(ion_test_class, NULL, ion_test_devno, NULL,
						 ION_TEST_DEVNAME);

	if (g_ion_device) {
		gClient = ion_client_create(g_ion_device, 0xFFFFFFFF, "ion_test_drv_client");
		if (IS_ERR_OR_NULL(gClient)) {
			pr_info("ion_test_drv: Cannot create client. gClient=0x%08X\n",
			       (unsigned int)gClient);
			return 0;
		}
	} else
		pr_info("ion_test_drv: ion device not initialized.\n");
...
{
	pr_info("ion_test_drv: ion_test_exit()\n");
