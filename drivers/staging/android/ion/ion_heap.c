...
static int ion_heap_sglist_zero(struct scatterlist *sgl, unsigned int nents, pgprot_t pgprot)
{
	int p = 0;
	int ret = 0;
...
static size_t _ion_heap_freelist_drain(struct ion_heap *heap, size_t size, bool skip_pools)
{
...
int ion_heap_init_deferred_free(struct ion_heap *heap)
{
	...
	heap->task = kthread_run(ion_heap_deferred_free, heap, "%s", heap->name);
...
static int ion_heap_shrink(struct shrinker *shrinker, struct shrink_control *sc)
{
	...
	if (heap->flags & ION_HEAP_FLAG_DEFER_FREE)
		freed = ion_heap_freelist_shrink(heap, to_scan * PAGE_SIZE) / PAGE_SIZE;
...
struct ion_heap *ion_heap_create(struct ion_platform_heap *heap_data)
{
	...
	case ION_HEAP_TYPE_DMA:
		...
		break;
	case ION_HEAP_TYPE_MM_CARVEOUT:
		heap = ion_mm_carveout_heap_create(heap_data);
		break;
	default:
...
void ion_heap_destroy(struct ion_heap *heap)
{
	...
	case ION_HEAP_TYPE_DMA:
		...
		break;
	case ION_HEAP_TYPE_MM_CARVEOUT:
		ion_mm_carveout_heap_destroy(heap);
		break;
	default:
		pr_err("%s: Invalid heap type %d\n", __func__, heap->type);
	}
