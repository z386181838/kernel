				 ...
				 enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[wireless_get_property] data->WIRELESS_ONLINE %d\n",
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	battery_xlog_printk(BAT_LOG_FULL, "Power/Battery",
			    "status_2nd = %d, capacity_2nd = %d, present_2nd = %d\n",
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if (resetBatteryMeter == KAL_TRUE) {
		...
		if (bat_is_recharging_phase() == KAL_TRUE) {
			BMT_status.UI_SOC = 100;
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[Battery_Recharging_phase] Keep UI as 100. BMT_status.UI_SOC=%d, BMT_status.SOC=%ld\r\n",
					    BMT_status.UI_SOC, BMT_status.SOC);
		} else {
...
{
#if defined(CONFIG_POWER_VERIFY)
	battery_xlog_printk(BAT_LOG_CRTI, "Power/Battery",
			    "[update_battery_2nd_info] no support\n");
#else
