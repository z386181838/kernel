...
atomic_t in_ffu_mode = ATOMIC_INIT(0);
/**
	...
	u8 *data, int buf_bytes)
{
	...
	atomic_set(&in_ffu_mode, 1);
	if (card->cid.manfid == CID_MANFID_KSI) {
		/* KSI eMMC need Re-read the EXT_CSD after switch to FFU mode */
		err = mmc_send_ext_csd(card, ext_csd);
		if (err) {
			pr_err("FFU: %s: error %d sending ext_csd\n",
					mmc_hostname(card->host), err);
	...
	if ( card->cid.manfid == CID_MANFID_SAMSUNG && cmd->arg == 0x0 ) {
		cmd->arg = 0xc7810000;
		mmc_set_clock(card->host, 50 * 1000 * 1000); /* set 50Mhz clock for Samsung eMMC */
	}
...
int mmc_ffu_install(struct mmc_card *card)
{
	...
	if (card->cid.manfid == CID_MANFID_HYNIX) {
		pr_info("FFU: Hynix eMMC has installed");
		return 0;
	}
