...
static int cyttsp4_si_get_test_data(struct cyttsp4_core_data *cd)
{
	...
	if (!(si->si_ptrs.test->post_codel & CY_POST_CODEL_CFG_DATA_CRC_FAIL))
		dev_info(cd->dev, "%s: %s codel=%02X\n", __func__,
			 "Config Data CRC FAIL", si->si_ptrs.test->post_codel);
...
static int cyttsp4_wait_sysinfo_mode(struct cyttsp4_core_data *cd)
{
	...
	return 0;
#else
	/* Here is patch from cypress, 2014.01.07, EVT1 */
	int i, rc;
	u8 mode[3];
	enum cyttsp4_mode cur_mode;
	...
	for (i = 0; i < 60; i++) {
		...
		if (!rc) {
			if ((mode[0] & CY_HST_DEVICE_MODE) == CY_HST_SYSINFO) {
				cur_mode = CY_MODE_SYSINFO;
				break;
			}
		}
	...
	if (i >= 60) {
		...
		if ((cd->int_status & CY_INT_MODE_CHANGE)
				&& (mode[0] & CY_HST_MODE_CHANGE) == 0) {
			cd->int_status &= ~CY_INT_MODE_CHANGE;
			dev_dbg(cd->dev, "%s: finish mode switch m=%d -> m=%d\n",
					__func__, cd->mode, cur_mode);
			cd->mode = cur_mode;
			wake_up(&cd->wait_q);

			/* handshake the event */
			dev_vdbg(cd->dev, "%s: Handshake mode=0x%02X r=%d\n",
					__func__, mode[0], rc);
			rc = cyttsp4_handshake(cd, mode[0]);
			if (rc < 0)
				dev_err(cd->dev, "%s: Fail handshake mode=0x%02X r=%d\n",
						__func__, mode[0], rc);
		}
	}
	return 0;
...
static int cyttsp4_request_exec_cmd_(struct cyttsp4_device *ttsp, u8 mode,
				     u8 * cmd_buf, size_t cmd_size,
				     u8 * return_buf, size_t return_buf_size,
				     int timeout_ms)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
				 ...
				 u32 * param_value)
{
	...
	if (rc < 0) {
		dev_err(cd->dev,
			"%s: Unable to execute get parameter command.\n",
			__func__);
		return rc;
	}
				 ...
				 u8 param_size, u32 param_value)
{
	...
	if (rc < 0) {
		dev_err(cd->dev,
			"%s: Unable to execute set parameter command.\n",
			__func__);
		return rc;
	}
...
no_offset:
	...
	if (cur_block == end_block) {
		...
		if (rc) {
			dev_err(cd->dev,
				"%s: Error on last write config block\n",
				__func__);
			goto free_row_data;
		}
	}
					       ...
					       u8 hst_mode_reg)
{
	...
	if (cd->pdata->power) {
		...
		disable_irq(cd->irq);
		rc = cd->pdata->power(cd->pdata, 0, cd->dev);
	} else {
						 ...
						 int timeout_ms)
{
	...
	if (cd->pdata->power) {
		...
		rc = cd->pdata->power(cd->pdata, 1, dev);
		enable_irq(cd->irq);
	} else {
...
static bool _is_irq_disabled(int irq)
{
	struct irq_data *data = irq_get_irq_data(irq);
	return irqd_irq_disabled(data);
}
				     ...
				     const char *buf, size_t size)
{
	...
	unsigned long value;
	int retval;
	bool irq_is_disabled = _is_irq_disabled(cd->irq);
...
static struct device_attribute attributes[] = {
	__ATTR(ic_ver, S_IRUSR | S_IRGRP | S_IROTH, cyttsp4_ic_ver_show, NULL),
	__ATTR(ttconfig_ver, S_IRUSR | S_IRGRP | S_IROTH,
	       cyttsp4_ttconfig_ver_show, NULL),
	__ATTR(drv_ver, S_IRUSR | S_IRGRP | S_IROTH, cyttsp4_drv_ver_show,
	       NULL),
	__ATTR(hw_reset, S_IWUSR | S_IWGRP, NULL,
	       cyttsp4_hw_reset_store),
	__ATTR(drv_irq, S_IRUGO | S_IWUSR | S_IWGRP, cyttsp4_drv_irq_show,
	       cyttsp4_drv_irq_store),
	__ATTR(drv_debug, S_IWUSR | S_IWGRP, NULL,
	       cyttsp4_drv_debug_store),
	__ATTR(sleep_status, S_IRUSR | S_IRGRP | S_IROTH,
	       cyttsp4_sleep_status_show, NULL),
