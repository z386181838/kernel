...
static int fwu_wait_for_idle(int timeout_ms)
{
	...
	do {
		...
		usleep_range(SLEEP_TIME_US-5, SLEEP_TIME_US+5);
...
static int fwu_set_fw_image_name(void)
{
	...
	default:
		dev_err(&fwu->rmi4_data->i2c_client->dev,
			"Unknown touch sensor id: %d\n",
			(u16)fwu->sensor_id);
		return -EINVAL;
	}
	dev_notice(&fwu->rmi4_data->i2c_client->dev,
		"%s: Sensor ID=%d; fw_image=%s\n",
		__func__, (u16)fwu->sensor_id, fwu->fw_name);
	return 0;
}
