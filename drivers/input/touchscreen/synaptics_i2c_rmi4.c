...
/*for FWK-8513*/
static int synaptics_rmi4_reset_device_pm(
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		...
		if (done) {
			dev_info(&rmi4_data->i2c_client->dev,
				"%s: Find F01 command addr at 0x%x\n",
...
*/
	...
	/*Modified for FWK-8513*/
	#ifdef CONFIG_PM
	rmi4_data->reset_device = synaptics_rmi4_reset_device_pm;
	#else
...
err_query_device:
	...
	list_for_each_entry_safe(
			fhandler, fhandler_temp, &rmi->support_fn_list, link) {
		list_del(&fhandler->link);
		if (fhandler->fn_number == SYNAPTICS_RMI4_F1A)
			synaptics_rmi4_f1a_kfree(fhandler);
		else
...
static int synaptics_rmi4_reset_device_pm(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->sensor_sleep == true) {
		...
		rmi4_data->touch_stopped = false;
		pr_debug("synaptics_rmi4_i2c,synaptics_rmi4_reset_device_pm");
