						 ...
						 size_t count, loff_t *data)
{
	...
	ret = sscanf(kbuf, "%x %x %x %x %x %x %x", &p1, &p2, &p3, &p4, &p5, &p6, &p7);
					    ...
					    loff_t *data)
{
	...
	ret = sscanf(kbuf, "%x %x %x %x %x %x %x %x", &cmd, &p1, &p2, &p3, &p4, &p5, &p6, &p7);
...
static int freqhopping_debug_proc_init(void)
{
	...
	if (!fh_proc_dir) {
		...
		if (prDebugEntry) {
			...
			FH_MSG("[%s]: failed to create /proc/freqhopping/freqhopping_debug",
			       __func__);
			return 1;
		}
		...
		prDramcEntry =
		    proc_create("dramc", S_IRUGO | S_IWUSR | S_IWGRP, fh_proc_dir, &dramc_fops);
		if (prDramcEntry) {
			/* prDramcEntry->read_proc  = g_p_fh_hal_drv->proc.dramc_read; */
			/* prDramcEntry->write_proc = g_p_fh_hal_drv->proc.dramc_write; */
			FH_MSG("[%s]: successfully create /proc/freqhopping/prDramcEntry",
			       __func__);
		} else {
		...
		prDramcEntry =
		    proc_create("dvfs", S_IRUGO | S_IWUSR | S_IWGRP, fh_proc_dir, &dvfs_fops);
		if (prDramcEntry) {
		...
		prDumpregEntry =
		    proc_create("dumpregs", S_IRUGO | S_IWUSR | S_IWGRP, fh_proc_dir,
				&dumpregs_fops);
		if (prDumpregEntry) {
		...
		prUserdefEntry =
		    create_proc_entry("clkgen", S_IRUGO | S_IWUSR | S_IWGRP, fh_proc_dir);
		if (prUserdefEntry) {
