...
static UINT32 passCnt __attribute__ ((unused))= 0;
static int wmt_dev_dbg_read(char *page, char **start, off_t off, int count, int *eof, void *data) __attribute__ ((unused));
static int wmt_dev_dbg_write(struct file *file, const char *buffer, unsigned long count, void *data) __attribute__ ((unused));
#endif
#endif
#if WMT_CREATE_NODE_DYNAMIC
...
#if !USE_NEW_PROC_FS_FLAG
int wmt_dev_dbg_read(char *page, char **start, off_t off, int count, int *eof, void *data){
    INT32 len = 0;
...
#if !USE_NEW_PROC_FS_FLAG
int wmt_dev_dbg_write(struct file *file, const char *buffer, unsigned long count, void *data){
...
INT32 wmt_dev_dbg_setup(VOID)
{
	INT32 i_ret = 0;
#if USE_NEW_PROC_FS_FLAG
	gWmtDbgEntry = proc_create(WMT_DBG_PROCNAME, 0664, NULL, &wmt_aee_fops);
