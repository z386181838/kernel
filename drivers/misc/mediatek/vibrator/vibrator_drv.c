...
static void vibrator_enable(struct timed_output_dev *dev, int value)
{
...
#endif
	pr_debug(KERN_DEBUG "[vibrator]vibrator_enable: vibrator first in value = %d\n", value);

	spin_lock_irqsave(&vibe_lock, flags);
	while (hrtimer_cancel(&vibe_timer))
		pr_debug(KERN_DEBUG "[vibrator]vibrator_enable: try to cancel hrtimer\n");

	if (value == 0 || shutdown_flag == 1) {
		pr_debug(KERN_DEBUG "[vibrator]vibrator_enable: shutdown_flag = %d\n", shutdown_flag);
		vibe_state = 0;
	} else {
#if 1
		pr_debug(KERN_DEBUG "[vibrator]vibrator_enable: vibrator cust timer: %d\n", hw->vib_timer);
	...
	pr_debug(KERN_DEBUG "[vibrator]vibrator_enable: vibrator start: %d\n", value);
...
{
	unsigned long flags;
	pr_debug(KERN_DEBUG "[vibrator]vib_shutdown: enter!\n");
	spin_lock_irqsave(&vibe_lock, flags);
	shutdown_flag = 1;
	if (vibe_state) {
		pr_debug(KERN_DEBUG "[vibrator]vib_shutdown: vibrator will disable\n");
		vibe_state = 0;
		spin_unlock_irqrestore(&vibe_lock, flags);
		vibr_Disable();
...
{
	if (buf != NULL && size != 0) {
		pr_debug(KERN_DEBUG "[vibrator]buf is %s and size is %d\n", buf, size);
		if (buf[0] == '0')
			vibr_Disable();
		else
			vibr_Enable();
...
static int vib_mod_init(void)
{
	...
	pr_debug(KERN_DEBUG "MediaTek MTK vibrator driver register, version %s\n", VERSION);
	vibr_power_set();	/* set vibr voltage if needs.  Before MT6320 vibr default voltage=2.8v
		but in MT6323 vibr default voltage=1.2v */
	ret = platform_device_register(&vibrator_device);
	if (ret != 0) {
		pr_debug(KERN_DEBUG "[vibrator]Unable to register vibrator device (%d)\n", ret);
		return ret;
	}

	vibrator_queue = create_singlethread_workqueue(VIB_DEVICE);
	if (!vibrator_queue) {
		pr_debug(KERN_DEBUG "[vibrator]Unable to create workqueue\n");
		return -ENODATA;
	}
	...
	if (ret) {
		pr_debug(KERN_DEBUG "[vibrator]Unable to register vibrator driver (%d)\n", ret);
		return ret;
	}

	ret = device_create_file(mtk_vibrator.dev, &dev_attr_vibr_on);
	if (ret)
		pr_debug(KERN_DEBUG "[vibrator]device_create_file vibr_on fail!\n");

	pr_debug(KERN_DEBUG "[vibrator]vib_mod_init Done\n");
...
{
	pr_debug(KERN_DEBUG "MediaTek MTK vibrator driver unregister, version %s\n", VERSION);
	if (vibrator_queue)
		destroy_workqueue(vibrator_queue);

	pr_debug(KERN_DEBUG "[vibrator]vib_mod_exit Done\n");
}
