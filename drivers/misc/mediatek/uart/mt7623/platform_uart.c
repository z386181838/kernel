				...
				size_t size)
{
    ...
    if (flag == TTY_NORMAL) {
        copied = tty_insert_flip_string(TTY_FLIP_ARG(tty), chars, size);
    } else {
        MSG(ERR, "error occurs\n");
        copied += tty_insert_flip_string(TTY_FLIP_ARG(tty), chars, size-1);
        copied += tty_insert_flip_char(TTY_FLIP_ARG(tty), chars[size-1], flag);
    }
