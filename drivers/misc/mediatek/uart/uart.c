...
static void adjust_kernel_cmd_line_setting_baudrate(char *u_boot_cmd_line, char *kernel_cmd_line);
/******************************************************************************
 * SYSFS support
******************************************************************************/
...
/*---------------------------------------------------------------------------*/
ssize_t mtk_uart_attr_store(struct kobject *kobj, struct attribute *attr, const char *buffer,
			    size_t size)
{
	struct mtuart_entry *entry = container_of(attr, struct mtuart_entry, attr);
...
void adjust_kernel_cmd_line_setting_for_console(char *u_boot_cmd_line, char *kernel_cmd_line)
{
	...
	/* Replace the baudrate of serial by uboot value */
	adjust_kernel_cmd_line_setting_baudrate(u_boot_cmd_line, kernel_cmd_line);
...
static int find_port_baudrate_setting(char str[], int *setting_offset, int *setting_length)
{
	char *console_start;
	char *baudrate_start;
	char *baudrate_end;
	const char *fingerprint = "console=ttyMT";
	console_start = strstr(str, fingerprint);
	if (console_start) {
		baudrate_start = strchr(console_start, ',');
		baudrate_end = strchr(console_start, ' ');
		if (baudrate_start) {
			baudrate_start++;
			if (baudrate_end && baudrate_end > baudrate_start) {
				if (setting_offset && setting_length) {
					*setting_offset = baudrate_start - str;
					*setting_length = baudrate_end - baudrate_start;
				}
			}
			return 0;	/* Find it. */
		}
...
/* Functions of replace string */
static int move_sub_string(char str[], int source_position, int sub_string_lengh, int destination_position)
{
	int i;

	if (destination_position < source_position) {
		/* move sub string left */
		for (i = 0; i < sub_string_lengh + 1; i++)
			str[destination_position + i] = str[source_position + i];
	} else {
		/* move sub string right */
		if (destination_position + sub_string_lengh > sizeof(str))
			return -1;

		for (i = sub_string_lengh; i >= 0; i--)
			str[destination_position + i] = str[source_position + i];
	}
	return 0;
}

static int move_cmdline_tail_for_new_baudrate(char str[], int setting_offset, int setting_length, char new_baudrate[])
{
	int source_position = setting_offset + setting_length;
	int sub_string_lengh = strlen(str) - source_position;
	int destination_position = setting_offset + strlen(new_baudrate);

	return move_sub_string(str, source_position, sub_string_lengh, destination_position);
}

static int modify_baudrate_setting(char str[], int setting_offset, int setting_length, char new_baudrate[])
{

	if (strlen(new_baudrate) == setting_length) {
		strncpy(str + setting_offset, new_baudrate, strlen(new_baudrate));
	} else {
		move_cmdline_tail_for_new_baudrate(str, setting_offset, setting_length, new_baudrate);
		strncpy(str + setting_offset, new_baudrate, strlen(new_baudrate));
	}
...
/* Check and change the baudrate value */
static void adjust_kernel_cmd_line_setting_baudrate(char *u_boot_cmd_line, char *kernel_cmd_line)
{
	int u_boot_console_baudrate = -1;
	int kernel_console_baudrate = -1;
	int baudrate_setting_offset = 0;
	int baudrate_setting_length = 0;
	char u_boot_baudrate[16] = {0};

	/* Check u-boot command line baudrate setting */
	u_boot_console_baudrate = find_port_baudrate_setting(u_boot_cmd_line, &baudrate_setting_offset,
		&baudrate_setting_length);
	if (-1 == u_boot_console_baudrate) {
		/* printf("U-boot does not have baudrate setting, return\n"); */
		return;
	}

	if (baudrate_setting_length > 0)
		strncpy(u_boot_baudrate, u_boot_cmd_line + baudrate_setting_offset, baudrate_setting_length);

	/* U-boot has console baudrate setting, check kernel console baudrate setting */
	kernel_console_baudrate = find_port_baudrate_setting(kernel_cmd_line, &baudrate_setting_offset,
		&baudrate_setting_length);
	if (-1 == kernel_console_baudrate) {
		/* printf("Kernel does not have console baudrate setting, return\n"); */
		return;
	}

	/* Enter here, it means both kernel and u-boot console baudrate setting are not null, using u-boot setting */
	modify_baudrate_setting(kernel_cmd_line, baudrate_setting_offset, baudrate_setting_length, u_boot_baudrate);
...
static int __init mtk_uart_console_setup(struct console *co, char *options)
{
	...
	pr_alert(DBG_TAG "mtk console setup : co->index %d options:%s\n", co->index, options);
	...
	pr_alert(DBG_TAG "mtk console setup : uart_set_option port(%d)baud(%d) parity(%c) bits(%d) flow(%c) - ret(%d)\n",
co->index, baud, parity, bits, flow, ret);
	pr_alert(DBG_TAG "mtk setting: (%d, %d, %d, %lu, %lu)\n",
...
{				/*NOTE: please save the phyiscal address in vff->dmahd */
	...
	for (idx = uart->nport * 2; idx < uart->nport * 2 + 2; idx++) {
		...
		if (vfifo->size && !vfifo->addr) {
			err = -ENOMEM;
			break;
		} else if (mtk_uart_vfifo_new_dbgbuf(vfifo)) {
			      ...
			      struct mtk_uart_dma *dma, int mode, struct mtk_uart_vfifo *vfifo)
{
	...
	case UART_TX_VFIFO_DMA:
		...
		if (!atomic_read(&vfifo->reg_cb)) {
			...
			ret =
			    request_irq(vfifo->irq_id, (irq_handler_t) mtk_vfifo_irq_handler,
...
static void mtk_uart_start_tx(struct uart_port *port)
{
	...
	if (uart->tx_mode == UART_TX_VFIFO_DMA) {
		if (UART_DEBUG_EVT(DBG_EVT_BUF))
			pr_info("[UART%d] mtk_uart_start_tx\n", uart->nport);
				 ...
				 struct ktermios *termios, struct ktermios *old)
{
	...
	if (HW_FLOW_CTRL_PORT(uart) && (termios->c_cflag & CRTSCTS)
		...
		pr_notice("Hardware Flow Control\n");
		mode = UART_FC_HW;
	} else if (termios->c_iflag & 0x80000000) {
		pr_notice("MTK Software Flow Control\n");
		mode = UART_FC_SW;
	} else if (termios->c_iflag & (IXON | IXOFF | IXANY)) {
		/* pr_notice("Linux default SW Flow Control\n"); */
		mode = UART_FC_NONE;
	} else {
		pr_notice("No Flow Control\n");
		mode = UART_FC_NONE;
	}
...
static int mtk_uart_pm_restore_noirq(struct device *device)
{
	...
	if (!uart) {
		dev_err(device, "[%s] uart is null!!\n", __func__);
		return 0;
	}
	if (!uart->setting) {
		dev_err(device, "[%s] uart (%p), uart->setting is null!!\n", __func__, uart);
		return 0;
	}
	mtk_uart_fifo_set_trig(uart, uart->tx_trig, uart->rx_trig);
		...
		if (uart->poweron_count > 0) {
			...
			while (!(UART_READ32(UART_SLEEP_ACK) & UART_CLK_OFF_ACK)) {
				if (i++ >= WAIT_UART_ACK_TIMES) {
					...
					MSG(ERR, "CANNOT GET UART[%d] SLEEP ACK\n", uart_idx);
					/*dump_uart_reg();*/
					return -EBUSY;
				}
				udelay(10);
	...
	for (uart_idx = 0; uart_idx < UART_NR; uart_idx++) {
		...
		if (uart->poweron_count > 0) {
			...
			while ((UART_READ32(UART_SLEEP_ACK) & UART_CLK_OFF_ACK)) {
				if (i++ >= WAIT_UART_ACK_TIMES) {
					reg_sync_writel(val1, UART_SLEEP_REQ);
					MSG(ERR, "CANNOT GET UART[%d] WAKE ACK\n", uart_idx);
					/*dump_uart_reg();*/
					return -EBUSY;
				}
			udelay(10);
