...
#define MHL_ERR_LOG(fmt, arg...) \
    do {\
        if ( s_mhl_log_on & MHL_ERR_LOG_ON ) { printk("[it668x:ERR] %s, %d: ", __func__, __LINE__); printk(fmt, ##arg); } \
    } while ( 0 )

#define MHL_DBG_LOG(fmt, arg...)     printk(KERN_ERR fmt, ##arg)

#define MHL_INT_LOG(fmt, arg...) \
    do {\
        if ( s_mhl_log_on & MHL_INT_LOG_ON ) { printk("[it668x:INT] %s, %d: ", __func__, __LINE__); printk(fmt, ##arg); } \
    } while ( 0 )

#define MHL_VERB_LOG(fmt, arg...) \
    do {\
        if ( s_mhl_log_on & MHL_VERB_LOG_ON ) { printk("[it668x] %s, %d: ", __func__, __LINE__); printk(fmt, ##arg); } \
    } while ( 0 )

#define MHL_INFO_LOG(fmt, arg...) \
    do {\
        if ( s_mhl_log_on & MHL_INFO_LOG_ON ) { printk("[it668x] %s, %d: ", __func__, __LINE__); printk(fmt, ##arg); } \
    } while ( 0 )

#define MHL_MSC_LOG(fmt, arg...) \
    do {\
        if ( s_mhl_log_on & MHL_MSC_LOG_ON ) { printk("[it668x] %s, %d: ", __func__, __LINE__); printk(fmt, ##arg); } \
    } while ( 0 )

#define MHL_HDCP_LOG(fmt, arg...) \
    do {\
        if ( s_mhl_log_on & MHL_HDCP_LOG_ON ) { printk("[it668x:HDCP] %s, %d: ", __func__, __LINE__); printk(fmt, ##arg); } \
    } while ( 0 )

#define MHL_HDMI_LOG(fmt, arg...) \
    do {\
        if ( s_mhl_log_on & MHL_HDMI_LOG_ON ) { printk("[it668x:HDMI] %s, %d: ", __func__, __LINE__); printk(fmt, ##arg); } \
    } while ( 0 )

#define MHL_MHL_LOG(fmt, arg...) \
    do {\
        if ( s_mhl_log_on & MHL_MHL_LOG_ON ) { printk("[it668x:MHL] %s, %d: ", __func__, __LINE__); printk(fmt, ##arg); } \
    } while ( 0 )

#define MHL_WARN_LOG(fmt, arg...) \
    do {\
        if ( s_mhl_log_on & MHL_WARN_LOG_ON ) { printk("[it668x:Warn] %s, %d: ", __func__, __LINE__); printk(fmt, ##arg); } \
    } while ( 0 )

#define MHL_MUTEX_LOG(fmt, arg...) \
    do {\
        if ( s_mhl_log_on & MHL_MUTEX_LOG_ON ) { printk("[it668x:MUTEX]"); printk(fmt, ##arg); } \
    } while ( 0 )
    
#define MHL_DEF_LOG(fmt, arg...) \
    do {\
        if ( s_mhl_log_on & MHL_DEF_LOG_ON ) { printk(KERN_ERR fmt, ##arg); } \
    } while ( 0 )
...
static int mhl_cbus_Write_state_int_lock(struct it668x_data *it668x ,unsigned char sub_code , unsigned char sub_data);
static bool is_key_supported(struct it668x_data *it668x, int keyindex);

static int it668x_hdcp_read(struct it668x_data *it668x, int mode, unsigned char offset, int len, char *p);
...
static struct task_struct *it668x_irq_task = NULL;
    ...
    for (i = 0; i < 0x100; i += 16) {
        ...
        for (j = 0; j < 16; j++) {
            ...
            if (ij == 0x17 || ij == 0x59 || ij == 0x5B) {
                ...
                if ( 0 == i2c_read_reg(mhl, (unsigned char)((i + j) & 0xFF), &uc_data ) ) {
                    sprintf(str0, " %02X", uc_data);
                    strcat(str, str0);
                }
                else {
...
static int it668x_hdmitx_enhdcp_start( struct it668x_data *it668x)
{
    ...
    i2c_set_reg(hdmitx, REG_HDMITX_HDCP_TEST, M_HDCP_4B_OPTION, EN_RI_COMB_READ|EN_R0_COMB_READ);
    i2c_write_reg(hdmitx, REG_HDMITX_REG_LOCK, LOCK_PWD );


    i2c_set_reg(hdmitx, REG_HDMITX_HDCP_CONFIG, B_EN_HDCP11_FEATURE|B_EN_HDCP12_SYNCDET , 0x00); 
    ...
    i2c_set_reg(hdmitx, REG_HDMITX_INT_08_MASK, B_MASK_HDCP_AUTH_FAIL | B_MASK_HDCP_AUTH_DONE | B_MASK_HDCP_KSVLIST_CHK, 0x00); 

    i2c_set_reg(hdmitx, REG_HDMITX_INT_08_MASK, INT_HDCP_PJ_CHK | INT_HDCP_RI_CHK, INT_HDCP_PJ_CHK | INT_HDCP_RI_CHK );    
...
hdcp_start:
    ...
    if( bksv[0]==0x27 && bksv[1]==0xF8 && bksv[2]==0xA0 && bksv[3]==0x6E && bksv[4]==0x2D ) {
        MHL_ERR_LOG(" The BKSV is in revocation list for ATC test !!!\n");
        MHL_ERR_LOG(" Abort HDCP Authentication !!!\n");
        goto hdcp_ena_err_return;
    }
    ...
    if(it668x->hdcp_state  != state){
        ...
        switch(state){
            ...
            case HDCP_CP_GOING:
                ...
                break;
            case HDCP_CP_DONE:
                i2c_set_reg(hdmitx, REG_HDMITX_AVMUTE_CTRL, B_EN_AVMUTE, 0x00);   // clear AVMute
            ...
            case HDCP_CP_FAIL:               
                ...
                i2c_set_reg(hdmitx, REG_HDMITX_HDCP_CONFIG, B_EN_CPDESIRE, 0x00);   // Disable CPDesired
                i2c_set_reg(hdmitx, REG_HDMITX_VIDEO_BLACK, B_EN_VIDEO_BLACK, B_EN_VIDEO_BLACK);   // Black Screen
                break;
            default:
                break;
        }
        ...
        switch (it668x->hdcp_cmd) {
                ...
                if(it668x->hdcp_enable == true){
                    ...
                    */
                    ...
                    do{
                        ...
                        wait_event_interruptible_timeout(it668x->hdcp_wq, NULL ,msecs_to_jiffies(150));
                        IT668X_MUTEX_LOCK(&it668x->lock);
                    ...
                    if(ret<0){
                        if ( hdcp_fire_count <= 10 ) {
                            MHL_VERB_LOG( " it668x_hdmitx_enhdcp_start fail .. \n" ); 
            ...
            case HDCP_KSV_CHK:
                ...
                for(i=0;i<25;i++){
                    ...
                    IT668X_MUTEX_UNLOCK(&it668x->lock);
                    wait_event_interruptible_timeout(it668x->hdcp_wq,NULL ,msecs_to_jiffies(200));
                    IT668X_MUTEX_LOCK(&it668x->lock);
                ...
                if(ksvready == false){
                    ...
                    i2c_set_reg(hdmitx,REG_HDMITX_HDCP_KSVLIST_CHECK, M_KSVLIST_CHECK, 0x03);
                    i2c_set_reg(hdmitx,REG_HDMITX_HDCP_KSVLIST_CHECK, M_KSVLIST_CHECK, 0x00);
                }

                if(it668x_hdcp_repeater_check(it668x) == true){


                    i2c_set_reg(hdmitx, REG_HDMITX_HDCP_KSVLIST_CHECK, B_KSVLIST_DONE, B_KSVLIST_DONE);
                    i2c_set_reg(hdmitx, REG_HDMITX_HDCP_KSVLIST_CHECK, M_KSVLIST_CHECK, 0x00);
    ...
    if (it668x->avi_work == true) {
        ...
        switch (it668x->avi_cmd) {
            case AVI_CDREST:
                ...
                wait_event_interruptible_timeout(it668x->avi_wq, NULL, msecs_to_jiffies(200));
            ...
            case HPD_HIGH_EVENT:
                ...
                if(it668x->mhl_devcap[MHL_DEVCAP_VID_LINK_MODE] & MHL_DEV_VID_LINK_SUPP_PPIXEL) {
                    it668x->sink_support_packet_pixel_mode = 1;
                } 
                ...
                it668x_mhl_config_path(it668x, it668x->ddc_bypass);//Tranmin remove to it668x_auto_set_vid_fmt
            ...
            case CEA_NEW_AVI:
                ...
                if(it668x->hdcp_enable == true){
                    ...
                    IT668X_MUTEX_UNLOCK(&it668x->lock);
                    wait_event_interruptible_timeout(it668x->avi_wq,NULL ,msecs_to_jiffies(500));
                    IT668X_MUTEX_LOCK(&it668x->lock);
...
static int it668x_hdmitx_reg_init(struct it668x_data *it668x)
{
    ...
    ret =i2c_set_reg(hdmitx, REG_HDMITX_SWREST, B_ASYNC_RESET|B_REST_RCLK | M_RESET_ALL, B_ASYNC_RESET|B_REST_RCLK | M_RESET_ALL);
    if (ret < 0)
        return ret;
    ...
    // MHL Slave Address
    ret =i2c_set_reg(hdmitx, REG_HDMITX_MHL_I2C_EN, MASK_ALL_BIT, IT668x_MHL_ADDR | B_EN_MHL_I2C); 
    if (ret < 0)
        return ret;
    ...
    ret =i2c_set_reg(hdmitx, REG_HDMITX_TEST_CTRL0, B_CBUS_DRVING | B_FORCE_VIDEO_STABLE, SET_CBUS_DRINING);
    if (ret < 0)
        return ret;
    ...
    ret =i2c_set_reg(hdmitx, REG_HDMITX_PULLUP_SEL, M_5KUP_SEL | M_10KUP_SEL, SET_R_PULLUP10K | SET_R_PULLUP5K);   
    if (ret < 0)
        return ret;

    ret =i2c_set_reg(hdmitx, REG_HDMITX_CSC_SIGNEDBIT, B_CSC_SIGNED|B_Y_SIGNED|B_CB_SIGNED|B_CR_SIGNED, 0x00); 
    if (ret < 0)
        return ret;
...
static int it668x_mhl_reg_init(struct it668x_data *it668x)
{
    ...
    if (ret < 0)
        return ret; 
    ret =i2c_set_reg(mhl, REG_MHL_INT_06_MASK0A, MASK_ALL_BIT, 0x80);    // Enable Disconnect and Discovery Interrupt
    if (ret < 0)
        return ret; 
    ret =i2c_set_reg(mhl, REG_MHL_INT_04_MASK08, MASK_ALL_BIT, 0xFF);    // Enable CBusNotDet Interrupt
    if (ret < 0)
        return ret; 
    ...
    ret =i2c_set_reg(mhl, REG_MHL_OSCD_CTRL, B_CBUS_DEGLITCH | M_100MS_ADJ, B_CBUS_DEGLITCH | SET_100MS_ADJ );
    if (ret < 0)
        return ret; 

    ret =i2c_set_reg(mhl, REG_MHL_PATHEN_CTROL, B_HW_AUTO_PATH_EN|B_PACKET_PIXEL_HDCP_OPT, B_PACKET_PIXEL_HDCP_OPT); 
    if (ret < 0)
        return ret; 

    ret =i2c_set_reg(mhl, REG_CBUS_LINK_LAYER_CTEL_36, M_T_ACK_HIGH|M_T_ACK_LOW, SET_T_ACK_HIGH|SET_T_ACK_LOW);
    if (ret < 0)
        return ret; 
    ...
    if (ret < 0)
        return ret;     
    ret =i2c_set_reg(mhl, REG_CBUS_DISCOVER_CONTROL1, M_CBUS_DISC_OPT_1, SET_CBUS_DISC_OPT_1);   //(Dis1KDet<<3)+(EnDynFloat<<2)+(CBusLtoDet<<1)); //IMPORT TANT FOR SUPPORT USB
    if (ret < 0)
        return ret;     

     // Enable MHL
    ret =i2c_set_reg(mhl, REG_MHL_CONTROL, B_PACKET_PIXEL_GB_SWAP|B_EN_VBUS_OUT, B_PACKET_PIXEL_GB_SWAP|B_EN_VBUS_OUT);
    if (ret < 0)
        return ret;     
...
static unsigned long it668x_read_default_calibration(struct it668x_data *it668x)
{
    ...
    i2c_set_reg(hdmitx, REG_HDMITX_SIPROM_CONFIG, B_ENA_SIPROM|B_SIPROM_SEL, B_ENA_SIPROM|B_SIPROM_SEL_EXT);
...
static int it668x_cal_oclk( struct it668x_data *it668x )
{
    ...
    ret =i2c_set_reg(hdmitx, REG_HDMITX_MHL_I2C_EN, 0xFF, IT668x_MHL_ADDR | B_EN_MHL_I2C); //CBUS Slave address enable
    if (ret < 0){
...
static int it668x_mhl_config_rclk(struct it668x_data *it668x)
{
    ...
    if (ret < 0)
        return ret;
    ret = i2c_write_reg(mhl ,REG_MHL_10US_TIME_FLOAT, (unsigned char) (((t10usint&0x100)>>1)+t10usflt));
    if (ret < 0)
        return ret;    
...
static int it668x_set_hdmitx_afe( struct it668x_data *it668x, unsigned long hclk, unsigned long tmdsclk )
{
    int ret = 0;
    struct i2c_client *hdmitx = it668x->pdata->hdmitx_client;
    struct i2c_client *hdmirx = it668x->pdata->hdmirx_client;
...
static int it668x_fire_afe(struct it668x_data *it668x, int on)
{
    ...
    if(on){
        ret = i2c_set_reg(hdmitx, REG_HDMITX_AFE_DRV_CONFIG, B_DRV_PWRDN | B_DEV_REST, 0x00);   
        if (ret < 0)
            return ret;
    }
    else{
        ret = i2c_set_reg(hdmitx, REG_HDMITX_AFE_DRV_CONFIG, B_DRV_PWRDN | B_DEV_REST, B_DRV_PWRDN | B_DEV_REST); 
        if (ret < 0)
            return ret;    
    }
...
static int it668x_is_usb_mode(struct it668x_data *it668x)
{
    ...
    if ( ret < 0 ) {
        MHL_ERR_LOG(" %s() error at line %d", __FUNCTION__, __LINE__);
    }
...
static int it668x_cbus_drive_low(struct it668x_data *it668x)
{
    ...
    if ( 0 == it668x_is_usb_mode(it668x) ) { // if mhl mode, pull down CBUS
        ...
        if ( ret < 0 ) {
            MHL_VERB_LOG(" %s() error at line %d", __FUNCTION__, __LINE__);
        }
...
// }
    ...
    switch(state){
        ...
        case MHL_LINK_DISCONNECT:
            ...
            ret  = i2c_set_reg(mhl ,REG_MHL_INT_06_MASK0A, B_MASK_1K_FAIL_IRQ,B_MASK_1K_FAIL_IRQ );
            if (ret < 0)
                return ret;
        ...
        case MHL_CBUS_START:
            ret = i2c_set_reg(mhl, REG_MHL_CONTROL, B_USB_SWITCH_ON|B_DISABLE_MHL, 0x00);    //  reset Cbus fsm
            if (ret < 0)
                return ret;
...
//          break;
        ...
        case MHL_CBUS_CONNECTED:
            ...
            ret = i2c_write_reg(mhl, REG_MHL_INT_04_MASK08, B_MASK_CBUS_RX_PKT_FAIL | 
                                    B_MASK_CBUS_RX_PKT_DONE | B_MASK_CBUS_TX_PKT_FAIL | B_MASK_CBUS_TX_PKT_DONE );    
            if (ret < 0)
                return ret;
...
static int it668x_hdmitx_set_color_scale(struct it668x_data *it668x)
{
    ...
    if(it668x->color_dyn_range == COLOR_RANGE_CEA){        //DynCEA(16-235), DynVESA(0-255)
...
static int it668x_config_3d_output(struct it668x_data *it668x )
{
    ...
    if(it668x->enable_3d == true){
        ...
        if(it668x->sel_3dformat == FRMEPKT){
            ...
            temp = (unsigned char) (((isfb_end&0xF00)>>4)+((isfb_start&0xF00)>>8));
...
static int it668x_auto_set_vid_fmt(struct it668x_data *it668x )
{
    ...
    i2c_set_reg(mhl, REG_MHL_PATHEN_CTROL, B_SET_PATHEN|B_CLR_PATHEN, B_SET_PATHEN);  //tirgger hw set path_en and clock mode
    ...
    if(it668x->video_state != state){
        ...
        switch(state){
            ...
            case HDMI_VIDEO_WAIT:
                ...
                i2c_set_reg(hdmitx, REG_HDMITX_INT_06_MASK, B_MASK_VIDEO_STABLE|B_MASK_VIDEO_UNSTABLE|
                                                            B_MASK_INT_3D_PKT_PRESENT|B_MASK_INT_3D_PKT_LOSS,
                                                             0x00); 
...
static int it668x_config_data_path(struct it668x_data *it668x, unsigned int packet_regen)
{
    ...
    if(packet_regen == 0){
        //bypass control pkt
        i2c_set_reg(hdmitx, REG_HDMITX_PATH_CTRL, B_EN_REPEAT_CTROL_PKT|B_EN_TX_GEN_CTROL_PKT, 0x00);
        
        i2c_set_reg(hdmitx, REG_HDMITX_MHL_DATA_PATH, B_EN_AUTO_TXSRC|M_MHL_DATA_PATH, MHL_BYPASS_HDCP);

    }
    else{
        //gen control packet
        i2c_set_reg(hdmitx, REG_HDMITX_PATH_CTRL, B_EN_REPEAT_CTROL_PKT|B_EN_TX_GEN_CTROL_PKT, B_EN_REPEAT_CTROL_PKT|B_EN_TX_GEN_CTROL_PKT);
        
        i2c_set_reg(hdmitx, REG_HDMITX_MHL_DATA_PATH, B_EN_AUTO_TXSRC|M_MHL_DATA_PATH, MHL_TX_CTRL_HDCP);
    }
...
static int it668x_mhl_config_path(struct it668x_data *it668x, unsigned int ddc_bypass)
{
    ...
    if ( ( it668x->hdcp_enable ) ||
         ( (it668x->mhl_active_link_mode & (MHL_STATUS_CLK_PACKED_PIXEL|MHL_STATUS_CLK_NORMAL)) == MHL_STATUS_CLK_PACKED_PIXEL ) ||
...
static int it668x_hdmirx_ini(struct it668x_data *it668x)
{
    ...
    i2c_set_reg(hdmirx, REG_HDMIRX_IO_CONFIG, B_INT_IO_TYPE|B_INT_ACT_POPARITY, IO_INT_PUSH_PULL|IO_INT_ACT_LOW);  //push_pull ,high active
...
static int it668x_initial_chip(struct it668x_data *it668x)
{
    ...
    unsigned char vender_id[2]={0xFF,0xFF}, device_id[2]={0xFF,0xFF};
    unsigned char temp;
    ...
    if( (vender_id[0]==0x54) && (vender_id[1]==0x49) && (device_id[0]==0x81) && (device_id[1] == 0x66) ){
...
static int it668x_hdmirx_irq( struct it668x_data *it668x )
{
    ...
    if(force_cdr_reset == true){
        ...
        MHL_VERB_LOG(" rx_clk_stable change =%d, it668x->hdmi_hpd_rxsen=%d ",rx_clk_stable,it668x->hdmi_hpd_rxsen);
...
static int it668x_hdmitx_video_audio_irq(struct it668x_data *it668x)
{
    ...
    if( hdmitx_reg07 & (INT_RXSEN | INT_HPD) ){
        ...
        MHL_DEF_LOG(" HPD Change Interrupt HPD/RXsen(it668x->RxSen) ===> %d/%d(%d)!! ",
                        (int)((rd_reg&0x02)>>1),(int)(rd_reg&0x01), it668x->hdmi_hpd_rxsen);
    ...
    if( hdmitx_reg08 & INT_HDCP_AUTH_FAIL ) {
        ...
        if(it668x->video_state == HDMI_VIDEO_ON){
            ...
            if( it668x->hdcp_fail_time_out > HDCP_FAIL_TIMEOUT ) {
                ...
                it668x_hdcp_state(it668x, HDCP_CP_FAIL);
                MHL_ERR_LOG(" ERROR: Set Black Screen because of hdcp_fail_time_out>%d !!!\n", it668x->hdcp_fail_time_out);
            }
            else{
...
static int it668x_cbus_irq(struct it668x_data *it668x)
{
    ...
    if( mhl_reg04 & INT_CBUS_TX_PKT_FAIL ) {
        ...
        if( it668x->cbus_packet_fail_counter++>CBUS_COMM_FAIL_TIMEOUT ) {
            MHL_INT_LOG(" ERROR: CBUS Link Layer Error ==> Retry CBUS Discovery Process !!!\n");
    ...
    if( mhl_reg04 & INT_CBUS_NOT_DETECT ) {
        ...
        if( it668x->cbus_detect_timeout>CBUS_NO_5V_1k_TIMEOUT ) {
            ...
            MHL_DEF_LOG(" ERROR: CBUS Low and VBus5V Detect Error ==> Switch to USB Mode !!!\n");
    ...
    if( mhl_reg05 & INT_CBUS_MSC_REQ_FAIL ){
        ...
        //if( rd_reg&0x20 )
        //    MHL_ERR_LOG(" MSC_MSG Requester Receive NACK Packet !!! ==> %dth NACK\n", it668x->MSGNackCnt);
        //if( rd_reg&0x40 )
        //    MHL_ERR_LOG(" Disable HW Retry and MSC Requester Arbitration Lose at 1st Packet !!! ==> %dth Lose\n", it668x->MSCBusyCnt);
        //if( rd_reg&0x80 )
        //    MHL_ERR_LOG(" Disable HW Retry and MSC Requester Arbitration Lose before 1st Packet !!! ==> %dth Lose\n", it668x->MSCBusyCnt);
        ...
        if(  it668x->cbus_msc_fail_counter++>CBUS_COMM_FAIL_TIMEOUT ) {
            MHL_ERR_LOG(" ERROR: CBUS MSC Channel Error ==> Retry CBUS Discovery Process !!!\n");
    ...
    if( mhl_reg05 & INT_CBUS_DDC_REQ_FAIL ) {
        ...
        //    MHL_ERR_LOG(" ERROR: Receive ABORT in non-Idle State !!!\n");
         ...
         if(  it668x->cbus_ddc_fail_counter++>CBUS_COMM_FAIL_TIMEOUT ) {
            MHL_ERR_LOG(" ERROR: CBUS DDC Channel Error ==> Retry CBUS Discovery Process !!!\n");
            it668x_mhl_switch_state(it668x, MHL_CBUS_START);
        }
        #endif
    ...
    if( mhl_reg06 & INT_MHL_PATHEN_CANGE ) {
        ...
        MHL_INT_LOG(" CBUS PATH_EN Change Interrupt ...\n");
            ...
            if( rd_reg & MHL_STATUS_PATH_ENABLED ){
                ...
                i2c_set_reg(mhl, REG_MHL_PATHEN_CTROL, B_SET_PATHEN|B_CLR_PATHEN, B_CLR_PATHEN); 
                it668x->mhl_active_link_mode &= (~MHL_STATUS_PATH_ENABLED);
    ...
    if ( 0 == mhl_read_edid_block(it668x, 0, it668x->edidbuf) ) {
        ...
        if ( block_count > 0 ) {
            ...
            for( block=1 ; block<block_count+1 ; block++ ) {
                ...
                if ( -1 == mhl_read_edid_block( it668x, block, &it668x->edidbuf[128*block])) {
                    ret = -1;
                    break; 
                }
                  ...
                  unsigned char first_data, unsigned char second_data)
{
    ...
    //MHL_VERB_LOG(" ret = %2.2x  \n",ret);
   
    return ret ? 0 : -EIO;

}

static int mhl_cbus_msg_send_lock(struct it668x_data *it668x ,unsigned char sub_code , unsigned char sub_data)
{
...
static int mhl_cbus_misc_cmd_lock(struct it668x_data *it668x ,enum mhl_cbus_cmd_reg req_type, unsigned char sub_code , unsigned char sub_data)
{
...
static int mhl_devcap_read_locked(struct it668x_data *it668x ,unsigned char offset)
{
    ...
    MHL_VERB_LOG("IT668x  : mhl_devcap_read_locked(%2.2X) \n",offset);
...
static int mhl_queue_cbus_cmd_locked(struct it668x_data *it668x,enum cbus_command command, unsigned char offset, unsigned char data)
{
    struct cbus_data *cbus_cmd;
    ...
    //MHL_VERB_LOG("  cbus_cmd->cmd =  %2.2X  \n",cbus_cmd->cmd);
    //MHL_VERB_LOG("  cbus_cmd->offset =  %2.2X  \n",cbus_cmd->offset);
    //MHL_VERB_LOG("  cbus_cmd->data =  %2.2X  \n",cbus_cmd->data);
...
static void mhl_read_msc_msg(struct it668x_data *it668x)
{
    ...
    //MHL_MSC_LOG(" cbus_cmd->cmd =  %2.2X  \n",cbus_cmd->cmd);
    //MHL_MSC_LOG(" cbus_cmd->offset =  %2.2X  \n",cbus_cmd->offset);
    //MHL_MSC_LOG(" cbus_cmd->data =  %2.2X  \n",cbus_cmd->data);
...
void mhl_process_mscmsg(struct it668x_data *it668x ,unsigned char sub_cmd , unsigned char sub_data)
{
    switch( sub_cmd) {
        ...
        case MSG_RCPE :
            switch( sub_data ){
                case 0x00: MHL_MSC_LOG(" No Error\n"); break;
                case 0x01: MHL_MSC_LOG(" ERROR: Ineffective RCP Key Code !!!\n"); break;
                case 0x02: MHL_MSC_LOG(" Responder Busy ...\n"); break;
                default  : MHL_MSC_LOG(" ERROR: Unknown RCP status code !!!\n");
            }
            break;
        ...
        case MSG_RAPK :
             ...
             switch(sub_data) {
                case 0x00: MHL_MSC_LOG(" No Error\n"); break;
                case 0x01: MHL_MSC_LOG(" ERROR: Unrecognized Action Code !!!\n"); break;
                case 0x02: MHL_MSC_LOG(" ERROR: Unsupported Action Code !!!\n"); break;
                case 0x03: MHL_MSC_LOG(" Responder Busy ...\n"); break;
                default  : MHL_MSC_LOG(" ERROR: Unknown RAP status code 0x%02X !!!\n",sub_data);
            }
            break;
        ...
        if(it668x->mhl_state == MHL_CBUS_CONNECTED){
            switch (data->cmd) {
                ...
                //
                    ...
                    ret = mhl_cbus_Write_state_int_lock(it668x, data->offset, data->data);

                    if (ret < 0)
                        MHL_ERR_LOG(" error STATE_INT req\n");

                    if(data->offset == CBUS_MHL_STATUS_OFFSET_0 && data->data == MHL_STATUS_DCAP_READY) {

                        mhl_cbus_Write_state_int_lock(it668x,CBUS_MHL_INTR_REG_0,MHL_INT_DCAP_CHG);
                    }
                ...
                case GET_STATE:
                    ret = mhl_cbus_misc_cmd_lock(it668x, FIRE_GET_STATE , data->offset, data->data);
                    break;
                case GET_VENDOR_ID:
                    ret = mhl_cbus_misc_cmd_lock(it668x, FIRE_GET_VENDOR_ID , data->offset, data->data);
                    break;
                /*
                //sink only command
                case SET_HPD:
                ...
                */
                case GET_MSC_ERR_CODE:
                    ret = mhl_cbus_misc_cmd_lock(it668x, FIRE_GET_MSC_ERRORCODE , data->offset, data->data);
                    break;
                //case GET_SC3_ERR_CODE:
                //case GET_SC1_ERR_CODE:
                case GET_DDC_ERR_CODE:
                    ret = mhl_cbus_misc_cmd_lock(it668x, FIRE_GET_DDC_ERRORCODE , data->offset, data->data);
                    break;
...
static int it668x_mhl_detect(void *data)
{
    ...
    for(;;){
        ...
        wait_event_interruptible_timeout(it668x->it668x_wq, it668x->mhl_state==MHL_CBUS_CONNECTED ,msecs_to_jiffies(2500));//100ms
...
void mhl_edid_raw_data(char *pbuf)
{
   ...
   for(bTemp=0; bTemp<2; bTemp++)
   {
     ...
     for(i=0; i< 8; i++)
     {
      ...
      for(k=0;k<16;k++)
      {
      	if(k==15)
      	{
      	  if((j+ (i*16+k))< MHL_EDID_SIZE)//for Buffer overflow error
      	  {
      	  if(pbuf[j+ (i*16+k)]> 0x0f)
          printk("%2x\n", pbuf[j+ (i*16+k)]);
          else
          printk("0%x\n", pbuf[j+ (i*16+k)]);
        }
      	...
      	else
      	{
      	   ...
      	   {
        if(pbuf[j+ (i*16+k)]> 0x0f)
          printk("%2x  ", pbuf[j+ (i*16+k)]);
        else
          printk("0%x  ", pbuf[j+ (i*16+k)]);
     }
     }
...
ssize_t it668x_fop_write (struct file * file,const char * buf, size_t count,loff_t * f_ops)
{
    ...
    if ( (count > cmp_len) && ( 0 == strncmp(buf, cmp_str, cmp_len)) )
    {
        ...
        MHL_DBG_LOG("[it668x] devcap:0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
                it668x->mhl_devcap[0],it668x->mhl_devcap[1],it668x->mhl_devcap[2],it668x->mhl_devcap[3],
                it668x->mhl_devcap[4],it668x->mhl_devcap[5],it668x->mhl_devcap[6],it668x->mhl_devcap[7],
                it668x->mhl_devcap[8],it668x->mhl_devcap[9],it668x->mhl_devcap[10],it668x->mhl_devcap[11],
                it668x->mhl_devcap[12],it668x->mhl_devcap[13],it668x->mhl_devcap[14],it668x->mhl_devcap[15]);
    ...
    {
        MHL_DBG_LOG("exec cmd : %s", buf);
        mhl_edid_raw_data(it668x->edidbuf);
        goto __leave;
    }
        ...
        const char *buf, size_t size)
{
    ...
    if( sscanf(buf,"%X %X ", &sub_code, &sub_data) ){
        ...
        const char *buf, size_t size)
{
    ...
    if( sscanf(buf,"%X %X ", &enable, &color) ){
...
static ssize_t it668x_devctl_show(struct class *class,
                    struct class_attribute *attr,
                    char *buf)
{
        ...
        const char *buf, size_t size)
{
    ...
    switch ( devctl->op ) {
        ...
        case DEV_CTL_SET_REG:
            ...
            {
                case IT668X_I2C_HDMI_RX: 
                    i2c_set_reg(hdmirx, devctl->set_reg.offset, devctl->set_reg.mask, devctl->set_reg.data);
                    break;
                case IT668X_I2C_HDMI_TX:
                    i2c_set_reg(hdmitx, devctl->set_reg.offset, devctl->set_reg.mask, devctl->set_reg.data);
                    break;
                case IT668X_I2C_MHL_TX:
                    i2c_set_reg(mhltx, devctl->set_reg.offset, devctl->set_reg.mask, devctl->set_reg.data);
                    break;
                default:
                    break;
            }
...
{
    it668x->debugfs = debugfs_create_file("it668x", S_IFREG|S_IALLUGO, NULL, (void*)0, &it668x_debug_fops);
...
static void it668x_switch_to_usb(struct it668x_data *it668x, bool keep_detection)
{
    ...
    if ( keep_detection ) {
        ret = i2c_set_reg( mhl, REG_MHL_CONTROL, B_USB_SWITCH_ON | B_DISABLE_MHL, B_USB_SWITCH_ON );    //Switch to USB, Cbus standby
    }
    else {
        ret = i2c_set_reg( mhl, REG_MHL_CONTROL, B_USB_SWITCH_ON | B_DISABLE_MHL, B_USB_SWITCH_ON | B_DISABLE_MHL );    //Switch to USB, fsm off
    }
...
static void it668x_switch_to_mhl(struct it668x_data *it668x)
{
    ...
    ret = i2c_set_reg( mhl, REG_MHL_CONTROL, B_USB_SWITCH_ON | B_DISABLE_MHL, 0x00 );    //Switch to MHL, Cbus standby
    if (ret < 0) {
        MHL_VERB_LOG(" %s: i2c_set_reg=%d\n", __FUNCTION__, ret);	
    }
    else{
...
int mhl_bridge_getedid(unsigned char *pedidbuf)
{
    ...
    if ( it668x->is_edid_ready ) {
        ...
        IT668X_MUTEX_LOCK(&it668x->lock);
        copy_len = min( IT668X_EDID_BUF_LEN, buf_len );
        memcpy( pedidbuf, &it668x->edidbuf[0], copy_len ); 
        IT668X_MUTEX_UNLOCK(&it668x->lock);
    }
    else {
        if ( it668x->use_internal_edid_before_edid_ready ) {
            MHL_DEF_LOG("[it668x] get from inter edid\n");
            copy_len = min( (int)sizeof(s_it668x_internal_edid), buf_len );
            memcpy( pedidbuf, &s_it668x_internal_edid[0], copy_len ); 
        }
        else{
            return -1;
        }
...
void mhl_bridge_mutehdmi(unsigned char enable)
{
    ...
    if ( NULL == it668x || NULL == it668x->pdata || NULL == it668x->pdata->hdmitx_client ) {
        // should not call this interface before mhl driver initialized.
        return;
    }
