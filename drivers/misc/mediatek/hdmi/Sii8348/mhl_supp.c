...
struct mhl_dev_context *get_mhl_device_context(void *context)
{
	...
	if (dev_context->signature != MHL_DEV_CONTEXT_SIGNATURE)
		dev_context = container_of(context, struct mhl_dev_context, drv_context);
	return dev_context;
}
...
static void return_cbus_queue_entry(struct mhl_dev_context *dev_context, struct cbus_req *pReq)
{
	list_add(&pReq->link, &dev_context->cbus_free_list);

}

void queue_cbus_transaction(struct mhl_dev_context *dev_context, struct cbus_req *pReq)
{
	MHL_TX_DBG_INFO(dev_context, "0x%02x 0x%02x 0x%02x\n",
			pReq->command,
			(MHL_MSC_MSG == pReq->command) ?
			pReq->msg_data[0] : pReq->reg,
			(MHL_MSC_MSG == pReq->command) ? pReq->msg_data[1] : pReq->reg_data);

	list_add_tail(&pReq->link, &dev_context->cbus_queue);
}

void queue_priority_cbus_transaction(struct mhl_dev_context *dev_context, struct cbus_req *req)
{
	MHL_TX_DBG_INFO(dev_context, "0x%02x 0x%02x 0x%02x\n",
			req->command,
			(MHL_MSC_MSG == req->command) ?
			req->msg_data[0] : req->reg,
			(MHL_MSC_MSG == req->command) ? req->msg_data[1] : req->reg_data);
 ...
 */
bool si_mhl_tx_set_status(struct mhl_dev_context *dev_context, uint8_t reg_to_write, uint8_t value,
...
bool si_mhl_tx_set_int(struct mhl_dev_context *dev_context,
		       uint8_t reg_to_write, uint8_t mask, uint8_t priority_level)
{
	struct cbus_req *req;
...
#ifdef MEDIA_DATA_TUNNEL_SUPPORT
	memset(dev_context->mdt_devs.is_dev_registered, INPUT_WAITING_FOR_REGISTRATION,
	       MDT_TYPE_COUNT);
...
int si_mhl_tx_initialize(struct mhl_dev_context *dev_context, bool bootup)
{
	...
	return dev_context->drv_info->mhl_device_initialize((struct drv_hw_context *)(&dev_context->
										      drv_context));
}
...
static void cbus_abort_timer_callback(void *callback_param)
{
	...
	MHL_TX_DBG_INFO(dev_context, "CBUS abort timer expired, " "enable CBUS messaging\n");
...
void si_mhl_tx_drive_states(struct mhl_dev_context *dev_context)
{
	...
	if (MHL_SET_INT == req->command) {
		if (MHL_RCHANGE_INT == req->reg) {
			...
			if (dev_context->misc_flags.flags.scratchpad_busy
			    || !dev_context->scratch_pad_read_done) {
	...
	if (req) {
		...
		success = si_mhl_tx_drv_send_cbus_command((struct drv_hw_context *)
...
enum scratch_pad_status si_mhl_tx_request_write_burst(struct mhl_dev_context *dev_context,
						      uint8_t reg_offset, uint8_t length,
						      uint8_t *data)
{
	struct cbus_req *req;
	enum scratch_pad_status status = SCRATCHPAD_BUSY;

	if (!(dev_context->dev_cap_cache.mdc.featureFlag & MHL_FEATURE_SP_SUPPORT)) {
 ...
 */
static bool si_mhl_rapk_send(struct mhl_dev_context *dev_context, uint8_t status)
{
	...
	if (dev_context->mhl_connection_event) {
		...
		case MHL_MSC_MSG_RAP:
			...
			if (MHL_RAP_POLL == dev_context->msc_msg_data) {
				...
				si_mhl_tx_drv_content_on((struct drv_hw_context *)&dev_context->
							 drv_context);
				...
				if (dev_context->misc_flags.flags.rap_content_on) {
					...
					si_mhl_tx_drv_content_off((struct drv_hw_context *)
...
bool si_mhl_tx_read_devcap(struct mhl_dev_context *dev_context, uint8_t offset)
{
	struct cbus_req *req;
...
void si_mhl_tx_msc_command_done(struct mhl_dev_context *dev_context, uint8_t data1)
{
	...
	if (req->status.flags.cancel == true) {
		...
		if (req->reg < DEVCAP_SIZE)	/* req->reg keep the current offset */
		{
			...
			MHL_TX_DBG_INFO(dev_context,
					"MHL_READ_DEVCAP DONE, idx: 0x%02x, data: 0x%02x\n",
					req->reg, data1);
		}
		...
		if (0 == data1) {
			si_mhl_tx_handle_atomic_hw_edid_read_complete(dev_context->
								      edid_parser_context, req);
		...
		si_mhl_tx_drv_set_hw_tpi_mode((struct drv_hw_context *)(&dev_context->drv_context),
					      true);
		...
		if (dev_context->intr_info.flags & DRV_INTR_FLAG_MSC_NAK) {
			...
			if (MHL_MSC_MSG_RCPE == req->msg_data[0]) {
				...
				MHL_TX_DBG_INFO(dev_context, "default\n"
						...
						req->command,
						req->msg_data[0], dev_context->msc_msg_last_data);
			}
		}
		 ...
		 * head of the queue.
		 */
		si_mhl_tx_set_int(dev_context, MHL_RCHANGE_INT, MHL_INT_DSCR_CHG, 0);
		...
		MHL_TX_DBG_INFO(dev_context, "default\n"
				...
				req->offset, req->msg_data[0], req->msg_data[1]);
	}
	...
	if (!(dev_context->misc_flags.flags.rcp_ready)) {
		MHL_TX_DBG_INFO(dev_context, "have(%s) sent(%s %s)\n",
				...
				"PATH_EN" : "",
				(dev_context->misc_flags.flags.sent_dcap_rdy) ? "DCAP_RDY" : "");
...
void si_mhl_tx_process_write_burst_data(struct mhl_dev_context *dev_context)
{
	...
	if (ret_val < 0) {
		...
		burst_id = BURST_ID(dev_context->incoming_scratch_pad.videoFormatData.burst_id);

		switch (burst_id) {
		case burst_id_3D_VIC:
			si_mhl_tx_process_3d_vic_burst(dev_context->edid_parser_context,
						       &dev_context->incoming_scratch_pad.
						       videoFormatData);
			break;

		case burst_id_3D_DTD:
			si_mhl_tx_process_3d_dtd_burst(dev_context->edid_parser_context,
						       &dev_context->incoming_scratch_pad.
						       videoFormatData);
			break;
...
static bool si_mhl_tx_set_path_en(struct mhl_dev_context *dev_context)
{
	...
	return si_mhl_tx_set_status(dev_context, MHL_STATUS_REG_LINK_MODE, dev_context->link_mode,
				    1);
}
...
static void si_mhl_tx_refresh_peer_devcap_entries(struct mhl_dev_context *dev_context)
{
	if (MHL_STATUS_DCAP_RDY & dev_context->status_0) {
		...
		MHL_TX_DBG_INFO(dev_context,
				"Can't read DEV CAP registers, DCAP_RDY not set yet\n");
	}
}
...
void si_mhl_tx_got_mhl_intr(struct mhl_dev_context *dev_context, uint8_t intr_0, uint8_t intr_1)
{
	...
	if (MHL_INT_REQ_WRT & intr_0) {
		...
		if (dev_context->misc_flags.flags.scratchpad_busy
			 ...
			 */
			si_mhl_tx_set_int(dev_context, MHL_RCHANGE_INT, MHL_INT_GRT_WRT, 1);
		} else {
			dev_context->misc_flags.flags.scratchpad_busy = true;
			/* use priority 0 to respond immediately */
			si_mhl_tx_set_int(dev_context, MHL_RCHANGE_INT, MHL_INT_GRT_WRT, 0);
		}
	}
 ...
 */
void si_mhl_tx_got_mhl_status(struct mhl_dev_context *dev_context, uint8_t status_0,
			      uint8_t status_1)
{
 ...
 */
bool si_mhl_tx_ucp_send(struct mhl_dev_context *dev_context, uint8_t ucp_key_code)
{
 ...
 * UCP keycode.
 */
bool si_mhl_tx_ucpe_send(struct mhl_dev_context *dev_context, uint8_t ucpe_error_code)
{
	MHL_TX_DBG_INFO(dev_context, "called\n");
 ...
 */
bool si_mhl_tx_rap_send(struct mhl_dev_context *dev_context, uint8_t rap_action_code)
{
	bool status;
 ...
 */
enum scratch_pad_status si_get_scratch_pad_vector(struct mhl_dev_context *dev_context,
						  uint8_t offset, uint8_t length, uint8_t *data)
{
	if (!(dev_context->dev_cap_cache.mdc.featureFlag & MHL_FEATURE_SP_SUPPORT)) {
...
void DumpIncomingInfoFrameImpl(char *pszId, char *pszFile, int iLine, info_frame_t *pInfoFrame,
			       uint8_t length)
{
	uint8_t j;
...
int si_mhl_tx_shutdown(struct mhl_dev_context *dev_context)
{
	MHL_TX_DBG_ERR(dev_context,
		       "SiI8348 may continue to output video. Driver features and APIs will not work.\n");
