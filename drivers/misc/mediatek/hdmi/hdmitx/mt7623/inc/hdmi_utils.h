...
#define HDMI_LOG(fmt, arg...) \
    do { \
        if (hdmi_log_on) {pr_err_ratelimited("[hdmi]#%d ", __LINE__); pr_err_ratelimited(fmt, ##arg);} \
    }while (0)

#define HDMI_FUNC()    \
    do { \
        if(hdmi_log_on) pr_err_ratelimited("[hdmi] %s\n", __func__); \
    }while (0)

#define HDMI_LINE()    \
    do { \
        if (hdmi_log_on) {pr_err_ratelimited("[hdmi]%s,%d ", __func__, __LINE__); pr_err_ratelimited(fmt, ##arg);} \
    }while (0)

#define HDMI_MSG(fmt, arg...) \
    do { \
        {pr_err_ratelimited("[hdmi]#%d ", __LINE__); pr_err_ratelimited(fmt, ##arg);} \
    }while (0)
