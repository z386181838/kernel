...
extern const MHL_BRIDGE_DRIVER * mhl_bridge_drv;
#endif
     ...
     for(i=0; i< 8; i++)
     {
      ...
      for(k=0;k<16;k++)
      {
      	...
      	else
      	{
      	   ...
      	   {
        if(_bEdidData[j+ (i*16+k)]> 0x0f)
          HDMI_LOG("%2x  ", _bEdidData[j+ (i*16+k)]);
        else
          HDMI_LOG("0%x  ", _bEdidData[j+ (i*16+k)]);
     }
     }
...
bool vGetTxPortEdid(unsigned char *prBuff, unsigned char u1BlcokNum, unsigned char u1Len)
{
	unsigned char u1Ind;

	if((u1BlcokNum > 3) || (u1Len > 128))
	{
		return FALSE;
	}
	if((i4SharedInfo(SI_EDID_PARSING_RESULT) == TRUE)&&(_HdmiSinkAvCap.b_sink_edid_ready == TRUE))
	{
		for(u1Ind =0; u1Ind < u1Len; u1Ind++)
		{
			*(prBuff+u1Ind) = _bEdidData[u1BlcokNum*128+ u1Ind];
		}
		return TRUE;
	}
	else
...
{
    return  _stAvdAVInfo.b_hotplug_state;
}
...
void hdmi_checkedid(unsigned char i1noedid)
{
  ...
  {
     HDMI_LOG("[it668x]can not support pp mode\n");
    ...
    HDMI_LOG("[it668x]support pp mode\n");
#endif
...
void vShowEdidInformation(void)
{
    ...
    if(u4Res & SINK_480I)
    HDMI_LOG("[HDMI]SUPPORT 1440x480I 59.94hz \n");
    if(u4Res & SINK_480I_1440)
    HDMI_LOG("[HDMI]SUPPORT 2880x480I 59.94hz \n");
    if(u4Res & SINK_480P)
    HDMI_LOG("[HDMI]SUPPORT 720x480P 59.94hz\n");
    if(u4Res & SINK_480P_1440)
    HDMI_LOG("[HDMI]SUPPORT 1440x480P 59.94hz \n");
    if(u4Res & SINK_480P_2880)
    HDMI_LOG("[HDMI]SUPPORT 2880x480P 59.94hz \n");
    if(u4Res & SINK_720P60)
    HDMI_LOG("[HDMI]SUPPORT 1280x720P 59.94hz \n");
    if(u4Res & SINK_1080I60)
    HDMI_LOG("[HDMI]SUPPORT 1920x1080I 59.94hz \n");
    if(u4Res & SINK_1080P60)
    HDMI_LOG("[HDMI]SUPPORT 1920x1080P 59.94hz \n");

    if(u4Res & SINK_576I)
    HDMI_LOG("[HDMI]SUPPORT 1440x576I 50hz \n");
    if(u4Res & SINK_576I_1440)
    HDMI_LOG("[HDMI]SUPPORT 2880x576I 50hz \n");
    if(u4Res & SINK_576P)
    HDMI_LOG("[HDMI]SUPPORT 720x576P 50hz \n");
    if(u4Res & SINK_576P_1440)
    HDMI_LOG("[HDMI]SUPPORT 1440x576P 50hz \n");
    if(u4Res & SINK_576P_2880)
    HDMI_LOG("[HDMI]SUPPORT 2880x576P 50hz \n");
    if(u4Res & SINK_720P50)
    HDMI_LOG("[HDMI]SUPPORT 1280x720P 50hz \n");
    if(u4Res & SINK_1080I50)
    HDMI_LOG("[HDMI]SUPPORT 1920x1080I 50hz \n");
    if(u4Res & SINK_1080P50)
    HDMI_LOG("[HDMI]SUPPORT 1920x1080P 50hz \n");
    if(u4Res & SINK_1080P30)
    HDMI_LOG("[HDMI]SUPPORT 1920x1080P 30hz \n");
    if(u4Res & SINK_1080P24)
    HDMI_LOG("[HDMI]SUPPORT 1920x1080P 24hz \n");
    if(u4Res & SINK_1080P25)
    HDMI_LOG("[HDMI]SUPPORT 1920x1080P 25hz \n");

    u4Res= (_HdmiSinkAvCap.ui4_sink_native_ntsc_resolution|_HdmiSinkAvCap.ui4_sink_native_pal_resolution);
    HDMI_LOG("[HDMI]NTSC Native =%x \n", _HdmiSinkAvCap.ui4_sink_native_ntsc_resolution);
    HDMI_LOG("[HDMI]PAL Native =%x \n", _HdmiSinkAvCap.ui4_sink_native_pal_resolution);
    if(u4Res & SINK_480I)
    HDMI_LOG("[HDMI]Native resolution is 1440x480I 59.94hz \n");
    if(u4Res & SINK_480I_1440)
    HDMI_LOG("[HDMI]Native resolution is 2880x480I 59.94hz \n");
    if(u4Res & SINK_480P)
    HDMI_LOG("[HDMI]Native resolution is 720x480P 59.94hz\n");
    if(u4Res & SINK_480P_1440)
    HDMI_LOG("[HDMI]Native resolution is 1440x480P 59.94hz \n");
    if(u4Res & SINK_480P_2880)
    HDMI_LOG("[HDMI]Native resolution is 2880x480P 59.94hz \n");
    if(u4Res & SINK_720P60)
    HDMI_LOG("[HDMI]Native resolution is 1280x720P 59.94hz \n");
    if(u4Res & SINK_1080I60)
    HDMI_LOG("[HDMI]Native resolution is 1920x1080I 59.94hz \n");
    if(u4Res & SINK_1080P60)
    HDMI_LOG("[HDMI]Native resolution is 1920x1080P 59.94hz \n");
    if(u4Res & SINK_576I)
    HDMI_LOG("[HDMI]Native resolution is 1440x576I 50hz \n");
    if(u4Res & SINK_576I_1440)
    HDMI_LOG("[HDMI]Native resolution is 2880x576I 50hz \n");
    if(u4Res & SINK_576P)
    HDMI_LOG("[HDMI]Native resolution is 720x576P 50hz \n");
    if(u4Res & SINK_576P_1440)
    HDMI_LOG("[HDMI]Native resolution is 1440x576P 50hz \n");
    if(u4Res & SINK_576P_2880)
    HDMI_LOG("[HDMI]Native resolution is 2880x576P 50hz \n");
    if(u4Res & SINK_720P50)
    HDMI_LOG("[HDMI]Native resolution is 1280x720P 50hz \n");
    if(u4Res & SINK_1080I50)
    HDMI_LOG("[HDMI]Native resolution is 1920x1080I 50hz \n");
    if(u4Res & SINK_1080P50)
    HDMI_LOG("[HDMI]Native resolution is 1920x1080P 50hz \n");
    if(u4Res & SINK_1080P30)
    HDMI_LOG("[HDMI]Native resolution is 1920x1080P 30hz \n");
    if(u4Res & SINK_1080P24)
    HDMI_LOG("[HDMI]Native resolution is 1920x1080P 24hz \n");
    if(u4Res & SINK_1080P25)
    HDMI_LOG("[HDMI]Native resolution is 1920x1080P 25hz \n");


    HDMI_LOG("[HDMI]SUPPORT RGB\n");
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_YCBCR_444)
    {
    HDMI_LOG("[HDMI]SUPPORT YCBCR 444\n");
    }
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_YCBCR_422)
    {
    HDMI_LOG("[HDMI]SUPPORT YCBCR 422\n");
    }
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_XV_YCC709)
    {
    HDMI_LOG("[HDMI]SUPPORT xvYCC 709\n");
    }
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_XV_YCC601)
    {
    HDMI_LOG("[HDMI]SUPPORT xvYCC 601\n");
    }
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_METADATA0)
    {
    HDMI_LOG("[HDMI]SUPPORT metadata P0\n");
    }
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_METADATA1)
    {
    HDMI_LOG("[HDMI]SUPPORT metadata P1\n");
    }
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_METADATA2)
    {
    HDMI_LOG("[HDMI]SUPPORT metadata P2\n");
    }


    if(_HdmiSinkAvCap.e_sink_ycbcr_color_bit & HDMI_SINK_DEEP_COLOR_10_BIT)
    HDMI_LOG("[HDMI]SUPPORT YCBCR 30 Bits Deep Color \n");
    if(_HdmiSinkAvCap.e_sink_ycbcr_color_bit & HDMI_SINK_DEEP_COLOR_12_BIT)
    HDMI_LOG("[HDMI]SUPPORT YCBCR 36 Bits Deep Color \n");
    if(_HdmiSinkAvCap.e_sink_ycbcr_color_bit & HDMI_SINK_DEEP_COLOR_16_BIT)
    HDMI_LOG("[HDMI]SUPPORT YCBCR 48 Bits Deep Color \n");
    if(_HdmiSinkAvCap.e_sink_ycbcr_color_bit == HDMI_SINK_NO_DEEP_COLOR)
    HDMI_LOG("[HDMI]Not SUPPORT YCBCR Deep Color \n");

    if(_HdmiSinkAvCap.e_sink_rgb_color_bit & HDMI_SINK_DEEP_COLOR_10_BIT)
    HDMI_LOG("[HDMI]SUPPORT RGB 30 Bits Deep Color \n");
    if(_HdmiSinkAvCap.e_sink_rgb_color_bit & HDMI_SINK_DEEP_COLOR_12_BIT)
    HDMI_LOG("[HDMI]SUPPORT RGB 36 Bits Deep Color \n");
    if(_HdmiSinkAvCap.e_sink_rgb_color_bit & HDMI_SINK_DEEP_COLOR_16_BIT)
    HDMI_LOG("[HDMI]SUPPORT RGB 48 Bits Deep Color \n");
    if(_HdmiSinkAvCap.e_sink_rgb_color_bit == HDMI_SINK_NO_DEEP_COLOR)
    HDMI_LOG("[HDMI]Not SUPPORT RGB Deep Color \n");

    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_LPCM)
    HDMI_LOG("[HDMI]SUPPORT LPCM \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_AC3)
    HDMI_LOG("[HDMI]SUPPORT AC3 Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_MPEG1)
    HDMI_LOG("[HDMI]SUPPORT MPEG1 Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_MP3)
    HDMI_LOG("[HDMI]SUPPORT AC3 Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_MPEG2)
    HDMI_LOG("[HDMI]SUPPORT MPEG2 Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_AAC)
    HDMI_LOG("[HDMI]SUPPORT AAC Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DTS)
    HDMI_LOG("[HDMI]SUPPORT DTS Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_ATRAC)
    HDMI_LOG("[HDMI]SUPPORT ATRAC Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DSD)
    HDMI_LOG("[HDMI]SUPPORT SACD DSD Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DOLBY_PLUS)
    HDMI_LOG("[HDMI]SUPPORT Dolby Plus Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DTS_HD)
    HDMI_LOG("[HDMI]SUPPORT DTS HD Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_MAT_MLP)
    HDMI_LOG("[HDMI]SUPPORT MAT MLP Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DST)
    HDMI_LOG("[HDMI]SUPPORT SACD DST Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_WMA)
    HDMI_LOG("[HDMI]SUPPORT  WMA Decode \n");
    ...
    HDMI_LOG("[HDMI]SUPPORT PCM Max 2CH, Fs is: %s \n", &cDstStr[0]);
    ...
    HDMI_LOG("[HDMI]SUPPORT PCM Max 6CH Fs is: %s \n", &cDstStr[0]);
    ...
    HDMI_LOG("[HDMI]SUPPORT PCM Max 7CH Fs is: %s \n", &cDstStr[0]);
    ...
    HDMI_LOG("[HDMI]SUPPORT PCM Max 8CH, FS is: %s \n", &cDstStr[0]);

    }

    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_FL_FR)
    HDMI_LOG("[HDMI]Speaker FL/FR allocated \n");
    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_LFE)
    HDMI_LOG("[HDMI]Speaker LFE allocated \n");
    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_FC)
    HDMI_LOG("[HDMI]Speaker FC allocated \n");
    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_RL_RR)
    HDMI_LOG("[HDMI]Speaker RL/RR allocated \n");
    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_RC)
    HDMI_LOG("[HDMI]Speaker RC allocated \n");
    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_FLC_FRC)
    HDMI_LOG("[HDMI]Speaker FLC/FRC allocated \n");
    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_RLC_RRC)
    HDMI_LOG("[HDMI]Speaker RLC/RRC allocated \n");

	HDMI_LOG("[HDMI]HDMI edid support content type =%x\n",_HdmiSinkAvCap.ui1_sink_content_cnc);
    HDMI_LOG("[HDMI]Lip Sync Progressive audio latency = %d\n", _HdmiSinkAvCap.ui1_sink_p_audio_latency);
    HDMI_LOG("[HDMI]Lip Sync Progressive video latency = %d\n", _HdmiSinkAvCap.ui1_sink_p_video_latency);
    if(_HdmiSinkAvCap.ui1_sink_i_latency_present)
    {
    HDMI_LOG("[HDMI]Lip Sync Interlace audio latency = %d\n", _HdmiSinkAvCap.ui1_sink_i_audio_latency);
    HDMI_LOG("[HDMI]Lip Sync Interlace video latency = %d\n", _HdmiSinkAvCap.ui1_sink_i_video_latency);
    }

    if(_HdmiSinkAvCap.ui1_sink_support_ai == 1)
    HDMI_LOG("[HDMI]Support AI \n");
    else
    HDMI_LOG("[HDMI]Not Support AI \n");

    HDMI_LOG("[HDMI]Monitor Max horizontal size = %d\n", _HdmiSinkAvCap.ui1_Display_Horizontal_Size);
    HDMI_LOG("[HDMI]Monitor Max vertical size = %d\n", _HdmiSinkAvCap.ui1_Display_Vertical_Size);


     if( _HdmiSinkAvCap.b_sink_hdmi_video_present==TRUE)
     	HDMI_LOG("[HDMI]HDMI_Video_Present\n");
     else
     	HDMI_LOG("[HDMI]No HDMI_Video_Present\n");

     if( _HdmiSinkAvCap.b_sink_3D_present==TRUE)
     	HDMI_LOG("[HDMI]3D_present\n");
     else
     	HDMI_LOG("[HDMI]No 3D_present\n");

}


void hdmi_appGetEdidRawData(char * edid_buf, int buf_size)
{
	...
	memcpy(edid_buf, _bEdidData, EDID_SIZE);
	return ;
}
