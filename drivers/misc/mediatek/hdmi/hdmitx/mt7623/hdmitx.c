...
#undef OUTREG32
#define OUTREG32(x, y) {/*HDMI_LOG("[hdmi]write 0x%08x to 0x%08x\n", (y), (x)); */__OUTREG32((x),(y))}
...
#if 1 // defined(CONFIG_MTK_INTERNAL_HDMI_SUPPORT) ||defined(CONFIG_MTK_INTERNAL_MHL_SUPPORT)
...
#if DISPLAY_V4L2
BUF_NOTIFY_CALLBACK    hdmitx_buf_receive_cb = NULL;
BUF_NOTIFY_CALLBACK    hdmitx_buf_remove_cb = NULL;
#endif
...
#define HDMI_BUFFER_LOG(fmt, arg...) \
    do { \
        if(enable_hdmi_buffer_log){HDMI_LOG("[hdmi_buffer] "); HDMI_LOG(fmt, ##arg);} \
    }while (0)
#else
bool enable_hdmi_buffer_log = 0;
#define HDMI_BUFFER_LOG(fmt, arg...)
...
static int hdmi_alloc_buffer(M4U_PORT_ID_ENUM port, unsigned int size, unsigned int *pVa, unsigned int *pMva)
{
    ...
    *pVa = va;
    ...
    HDMI_LOG("[Pattern] alloc_buffer va=0x%08x, mva=0x%08x\n", va, mva);
...
KREE_SESSION_HANDLE hdmi_pattern_secure_memory_session_handle(void)
{
    ...
    HDMI_LOG("hdmi_pattern_secure_memory_session_handle() acquire TEE session\n");
    if (NULL == hdmi_pattern_secure_memory_session)
    {
        TZ_RESULT ret;
        HDMI_LOG("hdmi_pattern_secure_memory_session_handle() create session\n");
        ret = KREE_CreateSession(TZ_TA_MEM_UUID, &hdmi_pattern_secure_memory_session);
        if (ret != TZ_RESULT_SUCCESS)
        {
            HDMI_LOG("KREE_CreateSession fail, ret=%d\n", ret);
            return NULL;
        }
    }

    HDMI_LOG("hdmi_pattern_secure_memory_session_handle() session=%x\n",
...
void *hdmi_pattern_allocate_secure_memory(int size)
{
    ...
    if (ret != TZ_RESULT_SUCCESS)
    {
		HDMI_LOG("KREE_AllocSecurechunkmem fail, ret=%d \n", ret);
        return NULL;
    }

    HDMI_LOG("KREE_AllocSecurchunkemem handle=0x%x \n", mem_handle);
	...
	if(ret!= TZ_RESULT_SUCCESS)
	{
		HDMI_LOG("KREE_TeeServiceCall(TZCMD_DDP_SECURE_MVA_MAP) fail, ret=%d \n", ret);
...
static struct sw_sync_timeline *hdmi_create_timeline(void)
{
    ...
    {
        HDMI_LOG(" error: cannot create timeline! \n");
    }
    else
    {
...
static struct fence_data hdmi_create_fence(void)
{
    ...
    if (hdmi_timeline != NULL)
    {
        ...
        {
            HDMI_LOG(" error: cannot create Fence Object! \n");
        }
        else
        {
            fenceFd = data.fence;
    ...
    {
        HDMI_LOG(" error: there is no Timeline to create Fence! \n");
    }
...
static int hdmi_rdma_update_kthread(void *data)
{
    ...
    for (;;)
    {
        ...
        if (wait_event_interruptible_timeout(hdmi_rdma_update_wq, atomic_read(&hdmi_rdma_update_event), HZ / 4) == 0)
        {
            ...
            {
                HDMI_LOG("[hdmi] sw rdma update. early_suspend=%d, %d (%lu)\n", is_early_suspended, p->is_clock_on, timeout_cnt);
            }
        ...
        else
        {
            if (timeout_cnt > 0)
            {
                HDMI_LOG("[hdmi] hw rdma update after %lu.\n", timeout_cnt);
                timeout_cnt = 0;
            }
        }
                ...
                while (pBuffList && !list_is_last(&pBuffList->list, &HDMI_Buffer_List))
                {
					if (pBuffList && ((pBuffList->buf_state == buf_read_done) || (pBuffList->buf_state == buf_dropped)))
					{
						...
						//do {
                            ...
                            #if DISPLAY_V4L2
                            if (pBuffList->buf_remove)
                            {
                                pBuffList->buf_remove(pBuffList->buffer_info.src_phy_addr);
                            }
                            #endif
...
void hdmi_config_pll(HDMI_VIDEO_RESOLUTION resolution)
{
	switch(resolution)
	{
		...
		case HDMI_VIDEO_720x576p_50Hz:
			vWriteHdmiANA(0x00, 0x7E00E131);
			mdelay(5);
			vWriteHdmiANA(0x00, 0x7E00E133);
			vWriteHdmiANA(0x25C, 0x00000003);
			mdelay(5);
			vWriteHdmiANA(0x25C, 0x00000001);
			mdelay(5);
			vWriteHdmiANA(0x254, 0x80109D8A);
			mdelay(5);
			vWriteHdmiANA(0x250, 0x00000141);
			break;
		...
		case HDMI_VIDEO_1920x1080p3d_24Hz:
			...
			vWriteHdmiANA(0x25C, 0x00000001);
			mdelay(5);
			vWriteHdmiANA(0x254, 0x8016D2B5);
			mdelay(5);
			vWriteHdmiANA(0x250, 0x00000141);
			break;
		...
		case HDMI_VIDEO_1920x1080p3d_23Hz:
			...
			vWriteHdmiANA(0x25C, 0x00000001);
			mdelay(5);
			vWriteHdmiANA(0x254, 0x8016D89D);
			mdelay(5);
			vWriteHdmiANA(0x250, 0x00000141);
			break;
		default:
...
static void _rdma1_irq_handler(unsigned int param)
{
	...
	if(param&(1<<2) || param&(1<<3))  //frame done or abnormal
	{
		...
		MMProfileLogEx(HDMI_MMP_Events.RDMA1RegisterUpdated, MMProfileFlagEnd, param, 4);
	}

    if (param & 2) // rdma1 frame start
    {
        MMProfileLogEx(HDMI_MMP_Events.RDMA1RegisterUpdated, MMProfileFlagStart, param, 2);
...
static HDMI_STATUS hdmi_drv_init(void)
{
    ...
    //HDMI_LOG("[hdmi]%s, hdmi_width=%d, hdmi_height=%d\n", __func__, p->hdmi_width, p->hdmi_height);
...
static int hdmi_video_config(HDMI_VIDEO_RESOLUTION vformat, HDMI_VIDEO_INPUT_FORMAT vin, HDMI_VIDEO_OUTPUT_FORMAT vout, HDMI_VIDEO_OUT_MODE mode)
{
    HDMI_FUNC();
    ...
    return hdmi_drv->video_config(vformat, vin, vout, mode);
}
...
/*static*/ void hdmi_power_on(void)
{
    ...
    {
        HDMI_LOG("[hdmi] wait rdma wq timeout @%s, %d, early_suspend=%d, %d\n",
    ...
    if (down_interruptible(&hdmi_update_mutex))
    {
        HDMI_LOG("[hdmi][HDMI] can't get semaphore in %s()\n", __func__);
        return;
    }
	...
	enable_clock(MT_CG_PERI_I2C3, "HDMITX");/*hdmi ddc clock*/
    hdmi_drv->power_on();
...
unsigned char _ucHdmiMode = HDMI_VIDEO_OUT_MODE_SRC;
#if CONFIG_MTK_HDMIRX_SUPPORT
size_t hdmibypass_colorspace = HDMI_RGB;
#endif
void hdmi_video_config_ext(HDMI_VIDEO_RESOLUTION res, HDMI_VIDEO_OUT_MODE mode)
{
        hdmi_video_buffer_info temp;
        int tmp = 0;
	...
	HDMI_LOG(" hdmi power state is not on,change resolution \n");
        ...
        if (hdmi_bufferdump_on > 0)
        {
            MMProfileLogEx(HDMI_MMP_Events.HDMIState, MMProfileFlagStart, ResChange, res);
        }
...
#if defined(CONFIG_MTK_INTERNAL_MHL_SUPPORT)
if((res == HDMI_VIDEO_1920x1080p_60Hz)
||(res == HDMI_VIDEO_1920x1080p_50Hz)
||(res == HDMI_VIDEO_1280x720p3d_60Hz)
||(res == HDMI_VIDEO_1280x720p3d_50Hz)
||(res == HDMI_VIDEO_1920x1080p3d_24Hz)
||(res == HDMI_VIDEO_1920x1080p3d_23Hz)
)
{
   ...
   hdmi_drv->colordeep(hdmi_colorspace);
	...
	if(fgHdmiRepeaterIsBypassMode())
	{
		...
		{//YUV
			if(!bHDMI422Input()) //YUV444 input
			{
				hdmibypass_colorspace  = HDMI_YCBCR_444;
				HDMI_LOG("RGB2HDMI HDMI_YCBCR_444\n");
			}
			else//YUV4222 input
			{
				hdmibypass_colorspace  = HDMI_YCBCR_422;
				HDMI_LOG("RGB2HDMI HDMI_YCBCR_422\n");
			}
			}
			else
			{ //RGB
				hdmibypass_colorspace  = HDMI_RGB_FULL;
				HDMI_LOG("RGB2HDMI HDMI_RGB\n");
			}
	}
	hdmi_drv->colordeep(hdmibypass_colorspace,1);
#endif  
		...
		if(mode == HDMI_VIDEO_OUT_MODE_SRC){
			hdmi_config_pll(res);
       		dpi_setting_res((u8)res);
			_ucHdmiMode = HDMI_VIDEO_OUT_MODE_SRC;
		}
		else
		{
#if CONFIG_MTK_HDMIRX_SUPPORT
			vconfig_hdmirx_sys(res);
			HDMI_LOG("%s @ %d\n", __FUNCTION__,__LINE__);
			rgb2hdmisetres(res,hdmibypass_colorspace);
			_ucHdmiMode = HDMI_VIDEO_OUT_MODE_REPEATER_BYPASS;
#endif
		}
        
        hdmi_video_config(p->output_video_resolution, HDMI_VIN_FORMAT_RGB888, HDMI_VOUT_FORMAT_RGB888, mode);

        {
            ...
            if((res == HDMI_VIDEO_1920x1080i_60Hz)
                ||(res == HDMI_VIDEO_1920x1080i_50Hz)
#if defined(CONFIG_MTK_INTERNAL_MHL_SUPPORT)
                || (res == HDMI_VIDEO_1920x1080i3d_sbs_60Hz)
                || (res == HDMI_VIDEO_1920x1080i3d_sbs_50Hz)
#endif
                )
            {
                line /= 8;
            ...
            HDMI_LOG("[hdmi] set rdma1 target line %d,hdmi_height %d,res %d\n",line,p->hdmi_height, res);
...
/*static*/ void hdmi_power_off(void)
{
	...
	disable_clock(MT_CG_PERI_I2C3, "HDMITX");
...
static long hdmi_ioctl_ex(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) && defined(CONFIG_MTK_HDMI_HDCP_SUPPORT)&&defined(MTK_DRM_KEY_MNG_SUPPORT))
	...
	#endif
#if DISPLAY_V4L2
    unsigned int from_v4l2 = file->private_data; // only v4l2 use private_data.
#endif
    ...
    switch (cmd)
    {
	   ...
	   case MTK_HDMI_GET_EDID_RAW:
	   {
	   		char edidbuffer[512];

			memset(edidbuffer, 0 , sizeof(edidbuffer));
	   		hdmi_drv->getedidrawdata(edidbuffer, sizeof(edidbuffer));
		    if (copy_to_user((void __user *)arg, edidbuffer, sizeof(edidbuffer))) {
	...
	case MTK_HDMI_COLOR_DEEP:
       ...
       case MTK_HDMI_AUDIO_VIDEO_ENABLE:
		...
		{
			hdmi_video_config_ext(arg, HDMI_VIDEO_OUT_MODE_SRC);
			break;
		}

		case MTK_HDMI_VIDEO_CONFIG_EXT:
		{
			HDMI_OUT_PARAM_T out_parm;
			if (copy_from_user(&out_parm, (void __user *)arg, sizeof(out_parm))) {
               HDMI_LOG("copy_from_user failed! line:%d \n", __LINE__);
               r = -EFAULT;
           } else {
           		hdmi_video_config_ext(out_parm.res, out_parm.mode);
           }
			break;
		}
        ...
        case MTK_HDMI_POST_VIDEO_BUFFER:
        {
            ...
            HDMI_LOG("POST Buffer Info: va:0x%x mva:0x%x fmt:0x%x, pitch:%d, ROI(%d,%d,%d,%d), "
            ...
            if(p->is_clock_on)
            {
                ...
                if (from_v4l2)
                {
                    pBuffList->buf_receive = hdmitx_buf_receive_cb;
                    pBuffList->buf_remove = hdmitx_buf_remove_cb;
                    pBuffList->buf_receive(pBuffList->buffer_info.src_phy_addr);
                }
                #endif
	    ...
	    *(volatile unsigned int *)0xf4014f00 = 0x00000041;
        ...
        case MTK_HDMI_PREPARE_BUFFER:
        {
            ...
            {
                HDMI_LOG("[HDMI]: copy_from_user failed! line:%d \n", __LINE__);
                r = -EFAULT;
                break;
            }
        ...
        default:
        {
            HDMI_LOG("[hdmi][HDMI] arguments error %x, %x, s=%d\n", MTK_HDMI_GET_DEV_INFO, cmd, sizeof(mtk_dispif_info_t));
            break;
        }
    }
...
long hdmi_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    if (fg_need_sync)
    {
        ...
        {
            HDMI_LOG("[HDMI]warning: down err\n!");
        }
    }
...
static int hdmi_probe(struct platform_device *pdev)
{
    ...
    if (ret)
    {
        HDMI_LOG("[hdmi]alloc_chrdev_region fail\n");
        return -1;
    }
    ...
    HDMI_LOG("[hdmi][%s] current=0x%08x\n", __func__, (unsigned int)current);
...
static int __init hdmi_init(void)
{
    ...
    {
        HDMI_LOG("[hdmi]failed to register hdmitx driver\n");
        return -1;
    }
    ...
    if (ret)
    {
        HDMI_LOG("[hdmi][HDMI]switch_dev_register returned:%d!\n", ret);
        return 1;
    }
