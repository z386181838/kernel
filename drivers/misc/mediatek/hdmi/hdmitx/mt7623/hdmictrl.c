...
{
  vWriteIoPadMsk(IO_PAD_MODE, PAD_MODE_HTPLG, PAD_MODE_MASK);
...
void vSetHDMITxPLL(unsigned char bResIndex, unsigned char bdeepmode)
{
  ...
  if(v4value1==0) v4value1 = 0x28;
  if(v4value2==0) v4value2 = 0x282828;
  ...
  if((u4Feq==2)&&(bdeepmode!=HDMI_NO_DEEP_COLOR))
  {
   ...
   vWriteIoPllMsk(HDMI_CON0, (0x1C<<RG_HDMITX_DRV_IBIAS), RG_HDMITX_DRV_IBIAS_MASK);
  }
  else
  {
...
void vResetHDMI(BYTE bRst)
{
  ...
  else
  {
    ...
    vWriteHdmiGRLMsk(DUMMY_304,0x06<<2,0x3F<<2);//swap tmds d0&d2 
...
void vHDMIResetGenReg(unsigned char ui1resindex, unsigned char ui1colorspace)
{
  ...
  if(_stAvdAVInfo.out_mode != HDMI_VIDEO_OUT_MODE_SRC)
  {
	vWriteHdmiGRLMsk(GRL_CFG2,0,MHL_DE_SEL);
	return ;
  }

  if((ui1resindex==HDMI_VIDEO_1920x1080i_50Hz)||(ui1resindex==HDMI_VIDEO_1920x1080i_60Hz))
   vWriteHdmiGRLMsk(GRL_CFG2,0,MHL_DE_SEL);
  else
...
extern unsigned char _ucHdmiMode;

void vSendAVIInfoFrame(unsigned char ui1resindex, unsigned char ui1colorspace)
{
  HDMI_VIDEO_FUNC();
#if CONFIG_MTK_HDMIRX_SUPPORT
	if(_ucHdmiMode == HDMI_VIDEO_OUT_MODE_REPEATER_BYPASS)
	{
		return;
	}
#endif
...
void vHalSendVendorSpecificInfoFrame(unsigned char fg3DRes,unsigned char bVIC,unsigned char b3dstruct)
{
   ...
   HDMI_VIDEO_LOG(" vendor: bVS_CHSUM = %d, bPB1 = %d, bPB2 = %d, bPB3 = %d, bPB4 = %d, bPB5 = %d\n", bVS_CHSUM, bPB1, bPB2, bPB3, bPB4, bPB5);
}
...
void vChgHDMIVideoResolution(unsigned char ui1resindex, unsigned char ui1colorspace, unsigned char ui1hdmifs, unsigned char bdeepmode)
{
...
void vChgtoSoftNCTS(unsigned char ui1resindex, unsigned char ui1audiosoft, unsigned char ui1hdmifs, unsigned char bdeepmode)
{
  HDMI_AUDIO_FUNC();
...
void vShowHpdRsenStatus(void)
{
  if(bCheckPordHotPlug(HOTPLUG_MODE)==TRUE)
    HDMI_LOG("[HDMI]HPD ON\n");
  else
    HDMI_LOG("[HDMI]HPD OFF\n");

  if(bCheckPordHotPlug(PORD_MODE)==TRUE)
    HDMI_LOG("[HDMI]RSEN ON\n");
  else
    HDMI_LOG("[HDMI]RSEN OFF\n");

  if(i4SharedInfo(SI_HDMI_RECEIVER_STATUS) == HDMI_PLUG_IN_ONLY)
  HDMI_LOG("[HDMI]SI_HDMI_RECEIVER_STATUS = HDMI_PLUG_IN_ONLY\n");
  else if(i4SharedInfo(SI_HDMI_RECEIVER_STATUS) == HDMI_PLUG_IN_AND_SINK_POWER_ON)
  HDMI_LOG("[HDMI]SI_HDMI_RECEIVER_STATUS = HDMI_PLUG_IN_AND_SINK_POWER_ON\n");
  else if(i4SharedInfo(SI_HDMI_RECEIVER_STATUS) == HDMI_PLUG_OUT)
  HDMI_LOG("[HDMI]SI_HDMI_RECEIVER_STATUS = HDMI_PLUG_OUT\n");
  else
  HDMI_LOG("[HDMI]SI_HDMI_RECEIVER_STATUS error\n");
}

void vShowOutputVideoResolution(void)
{
  HDMI_LOG("[HDMI]HDMI output resolution = %s \n", szHdmiResStr[_stAvdAVInfo.e_resolution]);//
...
{
  if(vIsDviMode())
    HDMI_LOG("[HDMI]DVI Mode\n");
  else
    HDMI_LOG("[HDMI]HDMI Mode\n");
...
void vShowDeepColor(void)
{
  ...
  HDMI_LOG("[HDMI]HDMI output deepcolor = HDMI_NO_DEEP_COLOR\n");
  else if(_stAvdAVInfo.e_deep_color_bit == HDMI_DEEP_COLOR_10_BIT)
  HDMI_LOG("[HDMI]HDMI output deepcolor = HDMI_DEEP_COLOR_10_BIT\n");
  else if(_stAvdAVInfo.e_deep_color_bit == HDMI_DEEP_COLOR_12_BIT)
  HDMI_LOG("[HDMI]HDMI output deepcolor = HDMI_DEEP_COLOR_12_BIT\n");
  else if(_stAvdAVInfo.e_deep_color_bit == HDMI_DEEP_COLOR_16_BIT)
  HDMI_LOG("[HDMI]HDMI output deepcolor = HDMI_DEEP_COLOR_16_BIT\n");
  else
  HDMI_LOG("[HDMI]HDMI output deepcolor error\n");
...
void vShowInforFrame(void)
{
   ...
   if(fgIsAcpEnable())
   {
     ...
     {
       HDMI_LOG("IEC 60958-Identified Audio\n");
       HDMI_LOG("Data Byte (1~8)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",_bAcpData[0],_bAcpData[1],_bAcpData[2],_bAcpData[3],_bAcpData[4],_bAcpData[5],_bAcpData[6],_bAcpData[7]);
       HDMI_LOG("Data Byte (9~16)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",_bAcpData[8],_bAcpData[9],_bAcpData[10],_bAcpData[11],_bAcpData[12],_bAcpData[13],_bAcpData[14],_bAcpData[15]);
     }
     else if(_bAcpType ==2)
     {
       HDMI_LOG("DVD Audio\n");
       HDMI_LOG("DVD-AUdio_TYPE_Dependent Generation = 0x%x \n", _bAcpData[0]);
       HDMI_LOG("Copy Permission = 0x%x \n", (_bAcpData[1]>>6)&0x03);
       HDMI_LOG("Copy Number = 0x%x \n", (_bAcpData[1]>>3)&0x07);
       HDMI_LOG("Quality = 0x%x \n", (_bAcpData[1]>>1)&0x03);
       HDMI_LOG("Transaction = 0x%x \n", _bAcpData[1]&0x01);

     }
     else if(_bAcpType ==3)
     {
       HDMI_LOG("SuperAudio CD\n");
       HDMI_LOG("CCI_1 Byte (1~8)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",_bAcpData[0],_bAcpData[1],_bAcpData[2],_bAcpData[3],_bAcpData[4],_bAcpData[5],_bAcpData[6],_bAcpData[7]);
       HDMI_LOG("CCI_1 Byte (9~16)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",_bAcpData[8],_bAcpData[9],_bAcpData[10],_bAcpData[11],_bAcpData[12],_bAcpData[13],_bAcpData[14],_bAcpData[15]);

     }
     HDMI_LOG("====================ACP inforFrame End ======================================\n");
   }

   if(fgIsISRC1Enable())
   {
     HDMI_LOG("====================ISRC1 inforFrame Start ====================================\n");
     HDMI_LOG("Data Byte (1~8)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",_bIsrc1Data[0],_bIsrc1Data[1],_bIsrc1Data[2],_bIsrc1Data[3],_bIsrc1Data[4],_bIsrc1Data[5],_bIsrc1Data[6],_bIsrc1Data[7]);
     HDMI_LOG("====================ISRC1 inforFrame End ======================================\n");
   }

   if(fgIsISRC2Enable())
   {
     HDMI_LOG("====================ISRC2 inforFrame Start ====================================\n");
     HDMI_LOG("Data Byte (1~8)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",_bIsrc1Data[8],_bIsrc1Data[9],_bIsrc1Data[10],_bIsrc1Data[11],_bIsrc1Data[12],_bIsrc1Data[13],_bIsrc1Data[14],_bIsrc1Data[15]);
     HDMI_LOG("====================ISRC2 inforFrame End ======================================\n");
   }
}
...
{
  HDMI_LOG("[HDMI]HDMI output audio Channel Number =%d \n", _stAvdAVInfo.ui1_aud_out_ch_number);
  HDMI_LOG("[HDMI]HDMI output Audio Fs = %s \n", cHdmiAudFsStr[_stAvdAVInfo.e_hdmi_fs]);
  HDMI_LOG("[HDMI]HDMI MCLK =%d \n", _stAvdAVInfo.u1HdmiI2sMclk);
  HDMI_LOG("[HDMI]HDMI output ACR N= %d, CTS = %d \n", u4ReadNValue(), u4ReadCtsValue());
}

void vShowHdmiDrmHdcpStatus(void)
{  
  HDMI_LOG("[HDMI]DrmHdcp _bflagvideomute =%d \n", _bflagvideomute);
  HDMI_LOG("[HDMI]DrmHdcp _bflagaudiomute =%d \n", _bflagaudiomute);
  HDMI_LOG("[HDMI]DrmHdcp _bsvpvideomute =%d \n", _bsvpvideomute);
  HDMI_LOG("[HDMI]DrmHdcp _bsvpaudiomute =%d \n", _bsvpaudiomute);    
  HDMI_LOG("[HDMI]DrmHdcp _bNeedSwHdcp =%d \n", _bHdcpOff);  
  HDMI_LOG("[HDMI]DrmHdcp _bHdcpOff =%d \n", _bHdcpOff);   
}
...
void vCheckHDMICRC(void)
{
  ...
  for(i=0; i<10; i++)
  {
    ...
    else
    {
     ...
     {
      HDMI_LOG("[HDMI]number = %d, u4Data = 0x%x \n", i, u4Data);
      HDMI_LOG("[HDMI]hdmi crc error \n");
      return ;
     }
    }
  }
  HDMI_LOG("[HDMI]hdmi crc pass \n");
}
...
void vHalSendHdmiPacket(unsigned char bType, bool fgEnable, unsigned char *pr_bData)
{
	unsigned char i;
	unsigned char bLength = 28;

	HDMI_LOG("RxTx packet bypass: bType = %d, fgEnable = %d, HB0 = 0x%x, HB1 = 0x%x, HB2 = 0x%x, PB0 = 0x%x\n",bType,fgEnable,*(pr_bData),*(pr_bData+1),*(pr_bData+2),*(pr_bData+3));
	...
	{
		vWriteHdmiGRLMsk(GRL_CTRL, 0, CTRL_MPEG_EN);
		bLength = 11;
	}
	...
	else if(bType == VENDOR_INFOFRAME)
	{
		...
		vWriteHdmiGRLMsk(GRL_ACP_ISRC_CTRL, 0,ACP_EN);
	else if(bType == ISRC1_PACKET)
		vWriteHdmiGRLMsk(GRL_ACP_ISRC_CTRL, 0,ISRC1_EN);
	else if(bType == ISRC2_PACKET)
		vWriteHdmiGRLMsk(GRL_ACP_ISRC_CTRL, 0,ISRC2_EN);
	...
	vWriteByteHdmiGRL(GRL_INFOFRM_TYPE, *(pr_bData));
	vWriteByteHdmiGRL(GRL_INFOFRM_VER, *(pr_bData+1));
	vWriteByteHdmiGRL(GRL_INFOFRM_LNG, *(pr_bData+2));


	for(i=0; i<bLength; i++)
	{
		vWriteByteHdmiGRL(GRL_IFM_PORT, *(pr_bData+3+i));
	  }
	...
	{
		if(bType == AVI_INFOFRAME)
			vWriteHdmiGRLMsk(GRL_CTRL, CTRL_AVI_EN, CTRL_AVI_EN);
		else if(bType == AUDIO_INFOFRAME)
			vWriteHdmiGRLMsk(GRL_CTRL, CTRL_AUDIO_EN, CTRL_AUDIO_EN);
		else if(bType == MPEG_INFOFRAME)
			vWriteHdmiGRLMsk(GRL_CTRL, CTRL_MPEG_EN, CTRL_MPEG_EN);
		else if(bType == SPD_INFOFRAME)
			vWriteHdmiGRLMsk(GRL_CTRL, CTRL_SPD_EN, CTRL_SPD_EN);
		else if(bType == VENDOR_INFOFRAME)
			vWriteHdmiGRLMsk(GRL_ACP_ISRC_CTRL, VS_EN,VS_EN);
		else if(bType == ACP_PACKET)
			vWriteHdmiGRLMsk(GRL_ACP_ISRC_CTRL, ACP_EN,ACP_EN);
		else if(bType == ISRC1_PACKET)
			vWriteHdmiGRLMsk(GRL_ACP_ISRC_CTRL, ISRC1_EN,ISRC1_EN);
		else if(bType == ISRC2_PACKET)
			vWriteHdmiGRLMsk(GRL_ACP_ISRC_CTRL, ISRC2_EN,ISRC2_EN);
		else if(bType == GAMUT_PACKET)
			vWriteHdmiGRLMsk(GRL_ACP_ISRC_CTRL, GAMUT_EN,GAMUT_EN);
	}

}
bool vIsTmdsOn(void)
{
	if(((dReadIoPll(HDMI_CON0)>>28)&0x0f)==0x0F)	
		return true; 
	else
		return false;
}

void vSendHdmiPacket(unsigned char bType, bool fgEnable, unsigned char *pr_bData)
{
	if((bType == ACP_PACKET)||(bType == ISRC1_PACKET)||(bType == ISRC2_PACKET))
	{
		if(0)//(fgIsHdmiRepeater()&&_fgHdmiRxInput&&fgHDMIAudRxBypassMode())
		{
			if(i4SharedInfo(SI_HDMI_SUPPORTS_AI)== 0)
				vHalSendHdmiPacket(bType, 0, pr_bData);
			else
