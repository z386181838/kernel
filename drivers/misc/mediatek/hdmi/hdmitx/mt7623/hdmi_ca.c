...
bool fgCaHDMICreate(void)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS)
	{
		...
		HDMI_LOG("Create ca_hdmi_handle Error: %d\n", tz_ret);
		return FALSE;
	}
	HDMI_LOG("[HDMI]Create ca_hdmi_handle ok: %d\n", tz_ret);
    }
...
bool fgCaHDMIClose(void)
{
	...
	{
		// Should provide strerror style error string in UREE.
		HDMI_LOG("Close ca_hdmi_handle Error: %d\n", tz_ret);
		return FALSE;
	}
	HDMI_LOG("[HDMI]Close ca_hdmi_handle ok: %d\n", tz_ret);
	return TRUE;
}
...
void vCaHDMIWriteReg(unsigned int u4addr,unsigned int u4data)
{
	...
	{
		HDMI_LOG("[HDMI] TEE ca_hdmi_handle=NULL\n");
		return ;
	}
	}
	...
	if (tz_ret != TZ_RESULT_SUCCESS)
	{
		HDMI_LOG("[HDMI]CA HDMI_TA_WRITE_REG err:%X\n",tz_ret);
	}
...
void vCaDPI1WriteReg(unsigned int u4addr,unsigned int u4data)
{
	...
	HDMI_LOG("[HDMI_DPI1]W:%X=%X\n",u4addr,u4data);
	
	if(ca_hdmi_handle == NULL)
	{
		HDMI_LOG("[HDMI_DPI1] TEE ca_dpi1_handle=NULL\n");
		return ;
	}
	...
	if (tz_ret != TZ_RESULT_SUCCESS)
	{
		HDMI_LOG("[HDMI_DPI1]CA HDMI_TA_DPI1_WRITE_REG err:%X\n",tz_ret);
	}
...
bool fgCaHDMIInstallHdcpKey(unsigned char *pdata,unsigned int u4Len)
{
	...
	HDMI_LOG("[HDMI]fgCaHDMIInstallHdcpKey,%d\n",u4Len);
	...
	if (tz_ret != TZ_RESULT_SUCCESS)
	{
		HDMI_LOG("[HDMI]CA HDMI_TA_INSTALL_HDCP_KEY err:%X\n",tz_ret);
		return FALSE;
	}
...
bool fgCaHDMIGetAKsv(unsigned char *pdata)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS)
	{
		HDMI_LOG("[HDMI]CA HDMI_TA_GET_HDCP_AKSV err:%X\n",tz_ret);
		return FALSE;
	}
	for(i=0;i<5;i++)
		pdata[i] = ptr[i];

	HDMI_LOG("[HDMI]hdcp aksv : %x %x %x %x %x\n",
		pdata[0],pdata[1],
...
bool fgCaHDMILoadHDCPKey(void)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS)
	{
		HDMI_LOG("[HDMI]CA HDMI_TA_LOAD_HDCP_KEY err:%X\n",tz_ret);
		return	FALSE;
	}
	return TRUE;
}
bool fgCaHDMIHDCPReset(bool fgen)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS)
	{
		HDMI_LOG("[HDMI]CA HDMI_TA_HDCP_RST err:%X\n",tz_ret);
		return	FALSE;
	}
	return TRUE;
}
bool fgCaHDMIHDCPEncEn(bool fgen)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS)
	{
		HDMI_LOG("[HDMI]CA HDMI_TA_HDCP_ENC_EN err:%X\n",tz_ret);
		return	FALSE;
	}
	return TRUE;
}
bool fgCaHDMIVideoUnMute(bool fgen)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS)
	{
		HDMI_LOG("[HDMI]CA HDMI_TA_VID_UNMUTE err:%X\n",tz_ret);
		return	FALSE;
	}
	return TRUE;
}
bool fgCaHDMIAudioUnMute(bool fgen)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS)
	{
		HDMI_LOG("[HDMI]CA HDMI_TA_AUD_UNMUTE err:%X\n",tz_ret);
		return	FALSE;
	}
	return TRUE;
}
