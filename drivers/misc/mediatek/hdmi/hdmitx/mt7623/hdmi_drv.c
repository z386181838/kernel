...
static int hdmi_internal_video_config(HDMI_VIDEO_RESOLUTION vformat, HDMI_VIDEO_INPUT_FORMAT vin, HDMI_VIDEO_OUTPUT_FORMAT vout ,  HDMI_VIDEO_OUT_MODE mode)
{
    HDMI_DRV_FUNC();

    _stAvdAVInfo.e_resolution = vformat;
	_stAvdAVInfo.out_mode = mode;
...
#if defined(MHL_BRIDGE_SUPPORT)
     mhl_bridge_drv->resolution((unsigned char)_stAvdAVInfo.e_resolution,(unsigned char)_stAvdAVInfo.e_video_color_space);
#endif
...
#if CONFIG_MTK_HDMIRX_SUPPORT
extern size_t hdmibypass_colorspace ;
#endif
void hdmi_colordeep(unsigned char u1colorspace, unsigned char u1deepcolor)
{
  ...
  {
	  HDMI_LOG("color_space:HDMI_YCBCR_444 = 2\n");
	  HDMI_LOG("color_space:HDMI_YCBCR_422 = 3\n");

      HDMI_LOG("deep_color:HDMI_NO_DEEP_COLOR = 1\n");
      HDMI_LOG("deep_color:HDMI_DEEP_COLOR_10_BIT = 2\n");
      HDMI_LOG("deep_color:HDMI_DEEP_COLOR_12_BIT = 3\n");
      HDMI_LOG("deep_color:HDMI_DEEP_COLOR_16_BIT = 4\n");
	...
	{
		_stAvdAVInfo.e_video_color_space  = hdmibypass_colorspace;
...
void hdmi_read(unsigned int u2Reg, unsigned int *p4Data)
{
	*p4Data = (*(volatile unsigned int*)(u2Reg));
    HDMI_LOG("Reg read= 0x%08x, data = 0x%08x\n", u2Reg, *p4Data);
}
...
void hdmi_write(unsigned int u2Reg, unsigned int u4Data)
{
	...
	if((u2Reg&0xFFFFF000)==0)
	{
      ...
      *(volatile unsigned int*)(u2Reg) = u4Data;
#endif
    HDMI_LOG("Reg write= 0x%08x, data = 0x%08x\n", u2Reg, u4Data);
}
...
#if CONFIG_MTK_HDMIRX_SUPPORT
		vIssueHdmiRxUpdateEdidCmd(i4GetHotPlugStatus());
#endif
    break;
...
void hdmi_drvlog_enable(unsigned short enable)
{
   ...
   if(enable==0)
   {
	   HDMI_LOG("hdmi_pll_log =   0x1\n");
	   HDMI_LOG("hdmi_dgi_log =   0x2\n");
	   HDMI_LOG("hdmi_plug_log =  0x4\n");
	   HDMI_LOG("hdmi_video_log = 0x8\n");
	   HDMI_LOG("hdmi_audio_log = 0x10\n");
	   HDMI_LOG("hdmi_hdcp_log =  0x20\n");
	   HDMI_LOG("hdmi_cec_log =   0x40\n");
	   HDMI_LOG("hdmi_ddc_log =   0x80\n");
	   HDMI_LOG("hdmi_edid_log =  0x100\n");
	   HDMI_LOG("hdmi_drv_log =   0x200\n");
	   
	   HDMI_LOG("hdmi_all_log =   0x3ff\n");
...
{
	static const HDMI_DRIVER HDMI_DRV_ops =
	{
        ...
        .getedidrawdata = hdmi_appGetEdidRawData,
	};
