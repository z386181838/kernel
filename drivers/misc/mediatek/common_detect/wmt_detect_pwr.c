...
static int wmt_detect_chip_pwr_on(void)
{
	...
	if ((COMBO_PMU_PIN == INVALID_PIN_ID) ||
	    (COMBO_RST_PIN == INVALID_PIN_ID) || (COMBO_WIFI_EINT_PIN == INVALID_PIN_ID)) {
		WMT_DETECT_ERR_FUNC
		    ("WMT-DETECT: either PMU(%d) or RST(%d) or WIFI_EINT(%d) is not set\n",
		     COMBO_PMU_PIN, COMBO_RST_PIN, COMBO_WIFI_EINT_PIN);
		return retval;
	}
