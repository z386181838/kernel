...
int static do_combo_common_drv_init(int chip_id)
{
	...
	WMT_DETECT_INFO_FUNC
	    ("COMBO chip is not supported, please check kernel makefile or project fonfig, i_ret:%d\n",
	     i_ret);
#endif
...
int static do_soc_common_drv_init(int chip_id)
{
	...
	i_ret = -1;
	WMT_DETECT_INFO_FUNC
	    ("SOC chip is not supported, please check kernel makefile or project fonfig, i_ret:%d\n",
	     i_ret);
#endif
