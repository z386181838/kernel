...
void msg_show(const char *prefix, AE_Msg *msg)
{
	...
	xlog_printk(ANDROID_LOG_DEBUG, AEK_LOG_TAG,
		    "%s: cmdType=%s[%d] cmdId=%s[%d] seq=%d arg=%x len=%d\n", prefix, cmd_type,
				...
				size_t count, loff_t *f_pos)
{
	...
	if ((*f_pos + count) > len) {
		xlog_printk(ANDROID_LOG_ERROR, AEK_LOG_TAG,
			    "read size overflow, count=%d, *f_pos=%d\n", count, (int)*f_pos);
		count = len - *f_pos;
		ret = -EFAULT;
		goto out;
	}
...
static long aed_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == AEEIOCTL_WDT_KICK_POWERKEY) {
		xlog_printk(ANDROID_LOG_DEBUG, AEK_LOG_TAG, "AEEIOCTL_WDT_Kick_Powkey ( %d)\n", (int)arg);
		aee_kernel_wdt_kick_Powkey_api("AEE Ioctl Call", (int)arg);
		return ret;
	}
	...
	if (cmd == AEEIOCTL_GET_PROCESS_BT) {
		...
		pr_info("%s: get process backtrace ioctl\n", __func__);
		...
		if (copy_from_user
		    (bt, (struct aee_process_bt __user *)arg, sizeof(struct aee_process_bt))) {
			kfree(bt);
			ret = -EFAULT;
			return ret;
		}
		...
		if (ret == 0) {
			if (copy_to_user
			    ((struct aee_process_bt __user *)arg, bt,
			     sizeof(struct aee_process_bt))) {
				kfree(bt);
				ret = -EFAULT;
				return ret;
			}
	...
	case AEEIOCTL_GET_THREAD_REG:
		{
			...
			pr_info("%s: get thread registers ioctl\n", __func__);
	...
	case AEEIOCTL_CHECK_SUID_DUMPABLE:
		{
			...
			pr_info("%s: check suid dumpable ioctl\n", __func__);
...
void aee_kernel_dal_api(const char *file, const int line, const char *msg)
{
	...
	if (down_interruptible(&aed_dal_sem) < 0) {
		xlog_printk(ANDROID_LOG_INFO, AEK_LOG_TAG,
			    "ERROR : aee_kernel_dal_api() get aed_dal_sem fail ");
		return;
	}
	xlog_printk(ANDROID_LOG_INFO, AEK_LOG_TAG, "aee_kernel_dal_api : <%s:%d> %s ", file, line,
		    msg);
	if (msg != NULL) {
		struct aee_dal_setcolor dal_setcolor;
		...
		if (dal_show == NULL) {
			xlog_printk(ANDROID_LOG_INFO, AEK_LOG_TAG,
				    "ERROR : aee_kernel_dal_api() kzalloc fail\n ");
			up(&aed_dal_sem);
			return;
		}
		if (aee_mode < AEE_MODE_CUSTOMER_ENG) {
			       ...
			       const int *phy, int phy_size, const char *detail)
{
	...
	if (in_interrupt()) {
		...
		xlog_printk(ANDROID_LOG_DEBUG, AEK_LOG_TAG,
			    "Modem exception occur in interrupt context, no modem coredump");
		phy_size = 0;
	} else if ((phy < 0) || (phy_size > MAX_EE_COREDUMP)) {
		xlog_printk(ANDROID_LOG_DEBUG, AEK_LOG_TAG,
			    "Modem Physical memory size(%d) too large or invalid", phy_size);
		phy_size = 0;
	}
	...
	if (phy_size > 0) {
		...
		if (aed_dev.eerec.ee_phy != NULL) {
			...
			xlog_printk(ANDROID_LOG_DEBUG, AEK_LOG_TAG,
				    "Losing ee phy mem due to vmalloc return NULL\n");
...
static int __init aed_init(void)
{
	...
	if (unlikely(err)) {
		xlog_printk(ANDROID_LOG_ERROR, AEK_LOG_TAG,
			    "aee: failed to register aed0(ee) device!\n");
		return err;
	}
	...
	if (unlikely(err)) {
		xlog_printk(ANDROID_LOG_ERROR, AEK_LOG_TAG,
			    "aee: failed to register aed1(ke) device!\n");
		return err;
	}
...
static void __exit aed_exit(void)
{
	...
	if (unlikely(err))
		xlog_printk(ANDROID_LOG_ERROR, AEK_LOG_TAG,
			    "xLog: failed to unregister aed(ee) device!\n");
	err = misc_deregister(&aed_ke_dev);
	if (unlikely(err))
		xlog_printk(ANDROID_LOG_ERROR, AEK_LOG_TAG,
			    "xLog: failed to unregister aed(ke) device!\n");
