...
/* bleow code is added by QHQ  for hang detect */
	...
	if (unlikely(err)) {
		xlog_printk(ANDROID_LOG_ERROR, AEK_LOG_TAG,
			    "aee: failed to register aed_wdt_RT_Monitor_dev device!\n");
		return err;
	}
#ifdef CONFIG_MTK_AEE_POWERKEY_HANG_DETECT
	...
	if (unlikely(err)) {
		xlog_printk(ANDROID_LOG_ERROR, AEK_LOG_TAG,
			    "aee: failed to register aed_wdt_tick_PowKey_dev device!\n");
		return err;
	}
#endif
...
static void __exit monitor_hang_exit(void)
{
	...
	if (unlikely(err))
		xlog_printk(ANDROID_LOG_ERROR, AEK_LOG_TAG,
			    "xLog: failed to unregister RT_Monitor device!\n");
#ifdef CONFIG_MTK_AEE_POWERKEY_HANG_DETECT
	err = misc_deregister(&aed_wdt_tick_PowKey_dev);
	if (unlikely(err))
		xlog_printk(ANDROID_LOG_ERROR, AEK_LOG_TAG,
			    "xLog: failed to unregister kick_powerkey device!\n");
#endif
}
	...
	if (pwk_start_monitor && (get_boot_mode() == NORMAL_BOOT)
		...
		if ((kinterval * hwt_kick_times > 180))	/* only monitor 3 min */
		{
			...
			if ((wdt_kick_status & (WDT_SETBY_Display | WDT_SETBY_SF)) !=
				...
				printk_sched
				    ("[WDK] Powerkey Tick fail,kick_status 0x%08x,RT[%lld]\n ",
