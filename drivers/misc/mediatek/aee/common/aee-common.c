...
void aee_trigger_kdb(void)
{
	...
	if (res) {
		pr_info("aee_trigger_kdb, get wd api error\n");
	} else {
...
void aed_md_exception(const int *log, int log_size, const int *phy, int phy_size,
		      const char *detail)
{
#ifdef CONFIG_MTK_AEE_AED
...
void aed_combo_exception(const int *log, int log_size, const int *phy, int phy_size,
			 const char *detail)
{
#ifdef CONFIG_MTK_AEE_AED
	...
	if (g_aee_api) {
		if (g_aee_api->combo_exception) {
			...
			xlog_printk(ANDROID_LOG_DEBUG, AEK_LOG_TAG,
				    "g_aee_api->combo_exception = 0x%x\n",
...
static int __init aee_common_init(void)
{
	...
	if (!trace_entry_ptr) {
		pr_info("allocate trace buffer fail:%d\n", (int)trace_entry_ptr);
		ret = -ENOMEM;
	}
	msgbuf = kmalloc(KERNEL_REPORT_LENGTH, GFP_KERNEL);
	if (!msgbuf) {
		pr_info("allocate msgbuf fail\n");
		ret = -ENOMEM;
	}
	msgbuf2 = kmalloc(KERNEL_REPORT_LENGTH, GFP_KERNEL);
	if (!msgbuf2) {
		pr_info("allocate msgbuf2 fail\n");
		ret = -ENOMEM;
	}
	return ret;
}
