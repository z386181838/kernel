...
extern void *ipanic_next_write(void
			       *(*next) (void *data, unsigned char *buffer, size_t sz_buf,
					 int *size), void *data, int off, size_t *sz_total);
/*
 * ramdump mini
			       ...
			       va_list ap)
{
	...
	for (i = 0; i < ELF_NGREG; i++) {
		...
		if (virt_addr_valid(reg)) {
			...
			size = end - start + 1;
			ipanic_next_write(mrdump_memory_region, &hdr->reg_desc[i],
					  mem_offset + offset, &total);
			offset += size;
		} else {
 ...
 ipanic_write:
	n = emmc_ipanic_write(mrdump_mini_buf, IPANIC_MRDUMP_OFFSET,
			      ALIGN(MRDUMP_MINI_HEADER_SIZE, SZ_512));
	if (IS_ERR(ERR_PTR(n))) {
		printk(KERN_ERR "card_dump_func_write failed (%d), size: 0x%llx\n",
		       i, (unsigned long long)offset);
	}
}
#else				/* CONFIG_MTK_EMMC_SUPPORT */
static void __mrdump_mini_core(AEE_REBOOT_MODE reboot_mode, struct pt_regs *regs, const char *msg,
			       va_list ap)
{
}
