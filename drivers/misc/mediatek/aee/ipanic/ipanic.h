...
static inline int ipanic_get_panic(void) { return 0; }
#endif
...
extern int ipanic_write_log_buf(unsigned int off, struct ipanic_log_index start,
				struct ipanic_log_index end);
#else

extern int ipanic_write_log_buf(struct mtd_info *mtd, unsigned int off,
				struct ipanic_log_index start, struct ipanic_log_index end);
#endif
