...
char *emmc_allocate_and_read(int offset, int length)
{
	...
	if (buff != NULL) {
		...
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "%s: Cannot allocate buffer to read(len:%d)\n", __func__, length);
		return NULL;
	}
...
static struct aee_oops *emmc_ipanic_oops_copy(void)
{
	...
	if (oops != NULL) {
		...
		if (oops_header == NULL) {
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "%s: Can't read oops header(len:%d)\n", __func__,
		...
		if (hdr->oops_detail_length != 0) {
			oops->detail =
			    emmc_allocate_and_read(hdr->oops_detail_offset,
...
#define TMPDETAILSTR  "panic detail is empty"
		...
		if (0 == hdr->userspace_info_length) {
			...
			oops->userspace_info =
			    emmc_allocate_and_read(hdr->userspace_info_offset,
						   hdr->userspace_info_length);
			oops->userspace_info_len = hdr->userspace_info_length;
			if (oops->userspace_info == NULL) {
				xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
					    "%s: read usrespace info failed, invalid length 0x%x\n",
					    __func__, oops->userspace_info_len);
				goto error_return;
			}
		}
		...
		oops->android_main =
		    emmc_allocate_and_read(hdr->android_main_offset, hdr->android_main_length);
		oops->android_main_len = hdr->android_main_length;
		if (oops->android_main == NULL) {
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "%s: read android_main failed, invalid length 0x%x\n",
				    __func__, oops->android_main_len);
			goto error_return;
		}

		oops->android_radio =
		    emmc_allocate_and_read(hdr->android_radio_offset, hdr->android_radio_length);
		oops->android_radio_len = hdr->android_radio_length;
		if (oops->android_radio == NULL) {
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "%s: read android_radio failed, invalid length 0x%x\n",
				    __func__, oops->android_radio_len);
		}

		oops->android_system =
		    emmc_allocate_and_read(hdr->android_system_offset, hdr->android_system_length);
		oops->android_system_len = hdr->android_system_length;
		if (oops->android_system == NULL) {
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "%s: read android_system failed, invalid length 0x%x\n",
				    __func__, oops->android_system_len);
			goto error_return;
		}
		...
		if (hdr->mmprofile_length == 0) {
			...
			oops->mmprofile =
			    emmc_allocate_and_read(hdr->mmprofile_offset, hdr->mmprofile_length);
			oops->mmprofile_len = hdr->mmprofile_length;
		}
		if (oops->mmprofile == NULL) {
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "%s: read mmprofile failed, offset - 0x%x, length - 0x%x\n",
				    __func__, oops->mmprofile, oops->mmprofile_len);
		}
		...
		if (hdr->mini_rdump_length == 0) {
			...
			oops->mini_rdump =
			    emmc_allocate_and_read(hdr->mini_rdump_offset, hdr->mini_rdump_length);
			oops->mini_rdump_len = hdr->mini_rdump_length;
		}
		if (oops->mini_rdump == NULL) {
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "%s: read mini_rdump failed, offset - 0x%x, length - 0x%x\n",
				    __func__, oops->mini_rdump, oops->mini_rdump_len);
		}
...
int emmc_ipanic_write(void *buf, int off, int len)
{
	...
	if (rem != 0) {
		...
		if (card_dump_func_write
		    ((unsigned char *)buf, EMMC_BLOCK_SIZE, off + len, DUMP_INTO_BOOT_CARD_IPANIC))
			return -1;
	}
	return len + rem;
}
...
static int ipanic_write_android_buf(unsigned int off, int type)
{
	...
	while (1) {
		...
		if (card_dump_func_write(emmc_bounce, PAGE_SIZE, off, DUMP_INTO_BOOT_CARD_IPANIC)) {
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "aee-ipanic-emmc(%s): android log %d write failed, offset %d\n",
				    __func__, type, off);
			return -1;
		}
		copy_count += rc;
...
static int ipanic_write_all_android_buf(int offset, struct ipanic_header *hdr)
{
	...
	if (rc > 0) {
		...
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "%s: Failed to write android main buffer\n", __func__);
	}
	...
	if (rc > 0) {
		...
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "%s: Failed to write android radio buffer\n", __func__);
	}
	...
	if (rc > 0) {
		...
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "%s: Failed to write android system buffer\n", __func__);
	}
	return offset;
}
...
static int ipanic_write_userspace(unsigned int off)
{
	...
	xlog_printk(ANDROID_LOG_DEBUG, IPANIC_LOG_TAG,
		    "%s: count %d, strlen(NativeInfo):%d, off:%d\n", __func__, copy_count,
		    strlen(NativeInfo), off);
	return copy_count;
}
...
static void ipanic_write_mmprofile(int offset, struct ipanic_header *hdr)
{
	...
	if (mmprofile_dump_size == 0 || mmprofile_dump_size > IPANIC_OOPS_MMPROFILE_LENGTH_LIMIT) {
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "%s: ignore INVALID MMProfile dump size 0x%x", mmprofile_dump_size);
		return;
	}
	...
	do {
		...
		if (rc < 0) {
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "%s: Error writing MMProfile to emmc! (%d)\n", __func__, rc);
...
#else
	/* CONFIG_MTK_MMPROFILE_SUPPORT disabled, no mmprofile dumped. */
	hdr->mmprofile_length = 0;
#endif
}
...
static int emmc_ipanic(struct notifier_block *this, unsigned long event, void *ptr)
{
	int rc;
	...
	iheader.oops_header_length =
	    emmc_ipanic_write(&oops_header, iheader.oops_header_offset,
			      sizeof(struct ipanic_oops_header));
	if (iheader.oops_header_length < 0) {
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "%s: Error writing oops header to panic log! (%d)\n", __func__,
			    iheader.oops_header_length);
	...
	iheader.oops_detail_length =
	    ipanic_write_log_buf(iheader.oops_detail_offset, ipanic_detail_start,
				 ipanic_detail_end);
...
#define __LOG_BUF_LEN	(1 << CONFIG_LOG_BUF_SHIFT)
	iheader.console_length =
	    ipanic_write_log_buf(iheader.console_offset, ipanic_get_log_start(),
				 ipanic_get_log_end());
	if (iheader.console_length < 0) {
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "%s: Error writing console to panic log! (%d)\n", __func__,
			    iheader.console_length);
		iheader.console_length = 0;
	}

	aee_rr_rec_fiq_step(AEE_FIQ_STEP_KE_IPANIC_USERSPACE);
	iheader.userspace_info_offset =
	    ALIGN(iheader.console_offset + iheader.console_length, EMMC_BLOCK_SIZE);
	iheader.userspace_info_length = ipanic_write_userspace(iheader.userspace_info_offset);
	if (iheader.userspace_info_length < 0) {
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "Error writing user space process to panic log (%d)\n",
			    iheader.console_length);
		iheader.userspace_info_length = 0;
	}

	aee_rr_rec_fiq_step(AEE_FIQ_STEP_KE_IPANIC_ANDROID);
	ipanic_write_all_android_buf(iheader.userspace_info_offset + iheader.userspace_info_length,
				     &iheader);

	ipanic_kick_wdt();

	aee_rr_rec_fiq_step(AEE_FIQ_STEP_KE_IPANIC_MMPROFILE);
	ipanic_write_mmprofile(iheader.android_system_offset + iheader.android_system_length,
			       &iheader);
	...
	rc = emmc_ipanic_write(emmc_bounce, 0,
			       ALIGN(sizeof(struct ipanic_header), EMMC_BLOCK_SIZE));
	if (rc <= 0) {
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "aee-ipanic: Header write failed (%d)\n", rc);
		goto out;
	}
	ipanic_kick_wdt();
	...
	xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
		    "aee-ipanic: Panic dump sucessfully written to emmc (detail len: %d, console len: %d)\n",
		    iheader.oops_detail_length, iheader.console_length);
	xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
		    "android log : 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n",
...
int __init aee_emmc_ipanic_init(void)
{
	...
	xlog_printk(ANDROID_LOG_INFO, IPANIC_LOG_TAG,
		    "aee-emmc-ipanic: startup, partition assgined %s\n", AEE_IPANIC_PLABEL);
	return 0;
}
