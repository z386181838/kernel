...
void mt_gpio_load_base(GPIO_REGS *regs) 
{
    ...
    for (idx = 0; idx < sizeof(pReg->dir1)/sizeof(pReg->dir1[0]); idx++)
        regs->dir1[idx].val = __raw_readl(&pReg->dir1[idx]);
    for (idx = 0; idx < sizeof(pReg->dir2)/sizeof(pReg->dir2[0]); idx++)
        regs->dir2[idx].val = __raw_readl(&pReg->dir2[idx]);
...
void mt_gpio_dump_base(GPIO_REGS *regs) 
{
    ...
    GPIOMSG("---# dir #-----------------------------------------------------------------\n");
    GPIOMSG("Offset 0x%04X\n",(void *)(&regs->dir1[0])-(void *)regs);
    for (idx = 0; idx < sizeof(regs->dir1)/sizeof(regs->dir1[0]); idx++) {
        GPIOMSG("0x%04X ", regs->dir1[idx].val);
        if (7 == (idx % 8)) GPIOMSG("\n");
    }
    GPIOMSG("Offset 0x%04X\n",(void *)(&regs->dir2[0])-(void *)regs);
    for (idx = 0; idx < sizeof(regs->dir2)/sizeof(regs->dir2[0]); idx++) {
        GPIOMSG("0x%04X ", regs->dir2[idx].val);
        if (7 == (idx % 8)) GPIOMSG("\n");
    }
    GPIOMSG("\n---# ies #-----------------------------------------------------------------\n");
...
void gpio_dump_regs(void)
{
	...
	GPIOMSG("PIN: [MODE] [PULL_SEL] [DIN] [DOUT] [PULL EN] [DIR]\n");
...
static void mt_gpio_read_pin_base(GPIO_CFG* cfg, int method)
{
    if (method == 0) {
        ...
        int num, bit; 
        VAL_REGS *dir = NULL; 
        if(cfg->no < GPIO_DIR_GRP2_PIN_BASE){
            dir = cur->dir1;
        }else{
            dir = cur->dir2;
            cfg->no -= GPIO_DIR_GRP2_PIN_BASE;
        }
		...
		if(cfg->no < MT_GPIO_BASE_MAX){
			...
			cfg->dir    = (dir[num].val & (1L << bit)) ? (1) : (0);
...
static ssize_t mt_gpio_dump_addr_base(void)
{
    ...
    GPIOMSG("# direction\n");
    for (idx = 0; idx < sizeof(reg->dir1)/sizeof(reg->dir1[0]); idx++)
        GPIOMSG("val[%2d] %p\nset[%2d] %p\nrst[%2d] %p\n", idx, &reg->dir1[idx].val, idx, &reg->dir1[idx].set, idx, &reg->dir1[idx].rst);
    GPIOMSG("# direction2\n");
    for (idx = 0; idx < sizeof(reg->dir2)/sizeof(reg->dir2[0]); idx++)
        GPIOMSG("val[%2d] %p\nset[%2d] %p\nrst[%2d] %p\n", idx, &reg->dir2[idx].val, idx, &reg->dir2[idx].set, idx, &reg->dir2[idx].rst);
    GPIOMSG("# ies\n");
...
static ssize_t mt_gpio_compare_base(void)
{
    ...
    for (idx = 0; idx < sizeof(reg->dir1)/sizeof(reg->dir1[0]); idx++)
        if (reg->dir1[idx].val != cur->dir1[idx].val)
            GPIOERR("mismatch dir[%2d]: %x <> %x\n", idx, reg->dir1[idx].val, cur->dir1[idx].val);
    for (idx = 0; idx < sizeof(reg->dir2)/sizeof(reg->dir2[0]); idx++)
        if (reg->dir2[idx].val != cur->dir2[idx].val)
            GPIOERR("mismatch dir[%2d]: %x <> %x\n", idx, reg->dir2[idx].val, cur->dir2[idx].val);
    for (idx = 0; idx < sizeof(reg->pullen)/sizeof(reg->pullen[0]); idx++)
        if (reg->pullen[idx].val != cur->pullen[idx].val)
...
static ssize_t mt_gpio_dump_regs(char *buf, ssize_t bufLen)
{
    int idx = 0, len = 0;
	/*char tmp[]="PIN: [MODE] [PULL_SEL] [DIN] [DOUT] [PULL EN] [DIR] [INV] [IES]\n";*/
	char tmp[]="PIN: [MODE] [PULL_SEL] [DIN] [DOUT] [PULL EN] [DIR]\n";
	len += snprintf(buf+len, bufLen-len, "%s",tmp);
    for (idx = MT_GPIO_BASE_START; idx < MT_GPIO_BASE_MAX; idx++) {
		len += snprintf(buf+len, bufLen-len, "%3d:%d%d%d%d%d%d\n",
		   idx,mt_get_gpio_mode_base(idx), mt_get_gpio_pull_select_base(idx), mt_get_gpio_in_base(idx),mt_get_gpio_out_base(idx),
		   mt_get_gpio_pull_enable_base(idx),mt_get_gpio_dir_base(idx)); 
    }
