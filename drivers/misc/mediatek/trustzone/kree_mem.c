...
static inline TZ_RESULT _allocFunc(uint32_t cmd, KREE_SESSION_HANDLE session,
				   uint32_t *mem_handle, uint32_t alignment, uint32_t size,
				   char *dbg)
{
	MTEEC_PARAM p[4];
	TZ_RESULT ret;
				      ...
				      uint32_t mem_handle, char *dbg)
{
	...
	ret = KREE_TeeServiceCall(session, cmd, TZ_ParamTypes1(TZPT_VALUE_INPUT), p);
	if (ret < 0) {
#ifdef DBG_KREE_MEM
		pr_info("[kree] %s Error: %d\n", dbg, ret);
#endif
...
static inline TZ_RESULT _handleOpFunc_1(uint32_t cmd, KREE_SESSION_HANDLE session,
					uint32_t mem_handle, uint32_t *count, char *dbg)
{
	MTEEC_PARAM p[4];
	TZ_RESULT ret;
			      ...
			      KREE_SECUREMEM_HANDLE *mem_handle, uint32_t alignment, uint32_t size)
{
	...
	ret =
	    _allocFunc(TZCMD_MEM_SECUREMEM_ALLOC, session, mem_handle, alignment, size,
		       "KREE_AllocSecuremem");
...
TZ_RESULT KREE_ReferenceSecuremem(KREE_SESSION_HANDLE session, KREE_SECUREMEM_HANDLE mem_handle)
{
	...
	ret =
	    _handleOpFunc(TZCMD_MEM_SECUREMEM_REF, session, mem_handle, "KREE_ReferenceSecuremem");
...
TZ_RESULT KREE_UnreferenceSecuremem(KREE_SESSION_HANDLE session, KREE_SECUREMEM_HANDLE mem_handle)
{
	...
	ret =
	    _handleOpFunc_1(TZCMD_MEM_SECUREMEM_UNREF, session, mem_handle, &count,
				   ...
				   uint32_t size)
{
	...
	ret =
	    _allocFunc(TZCMD_MEM_SECURECM_ALLOC, session, cm_handle, alignment, size,
					 ...
					 KREE_SECURECM_HANDLE cm_handle)
{
	...
	ret =
	    _handleOpFunc_1(TZCMD_MEM_SECURECM_UNREF, session, cm_handle, &count,
...
TZ_RESULT KREE_ReadSecurechunkmem(KREE_SESSION_HANDLE session, uint32_t offset, uint32_t size,
				  void *buffer)
{
	MTEEC_PARAM p[4];
	TZ_RESULT ret;
...
TZ_RESULT KREE_WriteSecurechunkmem(KREE_SESSION_HANDLE session, uint32_t offset, uint32_t size,
				   void *buffer)
{
	MTEEC_PARAM p[4];
	TZ_RESULT ret;
...
TZ_RESULT KREE_StartSecurechunkmemSvc(KREE_SESSION_HANDLE session, uint32_t start_pa, uint32_t size)
{
	MTEEC_PARAM p[4];
	TZ_RESULT ret;
...
TZ_RESULT KREE_StopSecurechunkmemSvc(KREE_SESSION_HANDLE session, void **cm_pa, uint32_t *size)
{
	MTEEC_PARAM p[4];
	TZ_RESULT ret;
...
TZ_RESULT KREE_QuerySecurechunkmem(KREE_SESSION_HANDLE session, void **cm_pa, uint32_t *size)
{
	MTEEC_PARAM p[4];
	TZ_RESULT ret;
