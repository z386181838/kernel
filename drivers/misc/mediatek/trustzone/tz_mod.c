...
static long _map_user_pages(struct MTIOMMU_PIN_RANGE_T *pinRange, uint32_t uaddr, uint32_t size,
			    uint32_t control)
{
	int nr_pages;
	...
	res = get_user_pages(current, current->mm, uaddr, nr_pages, write, 0,	/* don't force */
			     pages, NULL);
...
**************************************************************************/
static int tz_client_register_session(struct file *file, KREE_SESSION_HANDLE handle)
{
	...
	if (ret == -1) {
		...
		memset(&info->handles[num], 0, (nspace - num) * sizeof(KREE_SESSION_HANDLE));
	}
...
tz_client_register_sharedmem(struct file *file, KREE_SESSION_HANDLE handle,
			     KREE_SHAREDMEM_HANDLE mem_handle, uint32_t *resource)
{
	struct tz_client_info *info;
	int i, num, nspace, ret = -1;
	void *ptr;
	...
	if (ret == -1) {
		...
		memset(&info->shm_info[num], 0, (nspace - num) * sizeof(struct tz_sharedmem_info));
	}
...
static struct tz_sharedmem_info *tz_client_get_sharedmem(struct file *file,
							 KREE_SHAREDMEM_HANDLE handle)
{
	struct tz_client_info *info;
	struct tz_sharedmem_info *shm_info;
		...
		case TZPT_MEM_INOUT:
			if (type != TZPT_MEM_INPUT) {
				cret = copy_to_user((void *)oparam[i].mem.buffer,
						    param[i].mem.buffer, param[i].mem.size);
				if (cret) {
					cret = -EFAULT;
					goto error;
				}
...
static long tz_client_reg_sharedmem(struct file *file, unsigned long arg)
{
	...
	/* register it ...
	 */
	ret = kree_register_sharedmem(session, &mem_handle, (uint32_t) pin->start, pin->size,
				      (uint32_t) map_p);
	if (ret != TZ_RESULT_SUCCESS) {
	...
	/* register to fd
	 */
	cret = tz_client_register_sharedmem(file, session, (KREE_SHAREDMEM_HANDLE) mem_handle,
					 (uint32_t *) pin);
	if (cret < 0) {
...
static long tz_client_unreg_sharedmem(struct file *file, unsigned long arg)
{
	...
	if (shm_info == NULL) {
		pr_info("tz_client_unreg_sharedmem: get shared memory handle\n");
		return -EINVAL;
	}
...
static int __init register_tz_driver(void)
{
	...
	if (platform_device_register(&tz_device)) {
		ret = -ENODEV;
		xlog_printk(ANDROID_LOG_ERROR, MTEE_MOD_TAG,
...
register_early_suspend(&securetime_early_suspend);
	...
	if (tzret != TZ_RESULT_SUCCESS) {
		pr_info("tz_client_init: TZ Faild %d\n", (int)tzret);
		BUG();
	}
...
#ifdef TZ_PLAYREADY_SECURETIME_SUPPORT
	thread_securetime = kthread_run(update_securetime_thread, NULL, "update_securetime");
