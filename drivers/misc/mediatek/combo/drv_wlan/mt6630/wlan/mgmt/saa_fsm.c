...
*/
static VOID
saaFsmRunEventRxDeauthSub0(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb, IN P_STA_RECORD_T prStaRec);

static VOID
saaFsmRunEventRxDeauthSub1(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec, IN P_BSS_INFO_T prAisBssInfo);

static VOID
saaFsmRunEventRxDisassocSub0(IN P_ADAPTER_T prAdapter,
		IN P_WLAN_DISASSOC_FRAME_T prDisassocFrame, IN P_STA_RECORD_T prStaRec, IN P_SW_RFB_T prSwRfb);

static VOID
saaFsmRunEventRxDisassocSub1(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec);
			...
			if (prStaRec->ucStaState > STA_STATE_1) {
				...
				if (authProcessRxDeauthFrame(prSwRfb,
					...
					if (prAisSpecBssInfo->fgMgmtProtection && HAL_RX_STATUS_IS_CIPHER_MISMATCH(prSwRfb->prRxStatus)
						...
						saaFsmRunEventRxDeauthSub0(prAdapter, prSwRfb, prStaRec);
						return WLAN_STATUS_SUCCESS;
					}
#endif
					saaFsmRunEventRxDeauthSub1(prAdapter, prStaRec, prAisBssInfo);
				}
			}
		}
#if CFG_ENABLE_WIFI_DIRECT
			...
			if (prStaRec->ucStaState > STA_STATE_1) {
				...
				if (assocProcessRxDisassocFrame(prAdapter,
					...
					if (IS_STA_IN_AIS(prStaRec)
						...
						/* prDisassocFrame = (P_WLAN_DISASSOC_FRAME_T) prSwRfb->pvHeader; */
						saaFsmRunEventRxDisassocSub0(prAdapter, prDisassocFrame, prStaRec, prSwRfb);
						return WLAN_STATUS_SUCCESS;
					}
#endif
					saaFsmRunEventRxDisassocSub1(prAdapter, prStaRec);
				}
			}
		}
#if CFG_ENABLE_WIFI_DIRECT
