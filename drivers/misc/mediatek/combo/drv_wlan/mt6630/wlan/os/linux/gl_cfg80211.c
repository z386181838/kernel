				      ...
				      IN void *data, IN int len, IN P_GLUE_INFO_T prGlueInfo)
{
	...
	rStatus = kalIoctl(prGlueInfo,
			   ...
			   &rQueryLTEChn, sizeof(rQueryLTEChn), TRUE, FALSE, TRUE,
...
#endif
	{
		...
		if (prGlueInfo->prAdapter->rWifiVar.rConnSettings.u2CountryCode != 0x0000) {
			...
			rlmDomainGetChnlList(prGlueInfo->prAdapter,
					     BAND_2G4, 14, &ucMax_24G_Chn_List, aucChannelList);
		} else {
		...
		for (ucIdx = 0; ucIdx < ucMax_24G_Chn_List; ucIdx++) {
			if (!(u4TempSafeChannelBitmask & BIT(ucIdx + 1)))
				continue;
			prChnLoad =
			    (P_PARAM_CHN_LOAD_INFO) &(prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo.
						       rEachChnLoad[ucIdx]);
			if (PreferChannels[0].u2APNum >= prChnLoad->u2APNum) {
				...
				if ((PreferChannels[1].u2APNum >= prChnLoad->u2APNum)
				    || (fgIsFistRecord == 1)) {
