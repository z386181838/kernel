...
static VOID
nicTxReturnMsduInfoProfilingSub0(IN UINT_32 u4DeltaTime, IN P_STA_RECORD_T prStaRec, IN UINT_32 u4PktPrintPeriod, IN P_QUE_MGT_T prQM);
#endif
...
WLAN_STATUS nicTxResetResource(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(TX, TRACE,
	       ("Reset TCQ free resource to Page:Buf [%u:%u %u:%u %u:%u %u:%u %u:%u %u:%u ]\n",
	...
	DBGLOG(TX, TRACE,
	       ("Reset TCQ MAX resource to Page:Buf [%u:%u %u:%u %u:%u %u:%u %u:%u %u:%u]\n",
	...
	if (prWfdCfgSettings->ucWfdEnable > 0) {
		while (prMsduInfo) {
			...
			if (prPktProfile->fgIsValid) {
				...
				fgPrintCurPkt =
				    nicTxLifetimePrintCheckSnOrder(prPrevProfileMsduInfo,
								   prPrevRoundLastPkt, prPktProfile,
								   &fgGotFirst, 0);
#else
...
#if CFG_ENABLE_PER_STA_STATISTICS
				{
					...
					if (prStaRec) {
						...
						nicTxReturnMsduInfoProfilingSub0(u4DeltaTime, prStaRec, u4PktPrintPeriod, prQM);
					}
...
nicTxLifetimeCheck(IN P_ADAPTER_T prAdapter,
		   ...
		   IN UINT_8 ucPriorityParam, IN UINT_32 u4PacketLen, IN UINT_8 ucBssIdx)
{
	P_PKT_PROFILE_T prPktProfile = &prMsduInfo->rPktProfile;
		 ...
		 IN UINT_8 ucTxDescLength, IN BOOLEAN fgIsTemplate, OUT PUINT_8 prTxDescBuffer)
{
	...
	if (prMsduInfo->fgIs802_11) {
		P_WLAN_MAC_HEADER_T prWlanHeader =
		    (P_WLAN_MAC_HEADER_T) ((UINT_32) (prMsduInfo->prPacket) +
	...
	if (prStaRec->fgIsQoS) {
		...
		do {
			...
			for (ucTid = 0; ucTid < TX_DESC_TID_NUM; ucTid++) {
				prStaRec->aprTxDescTemplate[ucTid] = prTxDesc;
				DBGLOG(QM, INFO,
				       ("TXD template: TID[%u] Ptr[0x%08lx]\n", ucTid,
					(UINT_32) prTxDesc));
			}
		} while (FALSE);
	}
...
WLAN_STATUS nicTxCmd(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN UINT_8 ucTC)
{
	...
	if (prCmdInfo->eCmdType == COMMAND_TYPE_SECURITY_FRAME) {
		...
		u2OverallBufferLength =
		    TFCB_FRAME_PAD_TO_DW((prCmdInfo->u2InfoBufLen + ucTxDescAndPaddingLength));

		/* Tx byte count */
		HAL_MAC_TX_DESC_SET_TX_BYTE_COUNT(prTxDesc,
						  ucTxDescAndPaddingLength +
		...
		HAL_MAC_TX_DESC_SET_QUEUE_INDEX(prTxDesc,
						arTcResourceControl[ucTC].ucDestQueueIndex);
		...
		/* <3> Copy Frame Body Copy */
		kalCopyFrame(prAdapter->prGlueInfo,
			     prNativePacket, pucOutputBuf + ucTxDescAndPaddingLength);
#endif
		...
		if (prMsduInfo->ucRateMode == MSDU_RATE_MODE_AUTO) {
			...
			nicTxSetPktFixedRateOption(prMsduInfo,
						   u2RateCode, FIX_BW_NO_FIXED, FALSE, FALSE);
		}
		...
		DBGLOG(TX, TRACE, ("TX MGMT Frame: BSS[%u] SEQ[%u] STA[%u] LEN[%u]\n",
				   prCmdInfo->ucBssIndex,
				   prMsduInfo->ucTxSeqNum,
				   prMsduInfo->ucStaRecIndex, u2OverallBufferLength));
	} else {
		  ...
		  IN P_MSDU_INFO_T prMsduInfo, IN P_NATIVE_PACKET prPacket)
{
	...
	nicTxLifetimeCheck(prAdapter,
			   ...
			   prMsduInfo->u2FrameLength, prMsduInfo->ucBssIndex);
#endif
		...
		if (nicTxGetFreeCmdCount(prAdapter) < NIC_TX_CMD_INFO_RESERVED_COUNT) {
			...
			while (prMsduInfoHead) {
				...
				if (prCmdInfo) {
					...
					DBGLOG(TX, INFO,
					       ("%s: EN-Q MSDU[0x%08x] SEQ[%u] BSS[%u] STA[%u] to CMD Q\n",
