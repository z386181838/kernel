...
static VOID
nicpmSetAcpiPowerD0Sub0(IN P_ADAPTER_T prAdapter,
		IN P_FIRMWARE_DIVIDED_DOWNLOAD_T prFwHead, IN PVOID pvFwImageMapFile, OUT WLAN_STATUS * u4Status);
#endif
static VOID
nicpmSetAcpiPowerD0Sub1(IN P_ADAPTER_T prAdapter,
	...
	while (1) {
		...
		if (u4RegValue & WHLPCR_FW_OWN_REQ_SET) {
			...
			break;
			   ...
			   || fgTimeout || wlanIsChipNoAck(prAdapter)) {
			DBGLOG(INIT, ERROR,
			       ("LP cannot be own back, Timeout[%u](%ums), BusAccessError[%u], Reseting[%u], CardRemoved[%u] NoAck[%u]\n",
...
BOOLEAN nicpmSetAcpiPowerD0(IN P_ADAPTER_T prAdapter)
{
	...
	do {
		...
		if (pvFwImageMapFile) {
			...
			if (fgValidHead == TRUE) {
				nicpmSetAcpiPowerD0Sub0(prAdapter, prFwHead, pvFwImageMapFile, &u4Status);
			} else
#endif
			{
				if (wlanImageSectionConfig(prAdapter,
					...
					nicpmSetAcpiPowerD0Sub1(prAdapter, u4FwImgLength, pvFwImageMapFile, &u4Status);
				}
			}
			/* escape to top */
			if (u4Status != WLAN_STATUS_SUCCESS) {
