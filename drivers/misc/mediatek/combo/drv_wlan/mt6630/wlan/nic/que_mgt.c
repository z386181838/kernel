...
*/
static VOID
qmEnqueueTxPacketsDefaultStaAction(IN P_MSDU_INFO_T prCurrentMsduInfo,
		OUT ENUM_WMM_ACI_T * eAci, IN P_STA_RECORD_T prStaRec, IN P_BSS_INFO_T prBssInfo,
		OUT P_QUE_T * prTxQue, OUT UINT_8 *ucTC, OUT BOOLEAN * fgCheckACMAgain);
#if QM_ADAPTIVE_TC_RESOURCE_CTRL
static VOID
qmReassignTcResourceShareRes(IN UINT_32 u4Share,
	IN INT_32 * ai4PerTcResourceDemand, IN P_QUE_MGT_T prQM,
	OUT UINT_32 * u4ResidualResource);
#endif

static VOID
qmHandleRxPacketsAPModeAction(IN PUINT_8 pucEthDestAddr, IN P_SW_RFB_T prCurrSwRfb, IN P_BSS_INFO_T prBssInfo);

#if CFG_RX_REORDERING_ENABLED
static VOID
qmHandleRxPacketsAction(IN P_ADAPTER_T prAdapter, IN BOOLEAN fgIsBMC, IN P_SW_RFB_T prCurrentSwRfb, IN P_HW_MAC_RX_DESC_T prCurrRxStatus, OUT P_QUE_T prReturnedQue);
#endif

static VOID
mqmProcessAssocReqWmmInfo(IN P_ADAPTER_T prAdapter, IN PUINT_8 pucIE, IN P_STA_RECORD_T prStaRec);

static VOID
mqmProcessAssocRspWmmInfo(IN PUINT_8 pucIE, IN P_STA_RECORD_T prStaRec);

static VOID
mqmParseEdcaParametersWmmInfo(IN PUINT_8 pucIE, IN P_BSS_INFO_T prBssInfo, IN BOOLEAN fgForceOverride, OUT BOOLEAN * fgNewParameter);
/*----------------------------------------------------------------------------*/
/*!
		...
		if (!fgDropPacket) {
			...
			default:
				...
				do {
					...
					if (prStaRec->fgIsQoS) {
						qmEnqueueTxPacketsDefaultStaAction(prCurrentMsduInfo, &eAci, prStaRec, prBssInfo,
							&prTxQue, &ucTC, &fgCheckACMAgain);
					} else {
						prTxQue =
						    &prStaRec->arTxQueue[TX_QUEUE_INDEX_NON_QOS];
						ucTC =
						    arNetwork2TcResource[prCurrentMsduInfo->
									 ucBssIndex]
						    [NET_TC_NON_STAREC_NON_QOS_INDEX];
				   ...
				   IN UINT_32 u4CurrentQuota, IN UINT_32 u4TotalQuota)
{
	...
	DBGLOG(QM, LOUD,
	       ("Enter qmDequeueTxPacketsFromPerStaQueues (TC = %u, quota = %u)\n", ucTC,
		u4CurrentQuota));
	...
	for (u4Tc = 0; u4Tc < QM_ACTIVE_TC_NUM; u4Tc++) {
		...
		if (prQM->au4AverageQueLen[u4Tc] == 0) {
			prQM->au4AverageQueLen[u4Tc] =
			    (u4CurrQueLen << QM_QUE_LEN_MOVING_AVE_FACTOR);
		} else {
			prQM->au4AverageQueLen[u4Tc] -=
			    (prQM->au4AverageQueLen[u4Tc] >> QM_QUE_LEN_MOVING_AVE_FACTOR);
...
VOID qmReassignTcResource(IN P_ADAPTER_T prAdapter)
{
	...
	/* 4 <3> Case 2: Demand > Total Resource --> Guarantee a minimum amount of resource for each TC */
		...
		do {
			...
			u4Share = (u4ResidualResource / u4ShareCount);
			if (u4Share) {
				qmReassignTcResourceShareRes(u4Share, ai4PerTcResourceDemand, prQM, &u4ResidualResource);
			}
	...
	do {
		...
		if (HAL_RX_STATUS_GET_HEADER_TRAN(prRxStatus) == TRUE) {	/* (!HIF_RX_HDR_GET_80211_FLAG(prHifRxHdr)){ */
			...
			if (prAdapter->rRxCtrl.rFreeSwRfbList.u4NumElem
				...
				if (IS_BSS_ACTIVE(prBssInfo)) {
					if (OP_MODE_ACCESS_POINT == prBssInfo->eCurrentOPMode) {
						qmHandleRxPacketsAPModeAction(pucEthDestAddr, prCurrSwRfb, prBssInfo);
			...
			/* Check Class Error */
			{
				if (secCheckClassError
					...
					if (prReorderQueParm && prReorderQueParm->fgIsValid
						...
						qmHandleRxPacketsAction(prAdapter, fgIsBMC, prCurrSwRfb, prRxStatus, prReturnedQue);
					}
				} else {
		...
		if (HAL_RX_STATUS_GET_SEC_MODE(prRxStatus) == CIPHER_SUITE_TKIP_WO_MIC) {
			if (prCurrSwRfb->prStaRec) {
				...
				if (pucMicKey == NULL) {
				    ...
				    if (tkipMicDecapsulate(prCurrSwRfb, pucMicKey) == FALSE) {
					fgMicErr = TRUE;
				}
			}
...
VOID qmHandleReorderBubbleTimeout(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Param)
{
			...
			case ELEM_ID_WMM:
				...
				if ((WMM_IE_OUI_TYPE(pucIE) == VENDOR_OUI_TYPE_WMM) &&
					...
					case VENDOR_OUI_SUBTYPE_WMM_INFO:
						{
							mqmProcessAssocReqWmmInfo(prAdapter, pucIE, prStaRec);
						}
						break;
...
mqmProcessAssocReqWmmInfo(IN P_ADAPTER_T prAdapter, IN PUINT_8 pucIE, IN P_STA_RECORD_T prStaRec)
{
	...
	do {
		...
		prStaRec->fgIsUapsdSupported = ((ucQosInfoAC) ? TRUE : FALSE) & IS_FEATURE_ENABLED(prAdapter->rWifiVar.ucUapsd);
	...
	/* Determine whether QoS is enabled with the association */
		...
		IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
			...
			case ELEM_ID_WMM:
				if ((WMM_IE_OUI_TYPE(pucIE) == VENDOR_OUI_TYPE_WMM) &&
				    (!kalMemCmp(WMM_IE_OUI(pucIE), aucWfaOui, 3))) {
					mqmProcessAssocRspWmmInfo(pucIE, prStaRec);
				}
				/* else: VENDOR_OUI_TYPE_WPA, VENDOR_OUI_TYPE_WPS */
	...
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		...
		case ELEM_ID_WMM:
			...
			if ((WMM_IE_OUI_TYPE(pucIE) == VENDOR_OUI_TYPE_WMM) &&
				...
				switch (WMM_IE_OUI_SUBTYPE(pucIE)) {
				case VENDOR_OUI_SUBTYPE_WMM_PARAM:
					mqmParseEdcaParametersWmmInfo(pucIE, prBssInfo, fgForceOverride, &fgNewParameter);
					break;
				default:
...
static VOID
mqmParseEdcaParametersWmmInfo(IN PUINT_8 pucIE,
		IN P_BSS_INFO_T prBssInfo, IN BOOLEAN fgForceOverride,
		OUT BOOLEAN *fgNewParameter)
{
	do {
		if (IE_LEN(pucIE) != 24) {
			...
			*pucWmmParamSetCount = (prIeWmmParam->ucQosInfo & WMM_QOS_INFO_PARAM_SET_CNT);
			...
			for (eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++) {
				...
				DBGLOG(QM, WARN,
				       ("MQM: eAci = %d, ACM = %d, Aifsn = %d, CWmin = %d, CWmax = %d, TxopLimit = %d, *fgNewParameter = %d\n",
...
VOID qmDumpQueueStatus(IN P_ADAPTER_T prAdapter)
{
	...
	for (i = TC0_INDEX; i <= TC5_INDEX; i++) {
		...
		DBGLOG(SW4, INFO,
		       ("TC%u AvgQLen[%02u] minRsv[%02u] CurTcRes[%02u] GrtdTcRes[%02u]\n", i,
...
*/
/*----------------------------------------------------------------------------*/
VOID mqmTimeoutCheckIdleRxBa(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Param)
{
	INT_8 i;
			 ...
			 IN P_RX_BA_ENTRY_T prRxBaEntry, IN ENUM_BA_ENTRY_STATUS_T eStatus)
{
	...
	case BA_ENTRY_STATUS_ACTIVE:
		...
		if (!MQM_CHECK_FLAG(prAdapter->u4FlagBitmap, MQM_FLAG_IDLE_RX_BA_TIMER_STARTED)) {
			cnmTimerInitTimer(prAdapter, &prAdapter->rMqmIdleRxBaDetectionTimer, (PFN_MGMT_TIMEOUT_FUNC) mqmTimeoutCheckIdleRxBa, 0);	/* No parameter */
