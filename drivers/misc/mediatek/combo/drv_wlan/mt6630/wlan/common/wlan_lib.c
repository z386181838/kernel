		   ...
		   IN P_CMD_INFO_T prCmdInfo, IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	...
	case COMMAND_TYPE_MANAGEMENT_FRAME:
		...
		DBGLOG(REQ, INFO,
		       ("Free MGMT Frame: BSS[%u] WIDX:PID[%u:%u] SEQ[%u] STA[%u] RSP[%u]\n",
...
*/
/*----------------------------------------------------------------------------*/
VOID wlanReturnPacketDelaySetupTimeout(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Data)
{
...
BOOLEAN wlanProcessTxFrame(IN P_ADAPTER_T prAdapter, IN P_NATIVE_PACKET prPacket)
{
	...
	if (kalQoSFrameClassifierAndPacketInfo(prAdapter->prGlueInfo,
					       ...
					       &u4PacketLen,
					       aucEthDestAddr,
					       &fgIs1x, &fgIsPAL, &fgIs802_3, &fgIsVlanExists)) {
		...
		if (fgIs1x) {
			...
			prStaRec =
			    cnmGetStaRecByAddress(prAdapter, GLUE_GET_PKT_BSS_IDX(prPacket),
						  aucEthDestAddr);
...
VOID wlanInitFeatureOption(IN P_ADAPTER_T prAdapter)
{
	...
	prWifiVar->ucStaBfee = (UINT_8) wlanCfgGetUint32(prAdapter, "StaBfee", FEATURE_ENABLED);
