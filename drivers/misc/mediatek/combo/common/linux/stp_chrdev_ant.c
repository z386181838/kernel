...
#define ANT_DBG_FUNC(fmt, arg...)    if (gDbgLevel >= ANT_LOG_DBG) { printk(PFX "%s: "  fmt, __func__ , ##arg); }
#define ANT_INFO_FUNC(fmt, arg...)   if (gDbgLevel >= ANT_LOG_INFO) { printk(PFX "%s: "  fmt, __func__ , ##arg); }
#define ANT_WARN_FUNC(fmt, arg...)   if (gDbgLevel >= ANT_LOG_WARN) { printk(PFX "%s: "  fmt, __func__ , ##arg); }
#define ANT_ERR_FUNC(fmt, arg...)    if (gDbgLevel >= ANT_LOG_ERR) { printk(PFX "%s: "   fmt, __func__ , ##arg); }
...
static unsigned char g_bt_bd_addr[10] =
    { 0x01, 0x1a, 0xfc, 0x06, 0x00, 0x55, 0x66, 0x77, 0x88, 0x00 };
...
static int ANT_DownLoad_RAM_Code(void)
{
			...
			while ((i <= download_number)) {
				if (i < download_number) {
					ANT_INFO_FUNC("The ram code file location is %d\n",
						      pPatchExtFile->f_pos);
					if ((pPatchExtFile->f_op->read(pPatchExtFile, pbPatchExtBin,
								       read_ram_code_length,
								       &pPatchExtFile->f_pos)) <
					    0) {
						ANT_ERR_FUNC
						    ("fread ant rom code file fails errno: %d\n");
						filp_close(pPatchExtFile, NULL);
						return 0;
						break;
					} else {
