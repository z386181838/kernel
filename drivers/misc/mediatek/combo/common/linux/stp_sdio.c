...
********************************************************************************
*/
#define STPSDIO_LOUD_FUNC(fmt, arg...)    if (gStpSdioDbgLvl >= STPSDIO_LOG_LOUD) { printk(DFT_TAG "[L]%s:"  fmt, __func__ , ##arg); }
#define STPSDIO_DBG_FUNC(fmt, arg...)    if (gStpSdioDbgLvl >= STPSDIO_LOG_DBG) { printk(DFT_TAG "[D]%s:"  fmt, __func__ , ##arg); }
#define STPSDIO_HINT_FUNC(fmt, arg...)   if (gStpSdioDbgLvl >= STPSDIO_LOG_HINT) { printk(DFT_TAG "[I]%s:"  fmt, __func__ , ##arg); }
#define STPSDIO_INFO_FUNC(fmt, arg...)   if (gStpSdioDbgLvl >= STPSDIO_LOG_INFO) { printk(DFT_TAG "[I]%s:"  fmt, __func__ , ##arg); }
#define STPSDIO_WARN_FUNC(fmt, arg...)   if (gStpSdioDbgLvl >= STPSDIO_LOG_WARN) { printk(DFT_TAG "[W]%s:"  fmt, __func__ , ##arg); }
#define STPSDIO_ERR_FUNC(fmt, arg...)    if (gStpSdioDbgLvl >= STPSDIO_LOG_ERR) { printk(DFT_TAG "[E]%s(%d):"  fmt, __func__ , __LINE__, ##arg); }
...
INT32 stp_sdio_host_info_op(INT32 opId)
{
	INT32 iRet = 0;
#if KMALLOC_UPDATE
	static UINT8 *p_tx_buffer;
	static UINT8 *p_rx_buffer;
	if (NULL == p_tx_buffer) {
 ...
 */
#if STP_SDIO_NEW_TXRING
INT32 stp_sdio_tx(const UINT8 *data, const UINT32 size, UINT32 *written_size)
{
	PUINT8 pkt_bufp;
	UINT32 prev_wr_idx;
	UINT32 prev_size;
...
#endif
/* init stp sdio host private information */ 
/*TODO:[FixMe][George] Still need this ? */
