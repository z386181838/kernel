...
#endif
static int hif_sdio_probe(struct sdio_func *func, const struct sdio_device_id *id);

static void hif_sdio_remove(struct sdio_func *func);

static void hif_sdio_irq(struct sdio_func *func);

static int hif_sdio_clt_probe_func(MTK_WCN_HIF_SDIO_REGISTINFO *registinfo_p, INT8 probe_idx);

static void hif_sdio_clt_probe_worker(struct work_struct *work);

static int hif_sdio_find_probed_list_index_by_func(struct sdio_func *func);

#if 0				/* TODO:[ChangeFeature][George] remove obsolete function? */
static int hif_sdio_find_probed_list_index_by_clt_index(INT32 clt_index);
#endif

static int hif_sdio_find_probed_list_index_by_id_func(UINT16 vendor,
						      UINT16 device, UINT16 func_num);

static void hif_sdio_init_clt_list(INT32 index);

static int hif_sdio_find_clt_list_index(UINT16 vendor, UINT16 device, UINT16 func_num);

static int hif_sdio_check_supported_sdio_id(UINT16 vendor, UINT16 device);

static int hif_sdio_check_duplicate_sdio_id(UINT16 vendor, UINT16 device, UINT16 func_num);

static int hif_sdio_add_clt_list(INT32 *clt_index_p,
...
static int (*fp_wmt_tra_sdio_update) (void);
 ...
 */
static inline struct sdio_func *hif_sdio_ctx_to_func(MTK_WCN_HIF_SDIO_CLTCTX ctx)
{
	UINT32 probe_index;
...
static INT32 _hif_sdio_deep_sleep_info_init(VOID)
{
	...
	for (i = 0; i < array_size; i++) {
		...
		clt_info_size =
		    sizeof(g_hif_sdio_ds_info_list[i].clt_info) /
		    sizeof(g_hif_sdio_ds_info_list[i].clt_info[0]);
 ...
 * \retval none.
 */
void mtk_wcn_hif_sdio_set_drvdata(MTK_WCN_HIF_SDIO_CLTCTX ctx, void *private_data_p)
{
 ...
 */
void *mtk_wcn_hif_sdio_get_drvdata(MTK_WCN_HIF_SDIO_CLTCTX ctx)
{
	UINT8 probed_idx = CLTCTX_IDX(ctx);
 ...
 * \retval >= 0  return found index
 */
static int hif_sdio_find_probed_list_index_by_id_func(UINT16 vendor, UINT16 device, UINT16 func_num)
{
	int i;
	for (i = 0; i < CFG_CLIENT_COUNT; i++) {
		if (g_hif_sdio_probed_func_list[i].func) {
	...
	if (hif_sdio_probed_funcp->clt_idx >= 0) {	/* The clt contex has been registed */
		...
		ret =
		    sdio_set_block_size(func,
					g_hif_sdio_clt_drv_list[hif_sdio_probed_funcp->
								clt_idx].func_info->blk_sz);
		sdio_release_host(func);
