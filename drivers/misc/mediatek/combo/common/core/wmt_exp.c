...
ENUM_WMT_ANT_RAM_STATUS mtk_wcn_wmt_ant_ram_ctrl(ENUM_WMT_ANT_RAM_CTRL ctrlId, UCHAR *pBuf,
						 UINT32 length, ENUM_WMT_ANT_RAM_SEQ seq)
{
	...
	if ((WMT_ANT_RAM_DOWNLOAD == ctrlId) &&
		...
		eRet = WMT_ANT_RAM_PARA_ERR;
		WMT_ERR_FUNC
		    ("error parameter detected, ctrlId:%d, pBuf:0x%08x,length(0x%x),seq(%d) .\n",
