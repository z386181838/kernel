...
static UCHAR WMT_ANT_RAM_STA_GET_CMD[] = { 0x01, 0x06, 0x02, 0x00, 0x05, 0x02
};

static UCHAR WMT_ANT_RAM_STA_GET_EVT[] = { 0x02, 0x06, 0x03, 0x00	/*length */
	    , 0x05, 0x02, 0x00	/*S: result */
};

static UCHAR WMT_ANT_RAM_DWN_CMD[] = { 0x01, 0x15, 0x00, 0x00, 0x01
};

static UCHAR WMT_ANT_RAM_DWN_EVT[] = { 0x02, 0x15, 0x01, 0x00	/*length */
	, 0x00
};
...
INT32 wmt_core_ctrl(ENUM_WMT_CTRL_T ctrId, PUINT32 pPa1, PUINT32 pPa2)
{
	...
	if (iRet) {
		/* ERROR */
		WMT_ERR_FUNC
		    ("WMT-CORE: wmt_core_ctrl failed: id(%d), type(%d), value(%d) iRet:(%d)\n",
...
static INT32 wmt_core_stp_init(VOID)
{
	...
	if ((NULL != pctx->p_ic_ops->co_clock_ctrl) && (pWmtGenConf != NULL)) {
		(*(pctx->p_ic_ops->co_clock_ctrl)) (pWmtGenConf->co_clock_flag ==
						    0 ? WMT_CO_CLOCK_DIS : WMT_CO_CLOCK_EN);
	} else {
...
static VOID wmt_core_dump_func_state(CHAR *pSource)
{
	WMT_INFO_FUNC
	    ("[%s]status(b:%d f:%d g:%d w:%d lpbk:%d coredump:%d wmt:%d ant:%d sd1:%d sd2:%d stp:%d)\n",
	     (pSource == NULL ? (CHAR *) "CORE" : pSource), gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_BT],
			...
			if (WMTDRV_TYPE_WIFI == drvType) {
				if (WMT_HIF_UART == gMtkWmtCtx.wmtHifConf.hifType) {
					...
					if (DRV_STS_FUNC_ON ==
						 ...
						 **/
						WMT_ERR_FUNC
						    ("WMT-CORE: turn on Wi-Fi in comm SDIO2 but SDIO in FUNC_OFF state(0x%x)\n",
						     gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_SDIO2]);
						osal_assert(0);
						return -4;
					}
					...
					WMT_ERR_FUNC
					    ("WMT-CORE: error, not implemented yet gMtkWmtCtx.wmtHifConf.hifType: 0x%x, unspecified wifi_hif\n",
...
static INT32 opfunc_pwr_sv(P_WMT_OP pWmtOp)
{
	...
	if (SLEEP == pWmtOp->au4OpData[0]) {
		...
		if (ret || (u4_result != sizeof(WMT_SLEEP_CMD))) {
			WMT_ERR_FUNC("wmt_core: SLEEP_CMD ret(%d) cmd len err(%d, %d) ", ret,
		...
		if (osal_memcmp(evt_buf, WMT_SLEEP_EVT, sizeof(WMT_SLEEP_EVT)) != 0) {
			...
			wmt_core_rx_flush(WMT_TASK_INDX);
			WMT_ERR_FUNC
			    ("wmt_core: rx(%d):[%02X,%02X,%02X,%02X,%02X,%02X] exp(%d):[%02X,%02X,%02X,%02X,%02X,%02X]\n",
		...
		if (ret || (u4_result != sizeof(WMT_WAKEUP_CMD))) {
			wmt_core_rx_flush(WMT_TASK_INDX);
			WMT_ERR_FUNC("wmt_core: WAKEUP_CMD ret(%d) cmd len err(%d, %d) ", ret,
		...
		if (ret || (u4_result != sizeof(WMT_HOST_AWAKE_CMD))) {
			WMT_ERR_FUNC("wmt_core: HOST_AWAKE_CMD ret(%d) cmd len err(%d, %d) ", ret,
...
static INT32 opfunc_lpbk(P_WMT_OP pWmtOp)
{
	...
	do {
		...
		osal_memcpy((VOID *) pWmtOp->au4OpData[1],
...
static INT32 opfunc_therm_ctrl(P_WMT_OP pWmtOp)
{
	...
	iRet = wmt_core_rx(evtBuf, evtLen, &u4Res);
	if (iRet || ((u4Res != osal_sizeof(WMT_THERM_CTRL_EVT))
	...
	if (u4Res == osal_sizeof(WMT_THERM_CTRL_EVT)) {
		...
		if (osal_memcmp(evtBuf, WMT_THERM_READ_EVT, osal_sizeof(WMT_THERM_READ_EVT) - 1) !=
			...
			WMT_ERR_FUNC
			    ("WMT-CORE: rx(%d):[%02X,%02X,%02X,%02X,%02X,%02X] exp(%d):[%02X,%02X,%02X,%02X]\n",
			     u4Res, evtBuf[0], evtBuf[1], evtBuf[2], evtBuf[3], evtBuf[4],
...
INT32 opfunc_ant_ram_down(P_WMT_OP pWmtOp)
{
	...
	do {
		...
		if (osal_memcmp(antEvtBuf, WMT_ANT_RAM_DWN_EVT, sizeof(WMT_ANT_RAM_DWN_EVT)) != 0) {
			WMT_ERR_FUNC
			    ("wmt_core: compare WMT_ANT_RAM_DWN_EVT result error rx(%d):[%02X,%02X,%02X,%02X,%02X] exp(%d):[%02X,%02X,%02X,%02X,%02X]\n",
	...
	while (fragSeq < fragNum) {
		...
		if (osal_memcmp(antEvtBuf, WMT_SET_RAM_OP_EVT, sizeof(WMT_SET_RAM_OP_EVT)) != 0) {
			WMT_ERR_FUNC
			    ("wmt_core: compare WMT_SET_RAM_OP_EVT result error rx(%d):[%02X,%02X,%02X,%02X,%02X] exp(%d):[%02X,%02X,%02X,%02X,%02X]\n",
			     u4Res, antEvtBuf[0], antEvtBuf[1], antEvtBuf[2], antEvtBuf[3],
...
INT32 opfunc_ant_ram_stat_get(P_WMT_OP pWmtOp)
{
	...
	if (osal_memcmp(antEvtBuf, WMT_ANT_RAM_STA_GET_EVT, sizeof(WMT_ANT_RAM_STA_GET_EVT) - 1) !=
	    0) {
		WMT_ERR_FUNC
		    ("wmt_core: compare WMT_ANT_RAM_STA_GET_EVT result error rx(%d):[%02X,%02X,%02X,%02X,%02X] exp(%d):[%02X,%02X,%02X,%02X,%02X]\n",
