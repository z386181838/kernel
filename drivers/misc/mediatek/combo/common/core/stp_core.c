...
VOID stp_dbg_pkt_log(INT32 type, INT32 txAck, INT32 seq, INT32 crc, INT32 dir, const UINT8 *pBuf,
		     INT32 len)
{
	...
	/* sequence matches expected, enqueue packet */
		...
		if (fgRxOk == 0) {
			...
			if ((stp_core_ctx.parser.type == WMT_TASK_INDX) &&
			    (stp_core_ctx.parser.wmtsubtype == WMT_LTE_COEX_FLAG)) {
#if 0
				osal_buffer_dump(&stp_core_ctx.rx_buf[0], "WMT->LTE MSG",
		...
		while (i > 0) {
			...
			case MTKSTP_CRC2:
				...
				if ((stp_core_ctx.parser.type == BT_TASK_INDX)
					...
					is_function_active = ((*sys_check_function_status)
							      (stp_core_ctx.parser.type,
							       OP_FUNCTION_ACTIVE) ==
							      STATUS_FUNCTION_ACTIVE);
