...
#define STP_PSM_LOUD_FUNC(fmt, arg...)   if (gPsmDbgLevel >= STP_PSM_LOG_LOUD) { pr_debug(PFX_PSM "%s: "  fmt, __func__ , ##arg); }
#define STP_PSM_DBG_FUNC(fmt, arg...)    if (gPsmDbgLevel >= STP_PSM_LOG_DBG) {  pr_debug(PFX_PSM "%s: "  fmt, __func__ , ##arg); }
#define STP_PSM_INFO_FUNC(fmt, arg...)   if (gPsmDbgLevel >= STP_PSM_LOG_INFO) { pr_info(PFX_PSM "[I]%s: "  fmt, __func__ , ##arg); }
#define STP_PSM_WARN_FUNC(fmt, arg...)   if (gPsmDbgLevel >= STP_PSM_LOG_WARN) { pr_info(PFX_PSM "[W]%s: "  fmt, __func__ , ##arg); }
#define STP_PSM_ERR_FUNC(fmt, arg...)    if (gPsmDbgLevel >= STP_PSM_LOG_ERR) {  pr_info(PFX_PSM "[E]%s(%d):ERROR! "   fmt, __func__ , __LINE__, ##arg); }
#define STP_PSM_TRC_FUNC(f)              if (gPsmDbgLevel >= STP_PSM_LOG_DBG) {  pr_debug(PFX_PSM "<%s> <%d>\n", __func__, __LINE__); }
...
INT32
_stp_psm_hold_data(MTKSTP_PSM_T *stp_psm, const UINT8 *buffer, const UINT32 len, const UINT8 type)
{
...
static inline INT32 _stp_psm_wait_wmt_event_wq(MTKSTP_PSM_T *stp_psm)
{
	...
	if (stp_psm == NULL) {
		...
		osal_wait_for_event_timeout(&stp_psm->wait_wmt_q,
					    _stp_psm_wait_wmt_event, (void *)stp_psm);
...
INT32
stp_psm_hold_data(MTKSTP_PSM_T *stp_psm, const UINT8 *buffer, const UINT32 len, const UINT8 type)
{
