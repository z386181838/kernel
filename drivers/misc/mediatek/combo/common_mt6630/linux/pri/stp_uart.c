...
#define UART_DBG_FUNC(fmt, arg...)    if (gDbgLevel >= UART_LOG_DBG) {  pr_debug(PFX "%s: "  fmt, __func__ , ##arg); }
#define UART_INFO_FUNC(fmt, arg...)   if (gDbgLevel >= UART_LOG_INFO) { pr_info(PFX "%s: "  fmt, __func__ , ##arg); }
#define UART_WARN_FUNC(fmt, arg...)   if (gDbgLevel >= UART_LOG_WARN) { pr_info(PFX "%s: "  fmt, __func__ , ##arg); }
#define UART_ERR_FUNC(fmt, arg...)    if (gDbgLevel >= UART_LOG_ERR) {  pr_info(PFX "%s: "   fmt, __func__ , ##arg); }
#define UART_TRC_FUNC(f)              if (gDbgLevel >= UART_LOG_DBG) {  pr_debug(PFX "<%s> <%d>\n", __func__, __LINE__); }
