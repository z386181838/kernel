	...
	if (mail_box[0] >= 0 && mail_box[1] >= 0) {
		...
		case HIGH_STAT:
			if (throttling_pre_stat < HIGH_STAT) {
				if (cur_thro > 0) {	/*Wifi is working!! */
					thro_constraint =
					    (cur_thro * up_numerator) / up_denominator;
				} else {	/*At this moment, current throughput i */
					thro_constraint =
					    (thro_constraint * up_numerator) / up_denominator;
				}

				wmt_tm_printk("LOW/MID-->HIGH:%lu <- (%d / %d)", thro_constraint,
				...
				if ((over_up_time * interval) >= up_duration) {
					...
					wmt_tm_printk("HIGH-->HIGH:%lu <- (%d / %d)",
						      thro_constraint, up_numerator,
						      up_denominator);
		...
		case 0:
			if (throttling_pre_stat > LOW_STAT) {
				if (cur_thro > 0) {
					thro_constraint =
					    (cur_thro * low_numerator) / low_denominator;
				} else {
					thro_constraint =
					    (thro_constraint * low_numerator) / low_denominator;
				}

				wmt_tm_printk("MID/HIGH-->LOW:%lu <- (%d / %d)", thro_constraint,
