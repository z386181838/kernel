...
static INT32 mtk_wcn_sys_if_tx(const UINT8 *data, const UINT32 size, UINT32 *written_size)
{
...
static INT32 mtk_wcn_sys_event_set(UINT8 function_type)
{
	if ((function_type < MTKSTP_MAX_TASK_NUM) && (event_callback_tbl[function_type] != 0x0)) {
		...
		pr_info("[%s] STP set event fail. It seems the function is not active.\n",
			__func__);
	}
...
VOID mtk_wcn_stp_set_if_tx_type(ENUM_STP_TX_IF_TYPE stp_if_type)
{
	...
	pr_info("[%s] set STP_IF_TX to %s.\n",
		__func__,
		(STP_UART_IF_TX ==
		 stp_if_type) ? "UART" : ((STP_SDIO_IF_TX == stp_if_type) ? "SDIO" : "NULL"));
}
