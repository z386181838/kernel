...
#define BT_DBG_FUNC(fmt, arg...)    if (gDbgLevel >= BT_LOG_DBG) { pr_info(PFX "%s: "  fmt, __func__ , ##arg); }
#define BT_INFO_FUNC(fmt, arg...)   if (gDbgLevel >= BT_LOG_INFO) { pr_info(PFX "%s: "  fmt, __func__ , ##arg); }
#define BT_WARN_FUNC(fmt, arg...)   if (gDbgLevel >= BT_LOG_WARN) { pr_info(PFX "%s: "  fmt, __func__ , ##arg); }
#define BT_ERR_FUNC(fmt, arg...)    if (gDbgLevel >= BT_LOG_ERR) { pr_info(PFX "%s: "   fmt, __func__ , ##arg); }
#define BT_TRC_FUNC(f)              if (gDbgLevel >= BT_LOG_DBG) {pr_info(PFX "<%s> <%d>\n", __func__, __LINE__); }
