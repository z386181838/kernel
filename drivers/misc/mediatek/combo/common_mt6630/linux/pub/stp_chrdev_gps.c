...
#define GPS_DBG_FUNC(fmt, arg...)    if (gDbgLevel >= GPS_LOG_DBG) { pr_info(PFX "%s: "  fmt, __func__ , ##arg); }
#define GPS_INFO_FUNC(fmt, arg...)   if (gDbgLevel >= GPS_LOG_INFO) { pr_info(PFX "%s: "  fmt, __func__ , ##arg); }
#define GPS_WARN_FUNC(fmt, arg...)   if (gDbgLevel >= GPS_LOG_WARN) { pr_info(PFX "%s: "  fmt, __func__ , ##arg); }
#define GPS_ERR_FUNC(fmt, arg...)    if (gDbgLevel >= GPS_LOG_ERR) { pr_info(PFX "%s: "  fmt, __func__ , ##arg); }
#define GPS_TRC_FUNC(f)              if (gDbgLevel >= GPS_LOG_DBG) { pr_info(PFX "<%s> <%d>\n", __func__, __LINE__); }
...
long GPS_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMBO_IOC_GPS_HWVER:
		...
		GPS_INFO_FUNC(KERN_INFO
			      "GPS_ioctl(): get hw version = %d, sizeof(hw_ver_sym) = %d\n",
