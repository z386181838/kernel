	...
	if (0 != wmt_plat_merge_if_flag_get()) {
		...
		case PIN_STA_INIT:
			...
			mt_set_gpio_mode(GPIO_PCM_DAIPCMOUT_PIN,
					 GPIO_PCM_DAIPCMOUT_PIN_M_MRG_I2S_PCM_TX);
			mt_set_gpio_mode(GPIO_PCM_DAIPCMIN_PIN,
					 GPIO_PCM_DAIPCMIN_PIN_M_MRG_I2S_PCM_RX);
			mt_set_gpio_mode(GPIO_PCM_DAISYNC_PIN,
					 GPIO_PCM_DAISYNC_PIN_M_MRG_I2S_PCM_SYNC);
	...
	if (0 != wmt_plat_merge_if_flag_get()) {
		...
		case PIN_STA_MUX:
			...
			mt_set_gpio_mode(GPIO_COMBO_I2S_WS_PIN,
					 GPIO_COMBO_I2S_WS_PIN_M_MRG_I2S_PCM_SYNC);
			mt_set_gpio_mode(GPIO_COMBO_I2S_DAT_PIN,
					 GPIO_COMBO_I2S_DAT_PIN_M_MRG_I2S_PCM_RX);
			mt_set_gpio_mode(GPIO_PCM_DAIPCMOUT_PIN,
					 GPIO_PCM_DAIPCMOUT_PIN_M_MRG_I2S_PCM_TX);
...
#else
		WMT_INFO_FUNC
		    ("[MT662x]warnning:FM digital mode is not set, no I2S GPIO settings should be modified by combo driver\n");
#endif
#endif
	}
