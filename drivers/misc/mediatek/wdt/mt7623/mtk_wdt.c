...
/* only need in 6572 for hardware workaround*/
...
static int mtk_wdt_probe(struct platform_device *dev)
{
	...
	printk("mtk_wdt_probe : Initialize to dual mode \n");
    #ifdef  CONFIG_MTK_WD_KICKER_WCN 
			mtk_wdt_mode_config(TRUE, TRUE, TRUE, FALSE, TRUE);
		#else	
			mtk_wdt_mode_config(TRUE, TRUE, TRUE, FALSE, FALSE);//for WCN SW RESET
    #endif
...
static int mtk_wdt_suspend(struct platform_device *pdev, pm_message_t mesg)
{	
...
MODULE_AUTHOR("MTK");
MODULE_DESCRIPTION("MT7623 Watchdog Device Driver");
MODULE_LICENSE("GPL");
