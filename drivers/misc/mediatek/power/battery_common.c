...
{
	int ret;
	ret = sscanf(buf, "%d", &g_custom_charging_current);
	battery_xlog_printk(BAT_LOG_CRTI, "custom charging current = %d\n",
			    g_custom_charging_current);
	wake_up_bat();
	return size;
...
void bat_detect_event_enable(bool enable)
{
	mutex_lock(&bat_mutex);
...
EXPORT_SYMBOL(bat_detect_event_enable);
...
static int battery_probe(struct platform_device *pdev)
{
	...
	/* For EM */
	{
		...
		ret_device_file =
		    device_create_file(dev, &dev_attr_ADC_Channel_Is_Calibration);
		...
		ret_device_file =
		    device_create_file(dev, &dev_attr_FG_Battery_CurrentConsumption);
		...
		ret_device_file =
		    device_create_file(dev, &dev_attr_Custom_Charging_Current);
	}
...
#else
	if (!mutex_trylock(&bat_mutex))
		return -EBUSY;	/* skip this suspend operation */
