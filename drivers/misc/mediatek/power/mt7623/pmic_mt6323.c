...
rewait:
        ret2 = wait_event_interruptible(pmic_thread_wq, pmic_thread_timeout);
        if((ret2 == -ERESTARTSYS) && (pmic_thread_timeout == 0)){
            printk("interruptiable signal ignore and rewait\n");
            goto rewait;      
        }
...
{
    printk("Power/PMIC" "[show_pmic_access] 0x%x\n", g_reg_value);
...
static ssize_t store_pmic_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    U32 reg_value = 0;
    U32 reg_address = 0;
    printk("Power/PMIC" "[store_pmic_access] \n");
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[store_pmic_access] \n");
    if(buf != NULL && size != 0)
    {
        printk("Power/PMIC" "[store_pmic_access] buf is %s and size is %d \n",buf,size);
        ...
        if(size > 5)
        {
            ...
            printk("Power/PMIC" "[store_pmic_access] write PMU reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        else
        {
            ...
            printk("Power/PMIC" "[store_pmic_access] read PMU reg 0x%x with value 0x%x !\n",reg_address,g_reg_value);
            printk("Power/PMIC" "[store_pmic_access] Please use \"cat pmic_access\" to get value\r\n");
...
void PMIC_INIT_SETTING_V1(void)
{
    ...
    ret_val=pmic_config_interface(0x011A, 0x1, 0x1, 1); //Watch dog reset enable

    ret_val=pmic_config_interface(0x011A, 0x0, 0x1, 3);
    ret_val=pmic_config_interface(0x011A, 0x0, 0x1, 5); //Reset key long reset disable
    ret_val=pmic_config_interface(0x011A, 0x0, 0x1, 6); //Power key long reset disable
    ret_val=pmic_config_interface(0x011A, 0x0, 0x1, 4); //Disable SYSRSTB reset PCHR_DIG
    ret_val=pmic_read_interface( 0x011A, (&reg_val),0xFFFF, 0);
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[pmic6323_init] TOP MISC RESET [0x%x]=0x%x\n", 0x011A, reg_val); 
        
    xlog_printk(ANDROID_LOG_INFO,  "Power/PMIC", "set the vproc vosec... to 1.31v begin\n");
    //upmu_set_vproc_vosel(0x62);
    pmic_config_interface(VPROC_CON9, 0x60, PMIC_VPROC_VOSEL_MASK, PMIC_VPROC_VOSEL_SHIFT);
    pmic_config_interface(VPROC_CON10, 0x60, PMIC_VPROC_VOSEL_ON_MASK, PMIC_VPROC_VOSEL_ON_SHIFT);
    //get settings
    pmic_read_interface(VPROC_CON12,&reg_val,PMIC_NI_VPROC_VOSEL_MASK,PMIC_NI_VPROC_VOSEL_SHIFT);
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[VPROC 1.30] VPROC setting 0x%x, should be 0x60\n", reg_val);
    xlog_printk(ANDROID_LOG_INFO,  "Power/PMIC",  "set the vproc vosec... to 1.31v finish\n");
    xlog_printk(ANDROID_LOG_INFO,  "Power/PMIC",  "set VPA 1.0v, moved to preloader stage\n");
//    upmu_set_vpa_en(1);
//    upmu_get_qi_vpa_en();
//    upmu_set_vpa_vosel(0xa);
//    xlog_printk(ANDROID_LOG_INFO,  "Power/PMIC",  "set VPA 1.0v end\n");
