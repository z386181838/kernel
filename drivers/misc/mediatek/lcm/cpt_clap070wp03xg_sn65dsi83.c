...
/*int sn65dsi83_write_byte(kal_uint8 cmd, kal_uint8 Data);*/
#endif
...
static void dump_reg_table(sn65dsi8x_setting_table *table, unsigned int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		default:
			...
			dprintf(INFO, "dump cmd=0x%x  data=0x%x\n", cmd, data);
#else
			sn65dsi83_read_byte(cmd, &data);
			pr_info("dump cmd=0x%x  data=0x%x\n", cmd, data);
#endif
		}
	}
}
...
#ifdef BUILD_LK
	dprintf(INFO, "dump cmd=0xe5  data=0x%x\n", data);
#else
	pr_info("dump cmd=0xe5  data=0x%x\n", data);
#endif
...
static void lcm_set_gpio_output(unsigned int GPIO, unsigned int output)
{
	if (GPIO == 0xFFFFFFFF) {
		...
		dprintf(INFO, "[LK/LCM] GPIO_LCD_PWR2_EN =  0x%x\n", GPIO_LCD_PWR2_EN);
		dprintf(INFO, "[LK/LCM] GPIO_LCD_RST_EN =  0x%x\n", GPIO_LCD_RST_EN);
		dprintf(INFO, "[LK/LCM] GPIO_LCD_STB_EN =   0x%x\n", GPIO_LCD_STB_EN);
		dprintf(INFO, "[LK/LCM] GPIO_SHIFT_EN =   0x%x\n", GPIO_SHIFT_EN);
		dprintf(INFO, "[LK/LCM] GPIO_LCD_BL_EN =   0x%x\n", GPIO_LCD_BL_EN);
		dprintf(INFO, "[LK/LCM] GPIO_LCD_BRIDGE_EN =  0x%x\n", GPIO_LCD_BRIDGE_EN);
...
static void lcm_suspend(void)
{
	...
	pr_info("lcm_suspend  0x0a  value=0x%x\n", temp);

	sn65dsi83_read_byte(0x0d, &temp);
	pr_info("lcm_suspend  0x0d  value=0x%x\n", temp);
...
static void lcm_resume(void)
{
	...
	pr_info("lcm_resume cmd-- 0x0a=0x%x\n", temp);

	sn65dsi83_read_byte(0x0d, &temp);
	pr_info("lcm_resume cmd-- 0x0d=0x%x\n", temp);

	sn65dsi83_read_byte(0x09, &temp);
	pr_info("lcm_resume cmd-- 0x09=0x%x\n", temp);
#endif
...
{
#if defined(BUILD_LK)
	dprintf(INFO, "Sn65dsi83 lcm_compare_id enter\n");
#endif
