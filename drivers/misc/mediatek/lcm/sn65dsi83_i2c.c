...
static int sn65dsi83_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_info("[sn65dsi83_driver_probe]name=%s addr=0x%x\n", client->name, client->addr);
...
static int sn65dsi83_driver_remove(struct i2c_client *client)
{
	pr_info("sn65dsi83_driver_remove\n");
...
{
	pr_info("[sn65dsi83_init] init start\n");

	i2c_register_board_info(SN65DSI83_BUSNUM, &i2c_sn65dsi83, 1);

	if (i2c_add_driver(&sn65dsi83_driver) != 0)
		pr_info("[sn65dsi83_init] failed to register sn65dsi83 i2c driver.\n");
	else
		pr_info("[sn65dsi83_init] Success to register sn65dsi83 i2c driver.\n");
