	...
	while ((part = disk_part_iter_next(&piter))) {
		for (i = 0; i < PART_NUM; i++) {
			if (PartInfo[i].partition_idx != 0
				...
				sprintf(link_target, "/dev/%sp%d", disk->disk_name,
					part->partno);
...
ssize_t dumchar_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		printk
		    ("[dumchar_read] It is forbidded to access %s with dumchar(/dev/%s), the %s partition is managed by filesystem!\n",
...
ssize_t dumchar_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		printk
		    ("[dumchar_write] It is forbidded to access %s with dumchar(/dev/%s), the %s partition is managed by filesystem!\n",
		     dev->dumname, dev->dumname, dev->dumname);
...
static long dumchar_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		printk
		    ("[dumchar_ioctl] It is forbidded to access %s with dumchar(/dev/%s), the %s partition is managed by filesystem!\n",
		     dev->dumname, dev->dumname, dev->dumname);
...
loff_t dumchar_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		printk
		    ("[dumchar_llseek] It is forbidded to access %s with dumchar(/dev/%s), the %s partition is managed by filesystem!\n",
		     dev->dumname, dev->dumname, dev->dumname);
...
static int dumchar_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	for (i = 0; i < PART_NUM; i++) {
		...
		len += sprintf(page + len, "%-10s   0x%08x   0x%08x   1   %s   %s\n",
			       ...
			       (u32) dumchar_devices[i].size, dumchar_devices[i].mtd_index,
			       /*   "NAND", */
		...
		if (PartInfo[i].partition_idx == 0) {
			len +=
			    sprintf(page + len, "%-10s   0x%016llx   0x%016llx   2   %s     %s\n",
			...
			len +=
			    sprintf(page + len,
				    "%-10s   0x%016llx   0x%016llx   2   /dev/mmcblk0p%d   %s\n",
				    dumchar_devices[i].dumname, dumchar_devices[i].size,
				    dumchar_devices[i].start_address,
				    /* "EMMC", */
	...
	for (i = 0; i < PART_NUM; i++) {
		if (dumchar_devices[i].type == NAND) {
			...
			if (PartInfo[i].partition_idx == 0) {
				...
				len +=
				    sprintf(page + len,
					    "%-10s   0x%016llx   0x%016llx   2   /dev/mmcblk0p%d\n",
					    dumchar_devices[i].dumname, dumchar_devices[i].size,
					    dumchar_devices[i].start_address,
					    /* "EMMC", */
...
int dumchar_open(struct inode *inode, struct file *filp)
{
	...
	if (!strcmp(dev->dumname, "usrdata") || !strcmp(dev->dumname, "cache")
		...
		printk
		    ("[dumchar_open] It is forbidded to access %s with dumchar(/dev/%s), the %s partition is managed by filesystem!\n",
		     dev->dumname, dev->dumname, dev->dumname);
	...
	for (i = 0; i < part_num; i++) {
		...
		if (dumchar_devices[i].type == EMMC) {
			...
			case EMMC_PART_BOOT1:
				...
				break;
			case EMMC_PART_USER:
				sprintf(dumchar_devices[i].actname, "/dev/mmcblk0");
#ifdef CONFIG_MTK_NEW_COMBO_EMMC_SUPPORT
				dumchar_devices[i].start_address = PartInfo[i].start_address;
#else
				dumchar_devices[i].start_address =
