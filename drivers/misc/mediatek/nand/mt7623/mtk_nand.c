...
//#define SLC_SUPPORT (1) // for SLC ,added by bayi
//#endif
...
bool get_device_info(u8*id, flashdev_info *devinfo)
{
	...
	{
		use_default = 1;
		flash_number = sizeof(gen_FlashTable)/sizeof(flashdev_info);
		MSG(INIT, "flash_number: 0x%x, use default table \n",flash_number);
	}
    for (i = 0; i<flash_number; i++){
		if (use_default==1)
			memcpy(&gen_FlashTable_p[i], &gen_FlashTable[i], sizeof(flashdev_info));
...
{  
    ...
    while ((DRV_Reg32(ECC_DECFSM_REG32)&(0x7f0f0f0f)) != ECC_DECFSM_IDLE)
    {
        do_gettimeofday(&timer_cur);
 		if (timeval_compare(&timer_cur, &timer_timeout) >= 0)
	      {
	        MSG(INIT, "ECC_DECFSM_REG32: timeout 0x%x %d\n",DRV_Reg(ECC_DECDONE_REG16),u4SecNum);
					dump_nfi();
        	return false;
	      }        
	  }
...
static void mtk_nand_write_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, const uint8_t * buf, int oobrequired
		)
{
...
static void mtk_nand_gpio_init(void)
{
	...
	u8 is33v;
	 ...
	 NFI_GPIO_SET_FIELD(GPIO_BASE+0xcc0 ,   0x700, 0x2);	  //pullup with 50Kohm	 ----PAD_MSDC0_CLK for 1.8v/3.3v
	 NFI_GPIO_SET_FIELD(GPIO_BASE+0xcd0 ,   0x700, 0x3);   //pulldown with 50Kohm ----PAD_MSDC0_CMD for 1.8v/3.3v
	...
	NFI_GPIO_SET_FIELD(GPIO_BASE+0xe20 , 0xf,   0x0a); 	/* TDSEL change value to 0x0a*/
    NFI_GPIO_SET_FIELD(GPIO_BASE+0xe20 , 0x3f0, val); 	/* RDSEL change value to 0x0c*/

    NFI_GPIO_SET_FIELD(GPIO_BASE+0xe30 , 0xf,   0x0a);	/* TDSEL change value to 0x0a*/
    NFI_GPIO_SET_FIELD(GPIO_BASE+0xe30 , 0x3f0, val);	/* RDSEL change value to 0x0c*/

   	NFI_GPIO_SET_FIELD(GPIO_BASE+0xd20 , 0xf,   0x0a);	/* TDSEL change value to 0x0a*/
    NFI_GPIO_SET_FIELD(GPIO_BASE+0xd20 , 0x3f0,val);	    /* RDSEL change value to val*/  

	 if (is33v)
	 	 NFI_GPIO_SET_FIELD(GPIO_BASE+0xeb0 , 0x000f,  0x5);

	 NFI_GPIO_SET_FIELD(GPIO_BASE+0xcc0 , 0x7, 0x3);
  	 NFI_GPIO_SET_FIELD(GPIO_BASE+0xcd0 , 0x7, 0x3);
  	 NFI_GPIO_SET_FIELD(GPIO_BASE+0xce0 , 0x7, 0x3);
 	 NFI_GPIO_SET_FIELD(GPIO_BASE+0xf70 , 0x7000, 0x3);	//set driving more than 4mA
     NFI_GPIO_SET_FIELD(GPIO_BASE+0xf80 , 0x7, 0x3);	//set driving more than 4mA
...
int mtk_nand_unit_test(struct nand_chip *nand_chip, struct mtd_info *mtd)
{
    ...
    for (j = 0x400; j< 0x7A0; j++)
    {
        ...
        for (k = 0; k < p; k++)
        {
            //Qwert : mtk_nand_write_page api parameters mismatch
...
//          host->saved_para.sECC_FDMADDR_REG32 = DRV_Reg32(ECC_FDMADDR_REG32);
      ...
      {
          MSG(INIT, "[NFI] Suspend twice !\n");
      }
      #endif

      MSG(INIT, "[NFI] Suspend !\n");
      return 0;
}
