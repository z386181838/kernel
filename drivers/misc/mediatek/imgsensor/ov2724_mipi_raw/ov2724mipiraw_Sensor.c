...
static void OV2724MIPI_Write_Shutter(kal_uint16 iShutter)
{
	...
	SENSORDB("[OV2724MIPI]exit OV2724MIPI_Write_Shutter function 2722MIPI_frame_length=%x\n",
		 frame_height);
...
static void OV2724MIPI_Set_Dummy(const kal_uint16 iPixels, const kal_uint16 iLines)
{
	...
	SENSORDB("[OV2724MIPI]enter OV2724MIPI_Set_Dummy function iPixels:%x; iLines:%x\n",
		 iPixels, iLines);
	...
	if ((line_length >= 0xFFFF) || (frame_height >= 0xFFFF)) {
		pr_warning("Warnning: line length or frame height is overflow!!!!!!!! \n");
		return;
	}
	...
	SENSORDB("[OV2724MIPI_Set_Dummy]line_length:%x; frame_height:%x\n", line_length,
		 frame_height);
	...
	OV2724MIPI_write_cmos_sensor(0x0341, frame_height & 0xFF);
#endif
	SENSORDB("[OV2724MIPI]exit OV2724MIPI_Set_Dummy function\n");
...
static void OV2724MIPI_init(void)
{
	...
	OV2724MIPI_write_cmos_sensor(0x0101, 0x01);
	...
	OV2724MIPI_write_cmos_sensor(0x0100, 0x01);
...
kal_uint16 OV2724MIPIGetSensorID(UINT32 *sensorID)
{
	...
	const kal_uint8 i2c_addr[] = { OV2724MIPI_WRITE_ID_1, OV2724MIPI_WRITE_ID_2 };
	kal_uint16 i;
			 ...
			 MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	SENSORDB("[OV2724MIPI]enter OV2724MIPIPreview function\n");
	...
	SENSORDB("[OV2724MIPI]exit OV2724MIPIPreview function\n");
	return ERROR_NONE;
}				/*  OV2724MIPIPreview   */
...
UINT32 OV2724MIPIGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
	...
	pr_debug
	    ("[OV2724MIPI]exit OV2724MIPIGetResolution function SensorPreviewWidth=%d,SensorFullWidth=%d\n",
