...
#define HI704_TEST_PATTERN_CHECKSUM 0x369f144c
UINT32 HI704SetTestPatternMode(kal_bool bEnable)
{
	...
	if (bEnable) {
		...
		HI704_write_cmos_sensor(0x50, 0x01);	/* Sleep ON */
	} else {
		HI704_write_cmos_sensor(0x03, 0x00);	/* Page 0 */
...
{
	pr_debug("[Enter]:HI704 getInfo func:ScenarioId = %d\n", ScenarioId);
...
void HI704_AE_Set_Window(UINT32 zone_addr, UINT32 prevW, UINT32 prevH)
{
	...
	*/
	...
	SENSORDB("[%s] Remapping : (%d,%d)~(%d,%d)\n", __func__, (prevW-x1), (prevH-y1), (prevW-x0), (prevH-y0));
...
UINT32 HI704GetExposureTime(void)
{
	...
	ExposureTimems = (1000 * internal_exp_time) / (exp_time * freq);	/* Unit : ms */
	SENSORDB("[HI704GetExposureTime] ExposureTimems : %d ms\n", ExposureTimems);

	return ExposureTimems;
}

UINT32 HI704GetISOValues(void)
{
	UINT32 iso_val = 0;
	UINT32 AE_ISO_Values = AE_ISO_100;
	UINT32 map_table[4] = {0x28, 0x58, 0x88, 0xB8};
	HI704_write_cmos_sensor(0x03, 0x20);
	iso_val = HI704_read_cmos_sensor(0xB0);	/* read gain value */

	/* Mapp gain to AE ISO values, which is provided by Hynix */
	if (iso_val <= map_table[0])
		AE_ISO_Values = AE_ISO_100;
	else if ((iso_val > map_table[0]) && (iso_val <= map_table[1]))
		AE_ISO_Values = AE_ISO_200;
	else if ((iso_val > map_table[1]) && (iso_val <= map_table[2]))
		AE_ISO_Values = AE_ISO_400;
	else if ((iso_val > map_table[2]) && (iso_val <= map_table[3]))
		AE_ISO_Values = AE_ISO_800;
	else if (iso_val > map_table[3])
		AE_ISO_Values = AE_ISO_1600;
	else
		AE_ISO_Values = AE_ISO_AUTO;

	SENSORDB("[HI704GetISOValues] Read Gain: 0x%x, Map AE_ISO_Values : %d\n", iso_val, AE_ISO_Values);

	return AE_ISO_Values;
}
			   ...
			   UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
	...
	case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
		HI704SetMaxFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
					       *(pFeatureData32 + 1));
		break;
	case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
		HI704GetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
	...
	case SENSOR_FEATURE_SET_AE_WINDOW:
		...
		HI704_AE_Set_Window(*pFeatureReturnPara32, HI704_IMAGE_SENSOR_PV_WIDTH, HI704_IMAGE_SENSOR_PV_HEIGHT);
		break;
