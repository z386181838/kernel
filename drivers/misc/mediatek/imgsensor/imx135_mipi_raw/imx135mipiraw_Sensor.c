...
static kal_uint16 IMX135MIPI_IHDR_EN;
static kal_uint16 IMX135MIPI_CurretFPS;
...
SENSOR_WINSIZE_INFO_STRUCT SENSOR_WINSIZE_INFO[5] = { {4208, 3120, 0000, 0000, 4208, 3120, 2104, 1560, 0000, 0000, 2104, 1560, 0003, 0003, 2096, 1552},	/* Preview */
...
void IMX135MIPI_write_shutter(kal_uint16 shutter)
{
	...
	if (IMX135MIPI_sensor.pv_mode == KAL_TRUE) {
		max_exp_shutter =
		    IMX135MIPI_PV_FRAME_LENGTH_LINES + IMX135MIPI_sensor.pv_dummy_lines - 4;
	} else if (IMX135MIPI_sensor.video_mode == KAL_TRUE) {
		max_exp_shutter =
		    IMX135MIPI_VIDEO_FRAME_LENGTH_LINES + IMX135MIPI_sensor.video_dummy_lines - 4;
	} else if (IMX135MIPI_sensor.highspeedvideo_mode == KAL_TRUE) {
		max_exp_shutter =
		    IMX135MIPI_HSVIDEO_FRAME_LENGTH_LINES + IMX135MIPI_sensor.hsvideo_dummy_lines -
		    4;
	} else if (IMX135MIPI_sensor.slimvideo_mode == KAL_TRUE) {
		max_exp_shutter =
		    IMX135MIPI_SLIMVIDEO_FRAME_LENGTH_LINES +
		    IMX135MIPI_sensor.slimvideo_dummy_lines - 4;
	} else {
		max_exp_shutter =
		    IMX135MIPI_FULL_FRAME_LENGTH_LINES + IMX135MIPI_sensor.cp_dummy_lines - 4;
	}
	...
	if (IMX135MIPI_sensor.pv_mode == KAL_TRUE) {
		frame_length =
		    IMX135MIPI_PV_FRAME_LENGTH_LINES + IMX135MIPI_sensor.pv_dummy_lines +
		    extra_lines;
		...
		frame_length =
		    IMX135MIPI_VIDEO_FRAME_LENGTH_LINES + IMX135MIPI_sensor.video_dummy_lines +
		    extra_lines;
		line_length =
		    IMX135MIPI_VIDEO_LINE_LENGTH_PIXELS + IMX135MIPI_sensor.video_dummy_pixels;
		spin_lock_irqsave(&imx135_drv_lock, flags);
		...
		frame_length =
		    ...
		    extra_lines;
		line_length =
		    IMX135MIPI_HSVIDEO_LINE_LENGTH_PIXELS + IMX135MIPI_sensor.hsvideo_dummy_pixels;
		spin_lock_irqsave(&imx135_drv_lock, flags);
		...
		frame_length =
		    IMX135MIPI_SLIMVIDEO_FRAME_LENGTH_LINES +
		    IMX135MIPI_sensor.slimvideo_dummy_lines + extra_lines;
		line_length =
		    IMX135MIPI_SLIMVIDEO_LINE_LENGTH_PIXELS +
		    IMX135MIPI_sensor.slimvideo_dummy_pixels;
		spin_lock_irqsave(&imx135_drv_lock, flags);
		...
		frame_length =
		    IMX135MIPI_FULL_FRAME_LENGTH_LINES + IMX135MIPI_sensor.cp_dummy_lines +
		    extra_lines;
		line_length =
		    IMX135MIPI_FULL_LINE_LENGTH_PIXELS + IMX135MIPI_sensor.cp_dummy_pixels;
		spin_lock_irqsave(&imx135_drv_lock, flags);
...
static kal_uint8 IMX135MIPIGain2Reg(const kal_uint16 iGain)
{
	...
	if (iGain != IMX135MIPI_sensorGainMapping[iI][0]) {
		SENSORDB("[IMX135MIPIGain2Reg] Gain mapping don't correctly:%d %d\n", iGain,
			 IMX135MIPI_sensorGainMapping[iI][0]);
	}
...
void IMX135MIPI_camera_para_to_sensor(void)
{
	...
	for (i = 0; 0xFFFFFFFF != IMX135MIPISensorReg[i].Addr; i++) {
		IMX135MIPI_write_cmos_sensor(IMX135MIPISensorReg[i].Addr,
					     IMX135MIPISensorReg[i].Para);
	}
	...
	for (i = FACTORY_START_ADDR; i < FACTORY_END_ADDR; i++) {
		IMX135MIPI_write_cmos_sensor(IMX135MIPISensorCCT[i].Addr,
					     IMX135MIPISensorCCT[i].Para);
	}
...
void IMX135MIPI_get_sensor_group_info(kal_uint16 group_idx, kal_int8 *group_name_ptr,
...
void IMX135MIPI_get_sensor_item_info(kal_uint16 group_idx, kal_uint16 item_idx,
...
kal_bool IMX135MIPI_set_sensor_item_info(kal_uint16 group_idx, kal_uint16 item_idx,
					 kal_int32 ItemValue)
{
/* kal_int16 temp_reg; */
...
static void IMX135MIPI_SetDummy(const kal_uint16 iPixels, const kal_uint16 iLines)
{
	...
	if (IMX135MIPI_sensor.pv_mode == KAL_TRUE) {
		...
		IMX135MIPI_sensor.hsvideo_line_length =
		    IMX135MIPI_HSVIDEO_LINE_LENGTH_PIXELS + iPixels;
		IMX135MIPI_sensor.hsvideo_frame_length =
		    IMX135MIPI_HSVIDEO_FRAME_LENGTH_LINES + iLines;
		spin_unlock(&imx135_drv_lock);
		...
		IMX135MIPI_sensor.slimvideo_line_length =
		    IMX135MIPI_SLIMVIDEO_LINE_LENGTH_PIXELS + iPixels;
		IMX135MIPI_sensor.slimvideo_frame_length =
		    IMX135MIPI_SLIMVIDEO_FRAME_LENGTH_LINES + iLines;
		spin_unlock(&imx135_drv_lock);
...
UINT32 IMX135MIPIOpen(void)
{
	...
	do {
		...
		sensorid =
		    (kal_uint16) ((IMX135MIPI_read_cmos_sensor(0x0016) << 8) |
				  IMX135MIPI_read_cmos_sensor(0x0017));
/* Debug */
...
{
	return (UINT16) ((IMX135MIPI_read_cmos_sensor(0x0202) << 8) |
			 IMX135MIPI_read_cmos_sensor(0x0203));
}
...
void IMX135MIPI_NightMode(kal_bool bEnable)
{
	...
	if (bEnable) {
		if (OV5642_MPEG4_encode_mode == KAL_TRUE) {
			OV5642_MAX_EXPOSURE_LINES =
			    (kal_uint16) ((OV5642_sensor_pclk / 15) /
					  (OV5642_PV_PERIOD_PIXEL_NUMS + OV5642_PV_dummy_pixels));
			...
			OV5642_MAX_EXPOSURE_LINES =
			    OV5642_CURRENT_FRAME_LINES - OV5642_SHUTTER_LINES_GAP;
		}
	} else { 		/* Fix video framerate 30 fps */
		if (OV5642_MPEG4_encode_mode == KAL_TRUE) {
			OV5642_MAX_EXPOSURE_LINES =
			    (kal_uint16) ((OV5642_sensor_pclk / 30) /
					  (OV5642_PV_PERIOD_PIXEL_NUMS + OV5642_PV_dummy_pixels));
			if (OV5642_pv_exposure_lines < (OV5642_MAX_EXPOSURE_LINES - OV5642_SHUTTER_LINES_GAP)) {	/* for avoid the shutter > frame_lines,move the frame lines setting to shutter function */
				OV5642_write_cmos_sensor(0x350C,
							 (OV5642_MAX_EXPOSURE_LINES >> 8) & 0xFF);
				OV5642_write_cmos_sensor(0x350D, OV5642_MAX_EXPOSURE_LINES & 0xFF);
				OV5642_CURRENT_FRAME_LINES = OV5642_MAX_EXPOSURE_LINES;
			}
			OV5642_MAX_EXPOSURE_LINES =
			    OV5642_MAX_EXPOSURE_LINES - OV5642_SHUTTER_LINES_GAP;
		}
	}
#endif
			 ...
			 MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (sensor_config_data->SensorOperationMode == MSDK_SENSOR_OPERATION_MODE_VIDEO) {	/* MPEG4 Encode Mode */
		...
		IMX135MIPI_sensor.video_line_length =
		    IMX135MIPI_VIDEO_LINE_LENGTH_PIXELS + IMX135MIPI_sensor.video_dummy_pixels;
		IMX135MIPI_sensor.video_frame_length =
		    IMX135MIPI_VIDEO_FRAME_LENGTH_LINES + IMX135MIPI_sensor.video_dummy_lines;
		spin_unlock(&imx135_drv_lock);

		IMX135MIPI_SetDummy(IMX135MIPI_sensor.video_dummy_pixels,
				    IMX135MIPI_sensor.video_dummy_lines);
		/* IMX135MIPI_SetShutter(IMX135MIPI_sensor.video_shutter); */
		memcpy(&IMX135MIPISensorConfigData, sensor_config_data,
		...
		IMX135MIPI_sensor.slimvideo_dummy_pixels = 0;
		IMX135MIPI_sensor.slimvideo_dummy_lines = 0;
		IMX135MIPI_sensor.pv_line_length =
		    IMX135MIPI_PV_LINE_LENGTH_PIXELS + IMX135MIPI_sensor.pv_dummy_pixels;
		IMX135MIPI_sensor.pv_frame_length =
		    IMX135MIPI_PV_FRAME_LENGTH_LINES + IMX135MIPI_sensor.pv_dummy_lines;
		spin_unlock(&imx135_drv_lock);

		IMX135MIPI_SetDummy(IMX135MIPI_sensor.pv_dummy_pixels,
				    IMX135MIPI_sensor.pv_dummy_lines);
		...
		image_window->ExposureWindowHeight =
		    IMX135MIPI_IMAGE_SENSOR_PV_HEIGHT - 2 * iStartY;
			 ...
			 MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_lock(&imx135_drv_lock);
	IMX135MIPI_sensor.cp_line_length =
	    IMX135MIPI_FULL_LINE_LENGTH_PIXELS + IMX135MIPI_sensor.cp_dummy_pixels;
	IMX135MIPI_sensor.cp_frame_length =
	    IMX135MIPI_FULL_FRAME_LENGTH_LINES + IMX135MIPI_sensor.cp_dummy_lines;
	spin_unlock(&imx135_drv_lock);
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	IMX135MIPI_SetDummy(IMX135MIPI_sensor.hsvideo_dummy_pixels,
			    IMX135MIPI_sensor.hsvideo_dummy_lines);
	/* IMX135MIPI_SetShutter(IMX135MIPI_sensor.video_shutter); */
	memcpy(&IMX135MIPISensorConfigData, sensor_config_data, sizeof(MSDK_SENSOR_CONFIG_STRUCT));
	image_window->GrabStartX = iStartX;
	image_window->GrabStartY = iStartY;
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	IMX135MIPI_sensor.slimvideo_dummy_lines = 0;
	IMX135MIPI_sensor.slimvideo_line_length =
	    IMX135MIPI_SLIMVIDEO_LINE_LENGTH_PIXELS + IMX135MIPI_sensor.slimvideo_dummy_pixels;
	IMX135MIPI_sensor.slimvideo_frame_length =
	    IMX135MIPI_SLIMVIDEO_FRAME_LENGTH_LINES + IMX135MIPI_sensor.slimvideo_dummy_lines;
	spin_unlock(&imx135_drv_lock);

	IMX135MIPI_SetDummy(IMX135MIPI_sensor.slimvideo_dummy_pixels,
			    IMX135MIPI_sensor.slimvideo_dummy_lines);
	/* IMX135MIPI_SetShutter(IMX135MIPI_sensor.video_shutter); */
	memcpy(&IMX135MIPISensorConfigData, sensor_config_data, sizeof(MSDK_SENSOR_CONFIG_STRUCT));
	image_window->GrabStartX = iStartX;
	image_window->GrabStartY = iStartY;
...
UINT32 IMX135MIPIControl(MSDK_SCENARIO_ID_ENUM ScenarioId,
...
UINT32 IMX135MIPISetVideoMode(UINT16 u2FrameRate)
{
	...
	if (IMX135MIPI_sensor.highspeedvideo_mode == KAL_TRUE) {
		IMX135MIPI_Video_Max_Expourse_Time =
		    (kal_uint16) ((IMX135MIPI_sensor.highspeedvideo_pclk / u32FrameRate) /
				  IMX135MIPI_sensor.hsvideo_line_length);

		if (IMX135MIPI_Video_Max_Expourse_Time >
		    IMX135MIPI_HSVIDEO_FRAME_LENGTH_LINES /*IMX135MIPI_sensor.pv_frame_length */) {
			spin_lock(&imx135_drv_lock);
			IMX135MIPI_sensor.hsvideo_frame_length = IMX135MIPI_Video_Max_Expourse_Time;
			IMX135MIPI_sensor.hsvideo_dummy_lines =
			    IMX135MIPI_sensor.hsvideo_frame_length -
			    IMX135MIPI_HSVIDEO_FRAME_LENGTH_LINES;
			spin_unlock(&imx135_drv_lock);
			SENSORDB("[IMX135MIPI]%s():HS frame_length=%d,dummy_lines=%d\n",
				 __func__, IMX135MIPI_sensor.hsvideo_frame_length,
				 IMX135MIPI_sensor.hsvideo_dummy_lines);
			IMX135MIPI_SetDummy(IMX135MIPI_sensor.hsvideo_dummy_pixels,
					    IMX135MIPI_sensor.hsvideo_dummy_lines);
		}
		...
		IMX135MIPI_Video_Max_Expourse_Time =
		    (kal_uint16) ((IMX135MIPI_sensor.video_pclk / u32FrameRate) /
				  IMX135MIPI_sensor.video_line_length);

		if (IMX135MIPI_Video_Max_Expourse_Time >
		    IMX135MIPI_VIDEO_FRAME_LENGTH_LINES /*IMX135MIPI_sensor.pv_frame_length */) {
			spin_lock(&imx135_drv_lock);
	...
	if (bEnable) {		/* enable auto flicker */
		...
		if (IMX135MIPI_MPEG4_encode_mode == KAL_TRUE) {	/* in the video mode, reset the frame rate */
			pv_max_frame_rate_lines =
			    IMX135MIPI_MAX_EXPOSURE_LINES + (IMX135MIPI_MAX_EXPOSURE_LINES >> 7);
			IMX135MIPI_write_cmos_sensor(0x0104, 1);
		...
		if (IMX135MIPI_MPEG4_encode_mode == KAL_TRUE) {	/* in the video mode, restore the frame rate */
			IMX135MIPI_write_cmos_sensor(0x0104, 1);
			IMX135MIPI_write_cmos_sensor(0x0340,
						     (IMX135MIPI_MAX_EXPOSURE_LINES >> 8) & 0xFF);
...
UINT32 IMX135MIPIGetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId,
					       MUINT32 *pframeRate)
{
				...
				UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
	...
	case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
		if (*pFeatureParaLen >= sizeof(NVRAM_SENSOR_DATA_STRUCT)) {
			...
			memcpy(pSensorDefaultData->SensorEngReg, IMX135MIPISensorReg,
			       sizeof(SENSOR_REG_STRUCT) * ENGINEER_END);
			memcpy(pSensorDefaultData->SensorCCTReg, IMX135MIPISensorCCT,
	...
	case SENSOR_FEATURE_GET_CONFIG_PARA:
		memcpy(pSensorConfigData, &IMX135MIPISensorConfigData,
		       sizeof(MSDK_SENSOR_CONFIG_STRUCT));
	...
	case SENSOR_FEATURE_GET_GROUP_INFO:
		IMX135MIPI_get_sensor_group_info(pSensorGroupInfo->GroupIdx,
						 pSensorGroupInfo->GroupNamePtr,
						 &pSensorGroupInfo->ItemCount);
		*pFeatureParaLen = sizeof(MSDK_SENSOR_GROUP_INFO_STRUCT);
		break;
	case SENSOR_FEATURE_GET_ITEM_INFO:
		IMX135MIPI_get_sensor_item_info(pSensorItemInfo->GroupIdx, pSensorItemInfo->ItemIdx,
						pSensorItemInfo);
		*pFeatureParaLen = sizeof(MSDK_SENSOR_ITEM_INFO_STRUCT);
		break;

	case SENSOR_FEATURE_SET_ITEM_INFO:
		IMX135MIPI_set_sensor_item_info(pSensorItemInfo->GroupIdx, pSensorItemInfo->ItemIdx,
	...
	case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
		IMX135MIPISetMaxFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
						    *(pFeatureData32 + 1));
		break;
	case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
		IMX135MIPIGetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		...
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
			memcpy((void *)pwininfo, (void *)&SENSOR_WINSIZE_INFO[1],
			       sizeof(SENSOR_WINSIZE_INFO_STRUCT));
			break;
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			memcpy((void *)pwininfo, (void *)&SENSOR_WINSIZE_INFO[2],
			       sizeof(SENSOR_WINSIZE_INFO_STRUCT));
			break;
		case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
			memcpy((void *)pwininfo, (void *)&SENSOR_WINSIZE_INFO[3],
			       sizeof(SENSOR_WINSIZE_INFO_STRUCT));
			break;
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			memcpy((void *)pwininfo, (void *)&SENSOR_WINSIZE_INFO[4],
			       sizeof(SENSOR_WINSIZE_INFO_STRUCT));
			break;
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		default:
			memcpy((void *)pwininfo, (void *)&SENSOR_WINSIZE_INFO[0],
			       sizeof(SENSOR_WINSIZE_INFO_STRUCT));
			break;
		}
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		SENSORDB("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n", *pFeatureData32,
			 *(pFeatureData32 + 1), *(pFeatureData32 + 2));
		IMX135MIPI_IHDR_write_shutter_gain(*pFeatureData32, *(pFeatureData32 + 1),
						   *(pFeatureData32 + 2));
