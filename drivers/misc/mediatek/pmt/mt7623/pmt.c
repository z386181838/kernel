...
static const struct file_operations pmt_proc_fops = {
	.owner = THIS_MODULE,
...
static int __init pmt_init(void)
{
    ...
    pmt_upgrade_proc = proc_create("sd_upgrade", 0600, NULL, &pmt_proc_fops);
    if (pmt_upgrade_proc) {
        pmt_info("[%s]success to register /proc/sd_upgrade\n", __func__);
    } else {
