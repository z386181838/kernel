...
static int simple_sd_ioctl_single_rw(struct msdc_ioctl* msdc_ctl)
{
    ...
    MSDC_ASSERT(host_ctl);
    MSDC_ASSERT(host_ctl->mmc);
    MSDC_ASSERT(host_ctl->mmc->card);

    mmc_claim_host(host_ctl->mmc);

#if DEBUG_MMC_IOCTL
    MSDC_TRC_PRINT(MSDC_MISCDRV,"user want access %d partition\n",msdc_ctl->partition);
#endif
...
#if DEBUG_MMC_IOCTL
    MSDC_TRC_PRINT(MSDC_MISCDRV,"start MSDC_SINGLE_READ_WRITE !!\n");
#endif    
    ...
    if (!mmc_card_blockaddr(host_ctl->mmc->card)){
        MSDC_TRC_PRINT(MSDC_MISCDRV,"the device is used byte address!\n");
        msdc_cmd.arg <<= 9;
    }
...
#if DEBUG_MMC_IOCTL
    MSDC_TRC_PRINT(MSDC_MISCDRV,"single block: ueser buf address is 0x%p!\n",msdc_ctl->buffer);
#endif    
...
static int simple_sd_ioctl_multi_rw(struct msdc_ioctl* msdc_ctl)
{
    ...
    if (!mmc_card_blockaddr(host_ctl->mmc->card)){
        MSDC_TRC_PRINT(MSDC_MISCDRV,"this device use byte address!!\n");
        msdc_cmd.arg <<= 9;
    }
    msdc_cmd.flags = MMC_RSP_SPI_R1 | MMC_RSP_R1 | MMC_CMD_ADTC;
...
#if DEBUG_MMC_IOCTL
    MSDC_TRC_PRINT(MSDC_MISCDRV,"total size is %d\n",msdc_ctl->total_size);
#endif
    sg_init_one(&msdc_sg, sg_msdc_multi_buffer, msdc_ctl->total_size);
    mmc_set_data_timeout(&msdc_data, host_ctl->mmc->card);
    mmc_wait_for_req(host_ctl->mmc, &msdc_mrq);
...
#if DEBUG_MMC_IOCTL
    MSDC_TRC_PRINT(MSDC_MISCDRV,"user want the cid in msdc slot%d \n",msdc_ctl->host_num);
#endif
...
#if DEBUG_MMC_IOCTL
    MSDC_TRC_PRINT(MSDC_MISCDRV,"cid:0x%x,0x%x,0x%x,0x%x\n",host_ctl->mmc->card->raw_cid[0],
...
#if DEBUG_MMC_IOCTL
    MSDC_TRC_PRINT(MSDC_MISCDRV,"user want the csd in msdc slot%d \n",msdc_ctl->host_num);
#endif
...
#if DEBUG_MMC_IOCTL
    MSDC_TRC_PRINT(MSDC_MISCDRV,"csd:0x%x,0x%x,0x%x,0x%x\n",host_ctl->mmc->card->raw_csd[0],
...
static int simple_sd_ioctl_get_excsd(struct msdc_ioctl* msdc_ctl)
{
...
#if DEBUG_MMC_IOCTL
    MSDC_TRC_PRINT(MSDC_MISCDRV,"user want the extend csd in msdc slot%d \n",msdc_ctl->host_num);
#endif
    mmc_send_ext_csd(host_ctl->mmc->card,l_buf);
    ...
    for (i = 0; i < 512; i++)
    {
        MSDC_TRC_PRINT(MSDC_MISCDRV,"%x", l_buf[i]);
        if (0 == ((i + 1) % 16)){
            MSDC_TRC_PRINT(MSDC_MISCDRV,"\n");
        }
    }
#endif
...
static int simple_sd_ioctl_set_driving(struct msdc_ioctl* msdc_ctl)
{
    ...
    MSDC_ASSERT(msdc_ctl->host_num < HOST_MAX_NUM);	
    if (msdc_ctl->host_num >= HOST_MAX_NUM) {
        MSDC_TRC_PRINT(MSDC_MISCDRV,"host%d is not config\n", msdc_ctl->host_num);
        return -1;
    }	
   ...
   enable_clock(MT_CG_PERI_MSDC30_0 + host->id, "SD"); 
#endif
#if DEBUG_MMC_IOCTL
    //MSDC_TRC_PRINT(MSDC_MISCDRV,"set: clk pull down driving is 0x%x\n", msdc_ctl->clk_pd_driving);
    MSDC_TRC_PRINT(MSDC_MISCDRV,"set: clk driving is 0x%x\n", msdc_ctl->clk_pu_driving);
    //MSDC_TRC_PRINT(MSDC_MISCDRV,"set: cmd pull down driving is 0x%x\n", msdc_ctl->cmd_pd_driving);
    MSDC_TRC_PRINT(MSDC_MISCDRV,"set: cmd driving is 0x%x\n", msdc_ctl->cmd_pu_driving);
    //MSDC_TRC_PRINT(MSDC_MISCDRV,"set: dat pull down driving is 0x%x\n", msdc_ctl->dat_pd_driving);
    MSDC_TRC_PRINT(MSDC_MISCDRV,"set: dat driving is 0x%x\n", msdc_ctl->dat_pu_driving);
#endif
#ifndef FPGA_PLATFORM
    ...
    //MSDC_TRC_PRINT(MSDC_MISCDRV,"read: clk pull down driving is 0x%x\n", l_value);
    l_value = (msdc_dump_padctl0(host) & 0x7);
    //sdr_get_field(MSDC_PAD_CTL0, MSDC_PAD_CTL0_CLKDRVP, l_value);
    MSDC_TRC_PRINT(MSDC_MISCDRV,"read: clk driving is 0x%x\n", l_value);
    l_value = (msdc_dump_padctl1(host) & 0x7);;
    //sdr_get_field(MSDC_PAD_CTL1, MSDC_PAD_CTL1_CMDDRVN, l_value);
    MSDC_TRC_PRINT(MSDC_MISCDRV,"read: cmd driving is 0x%x\n", l_value);
    //l_value = 0;
    //sdr_get_field(MSDC_PAD_CTL1, MSDC_PAD_CTL1_CMDDRVP, l_value);
    //MSDC_TRC_PRINT(MSDC_MISCDRV,"read: cmd pull up driving is 0x%x\n", l_value);
    l_value = (msdc_dump_padctl2(host) & 0x7);;
    //sdr_get_field(MSDC_PAD_CTL2, MSDC_PAD_CTL2_DATDRVN, l_value);
    MSDC_TRC_PRINT(MSDC_MISCDRV,"read: dat driving is 0x%x\n", l_value);
    //l_value = 0;
    //sdr_get_field(MSDC_PAD_CTL2, MSDC_PAD_CTL2_DATDRVP, l_value);
    //MSDC_TRC_PRINT(MSDC_MISCDRV,"read: dat pull up driving is 0x%x\n", l_value);
#endif
...
//    unsigned int l_value;
...
#endif
    //sdr_get_field(MSDC_PAD_CTL0, MSDC_PAD_CTL0_CLKDRVN, l_value);
    msdc_ctl->clk_pu_driving = (msdc_dump_padctl0(host) & 0x7);
    ...
    //sdr_get_field(MSDC_PAD_CTL1, MSDC_PAD_CTL1_CMDDRVP, l_value);
    msdc_ctl->cmd_pu_driving = (msdc_dump_padctl1(host) & 0x7);

    //sdr_get_field(MSDC_PAD_CTL2, MSDC_PAD_CTL2_DATDRVN, l_value);
    //msdc_ctl->dat_pd_driving = l_value;

    //sdr_get_field(MSDC_PAD_CTL2, MSDC_PAD_CTL2_DATDRVP, l_value);
    msdc_ctl->dat_pu_driving = (msdc_dump_padctl2(host) & 0x7);


#if DEBUG_MMC_IOCTL
    //MSDC_TRC_PRINT(MSDC_MISCDRV,"read: clk pull down driving is 0x%x\n", msdc_ctl->clk_pd_driving);
    MSDC_TRC_PRINT(MSDC_MISCDRV,"read: clk driving is 0x%x\n", msdc_ctl->clk_pu_driving);
    //MSDC_TRC_PRINT(MSDC_MISCDRV,"read: cmd pull down driving is 0x%x\n", msdc_ctl->cmd_pd_driving);
    MSDC_TRC_PRINT(MSDC_MISCDRV,"read: cmd driving is 0x%x\n", msdc_ctl->cmd_pu_driving);
    //MSDC_TRC_PRINT(MSDC_MISCDRV,"read: dat pull down driving is 0x%x\n", msdc_ctl->dat_pd_driving);
    MSDC_TRC_PRINT(MSDC_MISCDRV,"read: dat driving is 0x%x\n", msdc_ctl->dat_pu_driving);
#endif
...
//	struct msdc_hw hw;
	...
	switch(msdc_ctl->sd30_mode){
		case SDHC_HIGHSPEED : msdc_host_mode[id] |= MMC_CAP_MMC_HIGHSPEED | MMC_CAP_SD_HIGHSPEED;
							  ...
							  MSDC_TRC_PRINT(MSDC_MISCDRV,"[****SD_Debug****]host will support Highspeed\n");
							  break;
		case UHS_SDR12		: msdc_host_mode[id] |= MMC_CAP_UHS_SDR12;
							  ...
							  msdc_host_mode2[id] &= ~MMC_CAP2_HS200_1_8V_SDR;
						  	  MSDC_TRC_PRINT(MSDC_MISCDRV,"[****SD_Debug****]host will support UHS-SDR12\n");
							  break;
		case UHS_SDR25		: msdc_host_mode[id] |= MMC_CAP_UHS_SDR12|MMC_CAP_UHS_SDR25;
							  ...
							  msdc_host_mode2[id] &= ~MMC_CAP2_HS200_1_8V_SDR;
						  	  MSDC_TRC_PRINT(MSDC_MISCDRV,"[****SD_Debug****]host will support UHS-SDR25\n");
						  	  break;
		case UHS_SDR50		: msdc_host_mode[id] |= MMC_CAP_UHS_SDR12|MMC_CAP_UHS_SDR25|MMC_CAP_UHS_SDR50;
							  ...
							  msdc_host_mode2[id] &= ~MMC_CAP2_HS200_1_8V_SDR;
						  	  MSDC_TRC_PRINT(MSDC_MISCDRV,"[****SD_Debug****]host will support UHS-SDR50\n");
						  	  break;
		case UHS_SDR104 	: msdc_host_mode[id] |= MMC_CAP_UHS_SDR12|MMC_CAP_UHS_SDR25|MMC_CAP_UHS_SDR50|MMC_CAP_UHS_DDR50|MMC_CAP_1_8V_DDR|MMC_CAP_UHS_SDR104;
                              msdc_host_mode2[id] |= MMC_CAP2_HS200_1_8V_SDR;
						 	  MSDC_TRC_PRINT(MSDC_MISCDRV,"[****SD_Debug****]host will support UHS-SDR104\n");
						  	  break;
		case UHS_DDR50		: msdc_host_mode[id] |= MMC_CAP_UHS_SDR12|MMC_CAP_UHS_SDR25|MMC_CAP_UHS_DDR50|MMC_CAP_1_8V_DDR;
							  ...
							  msdc_host_mode2[id] &= ~MMC_CAP2_HS200_1_8V_SDR;
						      MSDC_TRC_PRINT(MSDC_MISCDRV,"[****SD_Debug****]host will support UHS-DDR50\n");
						      break;
		default 			: break;
	...
	switch(msdc_ctl->sd30_max_current){
		case MAX_CURRENT_200 : msdc_host_mode[id] |= MMC_CAP_MAX_CURRENT_200;
							   ...
							   MSDC_TRC_PRINT(MSDC_MISCDRV,"[****SD_Debug****]host will support MAX_CURRENT_200\n");
							   break;
		case MAX_CURRENT_400 : msdc_host_mode[id] |= MMC_CAP_MAX_CURRENT_200 | MMC_CAP_MAX_CURRENT_400;
							   ...
							   MSDC_TRC_PRINT(MSDC_MISCDRV,"[****SD_Debug****]host will support MAX_CURRENT_400\n");
							   break;
		case MAX_CURRENT_600 : msdc_host_mode[id] |= MMC_CAP_MAX_CURRENT_200 | MMC_CAP_MAX_CURRENT_400|MMC_CAP_MAX_CURRENT_600;
							   ...
							   MSDC_TRC_PRINT(MSDC_MISCDRV,"[****SD_Debug****]host will support MAX_CURRENT_600\n");
							   break;
		case MAX_CURRENT_800 : msdc_host_mode[id] |= MMC_CAP_MAX_CURRENT_200 | MMC_CAP_MAX_CURRENT_400|MMC_CAP_MAX_CURRENT_600|MMC_CAP_MAX_CURRENT_800;
							   MSDC_TRC_PRINT(MSDC_MISCDRV,"[****SD_Debug****]host will support MAX_CURRENT_800\n");
							   break;
		default 			 : break;
	}
	switch(msdc_ctl->sd30_power_control){
		case SDXC_NO_POWER_CONTROL : msdc_host_mode[id] &= (~MMC_CAP_SET_XPC_330) & (~MMC_CAP_SET_XPC_300) & (~MMC_CAP_SET_XPC_180);
									 MSDC_TRC_PRINT(MSDC_MISCDRV,"[****SD_Debug****]host will not support SDXC power control\n");
							         break;
		case SDXC_POWER_CONTROL    : msdc_host_mode[id] |= MMC_CAP_SET_XPC_330 | MMC_CAP_SET_XPC_300 | MMC_CAP_SET_XPC_180;
									 MSDC_TRC_PRINT(MSDC_MISCDRV,"[****SD_Debug****]host will support SDXC power control\n");
							     	 break;
		default			           : break;
	}
#endif
...
#if DEBUG_MMC_IOCTL
                MSDC_TRC_PRINT(MSDC_MISCDRV,"part_name = %s    name = %s\n", PartInfo[i].name, name);
#endif                
...
#if DEBUG_MMC_IOCTL
    MSDC_TRC_PRINT(MSDC_MISCDRV,"erase done....\n");
#endif
end:
...
static int simple_mmc_erase_partition(unsigned char* name)
{
    ...
    if (('u' == *name && 's' == *(name + 1) && 'r' == *(name + 2) && 'd' == *(name + 3) && 
        ...
        if (l_ret == 0){
            /* do erase */
            MSDC_TRC_PRINT(MSDC_MISCDRV,"erase %s start sector: 0x%x size: 0x%x \n", mbr_part.part_name, mbr_part.start_sector, mbr_part.nr_sects);
...
static long simple_sd_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if(msdc_ctl == NULL){
		switch(cmd){
			...
			//#endif
        	default:
            MSDC_TRC_PRINT(MSDC_MISCDRV,"mt_sd_ioctl:this opcode value is illegal!!\n");
            return -EINVAL;
			}
		return ret;		
	}
    ...
    switch (msdc_ctl->opcode){
        ...
        case MSDC_DRIVING_SETTING:
            MSDC_TRC_PRINT(MSDC_MISCDRV,"in ioctl to change driving\n");
		...
		case MSDC_SD30_MODE_SWITCH:
			...
			break;
        default:
            MSDC_TRC_PRINT(MSDC_MISCDRV,"simple_sd_ioctl:this opcode value is illegal!!\n");
            return -EINVAL;
    }
...
static int simple_sd_probe(struct platform_device *pdev)
{
    ...
    MSDC_TRC_PRINT(MSDC_MISCDRV,"in misc_sd_probe function\n");
...
static int __init simple_sd_init(void)
{
    ...
    if( sg_msdc_multi_buffer ==NULL)
        MSDC_TRC_PRINT(MSDC_MISCDRV,"allock 64KB memory failed\n");
    ...
    if (ret) {
        MSDC_TRC_PRINT(MSDC_MISCDRV,"register MSDC Slot[0] misc driver failed (%d)\n",ret);
        return ret;
    }
