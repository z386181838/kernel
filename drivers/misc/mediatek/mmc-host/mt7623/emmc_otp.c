...
unsigned int emmc_get_wp_size(void)
{
    ...
    if (0 == sg_wp_size){
        ...
        MSDC_BUGON(!host_ctl);
        MSDC_BUGON(!host_ctl->mmc);
        MSDC_BUGON(!host_ctl->mmc->card);

        MSDC_TRC_PRINT(MSDC_OTPDRV,"[%s:%d]mmc_host is : 0x%p\n", __func__, __LINE__, host_ctl->mmc);
        //MSDC_TRC_PRINT(MSDC_OTPDRV,"[%s:%d]claim host done! claim_status = %d, claim_cnt = %d, claimer = 0x%x, current = 0x%x\n", __func__, __LINE__, 
...
#if EMMC_OTP_DEBUG
        {
            ...
            for (i = 0; i < 512; i++)
            {
                MSDC_TRC_PRINT(MSDC_OTPDRV,"%x", l_ext_csd[i]);
                if (0 == ((i + 1) % 16)){
                    MSDC_TRC_PRINT(MSDC_OTPDRV,"\n");
                }
            }
        }
#endif
...
unsigned int  emmc_otp_start(void)
{
    ...
    MSDC_TRC_PRINT(MSDC_OTPDRV,"find OTP start address is 0x%x \n", l_addr);
...
unsigned int emmc_otp_read(unsigned int blk_offset, void *BufferPtr)
{
...
#if EMMC_OTP_DEBUG
    MSDC_TRC_PRINT(MSDC_OTPDRV,"EMMC_OTP: start MSDC_SINGLE_READ_WRITE!\n");
#endif    
    ...
    MSDC_BUGON(!host_ctl->mmc->card);
    if (!mmc_card_blockaddr(host_ctl->mmc->card)){
        MSDC_TRC_PRINT(MSDC_OTPDRV,"the device is used byte address!\n");
        msdc_cmd.arg <<= 9;
    }
...
{
    MSDC_TRC_PRINT(MSDC_OTPDRV,"[%s]:(MAJOR)%d:(MINOR)%d\n", __func__, MAJOR(inode->i_rdev), MINOR(inode->i_rdev));
...
static int mt_otp_access(unsigned int access_type, unsigned int offset, void *buff_ptr, unsigned int length, unsigned int *status)
{
    ...
    MSDC_TRC_PRINT(MSDC_OTPDRV,"[%s]: %s (0x%x) length:(%d bytes) !\n", __func__, access_type?"WRITE":"READ", offset, length);
    ...
    do {
        ...
        if (FS_EMMC_OTP_READ == access_type){
            ...
            MSDC_TRC_PRINT(MSDC_OTPDRV,"[%s]: Read Access of page (%d)\n",__func__, blkno);

            Status = g_emmc_otp_func.read(blkno, p_D_Buff);
            *status = Status;

            if (OTP_SUCCESS != Status){
                MSDC_TRC_PRINT(MSDC_OTPDRV,"[%s]: Read status (%d)\n", __func__, Status);
                break;
            }
            ...
            *status = Status;
            ...
            if (OTP_SUCCESS != Status){
                MSDC_TRC_PRINT(MSDC_OTPDRV,"[%s]: read before write, Read status (%d) blkno (0x%x)\n", __func__, Status, blkno);
                break;
            }
            ...
            *status = Status;
            ...
            if (OTP_SUCCESS != Status){
                MSDC_TRC_PRINT(MSDC_OTPDRV,"[%s]: Write status (%d)\n",__func__, Status);
                break;
            }
        } else {
            MSDC_TRC_PRINT(MSDC_OTPDRV,"[%s]: Error, not either read nor write operations !\n",__func__);
            break;
        }
        ...
        if (length <= AccessLength){
            ...
            length -= AccessLength;
            MSDC_TRC_PRINT(MSDC_OTPDRV,"[%s]: Remaining %s (%d) !\n",__func__, access_type?"WRITE":"READ", length);
        }
...
static long mt_otp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    {
        MSDC_TRC_PRINT(MSDC_OTPDRV,"ERROR: EMMC Flash Not initialized !!\n");
        ret = -EFAULT;
        goto exit;
    }*/
    ...
    case EMMC_OTP_GET_LENGTH:
        MSDC_TRC_PRINT(MSDC_OTPDRV,"OTP IOCTL: EMMC_OTP_GET_LENGTH\n");
        ...
        MSDC_TRC_PRINT(MSDC_OTPDRV,"OTP IOCTL: The Length is %d\n", otpctl.QLength);
        break;
    case EMMC_OTP_READ:
        MSDC_TRC_PRINT(MSDC_OTPDRV,"OTP IOCTL: EMMC_OTP_READ Offset(0x%x), Length(0x%x) \n", otpctl.Offset, otpctl.Length);
        ...
        {
            MSDC_TRC_PRINT(MSDC_OTPDRV,"EMMC_OTP IOCTL: Copy to user buffer Error !\n");
            goto error;
        }
        break;
    case EMMC_OTP_WRITE:
        MSDC_TRC_PRINT(MSDC_OTPDRV,"OTP IOCTL: EMMC_OTP_WRITE Offset(0x%x), Length(0x%x) \n", otpctl.Offset, otpctl.Length);
        if (copy_from_user(pbuf, otpctl.BufferPtr, (sizeof(char)*otpctl.Length)))
        {
            MSDC_TRC_PRINT(MSDC_OTPDRV,"EMMC_OTP IOCTL: Copy from user buffer Error !\n");
            goto error;
        }
...
static int emmc_otp_probe(struct platform_device *pdev)
{
    ...
    MSDC_TRC_PRINT(MSDC_OTPDRV,"in emmc otp probe function\n");
...
static int __init emmc_otp_init(void)
{
    ...
    MSDC_TRC_PRINT(MSDC_OTPDRV,"MediaTek EMMC OTP misc driver init\n");
    ...
    if (entry == NULL) 
    {
        MSDC_TRC_PRINT(MSDC_OTPDRV,"emmc OTP: unable to create /proc entry\n");
        return -ENOMEM;
    }
    ...
    MSDC_TRC_PRINT(MSDC_OTPDRV,"OTP: register EMMC OTP device ...\n");
    err = misc_register(&emmc_otp_dev);
    if (unlikely(err))
    {
        MSDC_TRC_PRINT(MSDC_OTPDRV,"OTP: failed to register EMMC OTP device!\n");
        return err;
    }
...
{
    MSDC_TRC_PRINT(MSDC_OTPDRV,"MediaTek EMMC OTP misc driver exit\n");
