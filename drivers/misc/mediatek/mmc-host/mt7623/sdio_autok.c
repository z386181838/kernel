...
static void mt_vcore_dvfs_volt_set_by_sdio(unsigned int volt) 
{
    if(volt != 1250000){
        printk("%s:: Can't change to this volt %u, please check!\n",__func__,volt);
    }
}
#endif
...
{
    unsigned int vcore_uv_offset = 1250000;
...
#else   // PMIC MT6333
    //FIXME:   since mt6333_get_reg_value not implement zhanyong.wang
...
int msdc_autok_adjust_param(struct msdc_host *host, enum AUTOK_PARAM param, u32 *value, int rw)
{
    ...
    switch (param)
    {
        ...
        case CLK_DRV:
            ...
            reg = (u32)(MSDC_CTRL(MSDC2, 0));
            field = BIT_MSDC_CLK_E8E4E2;
            break;
        case CMD_DRV:
            if((rw == AUTOK_WRITE) && (*value > 7))
            {
            ...
            reg = (u32)(MSDC_CTRL(MSDC2, 1));
            field = BIT_MSDC_CMD_E8E4E2;
            break;
        case DAT_DRV:
            if((rw == AUTOK_WRITE) && (*value > 7))
            {
            ...
            reg = (u32)(MSDC_CTRL(MSDC2, 2));
            field = BIT_MSDC_DAT_E8E4E2;
            break;
        case DAT0_RD_DLY:
