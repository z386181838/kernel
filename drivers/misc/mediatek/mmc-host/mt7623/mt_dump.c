...
static void simp_msdc_dump_register(struct simp_msdc_host *host)
{
    ...
    printk("Reg[ 00] MSDC_CFG              = 0x%.8x\n", sdr_read32(MSDC_CFG));
    printk("Reg[ 04] MSDC_IOCON            = 0x%.8x\n", sdr_read32(MSDC_IOCON));
    printk("Reg[ 08] MSDC_PS               = 0x%.8x\n", sdr_read32(MSDC_PS));
    printk("Reg[ 0C] MSDC_INT              = 0x%.8x\n", sdr_read32(MSDC_INT));
    printk("Reg[ 10] MSDC_INTEN            = 0x%.8x\n", sdr_read32(MSDC_INTEN));    
    printk("Reg[ 14] MSDC_FIFOCS           = 0x%.8x\n", sdr_read32(MSDC_FIFOCS));
    printk("Reg[ 18] MSDC_TXDATA           = not read\n");                       
    printk("Reg[ 1C] MSDC_RXDATA           = not read\n");
    printk("Reg[ 30] SDC_CFG               = 0x%.8x\n", sdr_read32(SDC_CFG));
    printk("Reg[ 34] SDC_CMD               = 0x%.8x\n", sdr_read32(SDC_CMD));
    printk("Reg[ 38] SDC_ARG               = 0x%.8x\n", sdr_read32(SDC_ARG));
    printk("Reg[ 3C] SDC_STS               = 0x%.8x\n", sdr_read32(SDC_STS));
    printk("Reg[ 40] SDC_RESP0             = 0x%.8x\n", sdr_read32(SDC_RESP0));
    printk("Reg[ 44] SDC_RESP1             = 0x%.8x\n", sdr_read32(SDC_RESP1));
    printk("Reg[ 48] SDC_RESP2             = 0x%.8x\n", sdr_read32(SDC_RESP2));                                
    printk("Reg[ 4C] SDC_RESP3             = 0x%.8x\n", sdr_read32(SDC_RESP3));
    printk("Reg[ 50] SDC_BLK_NUM           = 0x%.8x\n", sdr_read32(SDC_BLK_NUM));
    printk("Reg[ 54] SDC_VOL_CHG           = 0x%.8x\n", sdr_read32(SDC_VOL_CHG));
    printk("Reg[ 58] SDC_CSTS              = 0x%.8x\n", sdr_read32(SDC_CSTS));
    printk("Reg[ 5C] SDC_CSTS_EN           = 0x%.8x\n", sdr_read32(SDC_CSTS_EN));
    printk("Reg[ 60] SDC_DCRC_STS          = 0x%.8x\n", sdr_read32(SDC_DCRC_STS));

	if((base == MSDC_0_BASE)||(base == MSDC_3_BASE)){
       printk("Reg[ 70] EMMC_CFG0             = 0x%.8x\n", sdr_read32(EMMC_CFG0));                        
       printk("Reg[ 74] EMMC_CFG1             = 0x%.8x\n", sdr_read32(EMMC_CFG1));
       printk("Reg[ 78] EMMC_STS              = 0x%.8x\n", sdr_read32(EMMC_STS));
       printk("Reg[ 7C] EMMC_IOCON            = 0x%.8x\n", sdr_read32(EMMC_IOCON));  
	}
	
    printk("Reg[ 80] SDC_ACMD_RESP         = 0x%.8x\n", sdr_read32(SDC_ACMD_RESP));
    printk("Reg[ 84] SDC_ACMD19_TRG        = 0x%.8x\n", sdr_read32(SDC_ACMD19_TRG));      
    printk("Reg[ 88] SDC_ACMD19_STS        = 0x%.8x\n", sdr_read32(SDC_ACMD19_STS));
    printk("Reg[ 8C]*MSDC_DMA_HIGH4BIT     = 0x%.8x\n", sdr_read32(MSDC_DMA_HIGH4BIT)); /* new register @mt7623 */
    printk("Reg[ 90] MSDC_DMA_SA           = 0x%.8x\n", sdr_read32(MSDC_DMA_SA));
    printk("Reg[ 94] MSDC_DMA_CA           = 0x%.8x\n", sdr_read32(MSDC_DMA_CA));
    printk("Reg[ 98] MSDC_DMA_CTRL         = 0x%.8x\n", sdr_read32(MSDC_DMA_CTRL));
    printk("Reg[ 9C] MSDC_DMA_CFG          = 0x%.8x\n", sdr_read32(MSDC_DMA_CFG));                        
    printk("Reg[ A0] MSDC_DBG_SEL          = 0x%.8x\n", sdr_read32(MSDC_DBG_SEL));
    printk("Reg[ A4] SW_DBG_OUT            = 0x%.8x\n", sdr_read32(MSDC_DBG_OUT));
    printk("Reg[ A8] MSDC_DMA_LEN          = 0x%.8x\n", sdr_read32(MSDC_DMA_LEN));
    printk("Reg[ B0] MSDC_PATCH_BIT0       = 0x%.8x\n", sdr_read32(MSDC_PATCH_BIT0));            
    printk("Reg[ B4] MSDC_PATCH_BIT1       = 0x%.8x\n", sdr_read32(MSDC_PATCH_BIT1));
    printk("Reg[ B8]*MSDC_PATCH_BIT2       = 0x%.8x\n", sdr_read32(MSDC_PATCH_BIT2)); /* new register @mt7623 */


	/* Only for SD/SDIO Controller 6 registers below */
    printk("Reg[ C0]*DAT0_TUNE_CRC         = 0x%.8x\n", sdr_read32(DAT0_TUNE_CRC)); /* new register @mt7623 */
    printk("Reg[ C4]*DAT1_TUNE_CRC         = 0x%.8x\n", sdr_read32(DAT1_TUNE_CRC)); /* new register @mt7623 */
    printk("Reg[ C8]*DAT2_TUNE_CRC         = 0x%.8x\n", sdr_read32(DAT2_TUNE_CRC)); /* new register @mt7623 */
    printk("Reg[ CC]*DAT3_TUNE_CRC         = 0x%.8x\n", sdr_read32(DAT3_TUNE_CRC)); /* new register @mt7623 */
    printk("Reg[ D0]*CMD_TUNE_CRC          = 0x%.8x\n", sdr_read32(CMD_TUNE_CRC)); /* new register @mt7623 */
    printk("Reg[ D4]*SDIO_TUNE_WIND        = 0x%.8x\n", sdr_read32(SDIO_TUNE_WIND)); /* new register @mt7623 */

    printk("Reg[ F0] MSDC_PAD_TUNE0	     = 0x%.8x\n", sdr_read32(MSDC_PAD_TUNE0)); 
    printk("Reg[ F4]*MSDC_PAD_TUNE1	     = 0x%.8x\n", sdr_read32(MSDC_PAD_TUNE1)); 
    printk("Reg[ F8] MSDC_DAT_RDDLY0	     = 0x%.8x\n", sdr_read32(MSDC_DAT_RDDLY0)); 
    printk("Reg[ FC] MSDC_DAT_RDDLY1	     = 0x%.8x\n", sdr_read32(MSDC_DAT_RDDLY1)); 
    printk("Reg[100]*MSDC_DAT_RDDLY2	     = 0x%.8x\n", sdr_read32(MSDC_DAT_RDDLY2)); 
    printk("Reg[104]*MSDC_DAT_RDDLY3	     = 0x%.8x\n", sdr_read32(MSDC_DAT_RDDLY3)); 
    printk("Reg[110] MSDC_HW_DBG	         = 0x%.8x\n", sdr_read32(MSDC_HW_DBG)); 
    printk("Reg[114] MSDC_VERSION	         = 0x%.8x\n", sdr_read32(MSDC_VERSION)); 
    printk("Reg[118] MSDC_ECO_VER  	     = 0x%.8x\n", sdr_read32(MSDC_ECO_VER)); 

    if(base == MSDC_3_BASE){
	    printk("Reg[180] EMMC50_PAD_CTL0	     = 0x%.8x\n", sdr_read32(EMMC50_PAD_CTL0));						  
	    printk("Reg[184] EMMC50_PAD_DS_CTL0	 = 0x%.8x\n", sdr_read32(EMMC50_PAD_DS_CTL0));						  
	    printk("Reg[188] EMMC50_PAD_DS_TUNE	 = 0x%.8x\n", sdr_read32(EMMC50_PAD_DS_TUNE));						  
	    printk("Reg[18C] EMMC50_PAD_CMD_TUNE   = 0x%.8x\n", sdr_read32(EMMC50_PAD_CMD_TUNE));						  
	    printk("Reg[190] EMMC50_PAD_DAT01_TUNE = 0x%.8x\n", sdr_read32(EMMC50_PAD_DAT01_TUNE));						  
	    printk("Reg[194] EMMC50_PAD_DAT23_TUNE = 0x%.8x\n", sdr_read32(EMMC50_PAD_DAT23_TUNE));						  
	    printk("Reg[198] EMMC50_PAD_DAT45_TUNE = 0x%.8x\n", sdr_read32(EMMC50_PAD_DAT45_TUNE));						  
	    printk("Reg[19C] EMMC50_PAD_DAT67_TUNE = 0x%.8x\n", sdr_read32(EMMC50_PAD_DAT67_TUNE));						  
	    printk("Reg[204] EMMC51_CFG0	         = 0x%.8x\n", sdr_read32(EMMC51_CFG0));						  
	    printk("Reg[208] EMMC50_CFG0	         = 0x%.8x\n", sdr_read32(EMMC50_CFG0));						  
	    printk("Reg[20C] EMMC50_CFG1	         = 0x%.8x\n", sdr_read32(EMMC50_CFG1));						  
	    printk("Reg[21C] EMMC50_CFG2	         = 0x%.8x\n", sdr_read32(EMMC50_CFG2));						  
	    printk("Reg[220] EMMC50_CFG3	         = 0x%.8x\n", sdr_read32(EMMC50_CFG3));						  
	    printk("Reg[224] EMMC50_CFG4	         = 0x%.8x\n", sdr_read32(EMMC50_CFG4));						  
	    printk("Reg[228] EMMC50_BLOCK_LENGTH   = 0x%.8x\n", sdr_read32(EMMC50_BLOCK_LENGTH));						  
    }                  
} 
...
static u32 simp_mmc_get_host(int card_type,bool boot)
{
	u32 index = 0;
	ASSERT(card_type < HOST_MAX_NUM);
