    ...
    if(pConfig->enable==1)
    {
...
#endif
		{
	        ...
	        if(pConfig->outFormat == eYUV_420_3P){
				...
				pr_err("pConfig->security=%d OvlSecure=%d",pConfig->security,OvlSecure);
				if(pConfig->security == 1)
				{
					MTEEC_PARAM param[4];
	                unsigned int paramTypes;
	                TZ_RESULT ret;  
	                param[0].value.a = pConfig->dstAddr;
	                param[1].value.a = picSz;
	                param[2].value.a = pConfig->srcROI.width;
					param[3].value.a = 1;  //isStartWDMA
					pr_err("[secured] WFD dstAddr:0x%08x picSz:0x%08x uAddr:0x%08x srcROI.width:0x%08x",param[0].value.a, param[1].value.a,param[2].value.a);
					paramTypes = TZ_ParamTypes4(TZPT_VALUE_INPUT, TZPT_VALUE_INPUT, TZPT_VALUE_INPUT, TZPT_VALUE_INPUT);
	                ret = KREE_TeeServiceCall(ddp_session_handle(), TZCMD_DDP_WDMA1_CONFIG_UV, paramTypes, param);
	                if(ret!= TZ_RESULT_SUCCESS)
	                {
	                    DISP_ERR("KREE_TeeServiceCall(TZCMD_DDP_WDMA1_CONFIG_UV) fail, ret=%d \n", ret);
	                }   
				}
				else
#endif
				{
					...
					vAddr = uAddr + (picSz>>2);		
					pr_err("[normal] WFD dstAddr:0x%08x picSz:0x%08x uAddr:0x%08x vAddr:0x%08x",pConfig->dstAddr,picSz,uAddr,vAddr);
