...
int OVLLayerSrcPitch(unsigned int layer,unsigned int src_pitch)
{
	...
	DISP_REG_SET_FIELD(L0_PITCH_FLD_L0_SRC_PITCH, DISP_REG_OVL_L0_PITCH+layer*0x20, src_pitch);
	DISP_MSG("get layer pitch 0x%x 0x%x\n",(DISP_REG_OVL_L0_PITCH+layer*0x20),DISP_REG_GET(DISP_REG_OVL_L0_PITCH+layer*0x20));
...
*/
    ...
    //--------------------------------------------------------
    address = addr+src_x*bpp+src_y*layer_pitch; // First byte of first pixel
    width = dst_w;
    start = address;
