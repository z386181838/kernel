...
extern unsigned int ddp_bitblt_ioctl_wait_reequest( unsigned long ioctl_user_param  );
extern unsigned int ddp_bitblt_ioctl_inform_done( unsigned long ioctl_user_param  );
...
static KREE_SESSION_HANDLE ddp_session = NULL;
static KREE_SESSION_HANDLE ddp_mem_session = NULL; // use [ddp_mem_session_handle()]
...
/* used by IRQ from TEE to REE
  ...
  */
#define MT7623_MDP_ROT_IRQ_BIT     0
#define MT7623_MDP_WDMA_IRQ_BIT    1
#define MT7623_DISP_OVL_IRQ_BIT    2
#define MT7623_DISP_WDMA_IRQ_BIT  3
#define MT7623_DISP_RDMA0_IRQ_BIT  4
#define MT7623_DISP_RDMA1_IRQ_BIT  5
...
static int disp_irq_log_kthread_func(void *data)
{
    ...
    while(1)
    {
        ...
        DISP_MSG("disp_irq_log_kthread_func dump intr register: disp_irq_log_module=0x%X \n", disp_irq_log_module);
...
//            disp_invoke_irq_callbacks(DISP_MODULE_COLOR, reg_val);
        ...
        case MT6582_DISP_MUTEX_IRQ_ID:  // can not do reg update done status after release mutex(for ECO requirement),
            ...
            if(reg_val & 0x01e00) // udpate timeout intr triggered
            {
                unsigned int reg = 0;
                unsigned int mutexID = 0;
                ...
                for(mutexID=0;mutexID<4;mutexID++)
                {
                    if((DISP_REG_GET(DISP_REG_CONFIG_MUTEX_INTSTA) & (1<<(mutexID+9))) == (1<<(mutexID+9)))
                    {
                        ...
                        {
                            DISP_ERR("disp_path_release_mutex() timeout! mutexID = %d\n", mutexID);
                            disp_irq_log_module |= (1<<DISP_MODULE_CONFIG);
                            disp_irq_log_module |= (1<<DISP_MODULE_MUTEX);
                            ddp_dump_info(DISP_MODULE_CONFIG);
                            ddp_dump_info(DISP_MODULE_MUTEX);
                            ...
                            if(reg!=0)
                            {
                                ...
                                if(reg&(1<<10)) {
                                	DISP_ERR(" RDMA0 update reg timeout! cnt=%d\n", cnt_mutex_timeout);
                                	disp_irq_log_module |= (1<<DISP_MODULE_RDMA0);
                                	disp_dump_reg(DISP_MODULE_RDMA0);
                                }
                                if(reg&(1<<9)) {
...
{
    DISP_REGISTER_IRQ(MT6582_DISP_RDMA1_IRQ_ID);
}

void disp_unregister_rdma1_irq(void)
{
    DISP_UNREGISTER_IRQ(MT6582_DISP_RDMA1_IRQ_ID);
}
...
int disp_secure_intr_callback(void *data)
{
	...
	ret = KREE_RegisterSharedmem (ddp_mem_session_handle(), &p_IRQ_status_share_handle, &sharedParam);
    ...
    static unsigned int ree_wdma0_cnt = 0;
	static unsigned int ree_rdma0_cnt = 0;
    ...
    param[3].value.a = 0;
    paramTypes = TZ_ParamTypes4(TZPT_VALUE_INPUT,TZPT_VALUE_OUTPUT,TZPT_MEMREF_OUTPUT,TZPT_VALUE_OUTPUT);
    ...
    while(1)
    {
        ret = KREE_TeeServiceCall(ddp_session_handle(), TZCMD_DDP_INTR_CALLBACK, paramTypes, param);
        if(ret!= TZ_RESULT_SUCCESS)
        {
        ...
        switch(irq)
        {
            case MT6582_DISP_MDP_WROT_IRQ_ID:                
                if(status_reg&(1<<0))
                {
                    g_disp_irq.irq_src |= (1<<DISP_MODULE_MDP_ROT);
                }   
                disp_invoke_irq_callbacks(DISP_MODULE_MDP_ROT, status_reg);
                break;
			case MT6582_DISP_RDMA0_IRQ_ID:
					if(status_reg&(1<<0))
					{
						g_disp_irq.irq_src |= (1<<DISP_MODULE_RDMA0);
						DISP_DBG("ree rdma0 intr done, cnt=%d \n", ree_rdma0_cnt++);
					}
					disp_invoke_irq_callbacks(DISP_MODULE_RDMA0, status_reg);
					break;
            case MT6582_DISP_WDMA_IRQ_ID:
                if(status_reg&(1<<0))
                {
                ...
                disp_invoke_irq_callbacks(DISP_MODULE_WDMA, status_reg);
                break;
            case MT6582_DISP_MDP_WDMA_IRQ_ID:
                if(status_reg&(1<<0))
                {
                    g_disp_irq.irq_src |= (1<<DISP_MODULE_MDP_WDMA);
                }
                disp_invoke_irq_callbacks(DISP_MODULE_MDP_WDMA, status_reg);
                break;
            case MT6582_DISP_OVL_IRQ_ID:
                if(status_reg&(1<<1))
                {
...
#endif
    	irq = param[1].value.a;
		DISP_DBG("[02420] irq=0x%08x ovl_status=0x%08x disp_wdma_status=0x%08x rdma0_status=0x%08x",irq,pIrqStatus->OVL_IRQ_STATUS,pIrqStatus->DISP_WDMA_IRQ_STATUS,pIrqStatus->RDMA0_IRQ_STATUS);

        if (1 == ((irq >> MT7623_MDP_ROT_IRQ_BIT) & 0x00000001))
		{               
            if((pIrqStatus->MDP_ROT_IRQ_STATUS) & (1<<0))
            {
                g_disp_irq.irq_src |= (1<<DISP_MODULE_MDP_ROT);
            }   
            disp_invoke_irq_callbacks(DISP_MODULE_MDP_ROT, pIrqStatus->MDP_ROT_IRQ_STATUS);
        }
		if (1 == ((irq >> MT7623_DISP_OVL_IRQ_BIT) & 0x00000001))
		{
		...
		if (1 == ((irq >> MT7623_MDP_WDMA_IRQ_BIT) & 0x00000001))
		{
            if((pIrqStatus->MDP_WDMA_IRQ_STATUS) & (1<<0))
            {
                g_disp_irq.irq_src |= (1<<DISP_MODULE_MDP_WDMA);
                DISP_DBG("[02420]ree mdp_wdma intr done, cnt=%d \n", ree_wdma0_cnt++);
            }
            disp_invoke_irq_callbacks(DISP_MODULE_MDP_WDMA, pIrqStatus->MDP_WDMA_IRQ_STATUS);
		}
		if (1 == ((irq >> MT7623_DISP_WDMA_IRQ_BIT) & 0x00000001))
		{
            if((pIrqStatus->DISP_WDMA_IRQ_STATUS) & (1<<0))
            {
                g_disp_irq.irq_src |= (1<<DISP_MODULE_WDMA);
				//DISP_DBG("[02420]ree wdma0/ovl intr done, cnt=%d \n", ree_ovl_cnt++);
            }
            disp_invoke_irq_callbacks(DISP_MODULE_WDMA, pIrqStatus->DISP_WDMA_IRQ_STATUS);
		}
		if (1 == ((irq >> MT7623_DISP_RDMA0_IRQ_BIT) & 0x00000001))
		{
		...
		if (1 == ((irq >> MT7623_DISP_RDMA1_IRQ_BIT) & 0x00000001))
		{
...
void disp_register_intr(unsigned int irq, unsigned int secure)
{
    ...
    if(secure==0)
    {
        ...
        ret = KREE_TeeServiceCall(ddp_session_handle(), TZCMD_DDP_REGISTER_INTR, paramTypes, param);
        if(ret!= TZ_RESULT_SUCCESS)
        {
            DISP_ERR("KREE_TeeServiceCall(TZCMD_DDP_REGISTER_INTR) fail, ret=%d \n", ret);
        }

        if (irq == MT6582_DISP_WDMA_IRQ_ID)
            MMProfileLogEx(MTKFB_MMP_Events.OVL_WDMA_IRQ_InTEE, MMProfileFlagEnd, irq, secure);
        else if (irq == MT6582_DISP_RDMA_IRQ_ID)
            MMProfileLogEx(MTKFB_MMP_Events.RDMA0_IRQ_InTEE, MMProfileFlagEnd, irq, secure);
        else if (irq == MT6582_DISP_RDMA1_IRQ_ID)
            MMProfileLogEx(MTKFB_MMP_Events.RDMA1_IRQ_InTEE, MMProfileFlagEnd, irq, secure);
    ...
    else
    {
        ...
        if (irq == MT6582_DISP_WDMA_IRQ_ID)
            MMProfileLogEx(MTKFB_MMP_Events.OVL_WDMA_IRQ_InTEE, MMProfileFlagStart, irq, secure);
        else if (irq == MT6582_DISP_RDMA_IRQ_ID)
            MMProfileLogEx(MTKFB_MMP_Events.RDMA0_IRQ_InTEE, MMProfileFlagStart, irq, secure);
        else if (irq == MT6582_DISP_RDMA1_IRQ_ID)
            MMProfileLogEx(MTKFB_MMP_Events.RDMA1_IRQ_InTEE, MMProfileFlagStart, irq, secure);
...
//            disp_set_needupdate(DISP_MODULE_BLS , 0);
...
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
        {
                ...
                if(copy_from_user(&mva_map_struct, (void *)arg, sizeof(struct disp_mva_map )))
                {
                ...
                else
                {
                    ...
                    if(ret!= TZ_RESULT_SUCCESS)
                    {
                        DISP_ERR("KREE_TeeServiceCall(TZCMD_DDP_SECURE_MVA_MAP) fail, ret=%d \n", ret);
                    }
                }
            }
#endif            
...
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
        {
                ...
                else
                {
                    ...
                    if(ret!= TZ_RESULT_SUCCESS)
                    {
                        DISP_ERR("KREE_TeeServiceCall(TZCMD_DDP_SECURE_MVA_UNMAP) fail, ret=%d \n", ret);
                    }
                }
            }
#endif            
...
static int disp_probe(struct platform_device *pdev)
{
	...
	DpEngine_COLORonInit();
	DpEngine_COLORonConfig(DISP_GetScreenWidth(),DISP_GetScreenHeight()); //height
...
static int disp_remove(struct platform_device *pdev)
{
    ...
    disable_irq(MT6582_DISP_RDMA1_IRQ_ID);
...
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
    if( (gBitbltSecure==1) && 
        (module==DISP_MODULE_MDP_ROT || module==DISP_MODULE_WDMA || module==DISP_MODULE_MDP_WDMA || module==DISP_MODULE_OVL))
    {
        MTEEC_PARAM param[4];
        unsigned int paramTypes;
        TZ_RESULT ret;
        ...
        ret = KREE_TeeServiceCall(ddp_session_handle(), TZCMD_DDP_DUMP_REG, paramTypes, param);
        if(ret!= TZ_RESULT_SUCCESS)
        {
