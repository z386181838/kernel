...
int Disp_Ovl_Engine_GetInstance(DISP_OVL_ENGINE_INSTANCE_HANDLE *pHandle,OVL_INSTANCE_MODE mode)
{
    ...
    for (i = 0; i< OVL_ENGINE_INSTANCE_MAX_INDEX; i++)
    {
        if (!disp_ovl_engine.Instance[i].bUsed)
        {
            *pHandle = i;
            ...
            {
                memset(&disp_ovl_engine.Instance[i].cached_layer_config[j],0, sizeof(OVL_CONFIG_STRUCT));
    ...
    {
        *pHandle = 0xFF;
        MMProfileLogEx(MTKFB_MMP_Events.GetInstance, MMProfileFlagEnd, *pHandle, disp_ovl_engine.bModeSwitch);
...
int Disp_Ovl_Engine_ReleaseInstance(DISP_OVL_ENGINE_INSTANCE_HANDLE handle)
{
    ...
    if (OVERLAY_STATUS_IDLE != disp_ovl_engine.Instance[handle].status)
    {
        ...
        DISP_OVL_ENGINE_ERR("Disp_Ovl_Engine_ReleaseInstance try to free a working instance \n");
        goto ReleaseInstanceFail;
    }
    ...
    MMProfileLogEx(MTKFB_MMP_Events.ReleaseInstance, MMProfileFlagEnd, handle, disp_ovl_engine.bModeSwitch);
...
int Disp_Ovl_Engine_Get_layer_info(DISP_OVL_ENGINE_INSTANCE_HANDLE handle, struct fb_overlay_layer * layerInfo)
{
    unsigned int layerpitch;
    OVL_CONFIG_STRUCT *ovl_config_tmp = &disp_ovl_engine.Instance[handle].cached_layer_config[layerInfo->layer_id];
    ...
    MMProfileLogStructure(MTKFB_MMP_Events.Get_layer_info, MMProfileFlagEnd, layerInfo, struct fb_overlay_layer);

    return 0;
}


int Disp_Ovl_Engine_Set_layer_info(DISP_OVL_ENGINE_INSTANCE_HANDLE handle, struct fb_overlay_layer* layerInfo)
{
    // Todo, check layer id is valid.
	unsigned int fmt;
    unsigned int layerpitch;
    ...
    MMProfileLogStructure(MTKFB_MMP_Events.Set_layer_info, MMProfileFlagStart, layerInfo, struct fb_overlay_layer);
    ...
    if(layerInfo->layer_enable)
    {
        ...
        if (ovl_config_tmp->dst_w >
            ovl_config_tmp->src_w)
            ovl_config_tmp->dst_w =
                ovl_config_tmp->src_w;
        if (ovl_config_tmp->dst_h >
            ovl_config_tmp->src_h)
            ovl_config_tmp->dst_h =
                ovl_config_tmp->src_h;
...
int Disp_Ovl_Engine_Get_Ovl_layer_info(DISP_OVL_ENGINE_INSTANCE_HANDLE handle, DISP_LAYER_INFO * layerInfo)
{
    ...
    layerInfo->next_en = disp_ovl_engine.Instance[handle].cached_layer_config[layerInfo->id].layer_en;
    ...
    layerInfo->next_idx = disp_ovl_engine.Instance[handle].cached_layer_config[layerInfo->id].buff_idx;
    ...
    layerInfo->next_identity = disp_ovl_engine.Instance[handle].cached_layer_config[layerInfo->id].identity;
    layerInfo->hw_identity = disp_ovl_engine.realtime_layer_config[layerInfo->id].identity;
    layerInfo->curr_conn_type = disp_ovl_engine.captured_layer_config[layerInfo->id].connected_type;
    layerInfo->next_conn_type = disp_ovl_engine.Instance[handle].cached_layer_config[layerInfo->id].connected_type;
    layerInfo->hw_conn_type = disp_ovl_engine.realtime_layer_config[layerInfo->id].connected_type;
    up(&disp_ovl_engine_semaphore);

    MMProfileLogStructure(MTKFB_MMP_Events.Get_Ovl_layer_info, MMProfileFlagEnd, layerInfo, DISP_LAYER_INFO);
    
    return OVL_OK;
}
int Disp_Ovl_Engine_Set_Overlayed_Buffer(DISP_OVL_ENGINE_INSTANCE_HANDLE handle, struct disp_ovl_engine_config_mem_out_struct *overlayedBufInfo)
{
	//DISP_OVL_ENGINE_INFO("Disp_Ovl_Engine_Set_Overlayed_Buffer %d\n",handle);
    //MMProfileLogStructure(MTKFB_MMP_Events.Set_Overlayed_Buffer, MMProfileFlagStart, overlayedBufInfo, struct disp_ovl_engine_config_mem_out_struct);
	...
	do {
		...
		//pr_err("[layeredbuf]overlayedBufInfo->dstAddr=0x%08x overlayedBufInfo->ion_fd=%d", overlayedBufInfo->dstAddr,overlayedBufInfo->ion_fd);
		...
		ion_handles = ion_import_dma_buf(disp_ovl_engine.ion_client, overlayedBufInfo->ion_fd);
		if (IS_ERR(ion_handles)) {
    ...
    memcpy(&(disp_ovl_engine.Instance[handle].MemOutConfig),overlayedBufInfo,sizeof(struct disp_ovl_engine_config_mem_out_struct));
...
int Disp_Ovl_Engine_Wait_Overlay_Complete(DISP_OVL_ENGINE_INSTANCE_HANDLE handle, int timeout)
{
	...
	/*no need to wait when suspend state*/
		...
		wait_ret = wait_event_interruptible_timeout(disp_ovl_complete_wq,
							 atomic_read(&disp_ovl_engine.Instance[handle].fgCompleted), HZ/10);
...
int Disp_Ovl_Engine_Get_Dirty_info(DISP_OVL_ENGINE_INSTANCE_HANDLE handle, int *OverlayDirty, int *MemOutDirty)
{
    if (handle >= OVL_ENGINE_INSTANCE_MAX_INDEX)
        return OVL_ERROR;
    ...
    *MemOutDirty = disp_ovl_engine.Instance[handle].MemOutConfig.dirty;
    ...
    MMProfileLogEx(MTKFB_MMP_Events.Get_Dirty_info, MMProfileFlagEnd, *OverlayDirty, *MemOutDirty);
    return OVL_OK;
}
int Disp_Ovl_Engine_Set_Path_Info(DISP_OVL_ENGINE_INSTANCE_HANDLE handle, struct disp_path_config_struct *info)
{
    if ((handle >= OVL_ENGINE_INSTANCE_MAX_INDEX) || !disp_ovl_engine.Instance[handle].bUsed)
        return OVL_ERROR;
    
    DISP_OVL_ENGINE_INFO("Disp_Ovl_Engine_Set_Path_Info handle %d srcModule %d dstModule %d\n",handle,info->srcModule,info->dstModule);
    MMProfileLogStructure(MTKFB_MMP_Events.Set_Path_Info, MMProfileFlagStart, info, struct disp_path_config_struct);
    
    if (down_interruptible(&disp_ovl_engine_semaphore))
    {
        return OVL_ERROR;
    }
    memcpy(&disp_ovl_engine.Instance[handle].path_info, info, sizeof(struct disp_path_config_struct));
...
int Disp_Ovl_Engine_Sync_Captured_layer_info(DISP_OVL_ENGINE_INSTANCE_HANDLE handle)
{
    ...
    disp_ovl_engine.captured_layer_config = disp_ovl_engine.layer_config[disp_ovl_engine.layer_config_index];
    //DISP_OVL_ENGINE_INFO("========= cached --> captured ===========\n");
    memcpy(disp_ovl_engine.captured_layer_config, disp_ovl_engine.Instance[handle].cached_layer_config, sizeof(OVL_CONFIG_STRUCT)*DDP_OVL_LAYER_MUN);
    up(&disp_ovl_engine_semaphore);
...
int Disp_Ovl_Engine_Dump_layer_info(DISP_OVL_ENGINE_INSTANCE_HANDLE handle, OVL_CONFIG_STRUCT *cached_layer, OVL_CONFIG_STRUCT *captured_layer, OVL_CONFIG_STRUCT *realtime_layer)
{
    if ((handle >= OVL_ENGINE_INSTANCE_MAX_INDEX) || !disp_ovl_engine.Instance[handle].bUsed)
        return OVL_ERROR;
    ...
    {
        memcpy(cached_layer, disp_ovl_engine.Instance[handle].cached_layer_config,sizeof(OVL_CONFIG_STRUCT) * DDP_OVL_LAYER_MUN);
    }
    if (captured_layer != NULL)
    {
        memcpy(captured_layer, disp_ovl_engine.captured_layer_config, sizeof(OVL_CONFIG_STRUCT) * DDP_OVL_LAYER_MUN);
    }
    if (realtime_layer != NULL)
    {
        memcpy(realtime_layer, disp_ovl_engine.realtime_layer_config, sizeof(OVL_CONFIG_STRUCT) * DDP_OVL_LAYER_MUN);
    }
    up(&disp_ovl_engine_semaphore);
...
static void Disp_Ovl_Engine_Trigger_Overlay_Handler(struct work_struct *work_)
{
    ...
    MMProfileLogEx(MTKFB_MMP_Events.Trigger_Overlay_Handler, MMProfileFlagPulse, instance->index, 0);

    // Wait input buffer fence
    for(layer=0; layer<DDP_OVL_LAYER_MUN; layer++)
    {
        if((instance->cached_layer_config[layer].layer_en) &&
           (work->fences[layer]))
        {
            DISP_OVL_ENGINE_INFO("Disp_Ovl_Engine_Trigger_Overlay_Handler %d layer %d wait fence\n",
    ...
    if(disp_ovl_engine.Instance[instance->index].status != OVERLAY_STATUS_IDLE)
    {
        ...
        wait_ret = wait_event_interruptible_timeout(disp_ovl_complete_wq, 
					atomic_read(&disp_ovl_engine.Instance[instance->index].fgCompleted),1000);
        ...
        if(wait_ret == 0)
        {
            ...
            DISP_OVL_ENGINE_ERR("Disp_Ovl_Engine_Trigger_Overlay_Handler %d: skip buffer\n",instance->index);
            skip = 1;
        }
    ...
    for(layer=0; layer<DDP_OVL_LAYER_MUN; layer++)
    {
        Disp_Ovl_Engine_Set_layer_info(instance->index,&(work->setting.overlay_layers[layer]));
    }
    ...
    struct disp_ovl_engine_setting *setting)
{
	...
	pt = sw_sync_pt_create(disp_ovl_engine.Instance[handle].timeline, disp_ovl_engine.Instance[handle].timeline_max);
        ...
        for(layer=0; layer<DDP_OVL_LAYER_MUN; layer++)
            if((setting->overlay_layers[layer].layer_enable) &&
               (setting->overlay_layers[layer].fence_fd >= 0))
                work->fences[layer] = 
                    sync_fence_fdget(disp_ovl_engine.Instance[handle].cached_layer_config[layer].fence_fd);
            else
