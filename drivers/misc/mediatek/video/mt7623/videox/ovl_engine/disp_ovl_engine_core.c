...
static int disp_ovl_engine_kthread(void *data)
{
	...
	//DISP_OVL_ENGINE_DBG("wait_event_interruptible\n");
        ...
        wait_ret = wait_event_interruptible(disp_ovl_engine_wq, atomic_read(&gWakeupOvlEngineThread));
...
static int disp_ovl_engine_rdma0_update_kthread(void *data)
{
    ...
    while(1)
    {
        ...
        wait_ret = wait_event_interruptible(disp_rdma0_update_wq, atomic_read(&gWakeupRdma0UpdateThread));
        atomic_set(&gWakeupRdma0UpdateThread, 0);

		//DISP_OVL_ENGINE_DBG("disp_ovl_engine_rdma0_update_kthread wake_up_interruptible\n");
