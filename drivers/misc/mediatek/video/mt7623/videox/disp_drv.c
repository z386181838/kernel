...
void _DISP_DumpLayer(OVL_CONFIG_STRUCT* pLayer)
{
    if (gCaptureLayerEnable && pLayer->layer_en)
    {
        ...
        switch (pLayer->fmt)
        {
            ...
            default: 
                pr_err("error: _DISP_DumpLayer(), unknow format=%d, dump raw\n", pLayer->fmt); 
                raw = 1;
        }
        ...
        if (pLayer->addr >= fb_pa &&
            ...
            if (!raw)
                ...
                MMProfileLogMeta(MTKFB_MMP_Events.Layer[pLayer->layer], MMProfileFlagPulse,
                        &meta);
            }
        }
        else {
...
static int _DISP_ConfigUpdateKThread(void *data)
{
    ...
    while(1)
    {
        if (wait_event_interruptible(config_update_wq,
                        atomic_read(&config_update_task_wakeup))== 0)
        ...
        if (down_interruptible(&sem_early_suspend)) {
            pr_err("[_DISP_ConfigUpdateKThread] can't get semaphore: sem_early_suspend\n");
            continue;
        }
