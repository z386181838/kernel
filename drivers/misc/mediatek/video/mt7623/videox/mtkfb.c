...
#else
    wait_ret = wait_event_interruptible_timeout(reg_update_wq, PanDispSettingApplied, HZ/10);
...
int mtkfb_queue_overlay_config_ex(struct mtkfb_device *fbdev, struct fb_overlay_config* config, unsigned int is_v4l2)
{
...
#else
int mtkfb_ioctl(struct file *file, struct fb_info *info, unsigned int cmd, unsigned long arg)
#endif
{
    ...
    pr_info("mtkfb_ioctl, info=0x%08x, cmd=0x%08x(%s), arg=0x%08x, v4l2=%d\n", (unsigned int)info,
        (unsigned int)cmd, _mtkfb_ioctl_spy(cmd), (unsigned int)arg, from_v4l2);
...
#endif
        if (!g_fb_pattern_en)
            r = mtkfb_queue_overlay_config_ex((struct mtkfb_device *)info->par, &config, from_v4l2);
        else
...
static int mtkfb_probe(struct device *dev)
{
	...
	p = strstr(saved_command_line, "use_lk");
    if(p == NULL)
		use_lk = 0;
    else
		use_lk = 1;
