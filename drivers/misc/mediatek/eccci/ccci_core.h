...
#define PORT_F_CHAR_KERNEL	(1<<4)	/* char device which is handled in kernel space */
...
struct ccci_modem_ops {
	...
	int (*napi_poll) (struct ccci_modem *md, unsigned char qno, struct napi_struct *napi,
			  int budget);
...
static inline int ccci_port_recv_request(struct ccci_modem *md, struct ccci_request *req)
{
	...
	if (ret == -CCCI_ERR_CHANNEL_NUM_MIS_MATCH) {
		CCCI_ERR_MSG(md->index, CORE, "drop on not supported channel %d", ccci_h->channel);
		list_del(&req->entry);
		req->policy = RECYCLE;
		ccci_free_req(req);
