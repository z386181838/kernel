...
static void control_msg_handler(struct ccci_port *port, struct ccci_request *req)
{
	...
	if (ccci_h->data[1] == MD_INIT_START_BOOT
	...
	} else if (ccci_h->data[1] == NORMAL_BOOT_ID && md->boot_stage == MD_BOOT_STAGE_1) {
		del_timer(&md->bootup_timer);
		...
		if (unlikely(ccci_h->reserved != MD_EX_CHK_ID)) {
			...
			md->ee_info_flag |=
			    ((1 << MD_EE_FLOW_START) | (1 << MD_EE_MSG_GET) | (1 << MD_STATE_UPDATE)
		...
		if (unlikely
			...
			CCCI_ERR_MSG(md->index, KERN,
				     "receive invalid MD_EX_REC_OK, resv=%x, len=%d\n",
			...
			if (need_update_state) {
				CCCI_ERR_MSG(md->index, KERN,
					     "get MD_EX_REC_OK without exception MD_EX\n");
				del_timer(&md->bootup_timer);
				md->ops->broadcast_state(md, EXCEPTION);
			}
			/* copy exception info */
			memcpy(&md->ex_info, skb_pull(req->skb, sizeof(struct ccci_header)),
			       sizeof(EX_LOG_T));
...
static int get_eint_attr(char *name, unsigned int name_len, unsigned int type, char *result,
			 unsigned int *len)
{
#if defined(FEATURE_GET_MD_EINT_ATTR)
	...
	case IPC_RPC_CPSVC_SECURE_ALGO_OP:
		{
			...
			CCCI_DBG_MSG(md->index, RPC,
				     ...
				     (unsigned int)ContentAddr, (unsigned int)p_rpc_buf,
				     sizeof(unsigned int) + RPC_MAX_BUF_SIZE);
			if (ContentAddr < (unsigned int)p_rpc_buf
			    || ContentAddr >
			    ((unsigned int)p_rpc_buf + sizeof(unsigned int) + RPC_MAX_BUF_SIZE)) {
				CCCI_ERR_MSG(md->index, RPC,
			...
			for (i = 0; i < ContentLen; i++) {
				if (i % 16 == 0) {
					...
					curr = 0;
					curr +=
					    snprintf(log_buf, sizeof(log_buf) - curr, "HACC_S: ");
				}
	...
	case IPC_RPC_GET_SECRO_OP:
		{
			...
			if (masp_secro_en()) {
				...
				addr =
				    (unsigned char *)&(p_rpc_buf->para_num) +
				    4 * sizeof(unsigned int);
				tmp_data[0] =
				    masp_secro_md_get_data(md->post_fix, addr, 0, img_len);
	...
	case IPC_RPC_GET_GPIO_ADC_OP:
		{
			...
			if (input->reqMask & RPC_REQ_GPIO_PIN) {
				for (i = 0; i < GPIO_MAX_COUNT; i++) {
					if (input->gpioValidPinMask & (1 << i)) {
						if ((num =
						     get_md_gpio_info(input->gpioPinName[i],
								      strlen(input->
									     gpioPinName[i]))) >=
						    0) {
			...
			if (input->reqMask & RPC_REQ_ADC_PIN) {
				if ((num =
				     get_md_adc_info(input->adcChName,
						     strlen(input->adcChName))) >= 0) {
					output->adcChNum = num;
				}
			}
...
static void rpc_msg_handler(struct ccci_port *port, struct ccci_request *req)
{
	...
	CCCI_DBG_MSG(md->index, RPC, "Write %d/%d, %08X, %08X, %08X, %08X, op_id=%x\n",
		     req->skb->len, data_len, rpc_buf->header.data[0], rpc_buf->header.data[1],
		     rpc_buf->header.channel, rpc_buf->header.reserved, rpc_buf->op_id);
...
static int port_kernel_thread(void *arg)
{
	...
	while (1) {
		if (list_empty(&port->rx_req_list)) {
			ret =
			    wait_event_interruptible(port->rx_wq, !list_empty(&port->rx_req_list));
			if (ret == -ERESTARTSYS) {
				continue;	/* FIXME */
			}
		}
...
static int port_kernel_recv_req(struct ccci_port *port, struct ccci_request *req)
{
 ...
 drop:
	/* drop this packet */
	CCCI_INF_MSG(port->modem->index, CHAR, "drop on %s, len=%d\n", port->name, port->rx_length);
	list_del(&req->entry);
	req->policy = RECYCLE;
	ccci_free_req(req);
...
void ccci_md_exception_notify(struct ccci_modem *md, MD_EX_STAGE stage)
{
	...
	case EX_INIT_DONE:
		break;
	case MD_NO_RESPONSE:
		md->ee_info_flag |= ((1 << MD_EE_FLOW_START) | (1 << MD_EE_PENDING_TOO_LONG));
...
static void ccci_aed(struct ccci_modem *md, unsigned int dump_flag, char *aed_str)
{
	...
	if (dump_flag & CCCI_AED_DUMP_CCIF_REG) {	/* check this first, as we overwrite share memory here */
		...
		ex_log_len = md->ops->dump_info(md, DUMP_FLAG_CCIF, ex_log_addr, ex_log_len);
	}
...
static void ccci_ee_info_dump(struct ccci_modem *md, DEBUG_INFO_T *debug_info)
{
	...
	CCCI_INF_MSG(md->index, KERN,
		     ...
		     (unsigned int)tv.tv_usec, tm_android.tm_hour, tm_android.tm_min,
	...
	case MD_EX_TYPE_ASSERT:
		...
		snprintf(ex_info, EE_BUF_LEN,
			 ...
			 debug_info->assert.line_num, debug_info->assert.parameters[0],
			 debug_info->assert.parameters[1], debug_info->assert.parameters[2]);
		break;
	...
	case DSP_EX_TYPE_ASSERT:
		...
		snprintf(ex_info, EE_BUF_LEN,
			 "\n[%s] file:%s line:%d\nexec:%s\np1:%d\np2:%d\np3:%d\n", debug_info->name,
			 debug_info->assert.file_name, debug_info->assert.line_num,
			 debug_info->dsp_assert.execution_unit,
			 debug_info->dsp_assert.parameters[0], debug_info->dsp_assert.parameters[1],
			 debug_info->dsp_assert.parameters[2]);
		break;
	...
	if (debug_info->more_info == MD_EE_CASE_NORMAL && md->boot_stage == MD_BOOT_STAGE_0) {	/* MD will not fill in share memory before we send runtime data */
		ccci_aed(md, CCCI_AED_DUMP_EX_PKT | CCCI_AED_DUMP_MD_IMG_MEM, ex_info);
...
void md_ex_monitor_func(unsigned long data)
{
	...
	if ((ee_info_flag &
		...
		if (ee_info_flag & (1 << MD_EE_AP_MASK_I_BIT_TOO_LONG)) {
		   ...
		   && (ee_info_flag & ((1 << MD_EE_MSG_GET) | (1 << MD_EE_SWINT_GET)))) {
		ee_case = MD_EE_CASE_SWINT_MISSING;
		CCCI_INF_MSG(md->index, KERN, "SWINT missing, ee_info_flag=%x\n", ee_info_flag);
	} else if ((ee_info_flag & ((1 << MD_EE_MSG_GET) | (1 << MD_EE_SWINT_GET))) &
		   (1 << MD_EE_MSG_GET)) {
	...
	} else if ((ee_info_flag & ((1 << MD_EE_OK_MSG_GET) | (1 << MD_EE_SWINT_GET))) &
		   (1 << MD_EE_OK_MSG_GET)) {
...
void md_bootup_timeout_func(unsigned long data)
{
	...
	if (md->boot_stage == MD_BOOT_STAGE_0) {
		...
		/* Handshake 2 fail */
#if defined(CONFIG_MTK_AEE_FEATURE)
		aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_FTRACE, "CCCI",
				       "modem boot up timeout");
#endif
		CCCI_INF_MSG(md->index, KERN, "Dump MD EX log\n");
		ccci_mem_dump(md->smem_layout.ccci_exp_smem_base_vir,
			      md->smem_layout.ccci_exp_smem_size);

		ccci_aed(md, CCCI_AED_DUMP_CCIF_REG | CCCI_AED_DUMP_EX_MEM, ex_info);
	}
}
