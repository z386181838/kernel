...
static ssize_t hwmsen_show_hwmdev(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	int len = 0;
	pr_info("sensor test: hwmsen_show_hwmdev function!\n");
/*
    if (!devobj || !devobj->dc) {
...
{
/*
	pr_info("sensor test: hwmsen_store_active function!\n");
    struct hwmdev_object *devobj = (struct hwmdev_object*)dev_get_drvdata(dev);
...
/*
    struct hwmdev_object *devobj = (struct hwmdev_object*)dev_get_drvdata(dev);
pr_info("sensor test: hwmsen_show_delay function!\n");
    if (!devobj || !devobj->dc) {
	HWM_ERR("null pointer!!\n");
	return 0;
    }
...
{
/*
	pr_info("sensor test: hwmsen_show_wake function!\n");
    struct hwmdev_object *devobj = (struct hwmdev_object*)dev_get_drvdata(dev);
...
/*
    struct hwmdev_object *devobj = (struct hwmdev_object*)dev_get_drvdata(dev);
    int wake, err;
pr_info("sensor test: hwmsen_store_wake function!\n");
    if (!devobj || !devobj->dc) {
	HWM_ERR("null pointer!!\n");
	return count;
    }
