...
void qmu_done_tx(struct musb *musb, u8 ep_num, unsigned long flags)
{
	...
	if (req != NULL) {
		if (request->length == 0) {
			...
			if (wait_for_value(USB_END_OFFSET(req->epnum, U3D_TX1CSR0),
						TX_FIFOEMPTY, TX_FIFOEMPTY, 1, 10) == RET_SUCCESS)
...
void qmu_done_rx(struct musb *musb, u8 ep_num, unsigned long flags)
{
	...
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		...
		dma_sync_single_for_cpu(musb->controller, (dma_addr_t) gpd, sizeof(TGPD),
					DMA_FROM_DEVICE);
				...
				if (request->request.length > 0) {
					...
					if (is_len_err == true) {
						mu3d_hal_insert_transfer_gpd(request->epnum, USB_TX,
							...
							true, true, false,
							((musb_ep->type == USB_ENDPOINT_XFER_ISOC) ? 0 : 1),
