	...
	if (g_ep0_mode == PIO_MODE) {
		...
		if (req->complete == 1) {
			...
			if (req->actual >= req->count) {
				...
				g_ep0_state = EP0_IDLE;
				req->complete = 1;
				dma_sync_single_for_cpu(NULL, (dma_addr_t) req->dma_adr,
							USB_BUF_SIZE, DMA_BIDIRECTIONAL);
				dma_unmap_single(NULL, req->dma_adr, USB_BUF_SIZE,
						 DMA_BIDIRECTIONAL);
				os_printk(K_DEBUG,
				...
				u3d_config_dma0(0, USB_TX,
						(DEV_UINT32) (req->dma_adr + req->actual),
						req->currentCount);
			}
		}
	}
...
void u3d_ep0_rx(void)
{
	...
	if (g_ep0_mode == PIO_MODE) {
		...
		if (req->currentCount == 0) {
			...
			u3d_config_dma0(0, USB_RX, (DEV_UINT32) (req->dma_adr + req->actual),
					req->currentCount);
		}
	}
...
#ifdef BOUNDARY_4K
		loopback_buffer =
		    g_loopback_buffer[1] + (0x1000 - (DEV_INT32) g_loopback_buffer[1] % 0x1000) -
...
void u3d_epx_handler(DEV_INT32 ep_num, USB_DIR dir)
{
	...
	if (dir == USB_TX) {
		...
		os_printk(K_DEBUG, "g_u3d_req[%d].buf@0x%08X\n", ep_index,
			  (DEV_UINT32) g_u3d_req[ep_index].buf);
#if (BUS_MODE == PIO_MODE)
		bp = req->buf + req->actual;
		length = req->count - req->actual;
	...
	if (DMAIntSts & EP0DMAISR) {	/* ep0 */
		...
		if ((os_readl(U3D_EP0CSR) & EP0_SETUPPKTRDY)	/* SETUPEND case; restarts from EP0_IDLE */
				...
				if (Request->bmRequestType & USB_DIR_IN) {
					...
					if (Request->bRequest == USB_REQ_GET_STATUS) {
						...
						*(ptr1 + 1) = 0x00;
						...
						req->needZLP = 0;
						mapping =
						    dma_map_single(NULL, req->buf, USB_BUF_SIZE,
								   DMA_BIDIRECTIONAL);
						dma_sync_single_for_device(NULL, mapping,
									   USB_BUF_SIZE,
									   DMA_BIDIRECTIONAL);
						req->dma_adr = mapping;
					}
			...
			if (Request->bmRequestType == 0x00C0) {
				...
				g_ep0_state = EP0_TX;
				os_writel(U3D_EP0CSR,
					  os_readl(U3D_EP0CSR) | EP0_SETUPPKTRDY | EP0_DPHTX);
				...
				mapping =
				    dma_map_single(NULL, req->buf, USB_BUF_SIZE, DMA_BIDIRECTIONAL);
				dma_sync_single_for_device(NULL, mapping, USB_BUF_SIZE,
							   DMA_BIDIRECTIONAL);
			...
			if ((count < ep_setting->maxp) || (req->actual == req->count)) {
				...
				Request->bCommand = AT_CMD->tsfun;
				dma_sync_single_for_cpu(NULL, req->dma_adr, USB_BUF_SIZE,
							DMA_BIDIRECTIONAL);
				dma_unmap_single(NULL, req->dma_adr, USB_BUF_SIZE,
						 DMA_BIDIRECTIONAL);
			}
