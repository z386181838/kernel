...
#ifdef BOUNDARY_4K
	qmu_loopback_buffer =
	    g_loopback_buffer[0] + (0x1000 - (DEV_INT32) g_loopback_buffer[0] % 0x1000) - 0x20 +
	    bDramOffset;
			    ...
			    DEV_UINT8 _is_bdp, DEV_UINT8 isHWO, DEV_UINT8 IOC)
{
	...
	os_printk(K_DEBUG,
		  "Rx gpd info { HWO %d, Next_GPD %x ,DataBufferLength %d, DataBuffer %x, Recived Len %d, Endpoint %d, TGL %d, ZLP %d}\r\n",
...
void proc_qmu_rx(DEV_INT32 ep_num)
{
	...
	if (g_run_stress) {
		...
		os_printk(K_INFO,
			  "ep_num : %d ,Rx_gpd_last : 0x%X, gpd_current : 0x%X, gpd_end : 0x%X \r\n",
			  ep_num, (DEV_UINT32) gpd, (DEV_UINT32) gpd_current,
			  (DEV_UINT32) Rx_gpd_end[ep_num]);
		...
		while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
			...
			if (TGPD_IS_FORMAT_BDP(gpd)) {
				...
				dma_sync_single_for_cpu(NULL, (dma_addr_t) bd, sizeof(TBD),
							DMA_BIDIRECTIONAL);
			...
			os_printk(K_INFO,
				  "Rx gpd info { HWO %d, Next_GPD %x ,DataBufferLength %d, DataBuffer %x, Recived Len %d, Endpoint %d, TGL %d, ZLP %d}\r\n",
				  (DEV_UINT32) TGPD_GET_FLAG(gpd), (DEV_UINT32) TGPD_GET_NEXT(gpd),
				  (DEV_UINT32) TGPD_GET_DataBUF_LEN(gpd),
				  (DEV_UINT32) TGPD_GET_DATA(gpd),
			...
			if (gpd)
				dma_sync_single_for_cpu(NULL, (dma_addr_t) gpd, sizeof(TGPD),
							DMA_BIDIRECTIONAL);
...
void proc_qmu_tx(DEV_INT32 ep_num)
{
	...
	if (g_run_stress) {
		...
		os_printk(K_DEBUG, "Tx_gpd_last 0x%x, gpd_current 0x%x, gpd_end 0x%x, \r\n",
			  (DEV_UINT32) gpd, (DEV_UINT32) gpd_current,
			  (DEV_UINT32) Tx_gpd_end[ep_num]);
		...
		while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
			os_printk(K_DEBUG,
				  "Tx gpd %x info { HWO %d, BPD %d, Next_GPD %x , DataBuffer %x, BufferLen %d, Endpoint %d}\r\n",
				  (DEV_UINT32) gpd, (DEV_UINT32) TGPD_GET_FLAG(gpd),
				  (DEV_UINT32) TGPD_GET_FORMAT(gpd),
				  (DEV_UINT32) TGPD_GET_NEXT(gpd), (DEV_UINT32) TGPD_GET_DATA(gpd),
				  (DEV_UINT32) TGPD_GET_BUF_LEN(gpd),
...
irqreturn_t u3d_inter_handler(int irq, void *dev_id)
{
	...
	if ((wIntrTxValue & 1) || (wIntrRxValue & 1)) {
		...
		os_printk(K_INFO, "EP0 interrupt EP0CSR[%x]\n", os_readl(U3D_EP0CSR));
