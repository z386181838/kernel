...
{
	if (argc > 10) {
		u3phy_ops->eyescan(u3phy, simple_strtol(argv[1], &argv[1], 10),	/* x_t1 */
...
void sram_write(DEV_UINT32 mode, DEV_UINT32 addr, DEV_UINT32 data)
{
	...
	os_writelmsk(U3D_SRAM_DBG_CTRL_1, 10 << SRAM_DEBUG_FIFOSEGSIZE_OFST,
		     SRAM_DEBUG_FIFOSEGSIZE);
	os_writelmsk(U3D_SRAM_DBG_CTRL_1, (addr / 1024) << SRAM_DEBUG_SLOT_OFST, SRAM_DEBUG_SLOT);
	os_writelmsk(U3D_SRAM_DBG_CTRL_1, (addr % 1024) << SRAM_DEBUG_DP_COUNT_OFST,
		     SRAM_DEBUG_DP_COUNT);
...
void sram_dbg(void)
{
	...
	DEV_UINT32 value[] =
	    { 0xaaaaaaaa, 0x55555555, 0x5a5a5a5a, 0xa5a5a5a5, 0x0, 0xffffffff, 0x12345678,
   0x87654321 };
	...
	for (j = 0; j < sizeof(en) / sizeof(en[0]); j++) {
			...
			/* addr = 0x100; */
		{
			...
			os_writelmsk(U3D_SRAM_DBG_CTRL_1, (addr / 1024) << SRAM_DEBUG_SLOT_OFST,
				     SRAM_DEBUG_SLOT);
			os_writelmsk(U3D_SRAM_DBG_CTRL_1, (addr % 1024) << SRAM_DEBUG_DP_COUNT_OFST,
				     SRAM_DEBUG_DP_COUNT);
			...
			for (i = 0; i < sizeof(value) / sizeof(value[0]); i++) {
				...
				if (os_readl(fifo[j]) != value[i]) {
					os_printk(K_ERR,
						  "[4-byte access]Write addr %x with value %x fail, got %x\n",
				...
				os_writelmsk(U3D_SRAM_DBG_CTRL_1,
					     ((addr + 2) % 1024) << SRAM_DEBUG_DP_COUNT_OFST,
					     SRAM_DEBUG_DP_COUNT);
				os_writel(fifo[j], (*(ptr + 2) | *(ptr + 3) << 8));
				/* restore to DWORD access */
				os_writel(U3D_RISC_SIZE, RISC_SIZE_4B);
				os_writelmsk(U3D_SRAM_DBG_CTRL_1,
					     (addr % 1024) << SRAM_DEBUG_DP_COUNT_OFST,
					     SRAM_DEBUG_DP_COUNT);
				if (value[i] != os_readl(fifo[j])) {
					os_printk(K_ERR,
						  "[2-byte access]Write addr %x with value %x fail, got %x\n",
						  addr, value[i], os_readl(fifo[j]));
				}
				/* test 1-byte access */
				ptr = (DEV_UINT8 *) &value[i];
				...
				os_writelmsk(U3D_SRAM_DBG_CTRL_1,
					     ((addr + 1) % 1024) << SRAM_DEBUG_DP_COUNT_OFST,
					     SRAM_DEBUG_DP_COUNT);
				...
				os_writelmsk(U3D_SRAM_DBG_CTRL_1,
					     ((addr + 3) % 1024) << SRAM_DEBUG_DP_COUNT_OFST,
					     SRAM_DEBUG_DP_COUNT);
				...
				if (value[i] != os_readl(fifo[j])) {
					os_printk(K_ERR,
						  "[1-byte access]Write addr %x with value %x fail, got %x\n",
						  addr, value[i], os_readl(fifo[j]));
					while (1);
				}
			}
		...
		for (addr = 0; addr < size[j]; addr = addr + 4) {
			...
			if (os_readl(fifo[j]) != data) {
				os_printk(K_ERR,
					  "addr %x data has been modified, should be %x, got %x\n",
	...
	while (g_run) {
		if (u3d_req_valid()) {
			...
			case RESET_STATE:
				...
				reset_dev(reset->speed & 0xf, 1, (reset->speed & 0x10) ? 0 : 1);
				tx_num = 0;
				rx_num = 0;
				speed = reset->speed;
				break;
			...
			case CONFIG_EP_STATE:
				...
				mu3d_hal_ep_enable(ep_info->ep_num, ep_info->dir, ep_info->type,
						   ep_info->ep_size, ep_info->interval,
						   ep_info->slot, ep_info->burst, ep_info->mult);
			...
			case STRESS:
				...
				for (i = 0; i < st_num; i++) {
					g_loopback_buffer[i] =
					    (DEV_UINT8 *) os_mem_alloc(g_dma_buffer_size);
				}
				...
				for (i = 0; i < st_num; i++) {
					dev_prepare_stress_gpd(MAX_GPD_NUM, USB_RX, EP_RX[i],
							       g_loopback_buffer[i]);
					dev_prepare_stress_gpd(MAX_GPD_NUM, USB_TX, EP_TX[i],
							       g_loopback_buffer[i]);
				}
			...
			case SINGLE:
				...
				for (i = 0; i < st_num; i++) {
					dev_prepare_gpd(STRESS_GPD_TH, dir, ep_n[i],
							g_loopback_buffer[0]);
				}
			...
			case POWER_STATE:
				...
				dev_power_mode(pw_info->mode, pw_info->u1_value, pw_info->u2_value,
					       pw_info->en_u1, pw_info->en_u2);
				break;


			case U1U2_STATE:
				u1u2_info = (U1U2_INFO *) u3d_req_buffer();
				dev_u1u2_en_cond(u1u2_info->opt, u1u2_info->cond, EP_RX[0],
						 EP_TX[0]);
				dev_u1u2_en_ctrl(u1u2_info->type, u1u2_info->u_num, u1u2_info->opt,
						 u1u2_info->cond, u1u2_info->u1_value,
						 u1u2_info->u2_value);
...
#if !LPM_STRESS
				dev_u1u2_en_cond(lpm_info->cond + 1, lpm_info->cond_en, EP_RX[0],
						 EP_TX[0]);
#endif
			...
			case RANDOM_STOP_STATE:
				...
				if (dir_1 == USB_RX) {
					ep_num1 = EP_RX[0];
					dev_prepare_gpd(STRESS_GPD_TH, USB_RX, ep_num1,
							g_loopback_buffer[0]);
				} else {
					ep_num1 = EP_TX[0];
					dev_prepare_gpd(STRESS_GPD_TH, USB_TX, ep_num1,
							g_loopback_buffer[0]);
				}
				if (dir_2 == USB_RX) {
					ep_num2 = (dir_1 == USB_RX) ? EP_RX[1] : EP_RX[0];
					dev_prepare_gpd(STRESS_GPD_TH, USB_RX, ep_num2,
							g_loopback_buffer[0]);
				} else {
					ep_num2 = (dir_1 == USB_TX) ? EP_TX[1] : EP_TX[0];
					dev_prepare_gpd(STRESS_GPD_TH, USB_TX, ep_num2,
							g_loopback_buffer[0]);
				}
				...
				while (run_test) {
					...
					if (run_stop) {
						...
						dev_prepare_gpd(STRESS_GPD_TH, dir, ep_num,
								g_loopback_buffer[0]);
			...
			case STOP_QMU_STATE:
				...
				dev_prepare_gpd(STRESS_GPD_TH, USB_RX, EP_RX[0],
						g_loopback_buffer[0]);
				dev_prepare_gpd(STRESS_GPD_TH, USB_TX, EP_TX[0],
						g_loopback_buffer[0]);
				dev_prepare_gpd(STRESS_GPD_TH, USB_TX, EP_TX[1],
						g_loopback_buffer[0]);
				...
				while (1) {
					...
					if (method == 2) {
						...
						dev_prepare_gpd_short(STRESS_GPD_TH, USB_TX,
								      EP_TX[0],
								      g_loopback_buffer[0]);
			...
			case RX_ZLP_STATE:
				...
				dev_qmu_rx(EP_RX[0]);
				os_printk(K_CRIT,
					  "rx_zlp:[%d], rx_coz:[%d], rx_done_count:[%d], rx_IOC_count:[%d]\r\n",
					  cfg_rx_zlp_en, cfg_rx_coz_en, rx_done_count,
					  rx_IOC_count);
			...
			case DEV_NOTIFICATION_STATE:
				...
				dev_notification(notif_info->type, notif_info->valuel,
						 notif_info->valueh);
				break;
...
DEV_INT32 otg_top(int argc, char **argv)
{
	...
	if (g_otg_mode) {
		...
		{
			kthread_run(otg_hnp_back_role_b_thread, NULL,
				    "otg_hnp_back_thread_role_b\n");
		...
		{
			kthread_run(otg_hnp_back_role_a_thread, NULL,
				    "otg_hnp_back_thread_role_a\n");
