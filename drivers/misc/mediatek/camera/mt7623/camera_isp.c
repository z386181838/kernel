...
{	
    MUINT32 i = 0;
    MUINT32 err_ctrl = (MUINT32)(ISP_RD32(0xF50043A4));
...
static MINT32 CAM_CLK_DumpRegToProc(struct file *file, char __user *pPage, size_t Count,
				   loff_t *ppos)
{
    return 0;
}
...
static MINT32  CAM_CLK_RegDebug(struct file *pFile, const char __user *pBuffer, size_t Count,
			      loff_t *ppos)
{
    char RegBuf[64];
    MUINT32 CopyBufSize = (Count < (sizeof(RegBuf) - 1)) ? (Count) : (sizeof(RegBuf) - 1);
...
static const struct file_operations g_cam_clk_fops = {
	.owner = THIS_MODULE,
	.read = CAM_CLK_DumpRegToProc,
	.write = CAM_CLK_RegDebug
};
static MINT32 __init ISP_Init(MVOID)
{
    ...
    pEntry = proc_create("driver/isp_reg", 0, NULL, &g_isp_reg_fops);
	if (!pEntry) {
		LOG_ERR("add /proc/driver/isp_reg entry fail");
	}

    pEntry = proc_create("driver/camio_reg", 0, NULL, &g_camio_reg_fops);
	if (!pEntry) {
		LOG_ERR("add /proc/driver/camio_reg entry fail");
	}
	
	pEntry = proc_create("driver/cam_clk", 0, NULL, &g_cam_clk_fops);
	if (!pEntry) {
		LOG_ERR("add /proc/driver/camio_reg entry fail");
	}
