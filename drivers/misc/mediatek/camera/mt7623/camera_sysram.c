...
#include <linux/camera_sysram.h>
#include <mach/mt_reg_base.h>
#include <camera_sysram_imp.h>
...
static const struct file_operations g_sysram_fops = {
	.owner = THIS_MODULE,
	.read = SYSRAM_DumpLayoutToProc
};

static const struct file_operations g_sysram_flag_fops = {
	.owner = THIS_MODULE,
...
static int __init SYSRAM_Init(void)
{
    ...
    //
    pEntry = proc_create("sysram", 0, NULL, &g_sysram_fops);
	if (!pEntry) {
		LOG_ERR("add /proc/sysram entry fail.");
	}
	//
	pEntry = proc_create("sysram_flag", 0, NULL, &g_sysram_flag_fops);
	if (!pEntry) {
