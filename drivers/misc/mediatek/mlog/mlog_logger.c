...
#define M_FILTER_ALL        (M_MEMFREE | M_SWAPFREE | M_CACHED | M_GPUUSE | M_MLOCK | M_ZRAM | M_ACTIVE | M_INACTIVE | M_SHMEM)
	...
	if (buddyinfo_filter) {
		...
		for (i = 0; i < 2; ++i) {
			...
			for (j = 0; j < MAX_ORDER - 2; ++j) {
				...
				/* pr_info("zmj: insert oder string format: %s\n", order_str); */
			}
...
void mlog_buddyinfo(void)
{
	...
	spin_lock_bh(&mlogbuf_lock);
	/* pr_info("[mlog]: normal zone:\n"); */
	for (order = 0; order < MAX_ORDER; ++order) {
		/* pr_info("%6lu ", normal_nr_free[order]); */
		mlog_emit_32(normal_nr_free[order]);
	}
	/* pr_info("\n"); */

	/* pr_info("[mlog]: high zone:\n"); */
	for (order = 0; order < MAX_ORDER; ++order) {
		mlog_emit_32(high_nr_free[order]);
		/* pr_info("%6lu ", high_nr_free[order]); */
	}
	/* pr_info("\n"); */
	spin_unlock_bh(&mlogbuf_lock);
}
...
static void mlog_procinfo(void)
{
	...
	for_each_process(tsk) {
		...
		/* pr_info("[mlog] p->pid: %d, p->signal->oom_score_adj:%d, oom_score_adj: %d\n", */
		/* p->pid, p->signal->oom_score_adj, oom_score_adj); */
#else
		oom_score_adj = p->signal->oom_score_adj;
		/* pr_info("[mlog] p->pid: %d, p->signal->oom_score_adj: %d\n", */
		/* p->pid, p->signal->oom_score_adj); */
#endif
