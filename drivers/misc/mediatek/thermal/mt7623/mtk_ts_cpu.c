...
static void thermal_reset_and_initial(void)
{
	...
	printk(KERN_CRIT  "[Reset and init thermal controller]\n");
...
int P_static(void)
{
	...
	mtktscpu_dprintk("P_static ,Num_of_OPP=%d\n",Num_of_OPP);
...
static ssize_t mtktscpu_read_opp(struct file *file, char *buf, size_t count, loff_t *data)
{
...
static ssize_t mtktscpu_read_temperature(struct file *file, char *buf, size_t count, loff_t *data)
{
	static unsigned char con = 0;
	char *p = buf;
...
static ssize_t mtktscpu_read_log(struct file *file, char *buf, size_t count, loff_t *data)
{
	static unsigned char con = 0;
	char *p = buf;
...
static ssize_t mtktscpu_read_cal(struct file *file, char *buf, size_t count, loff_t *data)
...
static ssize_t mtktscpu_read(struct file *file, char *buf, size_t count, loff_t *data)
{
	static unsigned char con = 0;
	char *p = buf;
...
static ssize_t mtktscpu_write_log(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	char desc[32];
	int log_switch;
	int len = 0;
	...
	if (sscanf(desc, "%d", &log_switch) == 1)
	{
		...
		printk("[mtktscpu_write_log] mtktscpu_debug_log=%d\n", mtktscpu_debug_log);
        return count;
	}
	else
	{
		//mtktscpu_dprintk("[mtktscpu_write_log] bad argument\n");
	    printk("[mtktscpu_write_log] bad argument\n");
    }
	return -EINVAL;
}

#if CPT_ADAPTIVE_AP_COOLER
static ssize_t mtktscpu_read_dtm_setting(struct file *file, char *buf, size_t count, loff_t *data)
{
	static unsigned char con = 0;
	char *p = buf;
    int i;
...
static ssize_t mtktscpu_write_dtm_setting(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	char desc[128];
	//char arg_name[32] = {0};
	//int arg_val = 0;
...
#if 1 //pause periodic temperature measurement
    	con = 0;
    	while(con < 10) //if bit7 and bit0=0
    	{
    		...
    		if(((temp & 0x81) == 0x00) || ((temp & 0x81) == 0x81)){
    			/*
       			Pause periodic temperature measurement for sensing point 0,sensing point 1,sensing point 2
              		*/
    			THERMAL_WRAP_WR32((temp | 0x0E), TEMPMSRCTL1);//set bit1=bit2=bit3=1 to pause sensing point 0,1,2

    			break;
    		}
	    	mtktscpu_dprintk("temp=0x%x, con=%d\n",temp,con);
    		udelay(10);
    		con++;
    	}
#endif
...
//	set_high_low_threshold(20000, 10000);//test
	...
	*/
	...
	else
	{
		...
		if (!entry)
		{
			mtktscpu_dprintk("[mtktscpu_init]: create /proc/mtktscpu/mtktscpu failed\n");
		}

		entry = proc_create("mtktscpu_log", S_IRUGO | S_IWUSR, mtktscpu_dir, &mtktscpu_log_fops);
		if (!entry)
		{
			mtktscpu_dprintk("[mtktscpu_init]: create /proc/mtktscpu/mtktscpu_log failed\n");
		}

		entry = proc_create("mtktscpu_opp", S_IRUGO | S_IWUSR, mtktscpu_dir, &mtktscpu_opp_fops);
		if (!entry)
		{
			mtktscpu_dprintk("[mtktscpu_init]: create /proc/mtktscpu/mtktscpu_opp failed\n");
		}
		
		entry = proc_create("mtktscpu_cal", S_IRUGO | S_IWUSR, mtktscpu_dir, &mtktscpu_cal_fops);
		if (!entry)
		{
			mtktscpu_dprintk("[mtktscpu_init]: create /proc/mtktscpu/mtktscpu_cal failed\n");
		}

		entry = proc_create("mtktscpu_temperature", S_IRUGO | S_IWUSR, mtktscpu_dir, &mtktscpu_temperature_fops);
		if (!entry)
		{
			mtktscpu_dprintk("[mtktscpu_init]: create /proc/mtktscpu/mtktscpu_temperature failed\n");
		}

#if CPT_ADAPTIVE_AP_COOLER
		entry = proc_create("mtktscpu_dtm_setting", S_IRUGO | S_IWUSR, mtktscpu_dir, &mtktscpu_dtm_setting_fops);
		if (NULL != entry)
		{
		...
		{
			mtktscpu_dprintk("[mtktscpu_init]: create /proc/mtktscpu/mtktscpu_dtm_setting failed\n");
		}
#endif
	}
#endif
...
#ifndef CONFIG_MT7623_FPGA	/* FPGA workaround */
