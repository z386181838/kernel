...
static void pmic_cali_prepare(void)
{
	...
	printk("Power/PMIC_Thermal: Reg(0x1E2)=0x%x, Reg(0x1EA)=0x%x, Reg(0x1EC)=0x%x\n", temp0, temp1, temp2);
...
static ssize_t mtktspmic_read_log(struct file *file, char *buf, size_t count, loff_t *data)
{
	static unsigned char con = 0;
	char *p = buf;
	...
	if (con)
	{
		p += sprintf(p, "[mtktspmic_read_log] log = %d\n", mtktspmic_debug_log);
	}
...
static ssize_t mtktspmic_write_log(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	char desc[32];
	int log_switch;
	int len = 0;
	...
	if (sscanf(desc, "%d", &log_switch) == 1)
	{
		...
		mtktspmic_dprintk("[mtktspmic_write_log] mtktspmic_debug_log=%d\n", mtktspmic_debug_log);
		return count;
	}
	else
	{
...
static int __init mtktspmic_init(void)
{
	...
	else
	{
		...
		if (!entry)
		{
			mtktspmic_dprintk("[mtktspmic_init]: create /proc/mtktspmic/mtktspmic failed\n");
		}

		entry = proc_create("mtktspmic_log", S_IRUGO | S_IWUSR, mtktspmic_dir, &mtktspmic_log_fops);
		if (!entry)
		{
			mtktspmic_dprintk("[mtktspmic_init]: create /proc/mtktspmic/mtktspmic_log failed\n");
		}
	}
