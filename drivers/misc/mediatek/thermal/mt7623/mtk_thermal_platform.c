...
/* Number of CPU CORE */
#if defined(CONFIG_ARCH_MT6589) || defined(CONFIG_ARCH_MT8127) || defined(CONFIG_ARCH_MT7623) 
...
#if defined(CONFIG_ARCH_MT8127) || defined(CONFIG_ARCH_MT7623)  
...
#elif defined(CONFIG_ARCH_MT6589) || defined(CONFIG_ARCH_MT8127) || defined(CONFIG_ARCH_MT7623)
...
static ssize_t mtk_thermal_validation_rd(struct file *file, char *buf, size_t count, loff_t *data)
{
...
static ssize_t mtk_thermal_validation_wr(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	char desc[32];
	int check_switch;
	int len = 0;
...
static const struct file_operations mtk_thermal_validation_ops = {
	.owner = THIS_MODULE,
	.read  = mtk_thermal_validation_rd,
...
static int __init mtk_thermal_platform_init(void)
{
    ...
    entry = proc_create("driver/tm_validation", S_IRUGO | S_IWUSR, NULL, &mtk_thermal_validation_ops);
    if (!entry)
    {
        THRML_ERROR_LOG("[mtk_thermal_platform_init] create /proc/driver/tm_validation failed\n");
    }
...
int mtk_thermal_get_cpu_info(
    ...
    int **cpuloading)
{
#if defined(CONFIG_ARCH_MT6577) || defined(CONFIG_ARCH_MT6589) || defined(CONFIG_ARCH_MT8127) || defined(CONFIG_ARCH_MT7623)
    //******************
    // CPU Usage
    //******************
    ...
    int **gpuloading)
{
    ...
    // GPU Index
    //******************
    //for mt7623 case, we has no gpu.
#if (0) && (defined(CONFIG_ARCH_MT8127) || defined(CONFIG_ARCH_MT7623))
    if (nocores)
...
int mtk_thermal_get_extra_info(
    ...
    char ***attr_units)
{
#if (0) && (defined(CONFIG_ARCH_MT6589) || defined(CONFIG_ARCH_MT8127) || defined(CONFIG_ARCH_MT7623))
    int size, i=0;
