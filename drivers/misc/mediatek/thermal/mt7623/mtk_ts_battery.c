...
static int mtktsbattery_get_hw_temp(void)
{
	...
	mtktsbattery_dprintk("[mtktsbattery_get_hw_temp] counter=%d\n", counter);
...
static ssize_t mtktsbattery_read(struct file *file, char *buf, size_t count, loff_t *data)
{
...
static ssize_t mtktsbattery_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
...
static int __init mtktsbattery_init(void)
{
	...
	{
		entry = proc_create("mtktsbattery", S_IRUGO | S_IWUSR, mtktsbattery_dir, &mtktsbattery_fops);
		if (!entry)
		{
			mtktsbattery_dprintk("[mtktsbattery_init]: create /proc/mtktsbattery/mtktsbattery failed\n");
		}
	}
