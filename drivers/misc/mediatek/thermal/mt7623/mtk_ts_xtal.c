...
#if 1
static ssize_t mtktsxtal_log_read(struct file *file, char *buf, size_t count, loff_t *data)
{
...
static ssize_t mtktsxtal_log_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	char desc[32];
	int len = 0;
...
static ssize_t mtktsxtal_read(struct file *file, char *buf, size_t count, loff_t *data)
{
    static unsigned char con = 0;
	char *p = buf;
...
static ssize_t mtktsxtal_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
...
static const struct file_operations mtktsxtal_fops = {
	.owner = THIS_MODULE,
	.read  = mtktsxtal_read,
	.write = mtktsxtal_write,
};

static const struct file_operations mtktsxtal_log_fops = {
	.owner = THIS_MODULE,
	.read  = mtktsxtal_log_read,
	.write = mtktsxtal_log_write,
};
...
static int __init mtktsxtal_init(void)
{
	...
	{
		entry = proc_create("mtktsxtal_log", S_IRUGO | S_IWUSR, mtktsxtal_dir, &mtktsxtal_log_fops);
		if (!entry)
		{
			mtktsxtal_dprintk("[mtktsxtal_init]: create /proc/mtktsxtal/mtktsxtal_log failed\n");
		}

		entry = proc_create("mtktsxtal", S_IRUGO | S_IWUSR, mtktsxtal_dir, &mtktsxtal_fops);
		if (NULL != entry)
		{
		...
		{
			mtktsxtal_dprintk("[mtktsxtal_init]: create /proc/mtktsxtal/mtktsxtal failed\n");
		}
	}
