...
static ssize_t wmt_wifi_tx_thro_read(struct file *file, char *buf, size_t count, loff_t *data)
{
...
/*New Wifi throttling Algo+*/
ssize_t wmt_wifi_algo_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	char desc[MAX_LEN] = {0};
	...
	count = (count < (sizeof(desc) - 1)) ? count : (sizeof(desc) - 1);
...
static ssize_t wmt_wifi_algo_read(struct file *file, char *buf, size_t count, loff_t *data)
{
	static unsigned char con = 0;
	char *p = buf;
...
ssize_t wmt_tm_wfd_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	int ret = 0;
	char tmp[MAX_LEN] = {0};
	...
	wmt_tm_printk("[%s] %s = %d, len=%d, ret=%d\n", __func__, tmp, tm_wfd_stat, count, ret);

	return count;
}

static ssize_t wmt_tm_wfd_read(struct file *file, char *buf, size_t count, loff_t *data)
{
	static unsigned char con = 0;
	char *p = buf;
	...
	if (con)
	{
		p += sprintf(p, "%d", tm_wfd_stat);

		wmt_tm_printk("[%s] %s = %d, len=%d\n", __func__, buf, tm_wfd_stat, p - buf);
	}

	return p - buf;
}

ssize_t wmt_tm_pid_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	int ret = 0;
	char tmp[MAX_LEN] = {0};
...
static ssize_t wmt_tm_pid_read(struct file *file, char *buf, size_t count, loff_t *data)
{
	static unsigned char con = 0;
	char *p = buf;
...
static ssize_t wmt_tm_read(struct file *file, char *buf, size_t count, loff_t *data)
{
    static unsigned char con = 0;
	char *p = buf;
...
static ssize_t wmt_tm_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    int i = 0;
...
static const struct file_operations wmt_tm_fops = {
	.owner = THIS_MODULE,
	.read  = wmt_tm_read,
	.write = wmt_tm_write,
};

static const struct file_operations wmt_tm_pid_fops = {
	.owner = THIS_MODULE,
	.read  = wmt_tm_pid_read,
	.write = wmt_tm_pid_write,
};

static const struct file_operations wmt_wifi_algo_fops = {
	.owner = THIS_MODULE,
	.read  = wmt_wifi_algo_read,
	.write = wmt_wifi_algo_write,
};

static const struct file_operations wmt_wifi_tx_thro_fops = {
	.owner = THIS_MODULE,
	.read  = wmt_wifi_tx_thro_read,
};

static const struct file_operations wmt_tm_wfd_fops = {
	.owner = THIS_MODULE,
...
static int wmt_tm_proc_register(void)
{
    ...
    if (!wmt_tm_proc_dir) {
		...
		entry = proc_create("wmt_tm", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, wmt_tm_proc_dir, &wmt_tm_fops);
		if (!entry)
		{
			wmt_tm_printk("[wmt_tm_proc_register]: create /proc/wmt_tm/wmt_tm failed\n");
		}

		entry = proc_create("tm_pid", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, wmt_tm_proc_dir, &wmt_tm_pid_fops);
		if (!entry)
		{
			wmt_tm_printk("[wmt_tm_proc_register]: create /proc/wmt_tm/tm_pid failed\n");
		}

		entry = proc_create("wmt_val", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, wmt_tm_proc_dir, &wmt_wifi_algo_fops);
		if (!entry)
		{
			wmt_tm_printk("[wmt_tm_proc_register]: create /proc/wmt_tm/wmt_val failed\n");
		}

		entry = proc_create("tx_thro", S_IRUSR | S_IWUSR, wmt_tm_proc_dir, &wmt_wifi_tx_thro_fops);
		if (!entry)
		{
			wmt_tm_printk("[wmt_tm_proc_register]: create /proc/wmt_tm/tx_thro failed\n");
		}

		entry = proc_create("wfd_stat", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, wmt_tm_proc_dir, &wmt_tm_wfd_fops);
		if (!entry)
		{
			wmt_tm_printk("[wmt_tm_proc_register]: create /proc/wmt_tm/wfd_stat failed\n");
		}
    }
    return 0;
}
