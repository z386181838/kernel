...
static ssize_t mtk_mdm_value_read(struct file *file, char *buf, size_t count, loff_t *data)
{
...
static ssize_t mtk_mdm_sw_read(struct file *file, char *buf, size_t count, loff_t *data)
{
    static unsigned char con = 0;
    char *p = buf;
...
static ssize_t mtk_mdm_sw_write(struct file *file, const char *buf, size_t len, loff_t *data)
{
...
static ssize_t mtk_mdm_proc_timeout_read(struct file *file, char *buf, size_t count, loff_t *data)
{
    static unsigned char con = 0;
    char *p = buf;
...
static ssize_t mtk_mdm_proc_timeout_write(struct file *file, const char *buf, size_t len, loff_t *data)
{
...
static int __init mtk_mdm_txpwr_init(void)
{
    ...
    if (!mdtxpwr_dir) {
        ...
        if (!entry) {
            mtk_mdm_dprintk("[mtk_mdm_init]: create /proc/mtk_mdm_txpwr/txpwr_sw failed\n");
        }
        entry = proc_create("txpwr_value", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, mdtxpwr_dir, &mtk_mdm_value_fops);
        if (!entry) {
            mtk_mdm_dprintk("[mtk_mdm_init]: create /proc/mtk_mdm_txpwr/txpwr_value failed\n");
        }
        entry = proc_create("timeout", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, mdtxpwr_dir, &mtk_mdm_proc_timeout_fops);
        if (!entry) {
            mtk_mdm_dprintk("[mtk_mdm_init]: create /proc/mtk_mdm_txpwr/timeout failed\n");
        }
