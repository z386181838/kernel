...
static ssize_t _mtk_cl_bcct_proc_read(struct file *file, char *buf, size_t count, loff_t *data)
{
...
#else
        if (NULL == PDE(file->f_path.dentry->d_inode)->data)
#endif
        {
...
static ssize_t _mtk_cl_bcct_proc_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    int len = 0;
    char desc[128];
...
static int __init mtk_cooler_bcct_init(void)
{
  ...
  /* create a proc file */
  {
    ...
    entry = proc_create_data("driver/mtk-cl-bcct", S_IRUGO | S_IWUSR | S_IWGRP, NULL, &_mtk_cl_bcct_proc_fops, cl_bcct_state);
	...
	{
		mtk_cooler_bcct_dprintk("[mtk_cooler_bcct_init] create /proc/driver/mtk-cl-bcct failed\n");
    }
