...
static void mtk_thermal_ext_set_tz_threshold(struct mtk_thermal_ext_tz_data *tzdata, int idx)
{
	...
	THRML_LOG
	    ("[mtk_thermal_ext_set_tz_threshold] id: %d, polling delay: %d, low trip: %d, high trip: %d\n",
	     idx, tz_threshold.polling_delay, tz_threshold.low_trip_point,
				...
				for (i = 0; i < MTK_THERMAL_EXT_SENSOR_COUNT; i++) {
					if (mtk_thermal_ext_tz_values[i].set) {
						...
						if (cancel_delayed_work
						    ...
						    == 0) {
							THRML_ERROR_LOG
							    ("[thermal start ack] work (tz %d) is running\n",
							     i);
						}
					}
				}
	...
	case THERMAL_MD32_IPI_MSG_REACH_THRESHOLD:
		{
			...
			if (tz != NULL) {
				...
				if (cancel_delayed_work(&(tz->poll_queue)) == 0) {
					THRML_ERROR_LOG
					    ("[thermal reach threshold] work (tz %d) is running\n",
					     tzidx);
				}
			}
			break;
...
static int mtkthermal_read(struct seq_file *m, void *v)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "THERMAL/MONITOR",
		    "[mtkthermal_monitor] enable_ThermalMonitor:%d\n", enable_ThermalMonitor);
...
static int __init mtkthermal_init(void)
{
	...
	if (!entry) {
		THRML_ERROR_LOG
		    ("[mtkthermal_init] Can not create /proc/driver/mtk_thermal_monitor\n");
	}
	...
	if (!entry) {
		THRML_ERROR_LOG
		    ("[mtkthermal_init] Can not create /proc/driver/mtk_thermal_indicator\n");
	}
...
struct thermal_zone_device *mtk_thermal_zone_device_register_wrapper
	...
	if (tzidx >= 0 && tzidx < MTK_THERMAL_EXT_SENSOR_COUNT) {
		...
		THRML_LOG
		    ("[mtk_thermal_ext_tz_values] %s, id: %d, temp: %d, polling delay: %d, low trip: %d, high trip: %d\n",
		     type, tzidx, mtk_thermal_ext_tz_values[tzidx].last_temperature,
		...
		if (g_controlState == MTK_THERMAL_CONTROL_STATE_INTERRUPT ||
			...
			if (g_controlState == MTK_THERMAL_CONTROL_STATE_INTERRUPT &&
				...
				THRML_ERROR_LOG
				    ("[mtk_thermal_zone_device_register_wrapper] cancel tz %d work, work is running\n",
				     type);
			}
		}
