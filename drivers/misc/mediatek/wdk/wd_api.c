...
#endif
#endif
	pr_info(" wd_api disable_local not support now\n");
	return 0;
}

static int set_mode(enum ext_wdt_mode mode)
{
	pr_info("support only irq mode-20140522\n");
	switch (mode) {
	case WDT_DUAL_MODE:
		break;
	...
	case WDT_IRQ_ONLY_MODE:
		pr_info("wd set only irq mode for debug\n");
...
{
	int res = 0;
	pr_info("thermal_direct_mode_config(en:0x%x,mode:0x%x)\n", en, mode);
	if (WD_REQ_EN == en) {
		/* g_ext_wd_drv.reques_en_set(MTK_WDT_REQ_SPM_THERMAL_MARK,WD_REQ_EN); */
#ifdef CONFIG_MTK_WD_KICKER
...
static unsigned int wd_get_check_bit(void)
{
	pr_info("dummy wd_get_check_bit");
	return 0;
}

static unsigned int wd_get_kick_bit(void)
{
	pr_info("dummy wd_get_kick_bit");
	return 0;
}

static int wd_restart(enum wd_restart_type type)
{
	pr_info("dummy wd_restart");
	return 0;
}
...
{
	int res = 0;
	pr_info("dummy wd_cpu_hot_plug_on_notify");
	return res;
}
...
{
	int res = 0;
	pr_info("dummy wd_cpu_hot_plug_off_notify");
	return res;
}

static int wd_sw_reset(int type)
{
	pr_info("dummy wd_sw_reset");
#ifdef CONFIG_MTK_WD_KICKER
	wdt_arch_reset(type);
#endif
	return 0;
...
static int mtk_wk_wdt_config(enum ext_wdt_mode mode, int timeout_val)
{
	...
	pr_info("dummy mtk_wk_wdt_config");
	return 0;
}

static int disable_ext(void)
{
	pr_info("dummy disable_ext");
	return 0;
}

static int disable_local(void)
{
	pr_info("dummy disable_local");
	return 0;
}

static int set_mode(enum ext_wdt_mode mode)
{
	pr_info("dummy set_mode");
	return 0;
...
static int confirm_hwreboot(void)
{
	pr_info("dummy confirm_hwreboot");
	return 0;
}

static void suspend_notify(void)
{
	pr_info("dummy suspend_notify\n");
}

static void resume_notify(void)
{
	pr_info("dummy resume_notify\n");
}

static int disable_all_wd(void)
{
	pr_info("dummy disable_all_wd\n");
	return 0;
}
...
{
	int res = 0;
	pr_info("dummy spmwdt_mode_config\n");
	return res;
}
...
{
	int res = 0;
	pr_info("dummy thermal_mode_config\n");
	return res;
}
...
{
	int res = 0;
	pr_info("dummy wd_dram_reserved_mode\n");
	return res;
}
...
{
	int res = 0;
	pr_info("thermal_direct_mode_config in dummy driver (en:0x%x,mode:0x%x)\n", en, mode);
	if (WD_REQ_EN == en) {
		/* g_ext_wd_drv.reques_en_set(MTK_WDT_REQ_SPM_THERMAL_MARK,WD_REQ_EN); */
#ifdef CONFIG_MTK_WD_KICKER
		res = mtk_wdt_request_en_set(MTK_WDT_REQ_THERMAL_MARK, WD_REQ_EN);
...
int wd_api_init(void)
{
	...
	for (i = 1; i < api_size; i++) {
		pr_info("p[%d]=%x\n", i, *(check_p + i));
		if (0 == check_p[i]) {
			pr_info("wd_api init fail the %d api not init\n", i);
...
int get_wd_api(struct wd_api **obj)
{
	...
	*obj = &g_wd_api_obj;
	if (NULL == *obj) {
		res = -1;
		/* pr_info("get_wd_public_interface_obj null pointer error\n"); */
	}
	if ((*obj)->ready == 0) {
		res = -2;
		/* pr_info("get_wd_public_api not ready\n"); */
	}
	return res;
}
