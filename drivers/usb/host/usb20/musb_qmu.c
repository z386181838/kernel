...
int musb_qmu_init(struct musb *musb)
{
	...
	printk("before-----iocdisr = %d \n",musb_readl((void __iomem*)(mtk_musb->mregs + MUSB_QISAR),0x30));
	musb_writel((void __iomem*)(mtk_musb->mregs + MUSB_QISAR),0x30,0);
	printk("after-----iocdisr = %d \n",musb_readl((void __iomem*)(mtk_musb->mregs + MUSB_QISAR),0x30));
...
//jingao:add tmp
extern void mtk_qmu_insert_task_ioc(u8 ep_num, u8 isRx, u8* buf, u32 length, u8 zlp,u8 ioc);
...
void musb_kick_D_CmdQ(struct musb *musb, struct musb_request *request)
{
    ...
    mtk_qmu_insert_task(request->epnum,
		...
		//musb_writel(musb->mregs,USB_L1INTM, musb_readl(musb->mregs,USB_L1INTM)| QINT_STATUS);
