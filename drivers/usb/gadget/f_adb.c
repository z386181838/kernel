...
static void adb_debug_read_copy_to_user(char __user *buf, struct usb_request *req)
{
	...
	if (dbg != NULL && dbg->command == A_DBUG && dbg->headtoken == DBGHEADTOKEN
		...
		pr_info("adb_read A_DBUG (0x%x) (0x%x) (0x%x)\n", dbg->command, dbg->msg_check,
			dbg->data_check);
	}
}
...
static struct usb_request *adb_request_new(struct usb_ep *ep, int buffer_size)
{
	...
	if (!req->buf) {
		pr_err(XLOG_TAG "%s %s %d: kmalloc failed\n", __FILE__,
			    __func__, __LINE__);
				     ...
				     struct usb_endpoint_descriptor *out_desc)
{
 ...
 fail:
	pr_err("%s %s %d: adb_bind() could not allocate requests\n", __FILE__, __func__, __LINE__);
	return -1;
}
...
static ssize_t adb_read(struct file *fp, char __user *buf, size_t count, loff_t *pos)
{
	...
	if (count > ADB_BULK_BUFFER_SIZE) {
		pr_err(XLOG_TAG "%s %s %d: count > ADB_BULK_BUFFER_SIZE\n",
			    __FILE__, __func__, __LINE__);
		return -EINVAL;
	}

	if (adb_lock(&dev->read_excl)) {
		pr_err(XLOG_TAG "%s %s %d: Failed due to lock busy\n",
			    __FILE__, __func__, __LINE__);
		return -EBUSY;
	}
 ...
 requeue_req:
	...
	if (ret < 0) {
		...
		pr_err(XLOG_TAG "%s %s %d: failed to queue req %p (%d)\n",
		...
		if (bitdebug_enabled == 1) {
			if (count == sizeof(amessage)) {
				...
				if (msg != NULL) {
					...
					case A_WRTE:
					case A_AUTH:
						/* pr_info("adb: adb_read (0x%x) (0x%x) (0x%x) (0x%x) (0x%x) (0x%x)\n", msg->command, msg->arg0, msg->arg1, */
						/* msg->data_length, msg->data_check, msg->magic); */
						break;
					default:
						/* pr_info("adb_read msg A_DATA\n"); */
						break;
					}
				}
			}
			...
			if (bitdebug_enabled == 1) {
				if (count == sizeof(amessage)) {
					...
					if (msg != NULL) {
						...
						case A_WRTE:
						case A_AUTH:
							/* pr_info("adb_write msg (0x%x) (0x%x) (0x%x) (0x%x) (0x%x) (0x%x)\n", msg->command, msg->arg0, msg->arg1, */
							/* msg->data_length, msg->data_check, msg->magic); */
							if (flow_state == 0) {
								flow_state = 1;
								/* no data packet */
				...
				if (data == true && bitdebug_enabled == 1) {
					if (flow_state == 1) {
						...
						pr_info("adb_write flow state data warning\n");
					}
					/* pr_info("adb_write data\n"); */
				}
			} else {
				if (count == sizeof(debuginfo)) {
					debuginfo *dbg = (debuginfo *) req->buf;
...
static int adb_open(struct inode *ip, struct file *fp)
{
	...
	/*pr_info("[adb]adb_open start, adb_open: %p check adb_release %p, open_release_pair: %d\n",
		adb_fops.open, adb_fops.release, open_release_pair);*/

	if (!_adb_dev) {
		pr_err(XLOG_TAG
			    "[adb]adb_open _adb_dev is NULL, open_release_pair: %d\n",
			    open_release_pair);
		open_release_pair = 0;
		ret = ENODEV;
		goto OPEN_END;
	...
	if (open_release_pair > 0) {
		pr_warn(XLOG_TAG
			    "[adb] open twice, %s %s %d: open_release_pair count: %d\n", __FILE__,
			    __func__, __LINE__, open_release_pair);
		ret = 0;
 ...
 OPEN_END:
	/*pr_info("[adb]adb_open end, open_release_pair: %d", open_release_pair);*/
...
static int adb_release(struct inode *ip, struct file *fp)
{
	...
	if (open_release_pair < 1) {
		pr_warn(XLOG_TAG
			    "[adb] close an unopened device, %s %s %d: open_release_pair count: %d\n",
 ...
 RELEASE_END:
	...
	/*pr_info("[adb]adb_release end, open_release_pair: %d", open_release_pair);*/
...
static int adb_function_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (ret) {
		pr_err(XLOG_TAG "%s %s %d: usb_ep_enable in failed\n",
			    __FILE__, __func__, __LINE__);
		return ret;
	}
	...
	if (ret) {
		...
		pr_err(XLOG_TAG "%s %s %d: usb_ep_enable out failed\n",
			    __FILE__, __func__, __LINE__);
		return ret;
	}
	dev->online = 1;
...
static int adb_setup(void)
{
 ...
 err:
	kfree(dev);
	pr_err("%s %s %d: adb gadget driver failed to initialize\n", __FILE__, __func__, __LINE__);
	return ret;
}
