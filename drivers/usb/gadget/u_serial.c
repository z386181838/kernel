...
static void UDVT_Direct_WriteComplete(struct gs_port *port)
{
  unsigned long	flags;
  port->port_write_direct.buf_size = 0;
  port->port_Active_write_buf_p = &port->port_write_buf;
  *(port->directCompleteSem) = 1;
  return;  
}
 ...
 */
static void UDVT_Direct_ReadComplete(struct gs_port		*port)
{
  unsigned long	flags;
...
//			pr_debug("gs_open: ttyGS%d (%p,%p) no buffer\n",port->port_num, tty, file);
  ...
  port->port_read_direct.buf_size = 0;
	...
	if (port->port_usb) {
		...
		//printk("UDVT_open: start ttyGS%d\n", port->port_num);
		gs_start_io(port);
...
int UDVT_Buffed_Write(int portn, const unsigned char *buf, int count)
{
	...
	int		status;
  port = ports[portn].port;
	//printk("UDVT_Buffed_Write: ttyGS%d () writing %d bytes\n",port->port_num, count);
	spin_lock_irqsave(&port->port_lock, flags);
	if (count)
		count = gs_buf_put(&port->port_write_buf, buf, count);
	if (port->port_usb)
...
int UDVT_Buffed_Read(int portn, unsigned char *buf, int count)
{
  ...
  port = ports[portn].port;
	...
	if (count)
		...
		if ( count)
			{
		//printk("UDVT_Buffed_Read:count =%d,packet = 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x \n",count,((&port->port_read_buf)->buf_buf)[0],((&port->port_read_buf)->buf_buf)[1],((&port->port_read_buf)->buf_buf)[2],((&port->port_read_buf)->buf_buf)[3],((&port->port_read_buf)->buf_buf)[4],((&port->port_read_buf)->buf_buf)[5]);
        //printk("UDVT_Buffed_Read:count =%d,packet buf %p  = 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x \n",count,buf,buf[0],buf[1],buf[2],buf[3],buf[4],buf[5]);
			}
	}
	spin_unlock_irqrestore(&port->port_lock, flags);
	return count;  
...
int UDVT_Direct_Write(int portn, unsigned char *buf, int count,int *completesem)
{
  ...
  unsigned long	flags;
  port = ports[portn].port;
	//printk("UDVT_Direct_Write: ttyGS%d () writing %d bytes\n",port->port_num, count);
	spin_lock_irqsave(&port->port_lock, flags);
	if (count > 0)
	{
