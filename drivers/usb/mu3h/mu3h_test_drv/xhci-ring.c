		...
		struct xhci_event_cmd *event)
{
	...
	xhci_err(xhci, "cmd : %d\n", ((xhci->cmd_ring->dequeue->generic.field[3] & TRB_TYPE_BITMASK)>>10));
	...
	case TRB_TYPE(TRB_ENABLE_SLOT):
		if (GET_COMP_CODE(event->status) == COMP_SUCCESS){
			...
			printk("1.cmd_status=%d\n", g_cmd_status);
		}
		else{
			g_slot_id = 0;
			g_cmd_status = CMD_FAIL;
			printk("2.cmd_status=%d\n", g_cmd_status);
		}
		break;
	...
	case TRB_TYPE(TRB_CONFIG_EP):
		if(GET_COMP_CODE(event->status) == COMP_SUCCESS){
			...
			g_cmd_status = CMD_DONE;
			printk("7.cmd_status=%d\n", g_cmd_status);
		}
		else{
			g_cmd_status = CMD_FAIL;
			printk("8.cmd_status=%d\n", g_cmd_status);
		}
		break;
	case TRB_TYPE(TRB_RESET_DEV):
		if(GET_COMP_CODE(event->status) == COMP_SUCCESS){
			...
			g_cmd_status = CMD_DONE;
			printk("9.cmd_status=%d\n", g_cmd_status);
		}
		else{
			xhci_dbg(xhci, "reset dev failed, code: %d\n", GET_COMP_CODE(event->status));
			g_cmd_status = CMD_FAIL;
			printk("10.cmd_status=%d\n", g_cmd_status);
		}
		break;
	case TRB_TYPE(TRB_STOP_RING):
		...
		if(GET_COMP_CODE(event->status) == COMP_SUCCESS){
					...
					g_cmd_status = CMD_DONE;
					printk("11.cmd_status=%d\n", g_cmd_status);
				}
				else{
					xhci_dbg(xhci, "stop ring failed, code: %d\n", GET_COMP_CODE(event->status));
					g_cmd_status = CMD_FAIL;
					printk("12.cmd_status=%d\n", g_cmd_status);
		}
	...
	case TRB_TYPE(TRB_SET_DEQ):
		...
		if(GET_COMP_CODE(event->status) == COMP_SUCCESS){
				...
				g_cmd_status = CMD_DONE;
				printk("13.cmd_status=%d\n", g_cmd_status);
			}
			else{
				xhci_dbg(xhci, "stop ring failed, code: %d\n", GET_COMP_CODE(event->status));
				g_cmd_status = CMD_FAIL;
				printk("14.cmd_status=%d\n", g_cmd_status);
		}
		break;
	case TRB_TYPE(TRB_EVAL_CONTEXT):
		...
		if(GET_COMP_CODE(event->status) == COMP_SUCCESS){
				g_cmd_status = CMD_DONE;
				printk("15.cmd_status=%d\n", g_cmd_status);
			}
			else{
				xhci_dbg(xhci, "eval context, code: %d\n", GET_COMP_CODE(event->status));
				g_cmd_status = CMD_FAIL;
				printk("16.cmd_status=%d\n", g_cmd_status);
		}
		break;
...
//	rh_port_clear_change(xhci, port_id);
	...
	if(port_status & (USB_PORT_STAT_C_CONNECTION << 16)){
		...
		if(port_status & USB_PORT_STAT_CONNECTION){
			xhci_err(xhci, "[OTG_H]connect port status event, connected\n");
			g_port_id = port_id;
			g_port_connect = true;
...
//	xhci_dbg(xhci, "td->urb 0x%x\n", td->urb);
	...
	case COMP_MISSED_INT:
		...
		printk("***** Miss service interval error, set skip flag *****\n");
