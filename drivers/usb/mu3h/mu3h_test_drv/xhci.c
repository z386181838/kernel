...
irqreturn_t xhci_mtk_irq(struct usb_hcd *hcd){
	...
	if(temp3 & SSUSB_CHG_A_ROLE_A){
		...
		writel((readl(SSUSB_OTG_STS_CLR) | SSUSB_CHG_A_ROLE_A_CLR), SSUSB_OTG_STS_CLR);
		//set host sel
	...
	if(temp3 & SSUSB_CHG_B_ROLE_A){
		...
		writel((readl(SSUSB_OTG_STS_CLR) | SSUSB_CHG_B_ROLE_A_CLR), SSUSB_OTG_STS_CLR);			
		spin_unlock(&xhci->lock);
		return IRQ_HANDLED;
	}
	if(temp3 & SSUSB_SRP_REQ_INTR){
		...
		enableXhciAllPortPower(xhci);
		writel((readl(SSUSB_OTG_STS_CLR) | SSUSB_SRP_REQ_INTR_CLR), SSUSB_OTG_STS_CLR);
...
irqreturn_t xhci_mtk_irq(struct usb_hcd *hcd){
	...
	u32 temp;
	u32 otg_status;
	u32 __iomem *otgstsaddr;
	u32 __iomem *otgclraddr;
	...
	otgstsaddr = (u32 __iomem *)(u3h_hw->ippc_virtual_base +SSUSB_OTG_STS);
	otgclraddr = (u32 __iomem *)(u3h_hw->ippc_virtual_base +SSUSB_OTG_STS_CLR);
	...
	spin_lock(&xhci->lock);
#if TEST_OTG
		otg_status = readl(otgstsaddr);
		printk(KERN_ERR "[OTG_H][IRQ] OTG_STS 0x%x\n", otg_status);
		#if 0
		if(otg_status & SSUSB_ATTACH_A_ROLE){
			//attached as device-A, turn on port power of all xhci port
			enableXhciAllPortPower(xhci);
			writel(SSUSB_ATTACH_A_ROLE_CLR, otgclraddr);			
			spin_unlock(&xhci->lock);
			return IRQ_HANDLED;
		}
		#endif
		if(otg_status & SSUSB_CHG_A_ROLE_A){
			g_otg_hnp_become_host = true;
			writel(SSUSB_CHG_A_ROLE_A_CLR, otgclraddr);
			//set host sel
			printk(KERN_ERR "[OTG_H] going to set dma to host\n");
			g_otg_wait_con = true;
			...
			//}
			while(readl(otgstsaddr) & SSUSB_DEV_DMA_REQ){
			}
			printk(KERN_ERR "[OTG_H] can set dma to host\n");
			temp = readl(u2_ctrl_addr);
			temp = temp | SSUSB_U2_PORT_HOST_SEL;
			writel(temp, u2_ctrl_addr);		
			//printk(KERN_ERR "[OTG_H][IRQ] SSUSB_U2_PORT_HOST_SEL 0x%x\n", readl(u2_ctrl_addr));
					//enable irq_en
		temp = xhci_readl(xhci, &xhci->ir_set->irq_pending);
		xhci_writel(xhci, ER_IRQ_ENABLE(temp),
			&xhci->ir_set->irq_pending);
		...
		if(otg_status & SSUSB_CHG_B_ROLE_A){
			g_otg_hnp_become_dev = true;
			writel((readl(otgclraddr) | SSUSB_CHG_B_ROLE_A_CLR), otgclraddr);
			spin_unlock(&xhci->lock);
			return IRQ_HANDLED;
		}
		if(otg_status & SSUSB_SRP_REQ_INTR){
			//set port_power
			enableXhciAllPortPower(xhci);
			writel((readl(otgclraddr) | SSUSB_SRP_REQ_INTR_CLR), otgclraddr);		
			spin_unlock(&xhci->lock);
			return IRQ_HANDLED;
		}
#endif
