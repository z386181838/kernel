...
static int t_dev_test_suite(int argc, char** argv);
...
//	{"dbg.sch", &dbg_sch_algorithm},
	...
	{"dev.lpm", &t_dev_lpm},
	{"dev.init", &t_dev_init},
	{"dev.test_suite", &t_dev_test_suite},
	{"", NULL},
	//{NULL, NULL}
};
				...
				for(maxp_index = 0; maxp_index < maxp_arr_size; maxp_index++){
					...
					for(mult_index = 0; mult_index < mult_arr_size; mult_index++){
						...
						for(burst_index = 0; burst_index < burst_arr_size; burst_index++){
							...
							/* if XHCI_DEBUG = 1, the debug log is so much that it 
							 * affects the timing : 
							 * when we read the port status, it is in connected 
							 * already and the disconnected status is ignored
							 * so, if XHCI_DEBUG = 1, we only wait the device connect
							 *  to the host. 
							 */
#if XHCI_DEBUG
							ret = f_enable_port(0);
				...
				for(interval_index = 0; interval_index < interval_arr_size; interval_index++){
					...
					for(mult_index = 0; mult_index < mult_arr_size; mult_index++){
						...
						for(burst_index = 0; burst_index < burst_arr_size; burst_index++){
							 ...
							 * when we read the port status, it is in connected 
							 * already and the disconnected status is ignored
							 * so, if XHCI_DEBUG = 1, we only wait the device 
							 * connect to the host. 
							 */
#if XHCI_DEBUG
							ret = f_enable_port(0);
...
static int t_u3auto_stress(int argc, char** argv){
	...
	if(dev_num == 0){
		...
		/* if XHCI_DEBUG = 1, the debug log is so much that it affects the timing : 
		 * when we read the port status, it is in connected already and the disconnected 
		 * status is ignored
		 * so, if XHCI_DEBUG = 1, we only wait the device connect to the host. 
		 */
#if XHCI_DEBUG
...
static int t_hub_configuredevice(int argc, char** argv){	
	...
	if(argc > 1){	//hub number
		hub_num = (int)simple_strtol(argv[1], &argv[1], 10);
		//xhci_info(xhci, "hub_num set to %d\n", hub_num);
		printk("hub_num set to %d\n", hub_num);

	}
	if(argc > 2){	//port number
		port_num = (int)simple_strtol(argv[2], &argv[2], 10);
		//xhci_info(xhci, "port_num set to %d\n", port_num);
		printk("port_num set to %d\n", port_num);

	}
	if(argc > 3){	//device number
		dev_num = (int)simple_strtol(argv[3], &argv[3], 10);
		//xhci_info(xhci, "dev_num set to %d\n", dev_num);
		printk("dev_num set to %d\n", dev_num);
...
static int t_dev_test_suite(int argc, char** argv)
{
	...
	int toggle_flag = 0;
	char command[256];
	char *test_suite_commands[] = {
#if 1 /* SS loopback */
	"auto.lbscan ss bulk", "auto.lbscan ss intr", 
	"auto.lbscan ss isoc", 
	"auto.lbctrl 0 0 ss", "auto.lbsgscan ss bulk", "auto.lbsgscan ss intr",
	"auto.lbscan ss intr true", 
	//"auto.lbscan ss isoc true",
#endif
#if 1 /* HS loopback */
	"dev.reset hs",
	"auto.lbscan hs bulk", "auto.lbscan hs intr", "auto.lbscan hs isoc", 
	"auto.lbctrl 0 0 hs", "auto.lbsgscan hs bulk", "auto.lbsgscan hs intr", 
	"auto.lbscan hs intr true", "auto.lbscan hs isoc true",
#endif
#if 1 /* FS loopback */
	"dev.reset fs",
	"auto.lbscan fs bulk", "auto.lbscan fs intr", "auto.lbscan fs isoc",
	"auto.lbctrl 0 0 fs", "auto.lbsgscan fs bulk", "auto.lbsgscan fs intr",
	"auto.lbscan fs intr true", "auto.lbscan fs isoc true",
#endif
#if 1 /* suspend/resume, remote wakeup */
	"dev.reset ss",
	"auto.randomsuspend ss 200", "auto.randomwakeup ss 200", 
	"dev.reset hs",
	"auto.randomsuspend hs 200", "auto.randomwakeup hs 200", 
	"dev.reset fs",
	"auto.randomsuspend fs 200", "auto.randomwakeup fs 200",
#endif
	"dev.reset ss",
	NULL};
	...
	while (1) {
		...
		if (argc > 1) {
			if (!strcmp(argv[1], "1"))
				writel(0x10e0e0c, 0xfa240950);
			else if (!strcmp(argv[1], "2")) {
				if (toggle_flag == 0) {
					writel(0x1020200, 0xfa240950);
					toggle_flag = 1;
				}
				else {
					writel(0x10e0e0c, 0xfa240950);
		...
		for (i = 0; test_suite_commands[i] != NULL; i++) {
			temp = readl(0xfa240950);
			printk("0x950=%x\n", temp);
			sprintf(command, "%s", test_suite_commands[i]);
			printk("Test suite command[%s]", command);
			if (call_function_mu3h(command) == RET_FAIL) {
				printk("command [%s] fail\n", test_suite_commands[i]);
				return RET_FAIL;
			}
			else
				printk("command [%s] pass\n", test_suite_commands[i]);
		}
	}
...
printk("==== debug2 ====\n");
	...
	for(u1RxSel = 0; u1RxSel<2; u1RxSel++){
		...
		for(u1TimeDelay=0; u1TimeDelay<32; u1TimeDelay++){
			printk(KERN_DEBUG "Test TxSel[0x%02x] RxSel[0x%02x] PhyDrv[0x%02x] TimingDelay[0x%02x]\n"
				, u1TxSel, u1RxSel, u1PhyDrv, u1TimeDelay);
			//write phase, drv
			u3phy_ops->change_pipe_phase(u3phy, u1PhyDrv, u1TimeDelay);
			enableAllClockPower(dev);	
			//set pipe latch
			addr = (u32 __iomem *)(u3h_hw->ippc_virtual_base + SSUSB_USB3_CSR_OFFSET 
				+ U3H_PIPE_LATCH_SELECT);
			data = (u1TxSel<<2) | u1RxSel;
			writel(data, addr);

			/* Make sure the HC is halted. */
			xhci_dbg(xhci, "Halt HCD\n");
			retval = xhci_halt(xhci);
			if (retval)
				xhci_err(xhci, "ERROR: Halt HCD\n");
			
			/* Reset the internal HC memory state and registers. */
			xhci_dbg(xhci, "Reset HCD\n");
			retval = xhci_reset(xhci);
			if (retval)
				xhci_err(xhci, "ERROR: Reset HCD\n");
			...
			for(i=0; i<200; i++){
				...
				data = readl(addr);
				u1Macltssm = (data & LTSSM);
				
				if(u1TmpMacltssm != u1Macltssm || u1TmpPortPlsVal != u1PortPlsVal){
					printk(KERN_DEBUG "Polling %d PLS=%d LTSSM=%d\n", i, u1PortPlsVal
						, u1Macltssm);
				...
				if(u4CorrectCnt==3){
					...
					addr = (u32 __iomem *)(u3h_hw->u3h_virtual_base + SSUSB_USB3_CSR_OFFSET 
						+ U3H_RECOVERY_COUNT);
					u3h_writelmsk(addr,0x1<<16,CLR_RECOV_CNT)
					u1IsPass = 0;
					//read error count
					addr = (u32 __iomem *)(u3h_hw->u3h_virtual_base + SSUSB_USB3_CSR_OFFSET 
						+ U3H_LINK_ERR_COUNT);
...
static int dbg_printportstatus(int argc, char** argv){
	...
	addr = &xhci->op_regs->port_status_base + NUM_PORT_REGS*((port_id-1) & 0xff);
	temp = xhci_readl(xhci, addr);
	xhci_info(xhci, "port %d = 0x%x, PLS=%d\n", port_id, temp, ((temp >>5) &0x0F));
	return RET_SUCCESS;
}
...
static int __init mu3h_test_init(void)
{
	...
	if(retval < 0)
	{
		printk(KERN_DEBUG "mu3h_test Init failed, %d\n", retval);
		goto fail;
	}
