...
int mtk_xhci_scheduler_remove_ep(struct usb_hcd *hcd, struct usb_device *udev, 
...
int mtk_xhci_scheduler_add_ep(struct usb_hcd *hcd, struct usb_device *udev, struct usb_host_endpoint *ep){
	...
	printk(KERN_DEBUG "add_ep parameters: rh_port_num %d, dev_speed %d, is_in %d, isTT %d \
		, ep_type %d, maxp %d, interval %d, burst %d, mult %d, ep 0x%x, ep_ctx 0x%x\n"
		, rh_port_num, dev_speed, is_in, isTT, ep_type, maxp, interval
		, burst, mult, (unsigned int)ep, (unsigned int)ep_ctx);	
	...
	if(isTT && ep_type == USB_EP_INT && ((dev_speed == USB_SPEED_LOW) || (dev_speed == USB_SPEED_FULL))){
		...
		for(frame_idx=0; frame_idx<frame_interval; frame_idx++){
			...
			if(count_tt_intr_bw(frame_interval, frame_idx, u3h_sch_port) == SCH_SUCCESS){
				...
				bw_cost = maxp;
				bRepeat = 0;
				if(add_sch_ep(rh_port_num, dev_speed, is_in, isTT, ep_type, maxp, frame_interval
					, burst, mult, bOffset, bRepeat, bPkts, bCsCount, bBm, maxp
					, (mtk_u32 *)ep,u3h_sch_port) == SCH_FAIL){
					return SCH_FAIL;
				}
				ret = SCH_SUCCESS;
				break;
		...
		if(best_bw_idx == -1){
			...
			if(add_sch_ep(rh_port_num, dev_speed, is_in, isTT, ep_type, maxp, interval, burst, mult
				, bOffset, bRepeat, bPkts, bCsCount, bBm, bw_cost, (mtk_u32 *)ep
				, u3h_sch_port) == SCH_FAIL){
				return SCH_FAIL;
			}
			ret = SCH_SUCCESS;
		}
		...
		for(frame_idx = 0; (frame_idx < interval) && !break_out; frame_idx++){
			for(repeat = max_repeat; repeat >= 0; repeat--){
				...
				printk(KERN_DEBUG "count_ss_bw, frame_idx %d, repeat %d, td_size %d \
		...
		if(best_bw_idx == -1){
			...
			if(add_sch_ep(rh_port_num, dev_speed, is_in, isTT, ep_type, maxp, interval, burst, mult
				, bOffset, bRepeat, bPkts, bCsCount, bBm, bw_cost, (mtk_u32 *)ep, u3h_sch_port) 
				== SCH_FAIL){
				return SCH_FAIL;
			}
			ret = SCH_SUCCESS;
