...
PHY_INT32 u2_slew_rate_calibration(struct u3phy_info *info)
{
	...
	printk("*** USB port0 slew rate calibration ***\n");
	if (slewrate_cal_flag == false) {
		...
		for(i=0; i<10; i++){
			...
			printk("Port0 FM_OUT value: u4FmOut = %d(0x%08X)\n", u4FmOut, u4FmOut);

			// check if FM detection done 
			if (u4FmOut != 0)
			{
				fgRet = 0;
				printk("Port0 FM detection done! loop = %d\n", i);
		...
		if(u4FmOut == 0){
			...
			printk("Port0 SR calibration value u1SrCalVal = %d\n", (PHY_UINT8)u4Tmp);
			U3PhyWriteField32(((PHY_UINT32)&info->u2phy_regs->usbphyacr5)
...
PHY_INT32 u2_slew_rate_calibration_p1(struct u3phy_info *info)
{
	...
	printk("*** USB port1 slew rate calibration ***\n");
	if (slewrate_cal_flag == false) {
		slewrate_cal_flag = true;
		...
		for(i=0; i<10; i++){
			...
			printk("Port1 FM_OUT value: u4FmOut = %d(0x%08X)\n", u4FmOut, u4FmOut);

			// check if FM detection done 
			if (u4FmOut != 0)
			{
				fgRet = 0;
				printk("Port 1 FM detection done! loop = %d\n", i);
		...
		if(u4FmOut == 0){
			...
			// set reg = (1024/FM_OUT) * 25 * 0.028 (round to the nearest digits)
			u4Tmp = (((1024 * 25 * U2_SR_COEF_7623) / u4FmOut) + 500) / 1000;
			printk("Port1 SR calibration value u1SrCalVal Port1= %d\n", (PHY_UINT8)u4Tmp);
			U3PhyWriteField32(((PHY_UINT32)&info->u2phy_regs_p1->usbphyacr5)
				, RG_USB20_HSTX_SRCTRL_OFST, RG_USB20_HSTX_SRCTRL, u4Tmp);
		}
	}
