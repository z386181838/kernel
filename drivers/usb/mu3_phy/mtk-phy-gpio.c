...
#define GPIO_BASE           	0xfa1c4700		//0x80080000
...
void gpio_dir_set(PHY_INT32 pin){
	...
	addr = SSUSB_I2C_OUT;
	temp = DRV_Reg32(addr);
//printk("1 gpio_dir_set addr=%x temp=%x\n", addr, temp);
	if(pin == SDA){
		temp |= SDA_OEN;
//printk("2 gpio_dir_set addr=%x temp=%x\n", addr, temp);
		DRV_WriteReg32(addr,temp);
	}
	else{
		temp |= SCL_OEN;
//printk("3 gpio_dir_set addr=%x temp=%x\n", addr, temp);
		DRV_WriteReg32(addr,temp);
	}
}
...
void gpio_dir_clr(PHY_INT32 pin){
	...
	addr = SSUSB_I2C_OUT;
	temp = DRV_Reg32(addr);
//printk("1 gpio_dir_clr addr=%x temp=%x\n", addr, temp);
	if(pin == SDA){
		temp &= ~SDA_OEN;
//printk("2 gpio_dir_clr addr=%x temp=%x\n", addr, temp);
		DRV_WriteReg32(addr,temp);
	}
	else{
		temp &= ~SCL_OEN;
//printk("3 gpio_dir_clr addr=%x temp=%x\n", addr, temp);
		DRV_WriteReg32(addr,temp);
	}
}
...
void gpio_dout_set(PHY_INT32 pin){
	...
	addr = SSUSB_I2C_OUT;
	temp = DRV_Reg32(addr);
//	printk("1 gpio_dout_set temp=%x\n", temp);
	if(pin == SDA){
		temp |= SDA_OUT;
//	printk("2 gpio_dout_set temp=%x\n", temp);
		DRV_WriteReg32(addr,temp);
	}
	else{
		temp |= SCL_OUT;
//	printk("3 gpio_dout_set temp=%x\n", temp);
		DRV_WriteReg32(addr,temp);
	}
}
...
void gpio_dout_clr(PHY_INT32 pin){
	...
	addr = SSUSB_I2C_OUT;
	temp = DRV_Reg32(addr);
//	printk("1 gpio_dout_clr temp=%x\n", temp);
	if(pin == SDA){
		temp &= ~SDA_OUT;
//	printk("2 gpio_dout_clr temp=%x\n", temp);
		DRV_WriteReg32(addr,temp);
	}
	else{
		temp &= ~SCL_OUT;
//	printk("3 gpio_dout_clr temp=%x\n", temp);
		DRV_WriteReg32(addr,temp);
	}
}
...
PHY_UINT32 SerialCommTxByte(PHY_UINT8 data) /* return 0 --> ack */
{
	...
	GPIO_InitIO(OUTPUT,SCL);
    GPIO_InitIO(OUTPUT,SDA);
//printk("SerialCommTxByte data=%x\n", data);
    for(i=8; --i>0;){
//printk("SerialCommTxByte writeIO i=%d\n", i);
...
PHY_INT32 I2cWriteReg(PHY_UINT8 dev_id, PHY_UINT8 Addr, PHY_UINT8 Data)
{
    ...
    //printk("I2cWriteReg dev_id=%x Addr=%x Data=%x\n", dev_id, Addr, Data);

#if 0
    	switch_device_address(0x60);
	random_write_one_byte(Addr, regValue);
	return 0;
#else
    SerialCommStart();
...
#if 0
	switch_device_address(0x60);
	*Data = random_read_one_byte(Addr);
	return 0;
#else
    PHY_INT32 acknowledge=0;    
    SerialCommStart();    
...
//printk("U3PhyReadReg32 bank=%x addr=%x\n", bank, addr8);
