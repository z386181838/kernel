...
#if defined(RLM_CAL_CACHE_SUPPORT) || defined(PRE_CAL_FLASH_SUPPORT)
#include "phy/rlm_cal_cache.h"
#endif /* defined(RLM_CAL_CACHE_SUPPORT) || defined(PRE_CAL_FLASH_SUPPORT) */
...
INT32 MtCmdGetPreCalResult(RTMP_ADAPTER *pAd, UINT8 CalId, UINT16 PreCalBitMap)
{
    ...
    PreCalCtrl.u2PreCalBitMap = PreCalBitMap;
...
INT32 MtCmdPATrimReStoreProc(RTMP_ADAPTER *pAd)
{
    ...
    /* Load data from bin file and store it to data buffer */
    Offset = pAd->CalFileOffset + PA_TRIM_OFFSET;
    Status = Cal_Data_Load_From_Bin(pAd, (UINT8 *)&PATrimCtrl.u4Data[0], Offset, PA_TRIM_SIZE);
          ...
          UCHAR        MUPowerCtrl)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
        ("%s: MUPowerForce: %d, MUPowerCtrl: %d \n", __FUNCTION__, MUPowerForce, MUPowerCtrl));
