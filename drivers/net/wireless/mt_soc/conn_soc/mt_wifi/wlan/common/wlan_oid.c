		     ...
		     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(PARAM_CUSTOM_CHIP_CONFIG_STRUCT_T);
	...
	rCmdChipConfig.aucReserved0[0] = prChipConfigInfo->aucReserved0[0];
	if (rCmdChipConfig.u2MsgSize > CHIP_CONFIG_RESP_SIZE) {
		DBGLOG(REQ, WARN,("Chip config Msg Size %u is not valid (set)\n", rCmdChipConfig.u2MsgSize));
...
#if 0
	DBGLOG(REQ, WARN, (" %d %d %d %d %d \n",rCmdChipConfig.u2Id,rCmdChipConfig.ucType,
			rCmdChipConfig.ucRespType,rCmdChipConfig.u2MsgSize,rCmdChipConfig.aucReserved0[0]));

	DBGLOG(REQ, WARN, (" %x %x %x %x %x %x %x %x %x %x\n",rCmdChipConfig.aucCmd[0],rCmdChipConfig.aucCmd[1],rCmdChipConfig.aucCmd[2]
		,rCmdChipConfig.aucCmd[3],rCmdChipConfig.aucCmd[4],rCmdChipConfig.aucCmd[5],rCmdChipConfig.aucCmd[6],rCmdChipConfig.aucCmd[7]
		,rCmdChipConfig.aucCmd[8],rCmdChipConfig.aucCmd[9]));
#endif
	rWlanStatus = wlanSendSetQueryCmd(prAdapter,
					  CMD_ID_CHIP_CONFIG,
					  TRUE,
    ...
    )
{
	...
	*pu4SetInfoLen = sizeof(PARAM_CUSTOM_MONITOR_SET_STRUC_T);
	...
	DBGLOG(INIT, TRACE, ("%s %d, ucEnable:%d, ucBand:%d, ucPriChannel:%d, ucSco:%d, ucChannelWidth:%d, ucChannelS1:%d, ucChannelS2:%d\n",
				__FUNCTION__, __LINE__,
				rCmdMonitorSetInfo.ucEnable,
				rCmdMonitorSetInfo.ucBand,
				rCmdMonitorSetInfo.ucPriChannel,
				rCmdMonitorSetInfo.ucSco,
