...
#if CFG_SUPPORT_SNIFFER
VOID nicRxFillRadiotapMCS(IN OUT P_MONITOR_RADIOTAP_T prMonitorRadiotap, IN P_SNIFFER_INFO_T prSnifferInfo)
{
	prMonitorRadiotap->ucMcsKnown = ( BITS(0,6) |
                                    (((prSnifferInfo->ucNess & BIT(1)) >> 1) << 7)
                                    );

    prMonitorRadiotap->ucMcsFlags = ( (prSnifferInfo->ucFrMode) |
                                    (prSnifferInfo->ucShortGI << 2) |
                                    ((prSnifferInfo->ucRxMode & BIT(0)) << 3) |
                                    (prSnifferInfo->ucLDPC << 4) |
                                    (prSnifferInfo->ucSTBC << 5) |
                                    ((prSnifferInfo->ucNess & BIT(0)) << 7)
                                    );
    /* Bit[6:0] for 802.11n, mcs0 ~ mcs7 */
	prMonitorRadiotap->ucMcsMcs = prSnifferInfo->ucMcs;
}
...
/*----------------------------------------------------------------------------*/
/*!
* @brief Get sniffer info at the tail of payload
*
* @param prSWRfb the RFB to receive rx data
* @param prSnifferInfo output of sniffer info
*
* @return (none)
*
*/
/*----------------------------------------------------------------------------*/
VOID nicRxGetSnifferInfo(IN P_SW_RFB_T prSwRfb, OUT P_SNIFFER_INFO_T prSnifferInfo)
{
	PUINT_8 pucBuf = (PUINT_8)prSwRfb->pvHeader;

	prSnifferInfo->u2RxRate = *((PUINT_16)(pucBuf + prSwRfb->u2PacketLen - 9));
	prSnifferInfo->ucRxMode = *(pucBuf + prSwRfb->u2PacketLen - 7);
	prSnifferInfo->ucFrMode = *(pucBuf + prSwRfb->u2PacketLen - 6);
	prSnifferInfo->ucShortGI = *(pucBuf + prSwRfb->u2PacketLen - 5);
	prSnifferInfo->ucLDPC = *(pucBuf + prSwRfb->u2PacketLen - 4);
	prSnifferInfo->ucSTBC = *(pucBuf + prSwRfb->u2PacketLen - 3); 
	prSnifferInfo->ucNess = *(pucBuf + prSwRfb->u2PacketLen - 2);
	prSnifferInfo->ucMcs = *(pucBuf + prSwRfb->u2PacketLen - 1);
}
...
VOID nicRxProcessMonitorPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	UINT_8 ucRxMode;
	UINT_8 ucChanNum;
	PUINT_8 pucBuffer[prSwRfb->u2PacketLen - prSwRfb->u2HeaderLen];
	...
	if(prSwRfb->u2PacketLen == 0) {
		DBGLOG(INIT, INFO, ("%s %d pktlen == 0\n", __FUNCTION__, __LINE__));
		return;
	}

	nicRxGetSnifferInfo(prSwRfb, &rSnifferInfo);
	...
	rRadiotapFieldVendor.ucData = rSnifferInfo.ucFrMode; /* VHTA1 B0-B1 */

	ucRxMode = rSnifferInfo.ucRxMode;
...
#else
	rMonitorRadiotap.ucFlags = 0;
#endif

	/* Bit Number 2 RATE */ 
	rMonitorRadiotap.ucRate = (rSnifferInfo.u2RxRate > 0xFF)? 0xFF: rSnifferInfo.u2RxRate;
	...
	ucChanNum = HIF_RX_HDR_GET_CHNL_NUM(prSwRfb->prHifRxHdr);
	...
	rMonitorRadiotap.ucAntennaSignal = RCPI_TO_dBm(prSwRfb->prHifRxHdr->ucRcpi);

	/* Bit Number 19 MCS */
	if(rSnifferInfo.ucRxMode >= RX_VT_MIXED_MODE)
		nicRxFillRadiotapMCS(&rMonitorRadiotap, &rSnifferInfo);
...
#else
	rMonitorRadiotap.u4AmpduRefNum = 0;
#endif
	...
	skb_trim(prSkb, 0);
	skb_put(prSkb, (u4RadiotapLen + prSwRfb->u2PacketLen - SNIFFER_INFO_LEN));
	...
	if(prAdapter->prGlueInfo->fgIsMonDbg)
