...
static struct mt53xx_nor *mt53xx_nor = NULL;
static inline void sflash_wreg8(struct mt53xx_nor *mt53xx_nor, u32 offset, u8 value)
{
	if (unlikely(!mt53xx_nor))
	{
		printk("access reg without init\n");
		return;
	}
	
	*(volatile unsigned int *)(mt53xx_nor->base + offset) = (value & 0xFF);
}

static inline u8 sflash_rreg8(struct mt53xx_nor *mt53xx_nor, u32 offset)
{
	if (unlikely(!mt53xx_nor))
	{
		printk("access reg without init\n");
	...
	return ((*(volatile unsigned int *)(mt53xx_nor->base + offset)) & 0xFF);
}

static inline void sflash_wreg32(struct mt53xx_nor *mt53xx_nor, u32 offset, u32 value)
{
	if (unlikely(!mt53xx_nor))
	{
		printk("access reg without init\n");
	...
	*(volatile unsigned int *)(mt53xx_nor->base + offset) = value;
}

static inline u32 sflash_rreg32(struct mt53xx_nor *mt53xx_nor, u32 offset)
{
	if (unlikely(!mt53xx_nor))
	{
		printk("access reg without init\n");
	...
	return (*(volatile unsigned int *)(mt53xx_nor->base + offset));
}


#define SFLASH_WREG8(offset, value)       sflash_wreg8(mt53xx_nor, offset, value)

#define SFLASH_RREG8(offset)              sflash_rreg8(mt53xx_nor, offset)

#define SFLASH_WREG32(offset, value)      sflash_wreg32(mt53xx_nor, offset, value)
#define SFLASH_RREG32(offset)             sflash_rreg32(mt53xx_nor, offset)
...
{                                           \
  SFLASH_WREG32(REG_FDMA_CTL, 0x0);          \
  SFLASH_WREG32(REG_FDMA_CTL, 0x2);          \
  SFLASH_WREG32(REG_FDMA_CTL, 0x0);          \
}
...
{                                                         \
  SFLASH_WREG32(REG_FDMA_FADR, src);                        \
  SFLASH_WREG32(REG_FDMA_DADR, deststart);                  \
  SFLASH_WREG32(REG_FDMA_END_DADR, deststart+len);              \
}

// DMA Trigger
#define SFLASH_DMA_START                    \
{											\
  SFLASH_WREG32(REG_FDMA_CTL, 0x05);			\
}

// DMA Completion Check
#define SFLASH_DMA_IS_COMPLETED            (0x0 == (SFLASH_RREG32(REG_FDMA_CTL) & 0x1))
...
static void mt53xx_nor_ClrInt(void)
{
...
static irqreturn_t mt53xx_nor_HandleIsr(int irq, void *dev_id)
{
  ...
  fdma_ctl = SFLASH_RREG8(REG_FDMA_CTL);


  //if(intvect & SFLASH_EN_INT)
  if ((REG_FDMA_CTL & 0x1) == 0)
  {
    //complete(&comp);
    up(&dma_complete);
  ...
  SFLASH_WREG8(REG_FDMA_CTL, 0x6);
  SFLASH_WREG8(SFLASH_SF_INTRSTUS_REG, SFLASH_EN_INT);

  while((SFLASH_RREG8(SFLASH_SF_INTRSTUS_REG)& SFLASH_EN_INT))
  	{
  		printk(KERN_ERR "Nor£ºinterrupt not be clear\n");
...
static u32 mt53xx_nor_RegIsr(struct mt53xx_nor *mt53xx_nor)
{
  ...
  if (request_irq(mt53xx_nor->irq, mt53xx_nor_HandleIsr, IRQF_TRIGGER_LOW, 
...
static void mt53xx_nor_DisableDualRead(struct mt53xx_nor *mt53xx_nor)
{
  ...
  if((attr & SFLASH_USE_DUAL_READ) &&
  	 (dualread != 0x00))
  {
	//printk(KERN_ERR "enable dualread !\n");
...
static int mt53xx_nor_erase(struct mtd_info *mtd, struct erase_info *instr)
{
  ...
  {
	 printk(KERN_ERR"Error: nor erase address at 0x%x < 0x30000",addr);
	  //while(1) {};
	 BUG_ON(1);
	 return -EINVAL;
  }
  ...
  {
  	printk(KERN_ERR "Error: nor erase address at 0x%x ",addr + len);
  	return -EINVAL;
  }
...
reMatch:
		...
		if((pattern_match>4)&&(delay_count<4))
			{
				printk(KERN_ERR "Nor:Match pattern %d,addr 0x%x, len %d \n",pattern_match,buf,len);
			pattern_match=0;
			delay_count++;
		...
		if(delay_count>=4)
			{
				printk("print error data:addr 0x%x len %d\n",buf,len);
				for(i=0;i<len;i++)
				{
...
static void mt53xx_nor_cal_checksum(u32 addr,u8 *buf,u32 len)
{
	...
	if(count)
		printk(KERN_ERR "Nor: recover %d data\n",len);
}
#endif
#if 0
static u32 mt53xx_nor_cmp_f8(u32 addr,u8 *buf,u32 len)
{
	...
	if(count)
		{
		printk(KERN_ERR "Nor: error data counts: %d \n",len);
		return 0;
		}
	else
		return 1;
...
{ 
  ...
  do {
	  ret = SFLASH_RREG32(REG_FDMA_CTL);
	  if (ret & 1)
	  {
		  printk(KERN_ERR "Nor: Dma not finished befor new trigger\n");
		  
	  }
	} while (SFLASH_RREG32(REG_FDMA_CTL) & 0x1);

 while(! down_trylock(&dma_complete))
 	{
 		printk(KERN_ERR "Nor: Get more UP signal from ISR \n");
 	}
  // reset DMA eginee
  SFLASH_DMA_RESET;
  ...
  mt53xx_nor_WaitInt(mt53xx_nor);
   do {
    ...
    if (ret & 1)
    {
    	printk(KERN_ERR "Nor: Dma not finished after trigger\n");
...
{
  u32 ret = 0, dram_addr_off = 0, len_align = 0;
  u8 info, attr;
  ...
  if(info & REGION_RANK_FIRST_FLASH)
  {
    ...
    if(0 != mt53xx_nor_WaitBusy(mt53xx_nor, SFLASH_WRITEBUSY_TIMEOUT))
    {
      printk(KERN_ERR "Read Wait Busy of flash#0 failed!\n");
      ret = -1;
      goto done;
    }
  }
  if(info & REGION_RANK_SECOND_FLASH)
  {
    ...
    if(0 != mt53xx_nor_WaitBusy(mt53xx_nor, SFLASH_WRITEBUSY_TIMEOUT))
    {
      printk(KERN_ERR " Read Wait Busy  of flash#1 failed!\n");
      ret = -1;
      goto done;
    }
  }  
		...
		if(multiReadEnable&&(multiReadCounts>1))
			{
				if(multiReadCounts==2)
					...
					printk(KERN_ERR"Nor: error read index\n");		
			}
		else
	...
	if(multiReadEnable)
	    ...
	    if(multiReadCounts==2)
			if(memcmp(_pu1NorDataBuf,_pu1NorTemp1Buf,(u32)(*retlen))!=0)
				{
					printk(KERN_ERR"Nor:Dma 1st != 2nd\n");
					multiReadCounts++;
...
//	readtime = (1000000000 * tv1.tv_sec + tv1.tv_nsec) - (1000000000 * tv.tv_sec + tv.tv_nsec);
  ...
  mutex_unlock(&mt53xx_nor->lock);
//  printk(KERN_ERR "%s (addr = %08x, len = %08x, retlen = %08x buf = %08x  readtime=%08x speed=%08x %08x)\n", __FUNCTION__,(u32)from,(u32)len,(u32)(*retlen),buf,readtime, tv1.tv_nsec, tv.tv_nsec);
...
static u32 mt53xx_nor_Write(struct mt53xx_nor *mt53xx_nor, u32 addr, u32 len, u32 *retlen, const u8 *buf)
{
  ...
  if(info & REGION_RANK_SECOND_FLASH)
  {
    ...
    if(protect_2nd_nor)
    	{
    		printk(KERN_ERR "Cancel the protect at 2nd nor before write data\n");
    		printk(KERN_ERR "[%s(%d)]%s (addr = %08x, len = %08x, retlen = %08x)\n", current->comm, current->pid, __FUNCTION__, 
...
static u32 Get_Kernel_rootfs_checksum()
{
	...
	for(i=0;i<0x93;i++)
		...
		printk(KERN_ERR "0X%08x checksum:0x%08x\n",nor_baseAddress,ichecksum);
...
static int mt53xx_nor_write(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const u_char *buf)
{
  struct mt53xx_nor *mt53xx_nor = mtd_to_mt53xx_nor(mtd);
  u32 ret = 0; // addr, u4len, i, count, pgalign, ret = 0, j;
  //u8 *u1buf, info;
#if NOR_DEBUG
  struct timeval t0;
#endif
 ...
 if((to < 0x30000)&&(enable_protect_ro==1))
  {
	 printk("Error: nor write address at 0x%x < 0x30000\n", (u32)to);
	 //while(1) {};
	 BUG_ON(1);
	 return -EINVAL;
  }
  ...
  {
  	printk(KERN_ERR "Error: nor write address to +len at 0x%x >0xc00000\n", (u32) to+len);
	//while(1) {};
    return -EINVAL;
  }
...
{
    int  len;
	printk(KERN_ERR "enable_protect_ro=%d\n",enable_protect_ro);			
	*eof = 1;
    return len;
}
                             ...
                             void *data)
{
	...
	enable_protect_ro=0;
	printk(KERN_ERR "Disable protect the READ_ONLY partition :enable_protect_ro=%d\n",enable_protect_ro);
...
{
    int  len;
	printk(KERN_ERR "MAXSIZE=%d\n",maxSize);
	if(protect_2nd_nor)
         printk(KERN_ERR "2nd Nor had been protect at write process\n");
	else
		 printk(KERN_ERR "2nd Nor not be protect at write process\n");
	*eof = 1;
    return len;
}
                             ...
                             void *data)
{
	...
	protect_2nd_nor=1;
	printk(KERN_ERR "Setting protect 2nd nor :protect_2nd_nor=%d\n",protect_2nd_nor);
    return count;
}
#endif
...
static int mt53xx_nor_init(void)
{
  int ret = -ENODEV;
  //int result;
  //u8 status;
  ...
  printk(KERN_ERR "<mt53xx_nor_init>hank:V0.6.9m(protect 0 flash)...\n");


  mt53xx_nor->base = (u32) ioremap(SFLASH_REG_BASE, 0x1000);
  mt53xx_nor->sys_base = (u32) ioremap(SYS_BASE, 0x300);
  mt53xx_nor->irq = VECTOR_FLASH;
  ...
  SFLASH_WRITE32(mt53xx_nor->sys_base + SYS_CLK_CFG_7, SFLASH_READ32(mt53xx_nor->sys_base + SYS_CLK_CFG_7) & ~0x3 | 0x4);
  /*
  ...
  //printk("nor buf w_buf=0x%08x , r_buf=0x%08x!\n",w_buf,r_buf);
  ...
  printk("read test strat total_sz =0x%x !\n",0x200000);
  mt53xx_nor_read(&(mt53xx_nor->mtd), 0x500000, 0x200000, &retlen, r_buf);
  printk("read test end total_sz =0x%x !\n",0x200000);
  //if(memcmp((void*)w_buf, (void*)r_buf, 0x10000) != 0)
  {
   // printk("nor RW test fail!!! buf w_buf=0x%08x , r_buf=0x%08x!\n",w_buf,r_buf);
  }
...
//#endif
	  ...
	  ret = add_mtd_device(&mt53xx_nor->mtd);
	  if (ret)
		printk(KERN_ERR "add_mtd_device failed...\n");
...
#else
	  ret = add_mtd_partitions(&mt53xx_nor->mtd, _partition, NUM_PARTITIONS);
	  if (ret)
		printk(KERN_ERR "add_mtd_partition failed...\n");
...
static void mt53xx_nor_exit(void)
{
	if (mt53xx_nor->flash_info)
	{
		kfree(mt53xx_nor->flash_info);
		mt53xx_nor->flash_info = NULL;
	}
...
{
	//struct mt53xx_nor *p = dev_get_platdata(&pdev->dev);
	int err;

	err = mt53xx_nor_init();
	if (err)
	{
		printk(KERN_ERR "<mt53xx_nor_init> failed...\n");
		return err;
	}
	return 0;
...
static struct resource mtk_nor_resource = {
	.name   = "mtk_nor_res",
	.flags  = IORESOURCE_MEM,
};

struct platform_device mtk_nor_dev = {
	.name           = "mtk_nor",
	.dev            = {
		.platform_data  = &mt53xx_nor,
	},
	.resource       = &mtk_nor_resource,
	.num_resources  = 1,
};

static struct platform_driver mtk_nor_dev_driver = {
	.probe = mtk_nor_probe,
...
static int __init mtk_nor_init(void)
{
	int err;


	err = platform_device_register(&mtk_nor_dev);
	if (err) 
	{
		printk(KERN_ERR "mtk_nor_dev register failed...\n");
		return err;
	}


	err = platform_driver_register(&mtk_nor_dev_driver);

	if (err)
		printk(KERN_ERR "mtk_nor_dev_driver register failed...\n");
	return err;
}

static void __exit mtk_nor_exit(void)
{
	platform_driver_unregister(&mtk_nor_dev_driver);
}

module_init(mtk_nor_init);
module_exit(mtk_nor_exit);
