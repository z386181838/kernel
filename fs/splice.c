...
#define MAX_PAGES_PER_RECVFILE  512
				     ...
				     loff_t __user *ppos, size_t count)
{
	...
	do {
		...
		ret = mapping->a_ops->write_begin(file, mapping, pos, bytes,
						  AOP_FLAG_UNINTERRUPTIBLE | AOP_FLAG_RECVFILE,
						  &pageP, &fsdata);
		...
		size_t, len, unsigned int, flags)
{
	...
	in_fd = fdget(fd_in);
	if (in_fd.file) {
		if (in_fd.file->f_mode & FMODE_READ) {
			out_fd = fdget(fd_out);
			if (out_fd.file) {
				if (out_fd.file->f_mode & FMODE_WRITE)
					error = do_splice(in_fd.file, off_in,
							  out_fd.file, off_out,
							  len, flags);
				fdput(out_fd);
			}
