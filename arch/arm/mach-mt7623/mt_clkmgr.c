...
static struct pll plls[NR_PLLS] = {
	...
	[AUD1PLL] = {
		...
		.feat = 0x0,
		.en_mask = 0x00000001,
		.base_addr = AUD1PLL_CON0,
		.pwr_addr = AUD1PLL_PWR_CON0,
		.ops = &sdm_pll_ops,
	},
	[AUD2PLL] = {
		...
		.feat = 0x0,
		.en_mask = 0x00000001,
		.base_addr = AUD2PLL_CON0,
		.pwr_addr = AUD2PLL_PWR_CON0,
		.ops = &sdm_pll_ops,
	},
#if 0
	[LVDSPLL] = {
...
static void sdm_pll_fsel_op(struct pll *pll, unsigned int value)
{
	...
	} else if ((pll->base_addr == AUD1PLL_CON0) ||
				(pll->base_addr == AUD2PLL_CON0)){
...
static struct subsys_ops bdp_sys_ops;
static struct subsys_ops eth_sys_ops;
static struct subsys_ops hif_sys_ops;
static struct subsys_ops ifrmisc_sys_ops;
...
static struct subsys syss[NR_SYSS] = {
	...
	[SYS_BDP] = {
		.name = __stringify(SYS_BDP),
		.type = SYS_TYPE_OTHER,
		.default_sta = PWR_ON,
		.sta_mask = 1U << 14,
		.ctl_addr = SPM_BDP_PWR_CON,
		.ops = &bdp_sys_ops,
	},
	[SYS_ETH] = {
		.name = __stringify(SYS_ETH),
		.type = SYS_TYPE_OTHER,
		.default_sta = PWR_ON,
		.sta_mask = 1U << 15,
		.ctl_addr = SPM_ETH_PWR_CON,
		.ops = &eth_sys_ops,
	},
	[SYS_HIF] = {
		.name = __stringify(SYS_HIF),
		.type = SYS_TYPE_OTHER,
		.default_sta = PWR_ON,
		.sta_mask = 1U << 16,
		.ctl_addr = SPM_HIF_PWR_CON,
		.ops = &hif_sys_ops,
	},
	[SYS_IFR_MISC] = {
		.name = __stringify(SYS_IFR_MISC),
		.type = SYS_TYPE_OTHER,
		.default_sta = PWR_ON,
		.sta_mask = 1U << 17,
		.ctl_addr = SPM_IFR_MISC_PWR_CON,
		.ops = &ifrmisc_sys_ops,
	},

#if 1
	/* not used, reserved for code building */
	[SYS_ISP] = {
...
#if 1 
static int bdp_sys_enable_op(struct subsys *sys)
{
	int err;
#ifdef SYS_LOG
	clk_info("[%s]: sys->name=%s\n", __func__, sys->name);
#endif
	err = spm_mtcmos_ctrl_bdpsys(STA_POWER_ON);
	return err;
}

static int bdp_sys_disable_op(struct subsys *sys)
{
	int err;
#ifdef SYS_LOG
	clk_info("[%s]: sys->name=%s\n", __func__, sys->name);
#endif
	err = spm_mtcmos_ctrl_bdpsys(STA_POWER_DOWN);
	return err;
}

static int eth_sys_enable_op(struct subsys *sys)
{
	int err;
#ifdef SYS_LOG
	clk_info("[%s]: sys->name=%s\n", __func__, sys->name);
#endif
	err = spm_mtcmos_ctrl_ethsys(STA_POWER_ON);
	return err;
}

static int eth_sys_disable_op(struct subsys *sys)
{
	int err;
#ifdef SYS_LOG
	clk_info("[%s]: sys->name=%s\n", __func__, sys->name);
#endif
	err = spm_mtcmos_ctrl_ethsys(STA_POWER_DOWN);
	return err;
}

static int hif_sys_enable_op(struct subsys *sys)
{
	int err;
#ifdef SYS_LOG
	clk_info("[%s]: sys->name=%s\n", __func__, sys->name);
#endif
	err = spm_mtcmos_ctrl_hifsys(STA_POWER_ON);
	return err;
}

static int hif_sys_disable_op(struct subsys *sys)
{
	int err;
#ifdef SYS_LOG
	clk_info("[%s]: sys->name=%s\n", __func__, sys->name);
#endif
	err = spm_mtcmos_ctrl_hifsys(STA_POWER_DOWN);
	return err;
}

static int ifrmisc_sys_enable_op(struct subsys *sys)
{
	int err;
#ifdef SYS_LOG
	clk_info("[%s]: sys->name=%s\n", __func__, sys->name);
#endif
	err = spm_mtcmos_ctrl_ifrmiscsys(STA_POWER_ON);
	return err;
}

static int ifrmisc_sys_disable_op(struct subsys *sys)
{
	int err;
#ifdef SYS_LOG
	clk_info("[%s]: sys->name=%s\n", __func__, sys->name);
#endif
	err = spm_mtcmos_ctrl_ifrmiscsys(STA_POWER_DOWN);
	return err;
}
#endif
...
#if 1 
static struct subsys_ops bdp_sys_ops = {
	.enable = bdp_sys_enable_op,
	.disable = bdp_sys_disable_op,
	.get_state = sys_get_state_op,
	.dump_regs = sys_dump_regs_op,
};

static struct subsys_ops eth_sys_ops = {
	.enable = eth_sys_enable_op,
	.disable = eth_sys_disable_op,
	.get_state = sys_get_state_op,
	.dump_regs = sys_dump_regs_op,
};

static struct subsys_ops hif_sys_ops = {
	.enable = hif_sys_enable_op,
	.disable = hif_sys_disable_op,
	.get_state = sys_get_state_op,
	.dump_regs = sys_dump_regs_op,
};

static struct subsys_ops ifrmisc_sys_ops = {
	.enable = ifrmisc_sys_enable_op,
	.disable = ifrmisc_sys_disable_op,
	.get_state = sys_get_state_op,
	.dump_regs = sys_dump_regs_op,
};
#endif
...
static struct clkmux muxs[NR_MUXS] = {
	...
	[MT_MUX_AUD2DVD] = {
		.name = __stringify(MT_MUX_AUD2DVD),
		.base_addr = CLK_CFG_15,
		.sel_mask = 0x00010000,
		.pdn_mask = 0x00800000,
		.offset = 16,
		.nr_inputs = 2,
...
#if defined(CONFIG_CLKMGR_BRINGUP) || defined(CONFIG_MT7623_FPGA)
	return 0;
#endif
...
static struct cg_grp grps[NR_GRPS] = {
	...
	[CG_AUDIO0] = {
		.name = __stringify(CG_AUDIO0),
		.sta_addr = AUDIO_TOP_CON0,
		.ops = &general_cg_grp_ops,
	},
	[CG_AUDIO4] = {
		.name = __stringify(CG_AUDIO4),
		.sta_addr = AUDIO_TOP_CON4,
		.ops = &general_cg_grp_ops,
	},
	[CG_AUDIO5] = {
		.name = __stringify(CG_AUDIO5),
		.sta_addr = AUDIO_TOP_CON5,
		.ops = &general_cg_grp_ops,
	},
	[CG_TOP_PWR2] = {
		.name = __stringify(CG_TOP_PWR2),
		.sta_addr = PWR2_TOP_CON,
		.ops = &general_cg_grp_ops,
	},
};
...
static struct cg_clk clks[NR_CLKS] = {
	...
	[MT_CG_AUDIO_LRCK_DETECT_CK] = {
		.name = "MT_CG_AUDIO_LRCK_DETECT_CK",
		.grp = &grps[CG_AUDIO0],
		.mask = BIT(4),
		.ops = &audsys_cg_clk_ops,
	},
	...
	[MT_CG_AUDIO_APLL] = {
		.name = "MT_CG_AUDIO_APLL",
		.grp = &grps[CG_AUDIO0],
		.mask = BIT(23),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_TML] = {
		.name = "MT_CG_AUDIO_TML",
		.grp = &grps[CG_AUDIO0],
		.mask = BIT(27),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_AHB_IDLE_EN_EXT] = {
		.name = "MT_CG_AUDIO_AHB_IDLE_EN_EXT",
		.grp = &grps[CG_AUDIO0],
		.mask = BIT(29),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_AHB_IDLE_EN_INT] = {
		.name = "MT_CG_AUDIO_AHB_IDLE_EN_INT",
		.grp = &grps[CG_AUDIO0],
		.mask = BIT(30),
		.ops = &audsys_cg_clk_ops,
	},
	...
	[MT_CG_AUDIO_I2SIN1] = {
		.name = "MT_CG_AUDIO_I2SIN1",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(0),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_I2SIN2] = {
		.name = "MT_CG_AUDIO_I2SIN2",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(1),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_I2SIN3] = {
		.name = "MT_CG_AUDIO_I2SIN3",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(2),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_I2SIN4] = {
		.name = "MT_CG_AUDIO_I2SIN4",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(3),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_I2SIN5] = {
		.name = "MT_CG_AUDIO_I2SIN5",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(4),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_I2SIN6] = {
		.name = "MT_CG_AUDIO_I2SIN6",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(5),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_I2SO1] = {
		.name = "MT_CG_AUDIO_I2SO1",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(6),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_I2SO2] = {
		.name = "MT_CG_AUDIO_I2SO2",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(7),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_I2SO3] = {
		.name = "MT_CG_AUDIO_I2SO3",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(8),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_I2SO4] = {
		.name = "MT_CG_AUDIO_I2SO4",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(9),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_I2SO5] = {
		.name = "MT_CG_AUDIO_I2SO5",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(10),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_I2SO6] = {
		.name = "MT_CG_AUDIO_I2SO6",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(11),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_ASRCI1] = {
		.name = "MT_CG_AUDIO_ASRCI1",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(12),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_ASRCI2] = {
		.name = "MT_CG_AUDIO_ASRCI2",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(13),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_ASRCO1] = {
		.name = "MT_CG_AUDIO_ASRCO1",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(14),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_ASRCO2] = {
		.name = "MT_CG_AUDIO_ASRCO2",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(15),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_ASRC11] = {
		.name = "MT_CG_AUDIO_ASRC11",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(16),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_ASRC12] = {
		.name = "MT_CG_AUDIO_ASRC12",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(17),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_HDMIRX] = {
		.name = "MT_CG_AUDIO_HDMIRX",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(18),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_INTDIR] = {
		.name = "MT_CG_AUDIO_INTDIR",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(20),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_A1SYS] = {
		.name = "MT_CG_AUDIO_A1SYS",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(21),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_A2SYS] = {
		.name = "MT_CG_AUDIO_A2SYS",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(22),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_AFE_CONN] = {
		.name = "MT_CG_AUDIO_AFE_CONN",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(23),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_AFE_PCMIF] = {
		.name = "MT_CG_AUDIO_AFE_PCMIF",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(24),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_AFE_MRGIF] = {
		.name = "MT_CG_AUDIO_AFE_MRGIF",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(25),
		.ops = &audsys_cg_clk_ops,
	},

	[MT_CG_AUDIO_MMIF_UL1] = {
		.name = "MT_CG_AUDIO_MMIF_UL1",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(0),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_UL2] = {
		.name = "MT_CG_AUDIO_MMIF_UL2",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(1),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_UL3] = {
		.name = "MT_CG_AUDIO_MMIF_UL3",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(2),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_UL4] = {
		.name = "MT_CG_AUDIO_MMIF_UL4",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(3),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_UL5] = {
		.name = "MT_CG_AUDIO_MMIF_UL5",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(4),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_UL6] = {
		.name = "MT_CG_AUDIO_MMIF_UL6",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(5),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_DL1] = {
		.name = "MT_CG_AUDIO_MMIF_DL1",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(6),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_DL2] = {
		.name = "MT_CG_AUDIO_MMIF_DL2",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(7),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_DL3] = {
		.name = "MT_CG_AUDIO_MMIF_DL3",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(8),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_DL4] = {
		.name = "MT_CG_AUDIO_MMIF_DL4",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(9),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_DL5] = {
		.name = "MT_CG_AUDIO_MMIF_DL5",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(10),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_DL6] = {
		.name = "MT_CG_AUDIO_MMIF_DL6",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(11),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_DLMCH] = {
		.name = "MT_CG_AUDIO_MMIF_DLMCH",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(12),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_ARB1] = {
		.name = "MT_CG_AUDIO_MMIF_ARB1",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(13),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_AWB1] = {
		.name = "MT_CG_AUDIO_MMIF_AWB1",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(14),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_AWB2] = {
		.name = "MT_CG_AUDIO_MMIF_AWB2",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(15),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MMIF_DAI] = {
		.name = "MT_CG_AUDIO_MMIF_DAI",
		.grp = &grps[CG_AUDIO5],
		.mask = BIT(16),
		.ops = &audsys_cg_clk_ops,
	},
	...
	[MT_CG_AUDIO_DMIC1] = {
		.name = "MT_CG_AUDIO_DMIC1",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(0),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_DMIC2] = {
		.name = "MT_CG_AUDIO_DMIC2",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(1),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_ASRCI3] = {
		.name = "MT_CG_AUDIO_ASRCI3",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(2),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_ASRCI4] = {
		.name = "MT_CG_AUDIO_ASRCI4",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(3),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_ASRCI5] = {
		.name = "MT_CG_AUDIO_ASRCI5",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(4),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_ASRCI6] = {
		.name = "MT_CG_AUDIO_ASRCI6",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(5),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_ASRCO3] = {
		.name = "MT_CG_AUDIO_ASRCO3",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(6),
		.ops = &audsys_cg_clk_ops,
	},
	...
	[MT_CG_AUDIO_MEM_ASRC1] = {
		.name = "MT_CG_AUDIO_MEM_ASRC1",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(10),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MEM_ASRC2] = {
		.name = "MT_CG_AUDIO_MEM_ASRC1",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(11),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MEM_ASRC3] = {
		.name = "MT_CG_AUDIO_MEM_ASRC1",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(12),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MEM_ASRC4] = {
		.name = "MT_CG_AUDIO_MEM_ASRC1",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(13),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_MEM_ASRC5] = {
		.name = "MT_CG_AUDIO_MEM_ASRC1",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(14),
		.ops = &audsys_cg_clk_ops,
	...
	[MT_CG_AUDIO_DSD_ENC] = {
		.name = "MT_CG_AUDIO_DSD_ENC",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(15),
		.ops = &audsys_cg_clk_ops,
	},
	[MT_CG_AUDIO_ASRC_BRG] = {
		.name = "MT_CG_AUDIO_ASRC_BRG",
		.grp = &grps[CG_TOP_PWR2],
		.mask = BIT(16),
		.ops = &audsys_cg_clk_ops,
	},
...
{
#ifdef CLK_LOG
	clk_info("[%s]: clk->grp->name=%s, clk->mask=0x%x, clk->name=%s\n", __func__, clk->grp->name, clk->mask, clk->name);
#endif
