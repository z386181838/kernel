...
static  fh_pll_t g_fh_pll[MT658X_FH_PLL_TOTAL_NUM] = { //keep track the status of each PLL 
	...
	{FH_FH_DISABLE,	    FH_PLL_ENABLE   , 0, 1500000 , 0},  //LVDSPLL  default SSC disable
	/* 8127 FHCTL ME */
};
...
static void mt_fh_hal_switch_register_to_FHCTL_control(enum FH_PLL_ID pll_id, int i_control)
{
	...
	else if (pll_id==MT658X_FH_LVDS_PLL) //FHCTL7
		{
			fh_set_field(PLL_HP_CON0, (0x1U<<7), i_control);
		}
	/* 8127 FHCTL ME */
}
static int mt_fh_hal_sync_ncpo_to_FHCTL_DDS(enum FH_PLL_ID pll_id)
{
	...
	/* 8127 FHCTL MB */
		...
		{
			fh_write32(REG_FHCTL7_DDS, (fh_read32(LVDSPLL_CON1)&0x1FFFFF)|(1U<<31));
		}
	/* 8127 FHCTL ME */
	else
		{
...
static int __freqhopping_ctrl(struct freqhopping_ioctl* fh_ctl,bool enable)
{
	...
	else if (fh_ctl->pll_id==MT658X_FH_LVDS_PLL)
	{
		g_fh_pll[fh_ctl->pll_id].curr_freq = 150000;
	}
	/* 8127 FHCTL ME */
	else
	{
...
static int mt_fh_hal_dvfs(enum FH_PLL_ID pll_id, unsigned int dds_value)
{
	...
	/* 8127 FHCTL MB */
		...
		{
			fh_write32(TVDPLL_CON1, (fh_read32(REG_FHCTL3_DVFS)&0x1FFFFF)|(fh_read32(TVDPLL_CON1)&0xFFE00000)|(1U<<31));
	...
	else if (pll_id==MT658X_FH_LVDS_PLL) //FHCTL3
		{
			fh_write32(LVDSPLL_CON1, (fh_read32(REG_FHCTL3_DVFS)&0x1FFFFF)|(fh_read32(LVDSPLL_CON1)&0xFFE00000)|(1U<<31));
			
			if(ilog)
			{
				FH_MSG("LVDSPLL_CON1: 0x%08x",(fh_read32(LVDSPLL_CON1)&0x1FFFFF));
			}
		}
	/* 8127 FHCTL ME */
