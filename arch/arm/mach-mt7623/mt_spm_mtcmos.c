...
int spm_mtcmos_ctrl_bdpsys(int state)
{
    ...
    if (state == STA_POWER_DOWN) {
        spm_write(SPM_BDP_PWR_CON, spm_read(SPM_BDP_PWR_CON) | PWR_ISO);

        val = spm_read(SPM_BDP_PWR_CON);
        val = (val & ~PWR_RST_B) | PWR_CLK_DIS;
        spm_write(SPM_BDP_PWR_CON, val);

        spm_write(SPM_BDP_PWR_CON, spm_read(SPM_BDP_PWR_CON) & ~(PWR_ON | PWR_ON_S));
        ...
        spm_write(SPM_BDP_PWR_CON, spm_read(SPM_BDP_PWR_CON) | PWR_ON);
        spm_write(SPM_BDP_PWR_CON, spm_read(SPM_BDP_PWR_CON) | PWR_ON_S);
#if 0
        udelay(1);
#else
        while (!(spm_read(SPM_PWR_STATUS) & BDP_PWR_STA_MASK) 
        ...
        spm_write(SPM_BDP_PWR_CON, spm_read(SPM_BDP_PWR_CON) & ~PWR_CLK_DIS);
        spm_write(SPM_BDP_PWR_CON, spm_read(SPM_BDP_PWR_CON) & ~PWR_ISO);
        spm_write(SPM_BDP_PWR_CON, spm_read(SPM_BDP_PWR_CON) | PWR_RST_B);
...
int spm_mtcmos_ctrl_ethsys(int state)
{
    ...
    if (state == STA_POWER_DOWN) {
        ...
        spm_write(SPM_ETH_PWR_CON, spm_read(SPM_ETH_PWR_CON) & ~(PWR_ON | PWR_ON_S));
        ...
        spm_write(SPM_ETH_PWR_CON, spm_read(SPM_ETH_PWR_CON) | PWR_ON_S);
#if 0
        udelay(1);
#else
        while (!(spm_read(SPM_PWR_STATUS) & ETH_PWR_STA_MASK) 
...
int spm_mtcmos_ctrl_hifsys(int state)
{
    ...
    if (state == STA_POWER_DOWN) {
        spm_write(SPM_HIF_PWR_CON, spm_read(SPM_HIF_PWR_CON) | PWR_ISO);

        val = spm_read(SPM_HIF_PWR_CON);
        val = (val & ~PWR_RST_B) | PWR_CLK_DIS;
        spm_write(SPM_HIF_PWR_CON, val);

        spm_write(SPM_HIF_PWR_CON, spm_read(SPM_HIF_PWR_CON) & ~(PWR_ON | PWR_ON_S));
        ...
        spm_write(SPM_HIF_PWR_CON, spm_read(SPM_HIF_PWR_CON) | PWR_ON);
        spm_write(SPM_HIF_PWR_CON, spm_read(SPM_HIF_PWR_CON) | PWR_ON_S);
#if 0
        udelay(1);
#else
        while (!(spm_read(SPM_PWR_STATUS) & HIF_PWR_STA_MASK) 
        ...
        spm_write(SPM_HIF_PWR_CON, spm_read(SPM_HIF_PWR_CON) & ~PWR_CLK_DIS);
        spm_write(SPM_HIF_PWR_CON, spm_read(SPM_HIF_PWR_CON) & ~PWR_ISO);
        spm_write(SPM_HIF_PWR_CON, spm_read(SPM_HIF_PWR_CON) | PWR_RST_B);
...
int spm_mtcmos_ctrl_ifrmiscsys(int state)
{
    ...
    if (state == STA_POWER_DOWN) {
        spm_write(SPM_IFR_MISC_PWR_CON, spm_read(SPM_IFR_MISC_PWR_CON) | PWR_ISO);

        val = spm_read(SPM_IFR_MISC_PWR_CON);
        val = (val & ~PWR_RST_B) | PWR_CLK_DIS;
        spm_write(SPM_IFR_MISC_PWR_CON, val);

        spm_write(SPM_IFR_MISC_PWR_CON, spm_read(SPM_IFR_MISC_PWR_CON) & ~(PWR_ON | PWR_ON_S));
        ...
        spm_write(SPM_IFR_MISC_PWR_CON, spm_read(SPM_IFR_MISC_PWR_CON) | PWR_ON);
        spm_write(SPM_IFR_MISC_PWR_CON, spm_read(SPM_IFR_MISC_PWR_CON) | PWR_ON_S);
#if 0
        udelay(1);
#else
        while (!(spm_read(SPM_PWR_STATUS) & IFR_MISC_PWR_STA_MASK) 
        ...
        spm_write(SPM_IFR_MISC_PWR_CON, spm_read(SPM_IFR_MISC_PWR_CON) & ~PWR_CLK_DIS);
        spm_write(SPM_IFR_MISC_PWR_CON, spm_read(SPM_IFR_MISC_PWR_CON) & ~PWR_ISO);
        spm_write(SPM_IFR_MISC_PWR_CON, spm_read(SPM_IFR_MISC_PWR_CON) | PWR_RST_B);
