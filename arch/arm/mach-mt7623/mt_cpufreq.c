...
static int mt_cpufreq_target(struct cpufreq_policy *policy, unsigned int target_freq, unsigned int relation)
{
    ...
    dprintk("policy->cpu=%u, num_possible_cpus=%u, mt_cpufreq_ready=%u\n", policy->cpu, num_possible_cpus(), mt_cpufreq_ready);
    ...
    ***************************************/
    ...
    dprintk("CPUFREQ_PRECHANGE=%u, CPUFREQ_POSTCHANGE=%u\n", CPUFREQ_PRECHANGE, CPUFREQ_POSTCHANGE);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0))
...
//sean 1
void hp_cooling(unsigned int cpus){
    ...
    printk("policy.governor->name=%s\n", policy.governor->name);
    gov_name = policy.governor->name;

    if(!strcmp(gov_name, "performance")){         
        total_cpus = num_possible_cpus();
        printk("possible_cpus = %d, prefered_cpus = %d", num_possible_cpus(), cpus);        
        //simple rules 
        if(cpus == 1)
            { cpu_down(3); cpu_down(2); cpu_down(1);}
        else if(cpus == 2)
            { cpu_down(3); cpu_down(2); cpu_up(1);}
        else if(cpus == 3)
            { cpu_down(3); cpu_up(2); cpu_up(1); }
        else if(cpus == 4)
            { cpu_up(3); cpu_up(2); cpu_up(1); }
    }
 
    else if (!strcmp(gov_name, "hotplug")){
        
        hp_limited_cpu_num(cpus);
        
        //if (num_online_cpus() > cpus)
        //    for (i = num_online_cpus(); i > cpus; i--){
        //           printk("turn off CPU%d due to thermal protection\n", (i - 1));
        //           cpu_down((i - 1));
        //       }
        }
}
...
{
    if (!mt_cpufreq_pause)
        seq_printf(s, "DVFS enabled\n");
    else
        seq_printf(s, "DVFS disabled\n");
...
*************************************/
static ssize_t mt_cpufreq_state_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    int enabled = 0;
...
***********************************/
static ssize_t mt_cpufreq_limited_power_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
...
*****************************/
static int mt_cpufreq_limited_load_show(struct seq_file* s, void* v)
{
    seq_printf(s, "g_limited_load_for_thermal_test = %d\n", g_limited_load_for_thermal_test);
    seq_printf(s, "g_limited_max_thermal_power = %d\n", g_limited_max_thermal_power);
    seq_printf(s, "g_cur_freq = %d, ncpu = %d\n", g_cur_freq, num_online_cpus());
...
{
    return single_open(file, mt_cpufreq_limited_load_show, NULL);
}
...
***********************************/
static ssize_t mt_cpufreq_limited_load_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    unsigned int load = 0;
...
{
    if (mt_cpufreq_debug)
        seq_printf(s, "cpufreq debug enabled\n");
    else
        seq_printf(s, "cpufreq debug disabled\n");

#ifdef CPUFREQ_SDIO_TRANSFER
	seq_printf(s, "g_cpufreq_get_vcore_corner = %d\n", g_cpufreq_get_vcore_corner);
#endif
...
************************/
static ssize_t mt_cpufreq_debug_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    int debug = 0;
...
****************************/
static int mt_cpufreq_sdio_info_show(struct seq_file* s, void* v)
{
    seq_printf(s, "mt_cpufreq_disabled_by_sdio_autoK = %d\n", mt_cpufreq_disabled_by_sdio_autoK);
    seq_printf(s, "mt_cpufreq_disabled_by_sdio_ot = %d\n", mt_cpufreq_disabled_by_sdio_ot);

    return 0;
}

static int mt_cpufreq_sdio_info_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_sdio_info_show, NULL);
}
#endif
...
* PTPOD test
************************/
static ssize_t mt_cpufreq_ptpod_test_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    int enable = 0;
...
#ifdef CPUFREQ_SDIO_TRANSFER
static const struct file_operations mt_cpufreq_sdio_info_fops = {
    .owner      = THIS_MODULE,
    .open       = mt_cpufreq_sdio_info_open,
    .read       = seq_read,
    .llseek     = seq_lseek,
    .release    = single_release,
};
...
static int __init mt_cpufreq_pdrv_init(void)
{
    ...
    else
    {
        ...
        if (!mt_entry)
        {
            pr_err("[%s]: mkdir /proc/cpufreq/cpufreq_limited_load failed\n", __FUNCTION__);
        }
        #endif
        ...
        if (!mt_entry)
        {
            pr_err("[%s]: mkdir /proc/cpufreq/cpufreq_ptpod_test failed\n", __FUNCTION__);
        }
        #endif
        ...
        #ifdef CPUFREQ_SDIO_TRANSFER
        mt_entry = proc_create("cpufreq_sdio_info", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_sdio_info_fops);
        if (!mt_entry)
        {
