...
#if defined(CONFIG_FIQ_GLUE) && !defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT)
...
#ifdef CONFIG_SMP
static int mt_irq_set_affinity(struct irq_data *data, const struct cpumask *dest, bool force){
        
    unsigned int irq = data->irq;
    unsigned long map = *cpus_addr(*dest);
    unsigned char  cpu_bitmap = 0;
    unsigned int  val = 0;
    unsigned int  base = 0;
    unsigned char offset = 0;
    unsigned long flags;
/*
    printk("[IRQ affinity] Set irq %d affinity to 0x%lx\n", irq, map);
  */
    ...
    spin_lock_irqsave(&irq_lock, flags);
    val = (*(volatile int*)(0xf0211800 + base));
   /*
    printk("1 %x\n", val);
    printk("2 %x\n",  ~(0xff << (offset << 3)));
    printk("2-1 %x\n", (val & ~(0xff << (offset << 3))));
    printk("3 %x\n",(cpu_bitmap << (offset << 3)));
    printk("4 %x\n", ((val & ~(0xff << (offset << 3))) | (cpu_bitmap << (offset << 3))));
    */

    val = ((val & ~(0xff << (offset << 3))) | (cpu_bitmap << (offset << 3)));
 
    /*
    printk("[IRQ affinity] before value in 0x%x is 0x%x, cpu_bitmap = 0x%x\n",\
            0xf0211800 + base, (*(volatile int*)(0xf0211800 + base)), cpu_bitmap);
    */

    (*(volatile int*)(0xf0211800 + base)) = val;
  
    /*
    printk("[IRQ affinity] after value in 0x%x is 0x%x and we expect 0x%x\n",\
            0xf0211800 + base, (*(volatile int*)(0xf0211800 + base)), val);
    */
    spin_unlock_irqrestore(&irq_lock, flags);
...
#if defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT)
    kree_irq_mask_restore(&mask->mask0, sizeof(struct mtk_irq_mask) -
                          sizeof(unsigned int)*2);
#else
...
#define line_set_fiq(no)   if ((no)<NR_IRQS) {is_fiq_set[(no)/32] |= (1<<((no)&31));}
