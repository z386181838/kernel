...
typedef struct {
	...
	VCODEC_DEC_ERROR_T(*pfnGetMemoryRequired) (IN VCODEC_DEC_INPUT_T * /* prInput */ ,
						   ...
						   OUT VCODEC_DEC_YUV_BUFFER_PARAM_T *
						   /* prBufferParameter */ ,
    ...
    ********************************************************/
	...
	VCODEC_DEC_ERROR_T(*pfnGetParameter) (IN HANDLE,	/* hCodec */
					      IN VCODEC_DEC_PARAM_TYPE_T, INOUT void *	/* pvParamData */
	    );
    /********************************************************
    *  wrapped for smart phone
    ********************************************************/
    ...
    ********************************************************/
	...
	VCODEC_DEC_ERROR_T(*pfnGetMemoryRequiredExtend) (IN VCODEC_DEC_INPUT_T * /* prInput */ ,
							 OUT VCODEC_MEMORY_SIZE_T * /* prMemSize */
							 ,
							 OUT VCODEC_DEC_YUV_BUFFER_PARAM_EXTEND_T *
							 /* prBufferParameter */ ,
							 OUT VCODEC_DEC_BITSTREAM_PARAM_T *
...
typedef struct {
    ...
    ********************************************************/
	VCODEC_ENC_ERROR_T(*pfnOpen) (IN HANDLE hDrv,
				      IN HANDLE hWrapper,
				      IN VCODEC_ENC_CALLBACK_T *, OUT HANDLE *hCodec);

	VCODEC_ENC_ERROR_T(*pfnInit) (IN HANDLE hCodec);

	VCODEC_ENC_ERROR_T(*pfnGetParameter) (IN HANDLE hCodec, OUT VCODEC_ENC_PARAM_TYPE_T,	/*VIDEO_ENC_MEMORY_T, */
					      void *);

	VCODEC_ENC_ERROR_T(*pfnSetParameter) (IN HANDLE hCodec, IN VCODEC_ENC_PARAM_TYPE_T,	/*VIDEO_ENCODER_QUALITY_T,VIDEO_ENCODER_CODEC_T,VIDEO_CODEC_ROTATE_ANGLE_T, */
					      void *);

    /********************************************************
    *  wrapped for smart phone
    ********************************************************/
	VCODEC_ENC_ERROR_T(*pfnGenerateHeader) (IN HANDLE hCodec, IN HANDLE hWrapper, IN VCODEC_ENC_BUFFER_INFO_T *prBufferInfo	/* for smart phone */
	    );

    /********************************************************
    *  wrapped for smart phone
    ********************************************************/
	VCODEC_ENC_ERROR_T(*pfnEncodeOneUnit) (IN HANDLE hCodec, IN HANDLE hWrapper, IN VIDEO_ENCODER_INPUT_PARAM_NC_T * prEncoderInputParamNC	/* for smart phone */
	    );
...
VCODEC_ENC_ERROR_T MPEG4EncoderGenerateVTHeader(IN MPEG4_VT_ENCODER_GEN_VT_HEADER_INPUT_T * prInput,
