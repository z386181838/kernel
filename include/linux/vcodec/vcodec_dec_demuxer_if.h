...
typedef struct 
{
    ...
    unsigned short u2FrameWidthInContainer;
    unsigned short u2FrameHeightInContainer;
} MPEG2_DECODER_PAYLOAD_INFO_T;

typedef struct
{
    unsigned int u4PayloadNumber;
    MPEG2_DECODER_PAYLOAD_INFO_T* pu1PayloadAddress;
} MPEG2_DECODER_INPUT_PARAM_T;

typedef struct __MPEG2_DRV_DATA_T
{
    MPEG2_DECODER_PAYLOAD_INFO_T     prMPEG2Payload;
    MPEG2_DECODER_INPUT_PARAM_T      prInputData;
    MPEG2_DECODER_PRIVATE_PARAM_T    rPrivateData;
} MPEG2_DRV_DATA_T, *P_MPEG2_DRV_DATA_T;
